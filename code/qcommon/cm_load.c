...
void CMod_LoadShaders(lump_t *l) {
	...
	if (l->filelen % sizeof(*in)) { Com_Error(ERR_DROP, "CMod_LoadShaders: funny lump size"); }
	count = l->filelen / sizeof(*in);

	if (count < 1) { Com_Error(ERR_DROP, "Map with no shaders"); }
...
void CMod_LoadSubmodels(lump_t *l) {
	...
	if (count > MAX_SUBMODELS) { Com_Error(ERR_DROP, "MAX_SUBMODELS exceeded"); }
	...
	for (i = 0; i < count; i++, in++) {
		...
		for (j = 0; j < out->leaf.numLeafBrushes; j++) { indexes[j] = LittleLong(in->firstBrush) + j; }

		out->leaf.numLeafSurfaces  = LittleLong(in->numSurfaces);
		indexes					   = Hunk_Alloc(out->leaf.numLeafSurfaces * 4, h_high);
		out->leaf.firstLeafSurface = indexes - cm.leafsurfaces;
		for (j = 0; j < out->leaf.numLeafSurfaces; j++) { indexes[j] = LittleLong(in->firstSurface) + j; }
	}
}
...
void CMod_LoadPatches(lump_t *surfs, lump_t *verts) {
	...
	for (i = 0; i < count; i++, in++) {
		...
		c	   = width * height;
		if (c > MAX_PATCH_VERTS) { Com_Error(ERR_DROP, "ParseMesh: MAX_PATCH_VERTS"); }
...
void CM_LoadMap(const char *name, qboolean clientload, int *checksum) {
	...
	if (!buf.i) { Com_Error(ERR_DROP, "Couldn't load %s", name); }
	...
	*checksum	  = last_checksum;
	...
	if (!clientload) { Q_strncpyz(cm.name, name, sizeof(cm.name)); }
}
...
cmodel_t *CM_ClipHandleToModel(clipHandle_t handle) {
	if (handle < 0) { Com_Error(ERR_DROP, "CM_ClipHandleToModel: bad handle %i", handle); }
...
clipHandle_t CM_InlineModel(int index) {
	if (index < 0 || index >= cm.numSubModels) { Com_Error(ERR_DROP, "CM_InlineModel: bad number"); }
	return index;
}
...
int CM_LeafCluster(int leafnum) {
	if (leafnum < 0 || leafnum >= cm.numLeafs) { Com_Error(ERR_DROP, "CM_LeafCluster: bad number"); }
	return cm.leafs[leafnum].cluster;
}

int CM_LeafArea(int leafnum) {
	if (leafnum < 0 || leafnum >= cm.numLeafs) { Com_Error(ERR_DROP, "CM_LeafArea: bad number"); }
