...
static const unsigned int pak_checksums[] = {1566731103u, 298122907u,  412165236u, 2991495316u, 1197932710u,
...
static fileHandle_t FS_HandleForFile(void) {
	...
	for (i = 1; i < MAX_FILE_HANDLES; i++) {
		if (fsh[i].handleFiles.file.o == NULL) { return i; }
	}
...
static FILE *FS_FileForHandle(fileHandle_t f) {
	...
	if (fsh[f].zipFile == qtrue) { Com_Error(ERR_DROP, "FS_FileForHandle: can't get FILE on zip file"); }
...
char *FS_BuildOSPath(const char *base, const char *game, const char *qpath) {
	...
	if (!game || !game[0]) { game = fs_gamedir; }
...
qboolean FS_CreatePath(char *OSPath) {
	...
	for (; ofs != NULL && *ofs; ofs++) {
		if (*ofs == PATH_SEP) {
			...
			*ofs = 0;
			if (!Sys_Mkdir(path)) { Com_Error(ERR_FATAL, "FS_CreatePath: failed to create path \"%s\"", path); }
			*ofs = PATH_SEP;
		}
	}
...
fileHandle_t FS_SV_FOpenFileWrite(const char *filename) {
	...
	if (fs_debug->integer) { Com_Printf("FS_SV_FOpenFileWrite: %s\n", ospath); }

	FS_CheckFilenameIsMutable(ospath, __func__);

	if (FS_CreatePath(ospath)) { return 0; }
	...
	if (!fsh[f].handleFiles.file.o) { f = 0; }
	return f;
}
...
long FS_SV_FOpenFileRead(const char *filename, fileHandle_t *fp) {
	...
	if (fs_debug->integer) { Com_Printf("FS_SV_FOpenFileRead (fs_homepath): %s\n", ospath); }
	...
	if (!fsh[f].handleFiles.file.o) {
		...
		if (Q_stricmp(fs_homepath->string, fs_basepath->string)) {
			...
			if (fs_debug->integer) { Com_Printf("FS_SV_FOpenFileRead (fs_basepath): %s\n", ospath); }
		...
		if (!fsh[f].handleFiles.file.o && fs_steampath->string[0]) {
			...
			if (fs_debug->integer) { Com_Printf("FS_SV_FOpenFileRead (fs_steampath): %s\n", ospath); }
		...
		if (!fsh[f].handleFiles.file.o && fs_gogpath->string[0]) {
			...
			if (fs_debug->integer) { Com_Printf("FS_SV_FOpenFileRead (fs_gogpath): %s\n", ospath); }
...
void FS_SV_Rename(const char *from, const char *to, qboolean safe) {
	...
	if (fs_debug->integer) { Com_Printf("FS_SV_Rename: %s --> %s\n", from_ospath, to_ospath); }

	if (safe) { FS_CheckFilenameIsMutable(to_ospath, __func__); }
...
void FS_Rename(const char *from, const char *to) {
	...
	if (fs_debug->integer) { Com_Printf("FS_Rename: %s --> %s\n", from_ospath, to_ospath); }
...
void FS_FCloseFile(fileHandle_t f) {
	...
	if (fsh[f].zipFile == qtrue) {
		...
		if (fsh[f].handleFiles.unique) { unzClose(fsh[f].handleFiles.file.z); }
		memset(&fsh[f], 0, sizeof(fsh[f]));
		return;
	}

	// we didn't find it as a pak, so close it as a unique file
	if (fsh[f].handleFiles.file.o) { fclose(fsh[f].handleFiles.file.o); }
	memset(&fsh[f], 0, sizeof(fsh[f]));
}
...
fileHandle_t FS_FOpenFileWrite(const char *filename) {
	...
	if (fs_debug->integer) { Com_Printf("FS_FOpenFileWrite: %s\n", ospath); }
...
fileHandle_t FS_FOpenFileAppend(const char *filename) {
	...
	if (fs_debug->integer) { Com_Printf("FS_FOpenFileAppend: %s\n", ospath); }
...
fileHandle_t FS_FCreateOpenPipeFile(const char *filename) {
	...
	if (fs_debug->integer) { Com_Printf("FS_FCreateOpenPipeFile: %s\n", ospath); }
...
qboolean FS_IsDemoExt(const char *filename, int namelen) {
	...
	if (ext_test && !Q_stricmpn(ext_test + 1, DEMOEXT, ARRAY_LEN(DEMOEXT) - 1)) {
		...
		for (index = 0; demo_protocols[index]; index++) {
			if (demo_protocols[index] == protocol) return qtrue;
		}
	}
...
long FS_FOpenFileReadDir(const char *filename, searchpath_t *search, fileHandle_t *file, qboolean uniqueFILE,
	...
	if (filename == NULL) Com_Error(ERR_FATAL, "FS_FOpenFileRead: NULL 'filename' parameter passed");
...
int FS_FindVM(void **startSearch, char *found, int foundlen, const char *name, int enableDll) {
	...
	if (enableDll) Com_sprintf(dllName, sizeof(dllName), "%s" ARCH_STRING DLL_EXT, name);
...
int FS_Read(void *buffer, int len, fileHandle_t f) {
	...
	if (fsh[f].zipFile == qfalse) {
		...
		while (remaining) {
			...
			if (read == -1) { Com_Error(ERR_FATAL, "FS_Read: -1 bytes read"); }
...
int FS_Seek(fileHandle_t f, long offset, int origin) {
	...
	if (fsh[f].zipFile == qtrue) {
		...
		if (offset < 0) {
			switch (origin) {
			case FS_SEEK_END: remainder = fsh[f].zipFileLen + offset; break;

			case FS_SEEK_CUR: remainder = currentPosition + offset; break;
		...
		switch (origin) {
		case FS_SEEK_SET:
			if (remainder == currentPosition) { return offset; }
		...
		default: Com_Error(ERR_FATAL, "Bad origin in FS_Seek"); return -1;
		}
	} else {
		FILE *file;
		file = FS_FileForHandle(f);
		...
		default: Com_Error(ERR_FATAL, "Bad origin in FS_Seek"); break;
		}
...
int FS_FileIsInPAK(const char *filename, int *pChecksum) {
	...
	// The searchpaths do guarantee that something will always
	// be prepended, so we don't need to worry about "c:" or "//limbo"
	if (strstr(filename, "..") || strstr(filename, "::")) { return -1; }
	...
	for (search = fs_searchpaths; search; search = search->next) {
		//
		if (search->pack) { hash = FS_HashFileName(filename, search->pack->hashSize); }
		// is the element a pak file?
		if (search->pack && search->pack->hashTable[hash]) {
			// disregard if it doesn't match one of the allowed pure pak files
			if (!FS_PakIsPure(search->pack)) { continue; }
			...
			do {
				// case and separator insensitive comparisons
				if (!FS_FilenameCompare(pakFile->name, filename)) {
					if (pChecksum) { *pChecksum = pak->pure_checksum; }
					return 1;
				}
				pakFile = pakFile->next;
			} while (pakFile != NULL);
	...
	if (strstr(qpath, ".cfg")) {
		...
		if (com_journal && com_journal->integer == 2) {
			...
			*buffer = buf;
			...
			if (r != len) { Com_Error(ERR_FATAL, "Read from journalDataFile failed"); }
...
============
*/
long FS_ReadFile(const char *qpath, void **buffer) { return FS_ReadFileDir(qpath, NULL, qfalse, buffer); }
...
void FS_FreeFile(void *buffer) {
	...
	if (fs_loadStack == 0) { Hunk_ClearTempMemory(); }
}
...
void FS_WriteFile(const char *qpath, const void *buffer, int size) {
	...
	if (!qpath || !buffer) { Com_Error(ERR_FATAL, "FS_WriteFile: NULL parameter"); }
...
static pack_t *FS_LoadZipFile(const char *zipfile, const char *basename) {
	...
	for (i = 1; i <= MAX_FILEHASH_SIZE; i <<= 1) {
		if (i > gi.number_entry) { break; }
	}

	pack			= Z_Malloc(sizeof(pack_t) + i * sizeof(fileInPack_t *));
	pack->hashSize	= i;
	pack->hashTable = (fileInPack_t **)(((char *)pack) + sizeof(pack_t));
	for (i = 0; i < pack->hashSize; i++) { pack->hashTable[i] = NULL; }
	...
	for (i = 0; i < gi.number_entry; i++) {
		err = unzGetCurrentFileInfo(uf, &file_info, filename_inzip, sizeof(filename_inzip), NULL, 0, NULL, 0);
		if (err != UNZ_OK) { break; }
		if (file_info.uncompressed_size > 0) { fs_headerLongs[fs_numHeaderLongs++] = LittleLong(file_info.crc); }
...
qboolean FS_CompareZipChecksum(const char *zipfile) {
	...
	for (index = 0; index < fs_numServerReferencedPaks; index++) {
		if (checksum == fs_serverReferencedPaks[index]) return qtrue;
	}
...
static int FS_AddFileToList(char *name, char *list[MAX_FOUND_FILES], int nfiles) {
	...
	if (nfiles == MAX_FOUND_FILES - 1) { return nfiles; }
	for (i = 0; i < nfiles; i++) {
...
char **FS_ListFilteredFiles(const char *path, const char *extension, char *filter, int *numfiles,
	...
	if (path[pathLength - 1] == '\\' || path[pathLength - 1] == '/') { pathLength--; }
		...
		if (search->pack) {
			...
			for (i = 0; i < pak->numfiles; i++) {
				...
				if (filter) {
					...
					if (Q_stricmp(name + length - extensionLength, extension)) { continue; }
					// unique the match
...
static char **Sys_ConcatenateFileLists(char **list0, char **list1) {
	...
	/* Copy over lists. */
	if (list0) {
		for (src = list0; *src; src++, dst++) *dst = *src;
	}
	if (list1) {
		for (src = list1; *src; src++, dst++) *dst = *src;
	}
...
void FS_GetModDescription(const char *modDir, char *description, int descriptionLen) {
	...
	if (nDescLen > 0) {
		...
		if (nDescLen >= 0) { description[nDescLen] = '\0'; }
	} else {
		Q_strncpyz(description, modDir, descriptionLen);
	}

	if (descHandle) { FS_FCloseFile(descHandle); }
}
...
int FS_GetModList(char *listbuf, int bufsize) {
	...
	*listbuf = 0;
	...
	for (i = 0; i < nPotential; i++) {
		...
		if (bDrop || Q_stricmp(name, com_basegame->string) == 0 || Q_stricmpn(name, ".", 1) == 0) { continue; }
...
void FS_Dir_f(void) {
	...
	for (i = 0; i < ndirs; i++) { Com_Printf("%s\n", dirnames[i]); }
...
void FS_SortFileList(char **filelist, int numfiles) {
	...
	for (i = 0; i < numfiles; i++) {
		...
		for (k = numsortedfiles; k > j; k--) { sortedlist[k] = sortedlist[k - 1]; }
...
void FS_Path_f(void) {
	...
	Com_Printf("\n");
	for (i = 1; i < MAX_FILE_HANDLES; i++) {
		if (fsh[i].handleFiles.file.o) { Com_Printf("handle %i: %s\n", i, fsh[i].name); }
	}
}
...
void FS_AddGameDirectory(const char *path, const char *dir) {
	...
	if (pakfiles) { qsort(pakfiles, numfiles, sizeof(char *), paksort); }
...
qboolean FS_idPak(char *pak, char *base, int numPaks) {
	...
	for (i = 0; i < NUM_ID_PAKS; i++) {
		if (!FS_FilenameCompare(pak, va("%s/pak%d", base, i))) { break; }
	}
...
qboolean FS_ComparePaks(char *neededpaks, int len, qboolean dlstring) {
	...
	if (!fs_numServerReferencedPaks) return qfalse; // Server didn't send any pack information along
...
#ifdef FS_MISSING
	if (closemfp) { fclose(missingFiles); }
#endif
}
...
static void FS_Startup(const char *gameName) {
	...
	if (!homePath || !homePath[0]) { homePath = fs_basepath->string; }
	fs_homepath	  = Cvar_Get("fs_homepath", homePath, CVAR_INIT | CVAR_PROTECTED);
	fs_gamedirvar = Cvar_Get("fs_game", "", CVAR_INIT | CVAR_SYSTEMINFO);

	if (!gameName[0]) { Cvar_ForceReset("com_basegame"); }
	...
	if (FS_InvalidGameDir(gameName)) { Com_Error(ERR_DROP, "Invalid com_basegame '%s'", gameName); }
	...
	if (fs_microsoftstorepath->string[0]) { FS_AddGameDirectory(fs_microsoftstorepath->string, gameName); }
	fs_gogpath = Cvar_Get("fs_gogpath", Sys_GogPath(), CVAR_INIT | CVAR_PROTECTED);
	if (fs_gogpath->string[0]) { FS_AddGameDirectory(fs_gogpath->string, gameName); }
	fs_steampath = Cvar_Get("fs_steampath", Sys_SteamPath(), CVAR_INIT | CVAR_PROTECTED);
	if (fs_steampath->string[0]) { FS_AddGameDirectory(fs_steampath->string, gameName); }
	if (fs_basepath->string[0]) { FS_AddGameDirectory(fs_basepath->string, gameName); }
	// fs_homepath is somewhat particular to *nix systems, only add if relevant

#ifdef __APPLE__
	fs_apppath = Cvar_Get("fs_apppath", Sys_DefaultAppPath(), CVAR_INIT | CVAR_PROTECTED);
	// Make MacOSX also include the base path included with the .app bundle
	if (fs_apppath->string[0]) FS_AddGameDirectory(fs_apppath->string, gameName);
#endif
	...
	if (fs_basegame->string[0] && Q_stricmp(fs_basegame->string, gameName)) {
		if (fs_gogpath->string[0]) { FS_AddGameDirectory(fs_gogpath->string, fs_basegame->string); }
		if (fs_steampath->string[0]) { FS_AddGameDirectory(fs_steampath->string, fs_basegame->string); }
	...
	if (fs_gamedirvar->string[0] && Q_stricmp(fs_gamedirvar->string, gameName)) {
		if (fs_gogpath->string[0]) { FS_AddGameDirectory(fs_gogpath->string, fs_gamedirvar->string); }
		if (fs_steampath->string[0]) { FS_AddGameDirectory(fs_steampath->string, fs_gamedirvar->string); }
	...
	if (!com_standalone->integer) {
		...
		if (fs_gamedirvar->string[0]) { Com_AppendCDKey(fs_gamedirvar->string); }
	}
#endif
...
#ifdef FS_MISSING
	if (missingFiles == NULL) { missingFiles = Sys_FOpen("\\missing.txt", "ab"); }
#endif
...
static void FS_CheckPak0(void) {
	...
	for (path = fs_searchpaths; path; path = path->next) {
		...
		if (!Q_stricmpn(curpack->pakGamename, "demoq3", MAX_OSPATH) && !Q_stricmpn(pakBasename, "pak0", MAX_OSPATH)) {
			if (curpack->checksum == DEMO_PAK0_CHECKSUM) founddemo = qtrue;
		}

		else if (!Q_stricmpn(curpack->pakGamename, BASEGAME, MAX_OSPATH) && strlen(pakBasename) == 4 &&
		...
		} else if (!Q_stricmpn(curpack->pakGamename, BASETA, MAX_OSPATH) && strlen(pakBasename) == 4 &&
				   !Q_stricmpn(pakBasename, "pak", 3) && pakBasename[3] >= '0' &&
...
void FS_PureServerSetLoadedPaks(const char *pakSums, const char *pakNames) {
	...
	for (i = 0; i < c; i++) { fs_serverPaks[i] = atoi(Cmd_Argv(i)); }
	...
	if (pakNames && *pakNames) {
		...
		for (i = 0; i < d; i++) { fs_serverPakNames[i] = CopyString(Cmd_Argv(i)); }
	}
}
...
void FS_PureServerSetReferencedPaks(const char *pakSums, const char *pakNames) {
	...
	for (i = 0; i < c; i++) { fs_serverReferencedPaks[i] = atoi(Cmd_Argv(i)); }
	...
	if (pakNames && *pakNames) {
		...
		for (i = 0; i < d; i++) { fs_serverReferencedPakNames[i] = CopyString(Cmd_Argv(i)); }
	}
...
void FS_InitFilesystem(void) {
	...
	if (!FS_FilenameCompare(Cvar_VariableString("fs_game"), com_basegame->string)) Cvar_Set("fs_game", "");
...
int FS_FOpenFileByMode(const char *qpath, fileHandle_t *f, fsMode_t mode) {
	...
	switch (mode) {
	case FS_READ: r = FS_FOpenFileRead(qpath, f, qtrue); break;
	case FS_WRITE:
		*f = FS_FOpenFileWrite(qpath);
		r  = 0;
		if (*f == 0) { r = -1; }
		break;
	case FS_APPEND_SYNC: sync = qtrue;
	case FS_APPEND:
		*f = FS_FOpenFileAppend(qpath);
		r  = 0;
		if (*f == 0) { r = -1; }
		break;
	default: Com_Error(ERR_FATAL, "FS_FOpenFileByMode: bad mode"); return -1;
	}

	if (!f) { return r; }

	if (*f) { fsh[*f].fileSize = r; }
