...
void Com_BeginRedirect(char *buffer, int buffersize, void (*flush)(char *)) {
	if (!buffer || !buffersize || !flush) return;
	rd_buffer	  = buffer;
...
void Com_EndRedirect(void) {
	if (rd_flush) { rd_flush(rd_buffer); }
...
void QDECL Com_Printf(const char *fmt, ...) {
	...
	if (com_logfile && com_logfile->integer) {
		...
		if (logfile && FS_Initialized()) { FS_Write(msg, strlen(msg), logfile); }
	}
}
...
void QDECL Com_Error(int code, const char *fmt, ...) {
	...
	if (com_errorEntered) Sys_Error("recursive error after: %s", com_errorMessage);
	...
	if (com_buildScript && com_buildScript->integer) { code = ERR_FATAL; }

	// if we are getting a solid stream of ERR_DROP, do an ERR_FATAL
	currentTime = Sys_Milliseconds();
	if (currentTime - lastErrorTime < 100) {
		if (++errorCount > 3) { code = ERR_FATAL; }
	} else {
		errorCount = 0;
	}
	...
	if (code != ERR_DISCONNECT && code != ERR_NEED_CD) Cvar_Set("com_errorMessage", com_errorMessage);
...
void Com_ParseCommandLine(char *commandLine) {
	...
	while (*commandLine) {
		if (*commandLine == '"') { inq = !inq; }
		// look for a + separating character
		// if commandLine came from a file, we might have real line seperators
		if ((*commandLine == '+' && !inq) || *commandLine == '\n' || *commandLine == '\r') {
			if (com_numConsoleLines == MAX_CONSOLE_LINES) { return; }
...
void Com_StartupVariable(const char *match) {
	...
	for (i = 0; i < com_numConsoleLines; i++) {
		Cmd_TokenizeString(com_consoleLines[i]);
		if (strcmp(Cmd_Argv(0), "set")) { continue; }
...
qboolean Com_AddStartupCommands(void) {
	...
	for (i = 0; i < com_numConsoleLines; i++) {
		if (!com_consoleLines[i] || !com_consoleLines[i][0]) { continue; }

		// set commands already added with Com_StartupVariable
		if (!Q_stricmpn(com_consoleLines[i], "set ", 4)) { continue; }
...
========================
*/
static int Z_AvailableZoneMemory(memzone_t *zone) { return zone->size - zone->used; }
...
void Z_Free(void *ptr) {
	...
	if (!ptr) { Com_Error(ERR_DROP, "Z_Free: NULL pointer"); }

	block = (memblock_t *)((byte *)ptr - sizeof(memblock_t));
	if (block->id != ZONEID) { Com_Error(ERR_FATAL, "Z_Free: freed a pointer without ZONEID"); }
	if (block->tag == 0) { Com_Error(ERR_FATAL, "Z_Free: freed a freed pointer"); }
	// if static memory
...
void *Z_TagMalloc(int size, int tag) {
	...
	if (!tag) { Com_Error(ERR_FATAL, "Z_TagMalloc: tried to use a 0 tag"); }
	...
	do {
		if (rover == start) {
			...
			Com_Error(ERR_FATAL, "Z_Malloc: failed on allocation of %i bytes from the %s zone: %s, line: %d (%s)", size,
					  zone == smallzone ? "small" : "main", file, line, label);
#else
...
void Z_LogZoneHeap(memzone_t *zone, char *name) {
	...
	if (!logfile || !FS_Initialized()) return;
	size = numBlocks = 0;
#ifdef ZONE_DEBUG
	allocSize = 0;
#endif
...
void Com_Meminfo_f(void) {
	...
	for (block = smallzone->blocklist.next;; block = block->next) {
		if (block->tag) { smallZoneBytes += block->size; }
	...
	if (hunk_low.temp != hunk_low.permanent) { Com_Printf("%8i low temp\n", hunk_low.temp); }
	Com_Printf("%8i low tempHighwater\n", hunk_low.tempHighwater);
	Com_Printf("\n");
	Com_Printf("%8i high mark\n", hunk_high.mark);
	Com_Printf("%8i high permanent\n", hunk_high.permanent);
	if (hunk_high.temp != hunk_high.permanent) { Com_Printf("%8i high temp\n", hunk_high.temp); }
...
void Com_InitZoneMemory(void) {
	...
	if (!mainzone) { Com_Error(ERR_FATAL, "Zone data failed to allocate %i megs", s_zoneTotal / (1024 * 1024)); }
...
void Hunk_SmallLog(void) {
	...
	if (!logfile || !FS_Initialized()) return;
	for (block = hunkblocks; block; block = block->next) { block->printed = qfalse; }
	size	  = 0;
	numBlocks = 0;
	Com_sprintf(buf, sizeof(buf), "\r\n================\r\nHunk Small log\r\n================\r\n");
	FS_Write(buf, strlen(buf), logfile);
...
qboolean Hunk_CheckMark(void) {
	if (hunk_low.mark || hunk_high.mark) { return qtrue; }
	return qfalse;
}
...
void Hunk_FreeTempMemory(void *buf) {
	...
	if (hdr->magic != HUNK_MAGIC) { Com_Error(ERR_FATAL, "Hunk_FreeTempMemory: bad magic"); }
...
sysEvent_t Com_GetRealEvent(void) {
	...
	if (com_journal->integer == 2) {
		...
		if (r != sizeof(ev)) { Com_Error(ERR_FATAL, "Error reading from journal file"); }
		if (ev.evPtrLength) {
			ev.evPtr = Z_Malloc(ev.evPtrLength);
			r		 = FS_Read(ev.evPtr, ev.evPtrLength, com_journalFile);
			if (r != ev.evPtrLength) { Com_Error(ERR_FATAL, "Error reading from journal file"); }
		}
	} else {
		...
		if (com_journal->integer == 1) {
			...
			if (r != sizeof(ev)) { Com_Error(ERR_FATAL, "Error writing to journal file"); }
			if (ev.evPtrLength) {
				r = FS_Write(ev.evPtr, ev.evPtrLength, com_journalFile);
				if (r != ev.evPtrLength) { Com_Error(ERR_FATAL, "Error writing to journal file"); }
			}
		}
	}
...
void Com_RunAndTimeServerPacket(netadr_t *evFrom, msg_t *buf) {
	...
	if (com_speeds->integer) { t1 = Sys_Milliseconds(); }
	...
	if (com_speeds->integer) {
		t2	 = Sys_Milliseconds();
		msec = t2 - t1;
		if (com_speeds->integer == 3) { Com_Printf("SV_PacketEvent time: %i\n", msec); }
	}
}
...
int Com_EventLoop(void) {
	...
	while (1) {
		...
		if (ev.evType == SE_NONE) {
			...
			while (NET_GetLoopPacket(NS_CLIENT, &evFrom, &buf)) { CL_PacketEvent(evFrom, &buf); }

			while (NET_GetLoopPacket(NS_SERVER, &evFrom, &buf)) {
				// if the server just shut down, flush the events
				if (com_sv_running->integer) { Com_RunAndTimeServerPacket(&evFrom, &buf); }
			}
		...
		switch (ev.evType) {
		case SE_KEY: CL_KeyEvent(ev.evValue, ev.evValue2, ev.evTime); break;
		case SE_CHAR: CL_CharEvent(ev.evValue); break;
		case SE_MOUSE: CL_MouseEvent(ev.evValue, ev.evValue2, ev.evTime); break;
		case SE_JOYSTICK_AXIS: CL_JoystickEvent(ev.evValue, ev.evValue2, ev.evTime); break;
		case SE_CONSOLE:
			Cbuf_AddText((char *)ev.evPtr);
			Cbuf_AddText("\n");
			break;
		default: Com_Error(ERR_FATAL, "Com_EventLoop: bad event type %i", ev.evType); break;
		}
...
int Com_Milliseconds(void) {
	...
	do {
		...
		ev = Com_GetRealEvent();
		if (ev.evType != SE_NONE) { Com_PushEvent(&ev); }
...
void Com_GameRestart(int checksumFeed, qboolean disconnect) {
	...
	if (!com_gameRestarting && com_fullyInitialized) {
		...
		if (com_sv_running->integer) SV_Shutdown("Game directory changed");

		if (com_gameClientRestarting) {
			if (disconnect) CL_Disconnect(qfalse);
...
static void Com_WriteCDKey(const char *filename, const char *ikey) {
	...
	if (!CL_CDKeyValidate(key, NULL)) { return; }
...
void Com_Init(char *commandLine) {
	...
	if (com_pipefile->string[0]) { pipefile = FS_FCreateOpenPipeFile(com_pipefile->string); }
...
void Com_WriteConfiguration(void) {
	...
	if (!(cvar_modifiedFlags & CVAR_ARCHIVE)) { return; }
	cvar_modifiedFlags &= ~CVAR_ARCHIVE;
...
int Com_ModifyMsec(int msec) {
	...
	if (com_dedicated->integer) {
		...
		// of time.
		if (com_sv_running->integer && msec > 500) Com_Printf("Hitch warning: %i msec frame time\n", msec);
...
void Com_Frame(void) {
	...
	// main event loop
	//
	if (com_speeds->integer) { timeBeforeFirstEvents = Sys_Milliseconds(); }
	...
	// server side
	//
	if (com_speeds->integer) { timeBeforeServer = Sys_Milliseconds(); }
	...
	//
	if (com_speeds->integer) { timeBeforeEvents = Sys_Milliseconds(); }
	Com_EventLoop();
	Cbuf_Execute();
	...
	// client side
	//
	if (com_speeds->integer) { timeBeforeClient = Sys_Milliseconds(); }

	CL_Frame(msec);

	if (com_speeds->integer) { timeAfter = Sys_Milliseconds(); }
#else
	if (com_speeds->integer) {
...
static void FindMatches(const char *s) {
	...
	if (Q_stricmpn(s, completionString, strlen(completionString))) { return; }
	matchCount++;
	if (matchCount == 1) {
	...
	for (i = 0; shortestMatch[i]; i++) {
		...
		if (tolower(shortestMatch[i]) != tolower(s[i])) { shortestMatch[i] = 0; }
	}
}
...
static void PrintMatches(const char *s) {
	if (!Q_stricmpn(s, shortestMatch, strlen(shortestMatch))) { Com_Printf("    %s\n", s); }
}
...
void Field_CompleteKeyname(void) {
	...
	if (!Field_Complete()) Key_KeynameCompletion(PrintMatches);
}
#endif
...
void Field_CompleteFilename(const char *dir, const char *ext, qboolean stripExt, qboolean allowNonPureFilesOnDisk) {
	...
	if (!Field_Complete()) FS_FilenameCompletion(dir, ext, stripExt, PrintMatches, allowNonPureFilesOnDisk);
}
...
void Field_CompleteCommand(char *cmd, qboolean doCommands, qboolean doCvars) {
	...
	if (con_autochat->integer && completionField->buffer[0] && completionField->buffer[0] != '\\') {
		if (completionField->buffer[0] != '/') {
			...
			if (strlen(completionField->buffer) + 1 >= sizeof(completionField->buffer)) return;
	...
	if (completionArgument > 1) {
		...
		if (completionString[0] == '\\' || completionString[0] == '/') completionString++;

		matchCount		 = 0;
		shortestMatch[0] = 0;

		if (strlen(completionString) == 0) return;

		if (doCommands) Cmd_CommandCompletion(FindMatches);

		if (doCvars) Cvar_CommandCompletion(FindMatches);

		if (!Field_Complete()) {
			// run through again, printing matches
			if (doCommands) Cmd_CommandCompletion(PrintMatches);

			if (doCvars) Cvar_CommandCompletion(PrintCvarMatches);
		}
	}
}
...
void Com_RandomBytes(uint8_t *string, int len) {
	...
	if (Sys_RandomBytes(string, len)) return;

	Com_Printf("Com_RandomBytes: using weak randomization\n");
	for (i = 0; i < len; i++) string[i] = (unsigned char)(rand() % 256);
}
...
qboolean Com_IsVoipTarget(uint8_t *voipTargets, int voipTargetsSize, int clientNum) {
	...
	if (clientNum < 0) {
		for (index = 0; index < voipTargetsSize; index++) {
			if (voipTargets[index]) return qtrue;
		}
	...
	if (index < voipTargetsSize) return (voipTargets[index] & (1 << (clientNum & 0x07)));
...
static void Name_PlayerNameCompletion(const char **names, int nameCount, void (*callback)(const char *s)) {
	...
	for (i = 0; i < nameCount; i++) { callback(names[i]); }
}
...
qboolean Com_FieldStringToPlayerName(char *name, int length, const char *rawname) {
	...
	if (name == NULL || rawname == NULL) return qfalse;
...
qboolean Com_PlayerNameToFieldString(char *str, int length, const char *name) {
	...
	if (str == NULL || name == NULL) return qfalse;
...
void Field_CompletePlayerName(const char **names, int nameCount) {
	...
	if (completionString[0] == '\0') { Com_PlayerNameToFieldString(shortestMatch, sizeof(shortestMatch), names[0]); }
...
int QDECL Com_strCompare(const void *const a, const void *const b) {
	const char *const *const pa = (const char *const *)a;
	const char *const *const pb = (const char *const *)b;
	return (int)(strcmp(*pa, *pb));
}
