...
static const char *opnames[256] = {
	"OP_UNDEF", "OP_IGNORE", "OP_BREAK", "OP_ENTER",  "OP_LEAVE",  "OP_CALL",	"OP_PUSH", "OP_POP",		"OP_CONST",
	"OP_LOCAL", "OP_JUMP",	 "OP_EQ",	 "OP_NE",	  "OP_LTI",	   "OP_LEI",	"OP_GTI",  "OP_GEI",		"OP_LTU",
	"OP_LEU",	"OP_GTU",	 "OP_GEU",	 "OP_EQF",	  "OP_NEF",	   "OP_LTF",	"OP_LEF",  "OP_GTF",		"OP_GEF",
	"OP_LOAD1", "OP_LOAD2",	 "OP_LOAD4", "OP_STORE1", "OP_STORE2", "OP_STORE4", "OP_ARG",  "OP_BLOCK_COPY", "OP_SEX8",
	"OP_SEX16", "OP_NEGI",	 "OP_ADD",	 "OP_SUB",	  "OP_DIVI",   "OP_DIVU",	"OP_MODI", "OP_MODU",		"OP_MULI",
	"OP_MULU",	"OP_BAND",	 "OP_BOR",	 "OP_BXOR",	  "OP_BCOM",   "OP_LSH",	"OP_RSHI", "OP_RSHU",		"OP_NEGF",
	"OP_ADDF",	"OP_SUBF",	 "OP_DIVF",	 "OP_MULF",	  "OP_CVIF",   "OP_CVFI",
};
...
static void VM_Destroy_Compiled(vm_t *vm) {
	if (vm->codeBase) {
		if (munmap(vm->codeBase, vm->codeLength)) Com_Printf(S_COLOR_RED "Memory unmap failed, possible memory leak\n");
	}
	vm->codeBase = NULL;
}
...
static int asmcall(int call, int pstack) {
	...
	if (sizeof(intptr_t) == sizeof(int)) {
		...
		for (i = 1; i < ARRAY_LEN(args); i++) args[i] = argPosition[i];
...
void _emit(vm_t *vm, unsigned isn, int pass) {
	...
	if (pass) memcpy(vm->codeBase + vm->codeLength, &isn, 4);
	vm->codeLength += 4;
}
...
static unsigned char off8(unsigned val) {
	if (val & 3) DIE("offset must be multiple of four");
	if (val > 1020) DIE("offset too large");
	return val >> 2;
}
...
static unsigned short rimm(unsigned val) {
	...
	if (shift > 15 || val > 255) { DIE("immediate cannot be encoded (%d, %d)\n", shift, val); }
...
static unsigned short can_encode(unsigned val) {
	unsigned shift = 0;
	if (!val) DIE("can_encode: invalid argument");
	if (val < 256) return val;
	// rotate the value until it fits
	while (shift < 16 && (val > 255 || !(val & 3))) {
		val = (val & 3) << 30 | val >> 2;
...
#define MOVT(dst, i)                                                                                                   \
	(AL | (0b11 << 24) | (0b0100 << 20) | ((((i) >> 12) & 0xF) << 16) | (dst << 12) | ((i) & ((1 << 12) - 1)))
...
#define LSL(dst, src, reg)                                                                                             \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | (reg << 8) | (0b0001 << 4) | src)
#define LSR(dst, src, reg)                                                                                             \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | (reg << 8) | (0b0011 << 4) | src)
#define ASR(dst, src, reg)                                                                                             \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | (reg << 8) | (0b0101 << 4) | src)
#define ROR(dst, src, reg)                                                                                             \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | (reg << 8) | (0b0111 << 4) | src)

#define LSLi(dst, src, i)                                                                                              \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | ((i & 0x1F) << 7) | (0b000 << 4) | src)
#define LSRi(dst, src, i)                                                                                              \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | ((i & 0x1F) << 7) | (0b010 << 4) | src)
#define ASRi(dst, src, i)                                                                                              \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | ((i & 0x1F) << 7) | (0b100 << 4) | src)
#define RORi(dst, src, i)                                                                                              \
	(AL | (0b000 << 25) | (0b1101 << 21) | (0 << 20) | (dst << 12) | ((i & 0x1F) << 7) | (0b110 << 4) | src)
...
#define LDRSBai(dst, base, off)                                                                                        \
	(AL | (0b000 << 25) | (0b0110 << 21) | (1 << 20) | base << 16 | dst << 12 | ((off & 0xF0) << 4) | 0b1101 << 4 |    \
	 (off & 0x0F))
#define STRBa(dst, base, off) (AL | (0b011 << 25) | (0b1110 << 21) | (0 << 20) | base << 16 | dst << 12 | off)

#define LDRHa(dst, base, off)                                                                                          \
	(AL | (0b000 << 25) | (0b1100 << 21) | (1 << 20) | base << 16 | dst << 12 | (0b1011 << 4) | off)
#define LDRSHai(dst, base, off)                                                                                        \
	(AL | (0b000 << 25) | (0b1110 << 21) | (1 << 20) | base << 16 | dst << 12 | ((off & 0xF0) << 4) | 0b1111 << 4 |    \
	 (off & 0x0F))
#define STRHa(dst, base, off)                                                                                          \
	(AL | (0b000 << 25) | (0b1100 << 21) | (0 << 20) | base << 16 | dst << 12 | (0b1011 << 4) | off)
...
#define Bi(i)				   (AL | (0b10) << 26 | (1 << 25) /*I*/ | (0 << 24) /*L*/ | (i))
// call subroutine
#define BLi(i)				   (AL | (0b10) << 26 | (1 << 25) /*I*/ | (1 << 24) /*L*/ | (i))
// branch and exchange (register)
#define BX(reg)				   (AL | 0b00010010 << 20 | 0b1111 << 16 | 0b1111 << 12 | 0b1111 << 8 | 0b0001 << 4 | reg)
// call subroutine (register)
#define BLX(reg)			   (AL | 0b00010010 << 20 | 0b1111 << 16 | 0b1111 << 12 | 0b1111 << 8 | 0b0011 << 4 | reg)
...
#define STM(base, regs)                                                                                                \
	(AL | 0b100 << 25 | 0 << 24 /*P*/ | 0 << 24 /*U*/ | 0 << 24 /*S*/ | 0 << 24 /*W*/ | (base << 16) |                 \
	 (regs & ~(1 << 16)))

// note: op1 and op2 must not be the same
#define MUL(op1, op2, op3) (AL | 0b0000000 << 21 | (1 << 20) /*S*/ | (op1 << 16) | (op3 << 8) | 0b1001 << 4 | (op2))

// puts integer in R0
#define emit_MOVR0i(arg)   emit_MOVRxi(R0, arg)

// puts integer arg in register reg
#define emit_MOVRxi(reg, arg)                                                                                          \
	do {                                                                                                               \
		emit(MOVW(reg, (arg & 0xFFFF)));                                                                               \
		if (arg > 0xFFFF) emit(MOVT(reg, (((arg >> 16) & 0xFFFF))));                                                   \
	} while (0)

// puts integer arg in register reg. adds nop if only one instr is needed to
// make size constant
#define emit_MOVRxi_or_NOP(reg, arg)                                                                                   \
	do {                                                                                                               \
		emit(MOVW(reg, (arg & 0xFFFF)));                                                                               \
		if (arg > 0xFFFF)                                                                                              \
...
#define VMOVass(Vn, Rt)                                                                                                \
	(AL | (0b1110 << 24) | (0b000 << 21) | (0 << 20) | ((Vn >> 1) << 16) | (Rt << 12) | (0b1010 << 8) |                \
	 ((Vn & 1) << 7) | (1 << 4))
// singe precision register -> arm core register
#define VMOVssa(Rt, Vn)                                                                                                \
	(AL | (0b1110 << 24) | (0b000 << 21) | (1 << 20) | ((Vn >> 1) << 16) | (Rt << 12) | (0b1010 << 8) |                \
	 ((Vn & 1) << 7) | (1 << 4))

#define _VCVT_F(Vd, Vm, opc2, op)                                                                                      \
	(AL | (0b11101 << 23) | ((Vd & 1) << 22) | (0b111 << 19) | (opc2 << 16) | ((Vd >> 1) << 12) | (0b101 << 9) |       \
	 (0 << 8) | (op << 7) | (1 << 6) | ((Vm & 1) << 5) | (Vm >> 1))
...
#define VLDRa(Vd, Rn, i)                                                                                               \
	(AL | (0b1101 << 24) | 1 << 23 | ((Vd & 1) << 22) | 1 << 20 | (Rn << 16) | ((Vd >> 1) << 12) | (0b1010 << 8) |     \
	 off8(i))
#define VSTRa(Vd, Rn, i)                                                                                               \
	(AL | (0b1101 << 24) | 1 << 23 | ((Vd & 1) << 22) | 0 << 20 | (Rn << 16) | ((Vd >> 1) << 12) | (0b1010 << 8) |     \
	 off8(i))

#define VNEG_F32(Vd, Vm)                                                                                               \
	(AL | (0b11101 << 23) | ((Vd & 1) << 22) | (0b11 << 20) | (1 << 16) | ((Vd >> 1) << 12) | (0b101 << 9) |           \
	 (0 << 8) | (1 << 6) | ((Vm & 1) << 5) | (Vm >> 1))

#define VADD_F32(Vd, Vn, Vm)                                                                                           \
	(AL | (0b11100 << 23) | ((Vd & 1) << 22) | (0b11 << 20) | ((Vn >> 1) << 16) | ((Vd >> 1) << 12) | (0b101 << 9) |   \
	 (0 << 8) | ((Vn & 1) << 7) | (0 << 6) | ((Vm & 1) << 5) | (Vm >> 1))
#define VSUB_F32(Vd, Vn, Vm)                                                                                           \
	(AL | (0b11100 << 23) | ((Vd & 1) << 22) | (0b11 << 20) | ((Vn >> 1) << 16) | ((Vd >> 1) << 12) | (0b101 << 9) |   \
	 (0 << 8) | ((Vn & 1) << 7) | (1 << 6) | ((Vm & 1) << 5) | (Vm >> 1))
#define VMUL_F32(Vd, Vn, Vm)                                                                                           \
	(AL | (0b11100 << 23) | ((Vd & 1) << 22) | (0b10 << 20) | ((Vn >> 1) << 16) | ((Vd >> 1) << 12) | (0b101) << 9 |   \
	 (0 << 8) | ((Vn & 1) << 7) | (0 << 6) | ((Vm & 1) << 5) | (Vm >> 1))
#define VDIV_F32(Vd, Vn, Vm)                                                                                           \
	(AL | (0b11101 << 23) | ((Vd & 1) << 22) | (0b00 << 20) | ((Vn >> 1) << 16) | ((Vd >> 1) << 12) | (0b101 << 9) |   \
	 (0 << 8) | ((Vn & 1) << 7) | (0 << 6) | ((Vm & 1) << 5) | (Vm >> 1))

#define _VCMP_F32(Vd, Vm, E)                                                                                           \
	(AL | (0b11101 << 23) | ((Vd & 1) << 22) | (0b11 << 20) | ((0b0100) << 16) | ((Vd >> 1) << 12) | (0b101 << 9) |    \
	 (0 << 8) | (E << 7) | (1 << 6) | ((Vm & 1) << 5) | (Vm >> 1))
...
#define CHECK_JUMP                                                                                                     \
	do {                                                                                                               \
		...
		emit(CMP(R0, R1));                                                                                             \
		if (branch == -1) branch = vm->codeLength;                                                                     \
		emit(cond(LT, Bi(j_rel(bytes_to_skip))));                                                                      \
		emit_MOVRxi_or_NOP(R12, (unsigned)ErrJump);                                                                    \
		emit(BLX(R12));                                                                                                \
		if (bytes_to_skip == -1) bytes_to_skip = vm->codeLength - branch;                                              \
	} while (0)
...
#define OFF_IMMEDIATES 1
	...
	for (pass = 0; pass < 2; ++pass) {
		...
		if (pass) {
			...
			if (vm->codeBase == MAP_FAILED) Com_Error(ERR_FATAL, "VM_CompileARM: can't mmap memory");
			vm->codeLength = 0;
		}
		...
		for (i_count = 0; i_count < header->instructionCount; i_count++) {
			...
			case OP_IGNORE: NOTIMPL(op); break;

			case OP_BREAK: emit(BKPT(0)); break;
			...
			case OP_CALL:
...
#endif
				{
					...
					if (start_block == -1) start_block = vm->codeLength;
					emit(cond(LT, Bi(j_rel(bytes_to_skip))));
					CHECK_JUMP;
					...
					if (bytes_to_skip == -1) bytes_to_skip = vm->codeLength - start_block;
					emit(MOV(R1, rPSTACK));
...
int VM_CallCompiled(vm_t *vm, int *args) {
	...
	*opStack = 0xDEADBEEF;
	...
	if (*opStack != 0xDEADBEEF) { Com_Error(ERR_DROP, "opStack corrupted in compiled code"); }
