...
ppc_instruction_t asm_instruction(powerpc_iname_t sname, const int argc, const long int *argv) {
	...
	while (opcode->operands[argi] != 0) {
		...
		if (operand->insert) {
			...
			if (errmsg) { printf("%s: error while inserting operand %d (0x%.2lx): %s\n", name, argi, op, errmsg); }
		} else {
...
static int valid_bo(long value, int dialect, int extract) {
	if ((dialect & PPC_OPCODE_POWER4) == 0) {
		...
		*/
		...
		case 0x4: valid = (value & 0x2) == 0; break;
		case 0x10: valid = (value & 0x8) == 0; break;
		case 0x14: valid = value == 0x14; break;
		}
		/* When disassembling with -Many, accept power4 encodings too.  */
		if (valid || (dialect & PPC_OPCODE_ANY) == 0 || !extract) return valid;
	}
...
static unsigned long insert_bo(unsigned long insn, long value, int dialect, const char **errmsg) {
	if (!valid_bo(value, dialect, 0)) *errmsg = _("invalid conditional option");
...
static unsigned long insert_ras(unsigned long insn, long value, int dialect ATTRIBUTE_UNUSED, const char **errmsg) {
	if (value == 0) *errmsg = _("invalid register operand when updating");
...
static unsigned long insert_rbs(unsigned long insn, long value ATTRIBUTE_UNUSED, int dialect ATTRIBUTE_UNUSED,
...
#define XO(op, xop, oe, rc)                                                                                            \
	(OP(op) | ((((unsigned long)(xop)) & 0x1ff) << 1) | ((((unsigned long)(oe)) & 1) << 10) |                          \
	 (((unsigned long)(rc)) & 1))
