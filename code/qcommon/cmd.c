...
void Cbuf_InsertText(const char *text) {
	...
	for (i = cmd_text.cursize - 1; i >= 0; i--) { cmd_text.data[i + len] = cmd_text.data[i]; }
...
char *Cmd_Args(void) {
	...
	for (i = 1; i < cmd_argc; i++) {
		...
		if (i != cmd_argc - 1) { strcat(cmd_args, " "); }
	}
...
void Cmd_Args_Sanitize(void) {
	...
	for (i = 1; i < cmd_argc; i++) {
		...
		if (strlen(c) > MAX_CVAR_VALUE_STRING - 1) c[MAX_CVAR_VALUE_STRING - 1] = '\0';
...
void Cmd_CompleteCfgName(char *args, int argNum) {
	if (argNum == 2) { Field_CompleteFilename("", "cfg", qfalse, qtrue); }
}
