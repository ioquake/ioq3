...
int COM_GetCurrentParseLine(void) {
	if (com_tokenline) { return com_tokenline; }
...
void COM_MatchToken(char **buf_p, char *match) {
	...
	if (strcmp(token, match)) { Com_Error(ERR_DROP, "MatchToken: %s != %s", token, match); }
}
...
void Parse2DMatrix(char **buf_p, int y, int x, float *m) {
	...
	for (i = 0; i < y; i++) { Parse1DMatrix(buf_p, x, m + i * x); }
...
void Parse3DMatrix(char **buf_p, int z, int y, int x, float *m) {
	...
	for (i = 0; i < z; i++) { Parse2DMatrix(buf_p, y, x, m + i * x * y); }
...
void Q_strncpyz(char *dest, const char *src, int destsize) {
	if (!dest) { Com_Error(ERR_FATAL, "Q_strncpyz: NULL dest"); }
	if (!src) { Com_Error(ERR_FATAL, "Q_strncpyz: NULL src"); }
	if (destsize < 1) { Com_Error(ERR_FATAL, "Q_strncpyz: destsize < 1"); }
...
int QDECL Com_sprintf(char *dest, int size, const char *fmt, ...) {
	...
	if (len >= size) Com_Printf("Com_sprintf: Output length %d too short, require %d bytes.\n", size, len + 1);
...
void Info_RemoveKey(char *s, const char *key) {
	...
	if (strlen(s) >= MAX_INFO_STRING) { Com_Error(ERR_DROP, "Info_RemoveKey: oversize infostring"); }
...
void Info_RemoveKey_Big(char *s, const char *key) {
	...
	if (strlen(s) >= BIG_INFO_STRING) { Com_Error(ERR_DROP, "Info_RemoveKey_Big: oversize infostring"); }
...
void Info_SetValueForKey(char *s, const char *key, const char *value) {
	...
	if (strlen(s) >= MAX_INFO_STRING) { Com_Error(ERR_DROP, "Info_SetValueForKey: oversize infostring"); }
...
void Info_SetValueForKey_Big(char *s, const char *key, const char *value) {
	...
	if (strlen(s) >= BIG_INFO_STRING) { Com_Error(ERR_DROP, "Info_SetValueForKey: oversize infostring"); }
...
char *Com_SkipTokens(char *s, int numTokens, char *sep) {
	...
	while (sepCount < numTokens) {
		if (Com_CharIsOneOfCharset(*p++, sep)) {
			sepCount++;
			while (Com_CharIsOneOfCharset(*p, sep)) p++;
		} else if (*p == '\0')
			break;
	}
