...
static void PPC_Append(dest_instruction_t *di_now, unsigned long int i_count) {
	...
	if (i_count != FALSE_ICOUNT) {
		if (!di_pointers[i_count]) di_pointers[i_count] = di_now;
	}
}
...
static void PPC_AppendInstructions(unsigned long int i_count, size_t num_instructions, const ppc_instruction_t *is) {
	if (num_instructions < 0) num_instructions = 0;
	...
	if (iBytes > 0) memcpy(&(di_now->code[0]), is, iBytes);
...
static void				PPC_PrepareJump(unsigned long int i_count, unsigned long int dest, long int bo, long int bi,
...
#define emitEnd()                                                                                                      \
	do {                                                                                                               \
		if (num_instructions || force_emit) PPC_AppendInstructions(i_count, num_instructions, instructions);           \
		num_instructions = 0;                                                                                          \
	} while (0)
...
#else
	fastMaskHi = 0;
	while ((mask & (0x80000000 >> fastMaskHi)) == 0) fastMaskHi++;

	fastMaskLo = 31;
	while ((mask & (0x80000000 >> fastMaskLo)) == 0) fastMaskLo--;
#endif
}
...
static void PPC_EmitConst(source_instruction_t *const i_const) {
	...
	if (!(i_const->regR & rTYPE_FLOAT)) {
		...
		if (i_const->arg.si >= -0x8000 && i_const->arg.si < 0x8000) {
			...
			if (i_const->arg.us[1] != 0) in(iORI, rFIRST, rFIRST, i_const->arg.us[1]);
		}
...
#define MAYBE_EMIT_CONST()                                                                                             \
	if (i_const) PPC_EmitConst(i_const)
 ...
 */
static void VM_AnalyzeFunction(source_instruction_t *const i_first, long int *prepareStack, long int *gpr_start_pos,
	...
	while ((i_now = i_now->next)) {
		...
		if (opi & opRetIF) {
			...
			i = i_now->regR & 2;
			needed_reg[i] += 1;
			if (max_reg[i] < needed_reg[i]) max_reg[i] = needed_reg[i];
	...
	if (max_reg[1] > 0 || max_reg[0] > gpr_volatile) {
		...
		if (max_reg[1] > max) max = max_reg[1];
		if (max > gpr_vstart) {
			/* error */
			DIE("Need more GPRs");
		}
	...
	*gpr_start_pos = gpr_start;
	...
	if (max_reg[3] > 0 || max_reg[2] > fpr_volatile) {
		...
		if (max_reg[3] > max) max = max_reg[3];
		if (max > fpr_vstart) {
			/* error */
			DIE("Need more FPRs");
		}
...
static void VM_CompileFunction(source_instruction_t *const i_first) {
	...
	{
		if (gpr_start_pos < gpr_vstart) save_space += (gpr_vstart - gpr_start_pos) * GPRLEN;
		save_space = (save_space + 15) & ~0x0f;

		if (fpr_start_pos < fpr_vstart) save_space += (fpr_vstart - fpr_start_pos) * FPRLEN;
		save_space = (save_space + 15) & ~0x0f;
	}
	...
	while ((i_now = i_now->next)) {
		...
		case OP_ENTER:
			if (i_const) DIE("Weird opcode order");
		...
		case OP_LEAVE:
			if (i_const) {
				...
				if (i_const->regR & rTYPE_FLOAT) DIE("constant float in OP_LEAVE");
				...
				if (i_const->arg.si >= -0x8000 && i_const->arg.si < 0x8000) {
					...
					if (i_const->arg.us[1] != 0) in(iORI, r3, r3, i_const->arg.us[1]);
				}
				gpr_pos--;
			} else {
				MAYBE_EMIT_CONST();
		...
		case OP_CALL:
			if (i_const) {
				...
				if (rFIRST != r3) in(iMR, r3, rFIRST); // push OPSTACK top value on argument list
				in(iMR, r4, rPSTACK);				   // push PSTACK on argument list
				...
				if (rFIRST != r3) in(iMR, rFIRST, r3); // push return value on the top of the opstack
			}
			break;
		...
		case OP_LOCAL:
			MAYBE_EMIT_CONST();
			{
				...
				if (hi == 0) {
					...
					if (lo != 0) in(iADDI, rFIRST, rFIRST, lo);
				}
			}
			break;
		...
		case OP_LOAD1: MAYBE_EMIT_CONST();
#if OPTIMIZE_MASK
		...
		case OP_LOAD2: MAYBE_EMIT_CONST();
#if OPTIMIZE_MASK
			in(iRLWINM, rFIRST, rFIRST, 0, fastMaskHi, fastMaskLo);
#else
			in(iLWZ, r0, VM_Data_Offset(dataMask), rVMDATA);
		...
		case OP_LOAD4: MAYBE_EMIT_CONST();
#if OPTIMIZE_MASK
			in(iRLWINM, rFIRST, rFIRST, 0, fastMaskHi, fastMaskLo);
#else
			in(iLWZ, r0, VM_Data_Offset(dataMask), rVMDATA);
		...
		case OP_STORE1: MAYBE_EMIT_CONST();
#if OPTIMIZE_MASK
		...
		case OP_STORE2: MAYBE_EMIT_CONST();
#if OPTIMIZE_MASK
			in(iRLWINM, rSECOND, rSECOND, 0, fastMaskHi, fastMaskLo);
#else
			in(iLWZ, r0, VM_Data_Offset(dataMask), rVMDATA);
		...
		case OP_BLOCK_COPY: MAYBE_EMIT_CONST();
#if OPTIMIZE_COPY
			if (i_now->arg.i <= SL(16, 32)) {
				...
				if (len >= GPRLEN) {
					...
					in(iLL, r0, 0, rFIRST);
					for (i = 1; i < words; i++) in(iLL, rTEMP(i - 1), GPRLEN * i, rFIRST);

					in(iSTL, r0, 0, rSECOND);
					for (i = 1; i < words; i++) in(iSTL, rTEMP(i - 1), GPRLEN * i, rSECOND);
				...
				if (left >= 4) { DIE("Bug in OP_BLOCK_COPY"); }
				if (left == 3) {
...
#endif
			{
				...
				if (r5_ori) in(iORI, r5, r5, r5_ori);

				in(iMTCTR, r0);

				if (rFIRST != r4) in(iMR, r4, rFIRST);
				if (rSECOND != r3) in(iMR, r3, rSECOND);
		...
		case OP_ADD:
			if (i_const) {
				...
				if (hi != 0) in(iADDIS, rSECOND, rSECOND, hi);
				if (lo != 0) in(iADDI, rSECOND, rSECOND, lo);

				// if both are zero no instruction will be written
				if (hi == 0 && lo == 0) force_emit = 1;
			} else {
				MAYBE_EMIT_CONST();
	...
	if (i_const) DIE("left (unused) OP_CONST");
...
static void PPC_ShrinkJumps(void) {
	...
	while ((sj_now = sj_now->nextJump)) {
		if (sj_now->bo == branchAlways)
			...
			if (!di) DIE("No instruction to jump to");
			if (ji->count > di->count) {
				do { jump_length += di->length; } while ((di = di->next) != ji);
			} else {
				jump_length = 1;
				while ((ji = ji->next) != di) jump_length += ji->length;
			}
...
static void PPC_ComputeCode(vm_t *vm) {
	...
	while ((di_now = di_now->next)) codeInstructions += di_now->length;

	size_t codeLength =
		sizeof(vm_data_t) + sizeof(unsigned int) * data_acc + sizeof(ppc_instruction_t) * codeInstructions;
	...
	if (dataAndCode == MAP_FAILED) DIE("Not enough memory");
	...
	while ((di_now = di_now->next)) {
		...
		if (i_count != FALSE_ICOUNT) {
			if (!di_pointers[i_count]) di_pointers[i_count] = (void *)codeNow;
		}
		...
		if (di_now->jump == NULL) {
			...
			long int		 i;
			symbolic_jump_t *sj;
			for (i = 0; i < di_now->length; i++) codeNow[i] = nop;
			codeNow += di_now->length;
	...
	while ((sj_now = sj_now->nextJump)) {
		...
		*jumpFrom = IN((sj_now->ext & branchExtLink ? iBL : iB), jumpLength * 4);
		if (sj_now->bo == branchAlways) continue;

		// there should have been additional space prepared for this case
		if (jumpFrom[-1] != nop) DIE("additional space for long jump not prepared");

		// invert instruction condition
		long int bo = 0;
		switch (sj_now->bo) {
		case branchTrue: bo = branchFalse; break;
		case branchFalse: bo = branchTrue; break;
		default: DIE("unrecognized branch type"); break;
		}
	...
	/* write dynamic data (float constants) */
	{
		...
		do {
			long int i;
			for (i = 0; i < d_now->count; i++) data->data[accumulated + i] = d_now->data[i];
	...
	/* free most of the compilation memory */
	{
		...
		while (di_now) {
			...
			if (di_now->jump) PPC_Free(di_now->jump);
			PPC_Free(di_now);
			di_now = di_first;
		}
	}
...
void VM_Compile(vm_t *vm, vmHeader_t *header) {
	...
	for (i_count = 0; i_count < header->instructionCount; ++i_count) {
		...
		if (op == OP_ENTER) {
			if (i_first->next) VM_CompileFunction(i_first);
			i_first->next = NULL;
			i_last		  = i_first;
		}
	...
	for (i = 0; i < header->instructionCount; i++)
		if (di_pointers[i] == 0) Com_Printf(S_COLOR_RED "Pointer %ld not initialized !\n", i);
#endif
...
int VM_CallCompiled(vm_t *vm, int *args) {
	...
	if (time_diff > 100) {
		printf("App clock: %ld, vm total: %ld, vm this: %ld, vm real: %ld, vm out: %ld\n"
			   ...
			   stop_time.tms_utime, time_total_vm, time_diff, time_diff - time_outside_vm, time_outside_vm,
