...
these, or some degree of prediction error will occur
...
typedef struct playerState_s {
	...
	int	torsoTimer;	// same as legsTimer
	int	torsoAnim;	// see legsAnim

	int	movementDir;	/* a number 0 to 7 that represents the relative angle of movement to the view
				angle (axial and diagonals) when at rest, the value will remain unchanged.
				It's used to twist the legs during strafing */
...
#define	BUTTON_WALKING		16	/* Walking can't just be inferred from MOVE_RUN because a key pressed late in the frame will only generate a
					small move value for that frame. Walking will use different animations and won't generate footsteps. */
...
/* entityState_t is the information conveyed from the server in an update message
about entities that the client will need to render in some way. Different eTypes
may use the information in different ways. The messages are delta compressed, so
it doesn't really matter if the structure size is fairly large */
