...
void MSG_Copy(msg_t *buf, byte *data, int length, msg_t *src) {
	if (length < src->cursize) { Com_Error(ERR_DROP, "MSG_Copy: can't copy into a smaller msg_t buffer"); }
...
void MSG_WriteBits(msg_t *msg, int value, int bits) {
	...
	if (bits == 0 || bits < -31 || bits > 32) { Com_Error(ERR_DROP, "MSG_WriteBits: bad bits %i", bits); }
...
int MSG_ReadBits(msg_t *msg, int bits) {
	...
	if (msg->oob) {
		...
		if (bits & 7) {
			...
			for (i = 0; i < nbits; i++) { value |= (Huff_getBit(msg->data, &msg->bit) << i); }
			bits = bits - nbits;
		}
		if (bits) {
			//			fp = fopen("c:\\netchan.bin", "a");
...
void MSG_WriteData(msg_t *buf, const void *data, int length) {
	int i;
	for (i = 0; i < length; i++) { MSG_WriteByte(buf, ((byte *)data)[i]); }
}
...
void MSG_WriteString(msg_t *sb, const char *s) {
	if (!s) {
		...
		for (i = 0; i < l; i++) {
			if (((byte *)string)[i] > 127 || string[i] == '%') { string[i] = '.'; }
		}
...
int MSG_ReadChar(msg_t *msg) {
	...
	if (msg->readcount > msg->cursize) { c = -1; }
...
float MSG_ReadFloat(msg_t *msg) {
	...
	if (msg->readcount > msg->cursize) { dat.f = -1; }
...
void MSG_ReadData(msg_t *msg, void *data, int len) {
	...
	for (i = 0; i < len; i++) { ((byte *)data)[i] = MSG_ReadByte(msg); }
}
...
#define LOG(x)                                                                                                         \
	if (cl_shownet && cl_shownet->integer == 4) { Com_Printf("%s ", x); };
...
int MSG_ReadDeltaKey(msg_t *msg, int key, int oldV, int bits) {
	if (MSG_ReadBits(msg, 1)) { return MSG_ReadBits(msg, bits) ^ (key & kbitmask[bits - 1]); }
	return oldV;
}
...
void MSG_ReportChangeVectors_f(void) {
	int i;
	for (i = 0; i < 256; i++) {
		if (pcount[i]) { Com_Printf("%d used %d\n", i, pcount[i]); }
	}
}
...
void MSG_ReadDeltaEntity(msg_t *msg, entityState_t *from, entityState_t *to, int number) {
	...
	if (lc > numFields || lc < 0) { Com_Error(ERR_DROP, "invalid entityState field count"); }
				...
				if (MSG_ReadBits(msg, 1) == 0) {
					*(float *)toF = 0.0f;
					...
					if (MSG_ReadBits(msg, 1) == 0) {
						...
						*(float *)toF = trunc;
						if (print) { Com_Printf("%s:%i ", field->name, trunc); }
					} else {
						// full floating point value
						*toF = MSG_ReadBits(msg, 32);
						if (print) { Com_Printf("%s:%f ", field->name, *(float *)toF); }
					}
				}
			} else {
				if (MSG_ReadBits(msg, 1) == 0) {
					...
					*toF = MSG_ReadBits(msg, field->bits);
					if (print) { Com_Printf("%s:%i ", field->name, *toF); }
				}
			}
			//			pcount[i]++;
		}
...
void MSG_WriteDeltaPlayerstate(msg_t *msg, struct playerState_s *from, struct playerState_s *to) {
	...
	for (i = 0; i < MAX_PERSISTANT; i++) {
		if (to->persistant[i] != from->persistant[i]) { persistantbits |= 1 << i; }
	}
	ammobits = 0;
	for (i = 0; i < MAX_WEAPONS; i++) {
		if (to->ammo[i] != from->ammo[i]) { ammobits |= 1 << i; }
	}
	powerupbits = 0;
	for (i = 0; i < MAX_POWERUPS; i++) {
		if (to->powerups[i] != from->powerups[i]) { powerupbits |= 1 << i; }
	}
	...
	if (ammobits) {
		...
		for (i = 0; i < MAX_WEAPONS; i++)
			if (ammobits & (1 << i)) MSG_WriteShort(msg, to->ammo[i]);
	} else {
		MSG_WriteBits(msg, 0, 1); // no change
	}
	...
	if (powerupbits) {
		...
		for (i = 0; i < MAX_POWERUPS; i++)
			if (powerupbits & (1 << i)) MSG_WriteLong(msg, to->powerups[i]);
	} else {
		MSG_WriteBits(msg, 0, 1); // no change
	}
}
...
void MSG_ReadDeltaPlayerstate(msg_t *msg, playerState_t *from, playerState_t *to) {
	...
	*to = *from;
	...
	if (lc > numFields || lc < 0) { Com_Error(ERR_DROP, "invalid playerState field count"); }
	...
	if (MSG_ReadBits(msg, 1)) {
		...
		if (MSG_ReadBits(msg, 1)) {
			...
			for (i = 0; i < MAX_STATS; i++) {
				if (bits & (1 << i)) { to->stats[i] = MSG_ReadShort(msg); }
			}
		}
		...
		if (MSG_ReadBits(msg, 1)) {
			...
			for (i = 0; i < MAX_PERSISTANT; i++) {
				if (bits & (1 << i)) { to->persistant[i] = MSG_ReadShort(msg); }
			}
		}
		...
		if (MSG_ReadBits(msg, 1)) {
			...
			for (i = 0; i < MAX_WEAPONS; i++) {
				if (bits & (1 << i)) { to->ammo[i] = MSG_ReadShort(msg); }
			}
		}
		...
		if (MSG_ReadBits(msg, 1)) {
			...
			for (i = 0; i < MAX_POWERUPS; i++) {
				if (bits & (1 << i)) { to->powerups[i] = MSG_ReadLong(msg); }
			}
		}
	}
