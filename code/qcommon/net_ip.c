...
static qboolean warned_ip_socket_fdset = qfalse;
static qboolean warned_ip6_socket_fdset = qfalse;
...
qboolean NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr)
{
	...
	socklen_t	fromlen;
	int		err;
	qboolean	checkAll = (fdr == NULL);

	if(ip_socket != INVALID_SOCKET && (checkAll || FD_ISSET(ip_socket, fdr)))
	{
		fromlen = sizeof(from);
	...
	if(ip6_socket != INVALID_SOCKET && (checkAll || FD_ISSET(ip6_socket, fdr)))
	{
		fromlen = sizeof(from);
	...
	if(multicast6_socket != INVALID_SOCKET && multicast6_socket != ip6_socket && (checkAll || FD_ISSET(multicast6_socket, fdr)))
	{
		fromlen = sizeof(from);
...
void NET_Config( qboolean enableNetworking ) {
	...
	warned_ip_socket_fdset = qfalse;
	warned_ip6_socket_fdset = qfalse;
...
void NET_Sleep(int msec)
{
	...
	qboolean hasOutOfRangeFd = qfalse;
	...
	if(ip_socket != INVALID_SOCKET)
	{
		...
		else if(!warned_ip_socket_fdset)
		{
			Com_Printf("Warning: ip_socket fd %d out of range for select() (FD_SETSIZE=%d), skipping until restart\n", (int)ip_socket, FD_SETSIZE);
			warned_ip_socket_fdset = qtrue;
		}
		hasOutOfRangeFd = qtrue;
	}
	if(ip6_socket != INVALID_SOCKET)
	{
		if(ip6_socket < FD_SETSIZE)
		{
		...
		else if(!warned_ip6_socket_fdset)
		{
			Com_Printf("Warning: ip6_socket fd %d out of range for select() (FD_SETSIZE=%d), skipping until restart\n", (int)ip6_socket, FD_SETSIZE);
			warned_ip6_socket_fdset = qtrue;
		}
		hasOutOfRangeFd = qtrue;
	}
