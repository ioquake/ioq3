...
qboolean Sys_StringToAdr(const char *s, netadr_t *a, netadrtype_t family) {
	...
	switch (family) {
	case NA_IP: fam = AF_INET; break;
	case NA_IP6: fam = AF_INET6; break;
	default: fam = AF_UNSPEC; break;
	}
	if (!Sys_StringToSockaddr(s, (struct sockaddr *)&sadr, sizeof(sadr), fam)) { return qfalse; }
...
qboolean NET_GetPacket(netadr_t *net_from, msg_t *net_message, fd_set *fdr) {
	...
	if (ip_socket != INVALID_SOCKET && FD_ISSET(ip_socket, fdr)) {
		fromlen = sizeof(from);
		ret =
			recvfrom(ip_socket, (void *)net_message->data, net_message->maxsize, 0, (struct sockaddr *)&from, &fromlen);
	...
	if (ip6_socket != INVALID_SOCKET && FD_ISSET(ip6_socket, fdr)) {
		fromlen = sizeof(from);
		ret		= recvfrom(ip6_socket, (void *)net_message->data, net_message->maxsize, 0, (struct sockaddr *)&from,
						   &fromlen);
	...
	if (multicast6_socket != INVALID_SOCKET && multicast6_socket != ip6_socket && FD_ISSET(multicast6_socket, fdr)) {
		fromlen = sizeof(from);
		ret = recvfrom(multicast6_socket, (void *)net_message->data, net_message->maxsize, 0, (struct sockaddr *)&from,
					   &fromlen);
...
void Sys_SendPacket(int length, const void *data, netadr_t to) {
	...
	if (ret == SOCKET_ERROR) {
		...
		if ((err == EADDRNOTAVAIL) && ((to.type == NA_BROADCAST))) { return; }
...
void NET_JoinMulticast6(void) {
	...
	if (IN6_IS_ADDR_MULTICAST(&boundto.sin6_addr) || IN6_IS_ADDR_UNSPECIFIED(&boundto.sin6_addr)) {
		// The way the socket was bound does not prohibit receiving multi-cast packets. So we don't need to open a new
		// one.
		multicast6_socket = ip6_socket;
	} else {
		if ((multicast6_socket = NET_IP6Socket(net_mcast6addr->string, ntohs(boundto.sin6_port), NULL, &err)) ==
			INVALID_SOCKET) {
			// If the OS does not support binding to multicast addresses, like WinXP, at least try with the normal file
			// descriptor.
			multicast6_socket = ip6_socket;
		}
	}
	...
	if (curgroup.ipv6mr_interface) {
		if (setsockopt(multicast6_socket, IPPROTO_IPV6, IPV6_MULTICAST_IF, (char *)&curgroup.ipv6mr_interface,
...
void NET_OpenSocks(int port) {
	...
	case 2: // username/password authentication
		break;
	default: Com_Printf("NET_OpenSocks: request denied\n"); return;
	}
	...
	if (buf[1] == 2) {
		...
		buf[1] = ulen;
		if (ulen) { memcpy(&buf[2], net_socksUsername->string, ulen); }
		buf[2 + ulen] = plen;
		if (plen) { memcpy(&buf[3 + ulen], net_socksPassword->string, plen); }
...
void NET_OpenIP(void) {
	...
	if (net_enabled->integer & NET_ENABLEV6) {
		...
		if (ip6_socket == INVALID_SOCKET) Com_Printf("WARNING: Couldn't bind to a v6 ip address.\n");
	}
	...
	if (net_enabled->integer & NET_ENABLEV4) {
		for (i = 0; i < 10; i++) {
			...
			if (ip_socket != INVALID_SOCKET) {
				...
				if (net_socksEnabled->integer) NET_OpenSocks(port + i);
...
void NET_Config(qboolean enableNetworking) {
	...
	if (!net_enabled->integer) { enableNetworking = 0; }

	// if enable state is the same and no cvars were modified, we have nothing to do
	if (enableNetworking == networkingEnabled && !modified) { return; }
	...
	if (stop) {
		...
		if (multicast6_socket != INVALID_SOCKET) {
			if (multicast6_socket != ip6_socket) closesocket(multicast6_socket);
...
void NET_Sleep(int msec) {
	...
	if (ip6_socket != INVALID_SOCKET) {
		...
		if (highestfd == INVALID_SOCKET || ip6_socket > highestfd) highestfd = ip6_socket;
	}
