...
#define ARG_RS1_RS2_RD                                                                                                 \
	{ ARG_RS1, ARG_RS2, ARG_RD }
#define ARG_RS1_SIMM13_RD                                                                                              \
	{ ARG_RS1, ARG_SIMM13, ARG_RD }
#define ARG_RS1_RS2                                                                                                    \
	{ ARG_RS1, ARG_RS2 }
#define ARG_RS2_RD                                                                                                     \
	{ ARG_RS2, ARG_RD }
...
#define FMT3F(op, op3, opf) ((op) << 30) | ((op3) << 19) | ((opf) << 5), (OP_MASK | OP3_MASK | OPF_MASK)
...
static struct dst_insn *dst_new(struct func_info *const fp, unsigned int length, struct jump_insn *jp, int insns_size) {
	...
	dp->next	= NULL;
	if (fp->saved_icount != THUNK_ICOUNT) fp->dst_by_i_count[fp->saved_icount] = dp;
...
static void dst_insn_append(struct func_info *const fp) {
	...
	if (insns_size) memcpy(&dp->code[0], fp->insn_buf, insns_size);
...
static void jump_insn_append(vm_t *vm, struct func_info *const fp, enum sparc_iname iname, int dest) {
	...
	if (dest < 0 || dest >= vm->instructionCount) ErrJump();
...
static void __do_emit_one(struct func_info *const fp, unsigned int insn) { fp->insn_buf[fp->insn_index++] = insn; }

#define in(inst, args...) __do_emit_one(fp, IN(inst, args))

static void end_emit(struct func_info *const fp) {
	if (fp->insn_index || fp->force_emit) dst_insn_append(fp);
}
...
static void blockcopy(unsigned int dest, unsigned int src, unsigned int count) {
	...
	if ((dest & dataMask) != dest || (src & dataMask) != src || ((dest + count) & dataMask) != dest + count ||
...
static void compile_one_insn(vm_t *vm, struct func_info *const fp, struct src_insn *sp) {
	...
	case OP_LEAVE:
		if (fp->cached_const && SIMM13_P(fp->cached_const->arg.si)) {
			EMIT_FALSE_CONST(fp);
			if (fp->cached_const->src1_reg_flags & REG_FLAGS_FLOAT) DIE("constant float in OP_LEAVE");
	...
	case OP_LOCAL:
		...
		break;
	case OP_CONST: MAYBE_EMIT_CONST(fp); break;
	case OP_LOAD4:
		MAYBE_EMIT_CONST(fp);
...
static void compile_function(vm_t *vm, struct func_info *const fp) {
	...
	sp = fp->first;
	while ((sp = sp->next) != NULL) compile_one_insn(vm, fp, sp);
...
static void sparc_compute_code(vm_t *vm, struct func_info *const fp) {
	...
	if (data_and_code == MAP_FAILED) DIE("Not enough memory");
	...
	while (dp) {
		...
		if (i_count != THUNK_ICOUNT) {
			if (!fp->dst_by_i_count[i_count]) fp->dst_by_i_count[i_count] = (void *)code_now;
		}
		if (!dp->jump) {
			...
			for (i = 0; i < dp->length; i++) code_now[i] = SPARC_NOP;
			code_now += dp->length;
		}
	...
	while (dhp) {
		...
		for (i = 0; i < dhp->count; i++) data->data[off + i] = dhp->data[i];
...
void VM_Compile(vm_t *vm, vmHeader_t *header) {
	...
	for (i_count = 0; i_count < header->instructionCount; i_count++) {
		...
		if (op == OP_CALL || op == OP_BLOCK_COPY) fi.has_call = 1;
		opi = vm_opInfo[op];
		if (op == OP_CVIF || op == OP_CVFI || (op == OP_LEAVE && (opi & opArgF))) fi.need_float_tmp = 1;

		if (op == OP_ENTER) {
			if (fi.first->next) compile_function(vm, &fi);
			fi.first->next = NULL;
			fi.last		   = fi.first;
