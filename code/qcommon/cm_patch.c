...
static qboolean CM_PlaneFromPoints(vec4_t plane, vec3_t a, vec3_t b, vec3_t c) {
	...
	if (VectorNormalize(plane) == 0) { return qfalse; }
...
static qboolean CM_NeedsSubdivision(vec3_t a, vec3_t b, vec3_t c) {
	...
	for (i = 0; i < 3; i++) { lmid[i] = 0.5 * (a[i] + c[i]); }

	// calculate the exact curve midpoint
	for (i = 0; i < 3; i++) { cmid[i] = 0.5 * (0.5 * (a[i] + b[i]) + 0.5 * (b[i] + c[i])); }
...
static void CM_SubdivideGridColumns(cGrid_t *grid) {
	...
	for (i = 0; i < grid->width - 2;) {
		...
		if (j == grid->height) {
			...
			for (j = 0; j < grid->height; j++) {
				// remove the column
				for (k = i + 2; k < grid->width; k++) { VectorCopy(grid->points[k][j], grid->points[k - 1][j]); }
			}
		...
		for (j = 0; j < grid->height; j++) {
			...
			for (k = grid->width - 1; k > i + 1; k--) { VectorCopy(grid->points[k][j], grid->points[k + 2][j]); }
...
static qboolean CM_ComparePoints(float *a, float *b) {
	...
	d = a[0] - b[0];
	if (d < -POINT_EPSILON || d > POINT_EPSILON) { return qfalse; }
	d = a[1] - b[1];
	if (d < -POINT_EPSILON || d > POINT_EPSILON) { return qfalse; }
	d = a[2] - b[2];
	if (d < -POINT_EPSILON || d > POINT_EPSILON) { return qfalse; }
...
static int CM_PointOnPlaneSide(float *p, int planeNum) {
	...
	if (d > PLANE_TRI_EPSILON) { return SIDE_FRONT; }

	if (d < -PLANE_TRI_EPSILON) { return SIDE_BACK; }
...
static qboolean CM_ValidateFacet(facet_t *facet) {
	...
	if (facet->surfacePlane == -1) { return qfalse; }
	...
	for (j = 0; j < 3; j++) {
		...
		if (bounds[0][j] >= MAX_MAP_BOUNDS) { return qfalse; }
		if (bounds[1][j] <= -MAX_MAP_BOUNDS) { return qfalse; }
	}
...
static void CM_PatchCollideFromGrid(cGrid_t *grid, patchCollide_t *pf) {
	...
	for (i = 0; i < grid->width - 1; i++) {
		for (j = 0; j < grid->height - 1; j++) {
			...
			if (numFacets == MAX_FACETS) { Com_Error(ERR_DROP, "MAX_FACETS"); }
...
struct patchCollide_s *CM_GeneratePatchCollide(int width, int height, vec3_t *points) {
	...
	for (i = 0; i < width; i++) {
		for (j = 0; j < height; j++) { VectorCopy(points[j * width + i], grid.points[i][j]); }
	}
	...
	for (i = 0; i < grid.width; i++) {
		for (j = 0; j < grid.height; j++) { AddPointToBounds(grid.points[i][j], pf->bounds[0], pf->bounds[1]); }
	}
...
#ifndef BSPC
	if (!cm_playerCurveClip->integer || !tw->isPoint) { return; }
#endif
	...
	for (i = 0; i < pc->numFacets; i++, facet++) {
		...
		for (j = 0; j < facet->numBorders; j++) {
			...
			if (frontFacing[k] ^ facet->borderInward[j]) {
				if (intersection[k] > intersect) { break; }
			} else {
				if (intersection[k] < intersect) { break; }
			}
		}
		if (j == facet->numBorders) {
			// we hit this facet
#ifndef BSPC
			if (!cv) { cv = Cvar_Get("r_debugSurfaceUpdate", "1", 0); }
			if (cv->integer) {
...
int CM_CheckFacetPlane(float *plane, vec3_t start, vec3_t end, float *enterFrac, float *leaveFrac, int *hit) {
	...
	*hit = qfalse;
	...
	if (d1 > 0 && (d2 >= SURFACE_CLIP_EPSILON || d2 >= d1)) { return qfalse; }

	// if it doesn't cross the plane, the plane isn't relevant
	if (d1 <= 0 && d2 <= 0) { return qtrue; }
...
qboolean CM_PositionTestInPatchCollide(traceWork_t *tw, const struct patchCollide_s *pc) {
	...
	for (i = 0; i < pc->numFacets; i++, facet++) {
		...
		if (DotProduct(plane, startp) - plane[3] > 0.0f) { continue; }
		...
		for (j = 0; j < facet->numBorders; j++) {
			...
			if (DotProduct(plane, startp) - plane[3] > 0.0f) { break; }
		}
		if (j < facet->numBorders) { continue; }
...
#ifndef BSPC
	if (!cv2) { cv2 = Cvar_Get("r_debugSurface", "0", 0); }
...
#ifndef BSPC
	if (!cv) { cv = Cvar_Get("cm_debugSize", "2", 0); }
#endif
