...
static cvar_t *Cvar_FindVar(const char *var_name) {
	...
	for (var = hashTable[hash]; var; var = var->hashNext) {
		if (!Q_stricmp(var_name, var->name)) { return var; }
	}
...
static const char *Cvar_Validate(cvar_t *var, const char *value, qboolean warn) {
	...
	if (changed) {
		if (Q_isintegral(valuef)) {
			...
			if (warn) Com_Printf(", setting to %d\n", (int)valuef);
		} else {
			Com_sprintf(s, sizeof(s), "%f", valuef);

			if (warn) Com_Printf(", setting to %f\n", valuef);
		}
...
cvar_t *Cvar_Get(const char *var_name, const char *var_value, int flags) {
	...
	if (var) {
		...
		if (var->flags & CVAR_VM_CREATED) {
			if (!(flags & CVAR_VM_CREATED)) var->flags &= ~CVAR_VM_CREATED;
		} else if (!(var->flags & CVAR_USER_CREATED)) {
			if (flags & CVAR_VM_CREATED) flags &= ~CVAR_VM_CREATED;
		}
		...
		if (var->flags & CVAR_SERVER_CREATED) {
			if (!(flags & CVAR_SERVER_CREATED)) var->flags &= ~CVAR_SERVER_CREATED;
		} else {
...
cvar_t *Cvar_Set2(const char *var_name, const char *value, qboolean force) {
	...
	if ((var->flags & CVAR_LATCH) && var->latchedString) {
		...
		if (!strcmp(value, var->latchedString)) return var;
...
void Cvar_WriteVariables(fileHandle_t f) {
	...
	for (var = cvar_vars; var; var = var->next) {
		if (!var->name || Q_stricmp(var->name, "cl_cdkey") == 0) continue;
...
char *Cvar_InfoString(int bit) {
	...
	for (var = cvar_vars; var; var = var->next) {
		if (var->name && (var->flags & bit)) Info_SetValueForKey(info, var->name, var->string);
	}
...
char *Cvar_InfoString_Big(int bit) {
	...
	for (var = cvar_vars; var; var = var->next) {
		if (var->name && (var->flags & bit)) Info_SetValueForKey_Big(info, var->name, var->string);
	}
	return info;
}
...
void Cvar_Update(vmCvar_t *vmCvar) {
	...
	if ((unsigned)vmCvar->handle >= cvar_numIndexes) { Com_Error(ERR_DROP, "Cvar_Update: handle out of range"); }
