...
static int NextConstant4(void) {
	return ((unsigned int)code[pc] | ((unsigned int)code[pc + 1] << 8) | ((unsigned int)code[pc + 2] << 16) |
...
#define SET_JMPOFS(x)                                                                                                  \
	do { buf[(x)] = compiledOfs - ((x) + 1); } while (0)
...
static void DoSyscall(void) {
	...
	if (vm_syscallNum < 0) {
		...
		for (index = 1; index < ARRAY_LEN(args); index++) args[index] = data[index];
		...
		*ret	= savedVM->systemCall((intptr_t *)data);
		...
		case VM_JMP_VIOLATION: ErrJump(); break;
		case VM_BLOCK_COPY:
			...
			break;
		default: Com_Error(ERR_DROP, "Unknown VM operation %d", vm_syscallNum); break;
		}
	}
...
void VM_Compile(vm_t *vm, vmHeader_t *header) {
	...
	for (i = 0; i < vm->numJumpTableTargets; i++) { JUSED(*(int *)(vm->jumpTableTargets + (i * sizeof(int)))); }
	...
	for (pass = 0; pass < 3; pass++) {
		...
		while (instruction < header->instructionCount) {
			...
			case OP_ARG:
				...
				EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
				break;
			case OP_CALL: EmitCallRel(vm, callProcOfs); break;
			case OP_PUSH: EmitPushStack(vm); break;
			case OP_POP:
				EmitCommand(LAST_COMMAND_SUB_BL_1); // sub bl, 1
				break;
			case OP_LEAVE:
			...
			case OP_LOAD4:
				...
				EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi + ebx * 4], eax
				break;
			case OP_LOAD2: EmitMovEAXStack(vm, vm->dataMask);
#if idx64
				...
				EmitCommand(LAST_COMMAND_MOV_STACK_EAX); // mov dword ptr [edi + ebx * 4], eax
				break;
			case OP_LOAD1: EmitMovEAXStack(vm, vm->dataMask);
#if idx64
			...
			case OP_JUMP:
				...
				break;
			default: VMFREE_BUFFERS(); Com_Error(ERR_DROP, "VM_CompileX86: bad opcode %i at offset %i", op, pc);
			}
			pop0 = pop1;
			pop1 = op;
		}
	...
	for (i = 0; i < header->instructionCount; i++) { vm->instructionPointers[i] += (intptr_t)vm->codeBase; }
}
