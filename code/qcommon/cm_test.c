...
int CM_PointContents(const vec3_t p, clipHandle_t model) {
	...
	for (k = 0; k < leaf->numLeafBrushes; k++) {
		...
		for (i = 0; i < b->numsides; i++) {
			...
			if (d > b->sides[i].plane->dist) { break; }
		}
...
#ifndef BSPC
	if (cm_noAreas->integer) { return qtrue; }
#endif

	if (area1 < 0 || area2 < 0) { return qfalse; }

	if (area1 >= cm.numAreas || area2 >= cm.numAreas) { Com_Error(ERR_DROP, "area >= cm.numAreas"); }

	if (cm.areas[area1].floodnum == cm.areas[area2].floodnum) { return qtrue; }
	return qfalse;
}
