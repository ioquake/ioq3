...
void Netchan_Transmit(netchan_t *chan, int length, const byte *data) {
	...
	if (length > MAX_MSGLEN) { Com_Error(ERR_DROP, "Netchan_Transmit: length = %i", length); }
...
qboolean Netchan_Process(netchan_t *chan, msg_t *msg) {
	...
	if (chan->sock == NS_SERVER) { MSG_ReadShort(msg); }
...
#endif
	{
		...
		if (NETCHAN_GENCHECKSUM(chan->challenge, sequence) != checksum) return qfalse;
	}
	...
	if (fragmented) {
		...
		if (fragmentLength == FRAGMENT_SIZE) { return qfalse; }
...
qboolean NET_GetLoopPacket(netsrc_t sock, netadr_t *net_from, msg_t *net_message) {
	...
	if (loop->get >= loop->send) return qfalse;
...
void NET_SendPacket(netsrc_t sock, int length, const void *data, netadr_t to) {
	...
	if (showpackets->integer && *(int *)data == -1) { Com_Printf("send packet %4i\n", length); }
...
void QDECL NET_OutOfBandData(netsrc_t sock, netadr_t adr, byte *format, int len) {
	...
	for (i = 0; i < len; i++) { string[i + 4] = format[i]; }
