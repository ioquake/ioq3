...
char *Com_MD5File(const char *fn, int length, const char *prefix, int prefix_len) {
	...
	if (filelen < length || !length) { length = filelen; }

	MD5Init(&md5);

	if (prefix_len && *prefix) MD5Update(&md5, (unsigned char *)prefix, prefix_len);
	...
	for (;;) {
		...
		if (r < sizeof(buffer) || total >= length) break;
	}
	FS_FCloseFile(f);
	MD5Final(&md5, digest);
	final[0] = '\0';
	for (i = 0; i < 16; i++) { Q_strcat(final, sizeof(final), va("%02X", digest[i])); }
	return final;
}
