...
void R_LoadTGA(const char *name, byte **pic, int *width, int *height) {
	...
	*pic = NULL;
	...
	if (length < 18) { ri.Error(ERR_DROP, "LoadTGA: header too short (%s)", name); }
	...
	if (targa_header.image_type != 2 && targa_header.image_type != 10 && targa_header.image_type != 3) {
		ri.Error(ERR_DROP, "LoadTGA: Only type 2 (RGB), 3 (gray), and 10 (RGB) TGA "
						   "images supported");
	}
	...
	if (targa_header.id_length != 0) {
		if (buf_p + targa_header.id_length > end) ri.Error(ERR_DROP, "LoadTGA: header too short (%s)", name);
	...
	if (targa_header.image_type == 2 || targa_header.image_type == 3) {
		...
		for (row = rows - 1; row >= 0; row--) {
			...
			for (column = 0; column < columns;) {
				if (buf_p + 1 > end) ri.Error(ERR_DROP, "LoadTGA: file truncated (%s)", name);
				packetHeader = *buf_p++;
				...
				if (packetHeader & 0x80) { // run-length packet
					...
					for (j = 0; j < packetSize; j++) {
						...
						default:
							ri.Error(ERR_DROP,
									 "LoadTGA: illegal pixel_size '%d' in "
									 "file '%s'",
	...
	// TTimo: this is the chunk of code to ensure a behavior that meets TGA
	// specs bit 5 set => top-down
