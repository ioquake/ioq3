...
static void CloseBufferedFile(struct BufferedFile *BF) {
	if (BF) {
		if (BF->Buffer) { ri.FS_FreeFile(BF->Buffer); }
...
static void *BufferedFileRead(struct BufferedFile *BF, unsigned Length) {
	...
	if (Length > BF->BytesLeft) { return (NULL); }
...
static qboolean BufferedFileRewind(struct BufferedFile *BF, unsigned Offset) {
	...
	if (Offset > BytesRead) { return (qfalse); }
...
static qboolean BufferedFileSkip(struct BufferedFile *BF, unsigned Offset) {
	...
	if (Offset > BF->BytesLeft) { return (qfalse); }
...
static qboolean FindChunk(struct BufferedFile *BF, uint32_t ChunkType) {
	...
	while (qtrue) {
		...
		if (Type == ChunkType) {
			...
			if (Length) {
				if (!BufferedFileSkip(BF, Length + PNG_ChunkCRC_Size)) { return (qfalse); }
			}
		}
	}
...
static uint32_t DecompressIDATs(struct BufferedFile *BF, uint8_t **Buffer) {
	...
	*Buffer			 = DecompressedData;
	...
	if (!FindChunk(BF, PNG_ChunkType_IDAT)) { return (-1); }
...
static qboolean UnfilterImage(uint8_t *DecompressedData, uint32_t ImageHeight, uint32_t BytesPerScanline,
	...
	if (!(DecompressedData && BytesPerPixel)) { return (qfalse); }

	/*
	 *  ImageHeight and BytesPerScanline can be zero in small interlaced images.
	 */

	if ((!ImageHeight) || (!BytesPerScanline)) { return (qtrue); }
	...
	for (h = 0; h < ImageHeight; h++) {
		...
		for (w = 0; w < (BytesPerScanline / BytesPerPixel); w++) {
			...
			if (h > 0) { PixelUpLeft = DecompPtr - (BytesPerScanline + 1); }
			...
			if (h > 0) { PixelUp = DecompPtr - (BytesPerScanline + 1); }
		}
	}
...
static qboolean ConvertPixel(struct PNG_Chunk_IHDR *IHDR, byte *OutPtr, uint8_t *DecompPtr,
							 qboolean HasTransparentColour, uint8_t *TransparentColour, uint8_t *OutPal) {
	/*
	 *  input verification
	 */

	if (!(IHDR && OutPtr && DecompPtr && TransparentColour && OutPal)) { return (qfalse); }
	...
	case PNG_ColourType_Grey: {
		...
		case PNG_BitDepth_4: {
			...
			if (HasTransparentColour) {
				if (TransparentColour[1] == DecompPtr[0]) { OutPtr[3] = 0x00; }
			}
	...
	case PNG_ColourType_True: {
		...
		case PNG_BitDepth_8: {
			...
			if (HasTransparentColour) {
				if ((TransparentColour[1] == DecompPtr[0]) && (TransparentColour[3] == DecompPtr[1]) &&
...
static qboolean DecodeImageNonInterlaced(struct PNG_Chunk_IHDR *IHDR, byte *OutBuffer, uint8_t *DecompressedData,
	...
	if (!(DecompressedDataLength == ((BytesPerScanline + 1) * IHDR_Height))) { return (qfalse); }

	/*
	 *  Unfilter the image.
	 */

	if (!UnfilterImage(DecompressedData, IHDR_Height, BytesPerScanline, BytesPerPixel)) { return (qfalse); }
			...
			if (PixelsPerByte > 1) {
				...
				for (p = 0; p < PixelsPerByte; p++) {
					if (CurrPixel < IHDR_Width) {
						...
						if (!ConvertPixel(IHDR, OutPtr, &SinglePixel, HasTransparentColour, TransparentColour,
										  OutPal)) {
							return (qfalse);
						}
...
static qboolean DecodeImageInterlaced(struct PNG_Chunk_IHDR *IHDR, byte *OutBuffer, uint8_t *DecompressedData,
									  uint32_t DecompressedDataLength, qboolean HasTransparentColour,
									  uint8_t *TransparentColour, uint8_t *OutPal) {
	uint32_t IHDR_Width;
	uint32_t IHDR_Height;
	...
	uint32_t WSkip[PNG_Adam7_NumPasses], WOffset[PNG_Adam7_NumPasses], HSkip[PNG_Adam7_NumPasses],
		HOffset[PNG_Adam7_NumPasses];
	uint32_t w, h, p, a;
	byte	*OutPtr;
	uint8_t *DecompPtr;
	...
	if (!(DecompressedDataLength == TargetLength)) { return (qfalse); }
	...
	for (a = 0; a < PNG_Adam7_NumPasses; a++) {
		if (!UnfilterImage(DecompPtr, PassHeight[a], BytesPerScanline[a], BytesPerPixel)) { return (qfalse); }
	...
	for (a = 0; a < PNG_Adam7_NumPasses; a++) {
		for (h = 0; h < PassHeight[a]; h++) {
			...
			if (BytesPerScanline[a]) { DecompPtr++; }
			...
			for (w = 0; w < (BytesPerScanline[a] / BytesPerPixel); w++) {
				if (PixelsPerByte > 1) {
					...
					OutPtr =
						OutBuffer + (((((h * HSkip[a]) + HOffset[a]) * IHDR_Width) + ((w * WSkip[a]) + WOffset[a])) *
									 Q3IMAGE_BYTESPERPIXEL);
...
void R_LoadPNG(const char *name, byte **pic, int *width, int *height) {
	...
	*pic = NULL;
	...
	if (FindChunk(ThePNG, PNG_ChunkType_tRNS)) {
		...
		case PNG_ColourType_Indexed: {
			...
			for (i = 0; i < ChunkHeaderLength; i++) { OutPal[i * Q3IMAGE_BYTESPERPIXEL + 3] = Trans[i]; }
	...
	case PNG_InterlaceMethod_NonInterlaced: {
		if (!DecodeImageNonInterlaced(IHDR, OutBuffer, DecompressedData, DecompressedDataLength, HasTransparentColour,
	...
	case PNG_InterlaceMethod_Interlaced: {
		if (!DecodeImageInterlaced(IHDR, OutBuffer, DecompressedData, DecompressedDataLength, HasTransparentColour,
								   TransparentColour, OutPal)) {
			ri.Free(OutBuffer);
			ri.Free(DecompressedData);
			CloseBufferedFile(ThePNG);
	...
	*pic = OutBuffer;
	...
	if (width) { *width = IHDR_Width; }

	if (height) { *height = IHDR_Height; }
