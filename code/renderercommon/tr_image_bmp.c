...
void R_LoadBMP(const char *name, byte **pic, int *width, int *height) {
	...
	*pic = NULL;
	...
	if (!buffer.b || length < 0) { return; }

	if (length < 54) { ri.Error(ERR_DROP, "LoadBMP: header too short (%s)", name); }
	...
	if (bmpHeader.bitsPerPixel == 8) {
		if (buf_p + sizeof(bmpHeader.palette) > end) ri.Error(ERR_DROP, "LoadBMP: header too short (%s)", name);
	...
	if (bmpHeader.fileSize != length) {
		ri.Error(ERR_DROP, "LoadBMP: header size does not match file size (%u vs. %u) (%s)", bmpHeader.fileSize, length,
				 name);
	}
	if (bmpHeader.compression != 0) { ri.Error(ERR_DROP, "LoadBMP: only uncompressed BMP files supported (%s)", name); }
	if (bmpHeader.bitsPerPixel < 8) {
	...
	case 24:
	case 32: break;
	default: ri.Error(ERR_DROP, "LoadBMP: illegal pixel_size '%hu' in file '%s'", bmpHeader.bitsPerPixel, name); break;
	}
