...
char *Sys_GetCurrentUser(void) {
	...
	if ((p = getpwuid(getuid())) == NULL) { return "player"; }
	return p->pw_name;
}
...
FILE *Sys_FOpen(const char *ospath, const char *mode) {
	...
	if (!stat(ospath, &buf) && S_ISDIR(buf.st_mode)) return NULL;
...
FILE *Sys_Mkfifo(const char *ospath) {
	...
	if (!stat(ospath, &buf) && S_ISFIFO(buf.st_mode)) FS_Remove(ospath);
...
char **Sys_ListFiles(const char *directory, const char *extension, char *filter, int *numfiles, qboolean wantsubs) {
	...
	while ((d = readdir(fdir)) != NULL) {
		...
		if ((dironly && !(st.st_mode & S_IFDIR)) || (!dironly && (st.st_mode & S_IFDIR))) continue;
...
static void Sys_AppendToExecBuffer(const char *text) {
	...
	if (length > size || execArgc >= ARRAY_LEN(execArgv)) return;
...
dialogResult_t Sys_Dialog(dialogType_t type, const char *message, const char *title) {
	typedef enum { NONE = 0, ZENITY, KDIALOG, XMESSAGE, NUM_DIALOG_PROGRAMS } dialogCommandType_t;
	...
	for (i = NONE + 1; i < NUM_DIALOG_PROGRAMS; i++) {
		if (preferredCommandType != NONE && preferredCommandType != i) continue;
...
qboolean Sys_DllExtension(const char *name) {
	...
	if (COM_CompareExtension(name, DLL_EXT)) { return qtrue; }

#ifdef __APPLE__
	// Allow system frameworks without dylib extensions
	// i.e., /System/Library/Frameworks/OpenAL.framework/OpenAL
	if (strncmp(name, "/System/Library/Frameworks/", 27) == 0) { return qtrue; }
#endif
