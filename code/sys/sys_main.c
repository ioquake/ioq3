...
static char *Sys_PIDFileName(const char *gamedir) {
	...
	if (*homePath == '\0') {
		Com_Printf(S_COLOR_YELLOW "Error: fs_homepath is not set.\n");
		return NULL;
	}

	char *pidFileName = va("%s/%s/%s", homePath, gamedir, PID_FILENAME);
	Com_Printf("PID file name: %s\n", pidFileName);
...
static qboolean Sys_WritePIDFile(const char *gamedir) {
	...
	if (pidFile == NULL) {
		Com_Printf(S_COLOR_YELLOW "Failed to get PID file name\n");
		return qfalse;
	}
	...
	if (FS_CreatePath(pidFile)) {
		Com_Printf(S_COLOR_YELLOW "Failed to create path for PID file: %s\n", pidFile);
		return qfalse;
	}
	...
	if ((f = fopen(pidFile, "w")) != NULL) {
		...
		Com_Printf(S_COLOR_YELLOW "Failed to open PID file for writing: %s\n", pidFile);
	}
...
void Sys_InitPIDFile(const char *gamedir) {
	if (Sys_WritePIDFile(gamedir)) {
		...
		if (Sys_Dialog(DT_YES_NO, message, "Abnormal Exit") == DR_YES) { Cvar_Set("com_abnormalExit", "1"); }
#endif
	}
}
...
=================
*/
void *Sys_LoadGameDll(const char *name, vmMainProc *entryPoint, int32_t (*systemcalls)(intptr_t, ...)) {
	void *libHandle;
	void (*dllEntry)(int32_t(*syscallptr)(intptr_t, ...));
...
#define MINSDL_VERSION XSTRING(MINSDL_MAJOR) "." XSTRING(MINSDL_MINOR) "." XSTRING(MINSDL_PATCH)
	...
	if (argc >= 2 && Q_strncmp(argv[1], "-psn", 4) == 0) argc = 1;
#endif
	...
	for (int i = argc-1; i > 0; i--) {
		...
		if (containsSpaces) Q_strcat(commandLine, sizeof(commandLine), "\"");
