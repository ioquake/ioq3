...
static WORD CON_ColorCharToAttrib(char color) {
	...
	if (color == COLOR_WHITE) {
		...
		attrib = (rgba[0] >= 0.5 ? FOREGROUND_RED : 0) | (rgba[1] >= 0.5 ? FOREGROUND_GREEN : 0) |
				 (rgba[2] >= 0.5 ? FOREGROUND_BLUE : 0) | (rgba[3] >= 0.5 ? FOREGROUND_INTENSITY : 0);
...
static void CON_HistAdd(void) {
	...
	if (qconsole_history_lines < QCONSOLE_HISTORY) qconsole_history_lines++;
...
static void CON_HistPrev(void) {
	...
	if (pos == qconsole_history_oldest || pos >= qconsole_history_lines) return;
...
static void CON_HistNext(void) {
	...
	// don' t allow looping through history
	if (qconsole_history_pos == qconsole_history_oldest) return;
...
static void CON_Show(void) {
	...
	if (qconsole_linelen > binfo.srWindow.Right) {
		WriteConsoleOutput(qconsole_hout, line + (qconsole_linelen - binfo.srWindow.Right), writeSize, writePos,
						   &writeArea);
	} else {
	...
	cursorPos.X = qconsole_cursor < qconsole_linelen		? qconsole_cursor
				  : qconsole_linelen > binfo.srWindow.Right ? binfo.srWindow.Right
															: qconsole_linelen;
...
void CON_Init(void) {
	...
	if (qconsole_hin == INVALID_HANDLE_VALUE) return;

	qconsole_hout = GetStdHandle(STD_OUTPUT_HANDLE);
	if (qconsole_hout == INVALID_HANDLE_VALUE) return;
	...
	qconsole_backgroundAttrib =
		qconsole_attrib & (BACKGROUND_BLUE | BACKGROUND_GREEN | BACKGROUND_RED | BACKGROUND_INTENSITY);
...
char *CON_Input(void) {
	...
	if (!GetNumberOfConsoleInputEvents(qconsole_hin, &events)) return NULL;
	...
	if (!ReadConsoleInput(qconsole_hin, buff, events, &count)) return NULL;

	FlushConsoleInputBuffer(qconsole_hin);

	for (i = 0; i < count; i++) {
		if (buff[i].EventType != KEY_EVENT) continue;
		if (!buff[i].Event.KeyEvent.bKeyDown) continue;
		...
		if (key == VK_RETURN) {
			...
			qconsole_cursor--;
			if (qconsole_cursor < 0) { qconsole_cursor = 0; }
			break;
		} else if (key == VK_RIGHT) {
			qconsole_cursor++;
			if (qconsole_cursor > qconsole_linelen) { qconsole_cursor = qconsole_linelen; }
			break;
		} else if (key == VK_HOME) {
			qconsole_cursor = 0;
			break;
			...
			if (key == VK_BACK) {
				if (qconsole_cursor > 0) {
					...
					if (qconsole_cursor < qconsole_linelen) {
						memmove(qconsole_line + qconsole_cursor - 1, qconsole_line + qconsole_cursor,
