...
char *Sys_DefaultHomePath(void) {
	...
	PFN_SHGetFolderPathA qSHGetFolderPath;
	...
	if (!*homePath && com_homepath) {
		qSHGetFolderPath = (PFN_SHGetFolderPathA)GetProcAddress(shfolder, "SHGetFolderPathA");
...
char *Sys_SteamPath(void) {
	...
	if (!steamPath[0] &&
		!RegOpenKeyEx(HKEY_LOCAL_MACHINE,
					  "SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\Steam App " STEAMPATH_APPID, 0,
	...
	if (steamPath[0]) {
		if (pathLen == MAX_OSPATH) pathLen--;

		steamPath[pathLen] = '\0';

		if (finishPath) Q_strcat(steamPath, MAX_OSPATH, "\\SteamApps\\common\\" STEAMPATH_NAME);
	}
#endif
...
char *Sys_GogPath(void) {
	...
	if (!gogPath[0] && !RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\GOG.com\\Games\\" GOGPATH_ID, 0,
									 KEY_QUERY_VALUE | KEY_WOW64_32KEY, &gogRegKey)) {
		pathLen = MAX_OSPATH;
		if (RegQueryValueEx(gogRegKey, "PATH", NULL, NULL, (LPBYTE)gogPath, &pathLen)) gogPath[0] = '\0';
...
char *Sys_MicrosoftStorePath(void) {
	...
	if (!microsoftStorePath[0]) {
		...
		Com_sprintf(microsoftStorePath, sizeof(microsoftStorePath), "%s%cModifiableWindowsApps%c%s%cEN", szPath,
					PATH_SEP, PATH_SEP, MSSTORE_PATH, PATH_SEP);
	}
#endif
	return microsoftStorePath;
}
...
================
*/
qboolean Sys_RandomBytes(uint8_t *string, int len) {
	HCRYPTPROV prov;

	if (!CryptAcquireContext(&prov, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) { return qfalse; }

	if (!CryptGenRandom(prov, len, (uint8_t *)string)) {
...
char *Sys_GetCurrentUser(void) {
	...
	if (!GetUserName(s_userName, &size)) strcpy(s_userName, "player");

	if (!s_userName[0]) { strcpy(s_userName, "player"); }
...
const char *Sys_Basename(char *path) {
	...
	while (length > 0 && base[length] == '\\') base[length--] = '\0';
...
const char *Sys_Dirname(char *path) {
	...
	while (length > 0 && dir[length] != '\\') length--;
...
FILE *Sys_FOpen(const char *ospath, const char *mode) {
	...
	if (length == 0 || ospath[length - 1] == ' ' || ospath[length - 1] == '.') { return NULL; }
...
static qboolean strgtr(const char *s0, const char *s1) {
	...
	for (i = 0; i < l0; i++) {
		if (s1[i] > s0[i]) { return qtrue; }
		if (s1[i] < s0[i]) { return qfalse; }
	}
	return qfalse;
}
	...
	if (Sys_Dialog(DT_YES_NO, va("%s. Copy console log to clipboard?", error), "Error") == DR_YES) {
		...
		if (clipMemory) {
			...
			*p = '\0';
			...
			if (OpenClipboard(NULL) && EmptyClipboard()) SetClipboardData(CF_TEXT, memoryHandle);
...
#ifndef DEDICATED
	if (timerResolution) timeEndPeriod(timerResolution);
#endif
}
...
qboolean Sys_PIDIsRunning(int pid) {
	...
	for (i = 0; i < numProcesses; i++) {
		if (processes[i] == pid) return qtrue;
	}
