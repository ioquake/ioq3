...
static void AddSkyPolygon(int nump, vec3_t vecs) {
	...
	for (i = 0, vp = vecs; i < nump; i++, vp += 3) { VectorAdd(vp, v, v); }
	av[0] = fabs(v[0]);
	av[1] = fabs(v[1]);
	av[2] = fabs(v[2]);
...
static void ClipSkyPolygon(int nump, vec3_t vecs, int stage) {
	...
	if (nump > MAX_CLIP_VERTS - 2) ri.Error(ERR_DROP, "ClipSkyPolygon: MAX_CLIP_VERTS");
...
static void DrawSkySide(struct image_s *image, const int mins[2], const int maxs[2]) {
	...
	*/
	...
	// qglDrawElements(GL_TRIANGLES, tess.numIndexes - tess.firstIndex, GL_INDEX_TYPE, BUFFER_OFFSET(tess.firstIndex *
	// sizeof(glIndex_t)));
		...
		for (t = sky_mins_subd[1] + HALF_SKY_SUBDIVISIONS; t <= sky_maxs_subd[1] + HALF_SKY_SUBDIVISIONS; t++) {
			for (s = sky_mins_subd[0] + HALF_SKY_SUBDIVISIONS; s <= sky_maxs_subd[0] + HALF_SKY_SUBDIVISIONS; s++) {
				MakeSkyVec((s - HALF_SKY_SUBDIVISIONS) / (float)HALF_SKY_SUBDIVISIONS,
						   (t - HALF_SKY_SUBDIVISIONS) / (float)HALF_SKY_SUBDIVISIONS, i, s_skyTexCoords[t][s],
						   s_skyPoints[t][s]);
			}
		}
...
static void FillCloudBox(const shader_t *shader, int stage) {
	...
	for (i = 0; i < 6; i++) {
		...
		if (1) // FIXME? shader->sky.fullClouds )
		{
			...
			case 1:
			case 2:
			case 3: MIN_T = -1; break;
			case 5:
				// don't draw clouds beneath you
				continue;
			case 4: // top
			default: MIN_T = -HALF_SKY_SUBDIVISIONS; break;
			}
		}
