...
qboolean R_LoadIQM(model_t *mod, void *buffer, int filesize, const char *mod_name) {
	...
	if (filesize < sizeof(iqmHeader_t)) { return qfalse; }

	header = (iqmHeader_t *)buffer;
	if (Q_strncmp(header->magic, IQM_MAGIC, sizeof(header->magic))) { return qfalse; }
	...
	LL(header->filesize);
	if (header->filesize > filesize || header->filesize > 16 << 20) { return qfalse; }
	...
	if (header->num_joints > IQM_MAX_JOINTS) {
		ri.Printf(PRINT_WARNING, "R_LoadIQM: %s has more than %d joints (%d).\n", mod_name, IQM_MAX_JOINTS,
				  header->num_joints);
		return qfalse;
	}

	for (i = 0; i < ARRAY_LEN(vertexArrayFormat); i++) { vertexArrayFormat[i] = -1; }
	...
	if (header->num_meshes) {
		...
		for (i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
			...
			case IQM_UBYTE:
				...
				if (IQM_CheckRange(header, vertexarray->offset, n, sizeof(byte))) { return qfalse; }
				break;
			case IQM_INT:
			case IQM_UINT:
			case IQM_FLOAT:
				// 4-byte swap
				if (IQM_CheckRange(header, vertexarray->offset, n, sizeof(float))) { return qfalse; }
		...
		if (vertexArrayFormat[IQM_POSITION] == -1 || vertexArrayFormat[IQM_NORMAL] == -1 ||
		...
		for (i = 0; i < header->num_triangles; i++, triangle++) {
			...
			if (triangle->vertex[0] > header->num_vertexes || triangle->vertex[1] > header->num_vertexes ||
		...
		if (IQM_CheckRange(header, header->ofs_meshes, header->num_meshes, sizeof(iqmMesh_t))) { return qfalse; }
		...
		for (i = 0; i < header->num_meshes; i++, mesh++) {
			...
			if (mesh->num_vertexes >= SHADER_MAX_VERTEXES) {
				ri.Printf(PRINT_WARNING, "R_LoadIQM: %s has more than %i verts on %s (%i).\n", mod_name,
						  SHADER_MAX_VERTEXES - 1, meshName[0] ? meshName : "a surface", mesh->num_vertexes);
				return qfalse;
			}
			if (mesh->num_triangles * 3 >= SHADER_MAX_INDEXES) {
				ri.Printf(PRINT_WARNING, "R_LoadIQM: %s has more than %i triangles on %s (%i).\n", mod_name,
						  (SHADER_MAX_INDEXES / 3) - 1, meshName[0] ? meshName : "a surface", mesh->num_triangles);
				return qfalse;
			}
			...
			if (mesh->first_vertex >= header->num_vertexes ||
				...
				mesh->first_triangle + mesh->num_triangles > header->num_triangles || mesh->name >= header->num_text ||
			...
			if (header->num_joints) {
				for (j = 0; j < mesh->num_vertexes; j++) {
					...
					for (k = 0; k < j; k++) {
						...
						if (*(int *)&blendIndexes[4 * influence] != *(int *)&blendIndexes[4 * vtx]) { continue; }
						...
						if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
							...
							if (*(int *)&blendWeights.b[4 * influence] == *(int *)&blendWeights.b[4 * vtx]) { break; }
						}
					}
	...
	if (header->num_joints) {
		...
		if (IQM_CheckRange(header, header->ofs_joints, header->num_joints, sizeof(iqmJoint_t))) { return qfalse; }
	...
	if (header->num_poses) {
		...
		if (IQM_CheckRange(header, header->ofs_poses, header->num_poses, sizeof(iqmPose_t))) { return qfalse; }
	...
	if (header->ofs_bounds) {
		...
		if (IQM_CheckRange(header, header->ofs_bounds, header->num_frames, sizeof(*bounds))) { return qfalse; }
	...
	if (header->num_meshes) {
		...
		for (i = 0; i < header->num_vertexarrays; i++, vertexarray++) {
			...
			switch (vertexarray->type) {
			case IQM_POSITION:
				memcpy(iqmData->positions, (byte *)header + vertexarray->offset, n * sizeof(float));
				break;
			case IQM_NORMAL:
				memcpy(iqmData->normals, (byte *)header + vertexarray->offset, n * sizeof(float));
				break;
			case IQM_TANGENT:
				memcpy(iqmData->tangents, (byte *)header + vertexarray->offset, n * sizeof(float));
				break;
			case IQM_TEXCOORD:
				memcpy(iqmData->texcoords, (byte *)header + vertexarray->offset, n * sizeof(float));
				break;
			case IQM_BLENDINDEXES:
			case IQM_BLENDWEIGHTS: break;
			case IQM_COLOR: memcpy(iqmData->colors, (byte *)header + vertexarray->offset, n * sizeof(byte)); break;
			}
		}
				...
				for (j = 0; j < surface->num_vertexes; j++) {
					...
					for (k = 0; k < surface->num_influences; k++) {
						...
						if (vertexArrayFormat[IQM_BLENDWEIGHTS] == IQM_FLOAT) {
							...
							if (*(int *)&iqmData->influenceBlendWeights.b[4 * influence] ==
								*(int *)&blendWeights.b[4 * vtx]) {
								break;
							}
						}
					}
	...
	if (header->num_joints) {
		...
		// copy joint parents
		joint = (iqmJoint_t *)((byte *)header + header->ofs_joints);
		for (i = 0; i < header->num_joints; i++, joint++) { iqmData->jointParents[i] = joint->parent; }
	...
	if (header->num_poses) {
		...
		for (i = 0; i < header->num_frames; i++) {
			...
			for (j = 0; j < header->num_poses; j++, pose++, transform++) {
				...
				if (pose->mask & 0x001) translate[0] += *framedata++ * pose->channelscale[0];
				translate[1] = pose->channeloffset[1];
				if (pose->mask & 0x002) translate[1] += *framedata++ * pose->channelscale[1];
				translate[2] = pose->channeloffset[2];
				if (pose->mask & 0x004) translate[2] += *framedata++ * pose->channelscale[2];

				rotate[0] = pose->channeloffset[3];
				if (pose->mask & 0x008) rotate[0] += *framedata++ * pose->channelscale[3];
				rotate[1] = pose->channeloffset[4];
				if (pose->mask & 0x010) rotate[1] += *framedata++ * pose->channelscale[4];
				rotate[2] = pose->channeloffset[5];
				if (pose->mask & 0x020) rotate[2] += *framedata++ * pose->channelscale[5];
				rotate[3] = pose->channeloffset[6];
				if (pose->mask & 0x040) rotate[3] += *framedata++ * pose->channelscale[6];

				scale[0] = pose->channeloffset[7];
				if (pose->mask & 0x080) scale[0] += *framedata++ * pose->channelscale[7];
				scale[1] = pose->channeloffset[8];
				if (pose->mask & 0x100) scale[1] += *framedata++ * pose->channelscale[8];
				scale[2] = pose->channeloffset[9];
				if (pose->mask & 0x200) scale[2] += *framedata++ * pose->channelscale[9];
	...
	if (iqmData->num_surfaces && iqmData->num_joints <= glRefConfig.glslMaxAnimatedBones) {
		...
		for (i = 0; i < iqmData->num_surfaces; i++, vaoSurf++, surf++) {
			...
			vaoSurf->vao = R_CreateVao(va("staticIQMMesh_VAO '%s'", surf->name), data, dataSize, (byte *)indexes,
...
static int R_CullIQM(iqmData_t *data, trRefEntity_t *ent) {
	...
	case CULL_IN: tr.pc.c_box_cull_md3_in++; return CULL_IN;
	case CULL_CLIP: tr.pc.c_box_cull_md3_clip++; return CULL_CLIP;
	case CULL_OUT:
	default: tr.pc.c_box_cull_md3_out++; return CULL_OUT;
	}
}
...
void R_AddIQMSurfaces(trRefEntity_t *ent) {
	...
	// range checked again.
	//
	if ((ent->e.frame >= data->num_frames) || (ent->e.frame < 0) || (ent->e.oldframe >= data->num_frames) ||
		(ent->e.oldframe < 0)) {
	...
	if (cull == CULL_OUT) { return; }

	//
	// set up lighting now that we know we aren't culled
	//
	if (!personalModel || r_shadows->integer > 1) { R_SetupEntityLighting(&tr.refdef, ent); }
	...
	for (i = 0; i < data->num_surfaces; i++) {
		...
		if (!personalModel) { R_AddDrawSurf(drawSurf, shader, fogNum, 0, 0, cubemapIndex); }
...
void RB_IQMSurfaceAnim(surfaceType_t *surface) {
	...
	if (data->num_poses > 0) {
		...
		for (i = 0; i < surf->num_vertexes; i++, xyz += 3, normal += 3, tangent += 4, texCoords += 2, outXYZ++,
			outNormal += 4, outTangent += 4, outTexCoord++) {
