...
void R_GammaCorrect(byte *buffer, int bufSize) {
	...
	for (i = 0; i < bufSize; i++) { buffer[i] = s_gammatable[buffer[i]]; }
}
...
void R_ImageList_f(void) {
	...
	for (i = 0; i < tr.numImages; i++) {
		...
		if (image->flags & IMGFLAG_MIPMAP) estSize += estSize / 2;
		...
		ri.Printf(PRINT_ALL, "%4i: %4ix%4i %s %4i%s %s\n", i, image->uploadWidth, image->uploadHeight, format,
				  displaySize, sizeSuffix, image->imgName);
...
static void ResampleTexture(byte *in, int inwidth, int inheight, byte *out, int outwidth, int outheight) {
	...
	if (outwidth > 2048) ri.Error(ERR_DROP, "ResampleTexture: max width");
...
static void RGBAtoNormal(const byte *in, byte *out, int width, int height, qboolean clampToEdge) {
	...
	for (y = 0; y < height; y++) {
		...
		for (x = 0; x < width; x++) {
			...
			if (!VectorNormalize2(normal, normal)) { VectorSet(normal, 0, 0, 1); }
...
// from Fast Artifacts-Free Image Interpolation
// (http://www.andreagiachetti.it/icbi/) assumes data has a 2 pixel thick border
// of clamped or wrapped data expects data to be a grid with even (0, 0), (2,
// 0), (0, 2), (2, 2) etc pixels filled only performs FCBI on specified
// component
static void DoFCBI(byte *in, byte *out, int width, int height, int component) {
	...
	for (y = 3; y < height - 3; y += 2) {
		...
		// COPYSAMPLE(se, line2); line2 += 8; COPYSAMPLE(sg, line3); line3 += 8;
		// COPYSAMPLE(sh, line3); line3 += 8; COPYSAMPLE(si, line3); line3 += 8;
		...
		for (; x < width - 3; x += 2) {
			...
			// original
			//                       SAMPLE2(sa, x-1, y-3); SAMPLE2(sb, x+1,
			//                       y-3);
			// SAMPLE2(sc, x-3, y-1); SAMPLE2(sd, x-1, y-1); SAMPLE2(se, x+1,
			// y-1); SAMPLE2(sf, x+3, y-1); SAMPLE2(sg, x-3, y+1); SAMPLE2(sh,
			// x-1, y+1); SAMPLE2(si, x+1, y+1); SAMPLE2(sj, x+3, y+1);
			//                       SAMPLE2(sk, x-1, y+3); SAMPLE2(sl, x+1,
			//                       y+3);
	...
	for (y = 2; y < height - 3; y++) {
		...
		for (; x < width - 3; x += 2) {
			...
			// SAMPLE2(sc, x-2, y-1); SAMPLE2(sd, x,   y-1); SAMPLE2(se, x+2,
			// y-1);
			//            SAMPLE2(sf, x-1, y  ); SAMPLE2(sg, x+1, y  );
			// SAMPLE2(sh, x-2, y+1); SAMPLE2(si, x,   y+1); SAMPLE2(sj, x+2,
			// y+1);
			...
			if (hd > 100 || vd > 100 || abs(hp - vp) > 200) {
				...
				// hdd = abs(sc[i] + sd[i] + se[i] - 3 * (sf[i] + sg[i]) + sh[i]
				// + si[i] + sj[i]); vdd = abs(sa[i] + sf[i] + sk[i] - 3 *
				// (sd[i] + si[i]) + sb[i] + sg[i] + sl[i]);
...
// Similar to DoFCBIQuick, but just takes the average instead of checking
// derivatives as well, this operates on all four components
static void DoLinear(byte *in, byte *out, int width, int height) {
	...
	for (y = 1; y < height - 1; y += 2) {
		...
		for (; x < width - 1; x += 2) {
			...
			for (i = 0; i < 4; i++) { *outbyte++ = (sd[i] + si[i] + se[i] + sh[i]) >> 2; }
	...
	for (y = 1; y < height - 1; y++) {
		...
		for (; x < width - 1; x += 2) {
			...
			for (i = 0; i < 4; i++) { *outbyte++ = (sf[i] + sg[i] + sd[i] + si[i]) >> 2; }
	...
	for (y = 0; y < height; y += WORKBLOCK_SIZE) {
		for (x = 0; x < width; x += WORKBLOCK_SIZE) {
			...
			if (!normalized) {
				...
				case 0: break;
				case 1: DoFCBIQuick(workdata, outdata, fullworkwidth, fullworkheight, 0); break;
				case 2:
				default: DoFCBI(workdata, outdata, fullworkwidth, fullworkheight, 0); break;
				}
			} else {
				switch (r_imageUpsampleType->integer) {
				case 0: break;
...
static void R_MipMapsRGB(byte *in, int inWidth, int inHeight) {
	...
	if (!downmipSrgbLookupSet) {
		for (x = 0; x < 256; x++) downmipSrgbLookup[x] = powf(x / 255.0f, 2.2f) * 0.25f;
		downmipSrgbLookupSet = 1;
	}

	if (inWidth == 1 && inHeight == 1) return;
	...
	for (y = inHeight; y; y--, in += stride, in2 += stride) {
		for (x = inWidth; x; x--) {
			for (c = 3; c; c--, in++, in2++) {
				total = downmipSrgbLookup[*(in)] + downmipSrgbLookup[*(in + 4)] + downmipSrgbLookup[*(in2)] +
...
===============
*/
static qboolean RawImage_ScaleToPower2(byte **data, int *inout_width, int *inout_height, imgType_t type,
									   imgFlags_t flags, byte **resampledBuffer) {
	...
	if (r_roundImagesDown->integer && scaled_width > width) scaled_width >>= 1;
	if (r_roundImagesDown->integer && scaled_height > height) scaled_height >>= 1;
	...
	if (picmip && data && resampledBuffer && r_imageUpsample->integer &&
		...
		*resampledBuffer = ri.Hunk_AllocateTempMemory(finalwidth * finalheight * 4);
		...
		if (type == IMGTYPE_COLORALPHA) RGBAtoYCoCgA(*resampledBuffer, *resampledBuffer, scaled_width, scaled_height);
		...
		// height, scaled_width, scaled_height, endTime - startTime);
...
static qboolean RawImage_HasAlpha(const byte *scan, int numPixels) {
	...
	if (!scan) return qtrue;

	for (i = 0; i < numPixels; i++) {
		if (scan[i * 4 + 3] != 255) { return qtrue; }
	}

	return qfalse;
}

static GLenum RawImage_GetFormat(const byte *data, int numPixels, GLenum picFormat, qboolean lightMap, imgType_t type,
								 imgFlags_t flags) {
	int		 samples			= 3;
	...
	if (picFormat != GL_RGBA8) return picFormat;
	...
	if (normalmap) {
		...
		if (RawImage_HasAlpha(data, numPixels)) { samples = 4; }
...
static void RawImage_UploadToRgtc2Texture(GLuint texture, int miplevel, int x, int y, int width, int height,
										  byte *data) {
	...
	qglCompressedTextureSubImage2DEXT(texture, GL_TEXTURE_2D, miplevel, x, y, width, height, GL_COMPRESSED_RG_RGTC2,
									  size, compressedData);
...
static int CalculateMipSize(int width, int height, GLenum picFormat) {
	...
	case GL_COMPRESSED_SIGNED_RED_RGTC1: return numBlocks * 8;
	...
	case GL_COMPRESSED_RGBA_BPTC_UNORM_ARB:
	case GL_COMPRESSED_SRGB_ALPHA_BPTC_UNORM_ARB: return numBlocks * 16;

	case GL_RGBA8:
	case GL_SRGB8_ALPHA8_EXT: return numPixels * 4;

	case GL_RGBA16: return numPixels * 8;

	default: ri.Printf(PRINT_ALL, "Unsupported texture format %08x\n", picFormat); return 0;
	}
...
static GLenum PixelDataFormatFromInternalFormat(GLenum internalFormat) {
	...
	case GL_DEPTH_COMPONENT32_ARB: return GL_DEPTH_COMPONENT;
	default: return GL_RGBA; break;
	}
}

static void RawImage_UploadTexture(GLuint texture, byte *data, int x, int y, int width, int height, GLenum target,
								   GLenum picFormat, int numMips, GLenum internalFormat, imgType_t type,
...
static void Upload32(byte *data, int x, int y, int width, int height, GLenum picFormat, int numMips, image_t *image,
	...
	if (cubemap) {
		for (i = 0; i < 6; i++) {
			...
			RawImage_UploadTexture(image->texnum, data, x, y, width, height, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
		...
		RawImage_UploadTexture(image->texnum, data, x, y, width, height, GL_TEXTURE_2D, picFormat, numMips,
...
image_t *R_CreateImage2(const char *name, byte *pic, int width, int height, GLenum picFormat, int numMips,
	...
	if (strlen(name) >= MAX_QPATH) { ri.Error(ERR_DROP, "R_CreateImage: \"%s\" is too long", name); }
	if (!strncmp(name, "*lightmap", 9)) { isLightmap = qtrue; }

	if (tr.numImages == MAX_DRAWIMAGES) { ri.Error(ERR_DROP, "R_CreateImage: MAX_DRAWIMAGES hit"); }
	...
	do {
		...
		if (cubemap) {
			...
			for (i = 0; i < 6; i++)
				qglTextureImage2DEXT(image->texnum, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, miplevel, internalFormat,
									 mipWidth, mipHeight, 0, dataFormat, GL_UNSIGNED_BYTE, NULL);
		} else {
			qglTextureImage2DEXT(image->texnum, GL_TEXTURE_2D, miplevel, internalFormat, mipWidth, mipHeight, 0,
	...
	// Upload data.
	if (pic) Upload32(pic, 0, 0, width, height, picFormat, numMips, image, scaled);

	if (resampledBuffer != NULL) ri.Hunk_FreeTempMemory(resampledBuffer);
	...
	if (cubemap) qglTextureParameteriEXT(image->texnum, textureTarget, GL_TEXTURE_WRAP_R, glWrapClampMode);
	...
	case GL_DEPTH_COMPONENT32_ARB:
		...
		break;
	default:
		qglTextureParameterfEXT(image->texnum, textureTarget, GL_TEXTURE_MIN_FILTER,
								mipmap ? gl_filter_min : GL_LINEAR);
		qglTextureParameterfEXT(image->texnum, textureTarget, GL_TEXTURE_MAG_FILTER,
								mipmap ? gl_filter_max : GL_LINEAR);
		break;
	}
...
================
*/
image_t *R_CreateImage(const char *name, byte *pic, int width, int height, imgType_t type, imgFlags_t flags,
					   int internalFormat) {
...
image_t *R_FindImageFile(const char *name, imgType_t type, imgFlags_t flags) {
				...
				for (y = 1; y < height - 1; y++) {
					...
					for (x = 1; x < width - 1; x++) {
						...
						result = *(picbyte - (width + 1) * 4) + *(picbyte - width * 4) + *(picbyte - (width - 1) * 4) +
								 *(picbyte - 1 * 4) + *(picbyte) + *(picbyte + 1 * 4) + *(picbyte + (width - 1) * 4) +
								 *(picbyte + width * 4) + *(picbyte + (width + 1) * 4);
	...
	if ((flags & IMGFLAG_MIPMAP) && picFormat != GL_RGBA8 && picFormat != GL_SRGB8_ALPHA8_EXT) {
		...
		if (neededMips > picNumMips) flags &= ~IMGFLAG_MIPMAP;
	}
...
static void R_CreateDlightImage(void) {
	...
	tr.dlightImage =
		R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE, IMGTYPE_COLORALPHA, IMGFLAG_CLAMPTOEDGE, 0);
}
...
float R_FogFactor(float s, float t) {
	...
	if (t < 31.0 / 32) { s *= (t - 1.0f / 32.0f) / (30.0f / 32.0f); }
...
static void R_CreateDefaultImage(void) {
	...
	tr.defaultImage =
		R_CreateImage("*default", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, IMGTYPE_COLORALPHA, IMGFLAG_MIPMAP, 0);
}
...
void R_CreateBuiltinImages(void) {
	...
	if (r_dlightMode->integer >= 2) {
		for (x = 0; x < MAX_DLIGHTS; x++) {
			tr.shadowCubemaps[x] = R_CreateImage(va("*shadowcubemap%i", x), NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE,
												 IMGTYPE_COLORALPHA, IMGFLAG_CLAMPTOEDGE | IMGFLAG_CUBEMAP, 0);
		}
	}
	...
	if (glRefConfig.framebufferObject) {
		...
		hdrFormat = GL_RGBA8;
		if (r_hdr->integer && glRefConfig.textureFloat) hdrFormat = GL_RGBA16F_ARB;
		...
		if (r_shadowBlur->integer)
			tr.screenScratchImage = R_CreateImage("screenScratch", NULL, width, height, IMGTYPE_COLORALPHA,
												  IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, rgbFormat);

		if (r_shadowBlur->integer || r_ssao->integer)
			tr.hdrDepthImage = R_CreateImage("*hdrDepth", NULL, width, height, IMGTYPE_COLORALPHA,
											 IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_R32F);

		if (r_drawSunRays->integer)
			tr.sunRaysImage = R_CreateImage("*sunRays", NULL, width, height, IMGTYPE_COLORALPHA,
											IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, rgbFormat);
		...
		tr.textureDepthImage =
			R_CreateImage("*texturedepth", NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE, IMGTYPE_COLORALPHA,
						  IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT24);

		{
			...
			tr.calcLevelsImage	 = R_CreateImage("*calcLevels", p, 1, 1, IMGTYPE_COLORALPHA,
												 IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
			tr.targetLevelsImage = R_CreateImage("*targetLevels", p, 1, 1, IMGTYPE_COLORALPHA,
												 IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
			tr.fixedLevelsImage	 = R_CreateImage("*fixedLevels", p, 1, 1, IMGTYPE_COLORALPHA,
												 IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
		}

		for (x = 0; x < 2; x++) {
			tr.textureScratchImage[x] = R_CreateImage(va("*textureScratch%d", x), NULL, 256, 256, IMGTYPE_COLORALPHA,
													  IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_RGBA8);
		}
		for (x = 0; x < 2; x++) {
			tr.quarterImage[x] = R_CreateImage(va("*quarter%d", x), NULL, width / 2, height / 2, IMGTYPE_COLORALPHA,
											   IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_RGBA8);
		}

		if (r_ssao->integer) {
			tr.screenSsaoImage = R_CreateImage("*screenSsao", NULL, width / 2, height / 2, IMGTYPE_COLORALPHA,
											   IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_RGBA8);
		}

		for (x = 0; x < MAX_DRAWN_PSHADOWS; x++) {
			tr.pshadowMaps[x] =
				R_CreateImage(va("*shadowmap%i", x), NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE, IMGTYPE_COLORALPHA,
							  IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT24);
			// qglTextureParameterfEXT(tr.pshadowMaps[x]->texnum, GL_TEXTURE_2D,
		...
		if (r_sunlightMode->integer) {
			for (x = 0; x < 4; x++) {
				tr.sunShadowDepthImage[x] = R_CreateImage(
					va("*sunshadowdepth%i", x), NULL, r_shadowMapSize->integer, r_shadowMapSize->integer,
				...
				qglTextureParameterfEXT(tr.sunShadowDepthImage[x]->texnum, GL_TEXTURE_2D, GL_TEXTURE_COMPARE_FUNC,
										GL_LEQUAL);
			}

			tr.screenShadowImage = R_CreateImage("*screenShadow", NULL, width, height, IMGTYPE_COLORALPHA,
												 IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_RGBA8);
		}

		if (r_cubeMapping->integer) {
			tr.renderCubeImage = R_CreateImage(
				"*renderCube", NULL, r_cubemapSize->integer, r_cubemapSize->integer, IMGTYPE_COLORALPHA,
				IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE | IMGFLAG_MIPMAP | IMGFLAG_CUBEMAP, rgbFormat);
		}
	}
}
...
void R_SetColorMappings(void) {
	...
	if (tr.overbrightBits > r_mapOverBrightBits->integer) { tr.overbrightBits = r_mapOverBrightBits->integer; }

	tr.identityLight	 = 1.0f / (1 << tr.overbrightBits);
	tr.identityLightByte = 255 * tr.identityLight;

	if (r_intensity->value <= 1) { ri.Cvar_Set("r_intensity", "1"); }
	...
	if (glConfig.deviceSupportsGamma) { GLimp_SetGamma(s_gammatable, s_gammatable, s_gammatable); }
}
...
void R_DeleteTextures(void) {
	...
	for (i = 0; i < tr.numImages; i++) { qglDeleteTextures(1, &tr.images[i]->texnum); }
...
qhandle_t RE_RegisterSkin(const char *name) {
	...
	if (totalSurfaces > MAX_SKIN_SURFACES) {
		ri.Printf(PRINT_WARNING,
				  "WARNING: Ignoring excess surfaces (found %d, max is %d) in "
				  "skin '%s'!\n",
...
skin_t *R_GetSkinByHandle(qhandle_t hSkin) {
	if (hSkin < 1 || hSkin >= tr.numSkins) { return tr.skins[0]; }
