...
#define SIDE_BACK		  1
#define SIDE_ON			  2
static void R_ChopPolyBehindPlane(int numInPoints, vec3_t inPoints[MAX_VERTS_ON_POLY], int *numOutPoints,
								  vec3_t outPoints[MAX_VERTS_ON_POLY], vec3_t normal, vec_t dist, vec_t epsilon) {
	...
	*numOutPoints = 0;
	...
	for (i = 0; i < numInPoints; i++) {
		...
		for (j = 0; j < 3; j++) { clip[j] = p1[j] + dot * (p2[j] - p1[j]); }
...
void R_AddMarkFragments(int numClipPoints, vec3_t clipPoints[2][MAX_VERTS_ON_POLY], int numPlanes, vec3_t *normals,
	...
	for (i = 0; i < numPlanes; i++) {
		...
		R_ChopPolyBehindPlane(numClipPoints, clipPoints[pingPong], &numClipPoints, clipPoints[!pingPong], normals[i],
							  dists[i], 0.5);
		pingPong ^= 1;
...
int R_MarkFragments(int numPoints, const vec3_t *points, const vec3_t projection, int maxPoints, vec3_t pointBuffer,
	...
	for (i = 0; i < numsurfaces; i++) {
		...
		if (*surfaces[i] == SF_GRID) {
			...
			for (m = 0; m < cv->height - 1; m++) {
				for (n = 0; n < cv->width - 1; n++) {
					...
					if (DotProduct(normal, projectionDir) < -0.1) {
						...
						R_AddMarkFragments(numClipPoints, clipPoints, numPlanes, normals, dists, maxPoints, pointBuffer,
			...
			if (DotProduct(surf->cullPlane.normal, projectionDir) > -0.5) { continue; }
			...
			for (k = 0, tri = surf->indexes; k < surf->numIndexes; k += 3, tri += 3) {
				...
				R_AddMarkFragments(3, clipPoints, numPlanes, normals, dists, maxPoints, pointBuffer, maxFragments,
								   fragmentBuffer, &returnedPoints, &returnedFragments, mins, maxs);
				if (returnedFragments == maxFragments) {
					return returnedFragments; // not enough space for more fragments
				}
			}
