...
static void R_BindAnimatedImageToTMU(textureBundle_t *bundle, int tmu) {
	...
	while (index >= bundle->numImageAnimations) { index -= bundle->numImageAnimations; }
...
void RB_BeginSurface(shader_t *shader, int fogNum, int cubemapIndex) {
	...
	if (backEnd.viewParms.flags & VPF_SHADOWMAP) { tess.currentStageIteratorFunc = RB_StageIteratorGeneric; }
}
...
static void ComputeTexMods(shaderStage_t *pStage, int bundleNum, float *outMatrix, float *outOffTurb) {
	...
	for (tm = 0; tm < bundle->numTexMods; tm++) {
		...
		case TMOD_TURBULENT: RB_CalcTurbulentFactors(&bundle->texMods[tm].wave, &outOffTurb[2], &outOffTurb[3]); break;
		...
		default:
			ri.Error(ERR_DROP, "ERROR: unknown texmod '%d' in shader '%s'", bundle->texMods[tm].type,
					 tess.shader->name);
			break;
		}
...
static void ProjectDlightTexture(void) {
	...
	if (!backEnd.refdef.num_dlights) { return; }
...
static void ComputeFogValues(vec4_t fogDistanceVector, vec4_t fogDepthVector, float *eyeT) {
	...
	if (fog->hasSurface) {
		fogDepthVector[0] = fog->surface[0] * backEnd.or.axis[0][0] + fog->surface[1] * backEnd.or
															.axis[0][1] + fog->surface[2] * backEnd.or.axis[0][2];
		fogDepthVector[1] = fog->surface[0] * backEnd.or.axis[1][0] + fog->surface[1] * backEnd.or
															.axis[1][1] + fog->surface[2] * backEnd.or.axis[1][2];
		fogDepthVector[2] = fog->surface[0] * backEnd.or.axis[2][0] + fog->surface[1] * backEnd.or
...
static void ProjectPshadowVBOGLSL(void) {
	...
	if (!backEnd.refdef.num_pshadows) { return; }
	...
	for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
		...
		if (backEnd.depthFill) {
			if (pStage->glslShaderGroup == tr.lightallShader) {
				...
				if (pStage->stateBits & GLS_ATEST_BITS) { index |= LIGHTDEF_USE_TCGEN_AND_TCMOD; }
				...
				if (pStage->stateBits & GLS_ATEST_BITS) { shaderAttribs |= GENERICDEF_USE_TCGEN_AND_TCMOD; }
		...
		if (backEnd.depthFill) {
			...
			if ((r_lightmap->integer == 1 || r_lightmap->integer == 2) && pStage->bundle[TB_LIGHTMAP].image[0]) {
				...
				enableTextures[3] =
					(r_cubeMapping->integer && !(tr.viewParms.flags & VPF_NOCUBEMAPS) && input->cubemapIndex) ? 1.0f
																											  : 0.0f;
			}
		...
		if (r_lightmap->integer && (pStage->bundle[0].isLightmap || pStage->bundle[1].isLightmap)) { break; }
...
void RB_StageIteratorGeneric(void) {
	...
	if (!input->numVertexes || !input->numIndexes) { return; }

	if (tess.useInternalVao) { RB_DeformTessGeometry(); }
	...
	if (input->shader->polygonOffset) { qglEnable(GL_POLYGON_OFFSET_FILL); }
	...
	if (backEnd.depthFill) {
		...
		//
		if (input->shader->polygonOffset) { qglDisable(GL_POLYGON_OFFSET_FILL); }
	...
	if (backEnd.viewParms.flags & VPF_SHADOWMAP) {
		if (input->shader->sort == SS_OPAQUE) { RB_RenderShadowmap(input); }
		//
		// reset polygon offset
		//
		if (input->shader->polygonOffset) { qglDisable(GL_POLYGON_OFFSET_FILL); }
	...
	// now do fog
	//
	if (tess.fogNum && tess.shader->fogPass) { RB_FogPass(); }
...
void RB_EndSurface(void) {
	...
	if (input->numIndexes == 0 || input->numVertexes == 0) { return; }

	if (input->indexes[SHADER_MAX_INDEXES - 1] != 0) { ri.Error(ERR_DROP, "RB_EndSurface() - SHADER_MAX_INDEXES hit"); }
