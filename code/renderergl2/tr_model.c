...
qhandle_t R_RegisterMDR(const char *name, model_t *mod) {
	...
	ident = LittleLong(*(unsigned *)buf.u);
	if (ident == MDR_IDENT) loaded = R_LoadMDR(mod, buf.u, filesize, name);
...
static modelExtToLoaderMap_t modelLoaders[] = {{"iqm", R_RegisterIQM}, {"mdr", R_RegisterMDR}, {"md3", R_RegisterMD3}};
...
model_t *R_GetModelByHandle(qhandle_t index) {
	...
	if (index < 1 || index >= tr.numModels) { return tr.models[0]; }
...
model_t *R_AllocModel(void) {
	...
	if (tr.numModels == MAX_MOD_KNOWN) { return NULL; }
...
static qboolean R_LoadMD3(model_t *mod, int lod, void *buffer, int bufferSize, const char *modName) {
	...
	for (i = 0; i < md3Model->numSurfaces; i++) {
		...
		if (md3Surf->numVerts >= SHADER_MAX_VERTEXES) {
			ri.Printf(PRINT_WARNING, "R_LoadMD3: %s has more than %i verts on %s (%i).\n", modName,
					  SHADER_MAX_VERTEXES - 1, md3Surf->name[0] ? md3Surf->name : "a surface", md3Surf->numVerts);
			return qfalse;
		}
		if (md3Surf->numTriangles * 3 >= SHADER_MAX_INDEXES) {
			ri.Printf(PRINT_WARNING, "R_LoadMD3: %s has more than %i triangles on %s (%i).\n", modName,
					  (SHADER_MAX_INDEXES / 3) - 1, md3Surf->name[0] ? md3Surf->name : "a surface",
		...
		j = strlen(surf->name);
		if (j > 2 && surf->name[j - 2] == '_') { surf->name[j - 2] = 0; }
...

	{
		...
		for (i = 0; i < mdvModel->numSurfaces; i++, vaoSurf++, surf++) {
			...
			vaoSurf->vao = R_CreateVao(va("staticMD3Mesh_VAO '%s'", surf->name), data, dataSize, (byte *)surf->indexes,
...
void R_Modellist_f(void) {
	...
	for (i = 1; i < tr.numModels; i++) {
		...
		for (j = 1; j < MD3_MAX_LODS; j++) {
			if (mod->mdv[j] && mod->mdv[j] != mod->mdv[j - 1]) { lods++; }
		}
...
static mdvTag_t *R_GetTag(mdvModel_t *mod, int frame, const char *_tagName) {
	...
	for (i = 0; i < mod->numTags; i++, tag++, tagName++) {
		if (!strcmp(tagName->name, _tagName)) { return tag; }
	}
...
mdvTag_t *R_GetAnimTag(mdrHeader_t *mod, int framenum, const char *tagName, mdvTag_t *dest) {
	...
	for (i = 0; i < mod->numTags; i++, tag++) {
		if (!strcmp(tag->name, tagName)) {
			...
			for (j = 0; j < 3; j++) {
				for (k = 0; k < 3; k++) dest->axis[j][k] = frame->bones[tag->boneIndex].matrix[k][j];
			}
