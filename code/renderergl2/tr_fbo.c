...
FBO_t *FBO_Create(const char *name, int width, int height) {
	...
	if (strlen(name) >= MAX_QPATH) { ri.Error(ERR_DROP, "FBO_Create: \"%s\" is too long", name); }
	...
	if (tr.numFBOs == MAX_FBOS) { ri.Error(ERR_DROP, "FBO_Create: MAX_FBOS hit"); }
...
void FBO_CreateBuffer(FBO_t *fbo, int format, int index, int multisample) {
	...
	default: ri.Printf(PRINT_WARNING, "FBO_CreateBuffer: invalid format %d\n", format); return;
	}

	absent = *pRenderBuffer == 0;
	if (absent) qglGenRenderbuffers(1, pRenderBuffer);
	...
	if (absent) {
		if (attachment == 0) {
			...
			qglNamedFramebufferRenderbufferEXT(fbo->frameBuffer, GL_STENCIL_ATTACHMENT, GL_RENDERBUFFER,
											   *pRenderBuffer);
		} else {
...
void FBO_AttachImage(FBO_t *fbo, image_t *image, GLenum attachment, GLuint cubemapside) {
	...
	if (image->flags & IMGFLAG_CUBEMAP) target = GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + cubemapside;

	qglNamedFramebufferTexture2DEXT(fbo->frameBuffer, attachment, target, image->texnum, 0);
	index = attachment - GL_COLOR_ATTACHMENT0;
	if (index >= 0 && index <= 15) fbo->colorImage[index] = image;
}
...
void FBO_Bind(FBO_t *fbo) {
	...
	if (glState.currentFBO == fbo) return;
...
void FBO_Init(void) {
	...
	if (!glRefConfig.framebufferObject) return;
	...
	if (glRefConfig.framebufferMultisample) qglGetIntegerv(GL_MAX_SAMPLES, &multisample);

	if (r_ext_framebuffer_multisample->integer < multisample) multisample = r_ext_framebuffer_multisample->integer;

	if (multisample < 2 || !glRefConfig.framebufferBlit) multisample = 0;
	...
	if (MAX_DRAWN_PSHADOWS && tr.pshadowMaps[0]) {
		for (i = 0; i < MAX_DRAWN_PSHADOWS; i++) {
			...
			// FIXME: this next line wastes 16mb with 16x512x512 sun shadow maps, skip if OpenGL 4.3+ or
	...
	if (tr.sunShadowDepthImage[0]) {
		for (i = 0; i < 4; i++) {
			tr.sunShadowFbo[i] =
				FBO_Create("_sunshadowmap", tr.sunShadowDepthImage[i]->width, tr.sunShadowDepthImage[i]->height);
			// FIXME: this next line wastes 16mb with 4x1024x1024 sun shadow maps, skip if OpenGL 4.3+ or
			// ARB_framebuffer_no_attachments This at least gets sun shadows working on older GPUs (Intel)
	...
	if (tr.textureScratchImage[0]) {
		for (i = 0; i < 2; i++) {
			tr.textureScratchFbo[i] = FBO_Create(va("_texturescratch%d", i), tr.textureScratchImage[i]->width,
...
void FBO_Shutdown(void) {
	...
	for (i = 0; i < tr.numFBOs; i++) {
		...
		for (j = 0; j < glRefConfig.maxColorAttachments; j++) {
			if (fbo->colorBuffers[j]) qglDeleteRenderbuffers(1, &fbo->colorBuffers[j]);
		}

		if (fbo->depthBuffer) qglDeleteRenderbuffers(1, &fbo->depthBuffer);

		if (fbo->stencilBuffer) qglDeleteRenderbuffers(1, &fbo->stencilBuffer);

		if (fbo->frameBuffer) qglDeleteFramebuffers(1, &fbo->frameBuffer);
	}
}
...
void FBO_BlitFromTexture(struct image_s *src, vec4_t inSrcTexCorners, vec2_t inSrcTexScale, FBO_t *dst,
	...
	if (!shaderProgram) { shaderProgram = &tr.textureColorShader; }
...
void FBO_Blit(FBO_t *src, ivec4_t inSrcBox, vec2_t srcTexScale, FBO_t *dst, ivec4_t dstBox,
	...
	FBO_BlitFromTexture(src->colorImage[0], srcTexCorners, srcTexScale, dst, dstBox, shaderProgram, color,
...
void FBO_FastBlit(FBO_t *src, ivec4_t srcBox, FBO_t *dst, ivec4_t dstBox, int buffers, int filter) {
	...
	qglBlitFramebuffer(srcBoxFinal[0], srcBoxFinal[1], srcBoxFinal[2], srcBoxFinal[3], dstBoxFinal[0], dstBoxFinal[1],
					   dstBoxFinal[2], dstBoxFinal[3], buffers, filter);
