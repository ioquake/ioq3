...
void GL_BindToTMU(image_t *image, int tmu) {
	...
	if (image) {
		if (image->flags & IMGFLAG_CUBEMAP) target = GL_TEXTURE_CUBE_MAP;
...
void GL_Cull(int cullType) {
	if (glState.faceCulling == cullType) { return; }

	if (cullType == CT_TWO_SIDED) {
		if (glState.faceCulling != CT_TWO_SIDED) { qglDisable(GL_CULL_FACE); }
	} else {
		qboolean cullFront = (cullType == CT_FRONT_SIDED);

		if (glState.faceCulling == CT_TWO_SIDED) { qglEnable(GL_CULL_FACE); }
...
void GL_State(unsigned long stateBits) {
	...
	if (diff & (GLS_SRCBLEND_BITS | GLS_DSTBLEND_BITS)) {
		...
		if (newState != 0 && storedState != newState) {
			...
			default: ri.Error(ERR_DROP, "GL_State: invalid src blend state bits"); break;
			}
			...
			default: ri.Error(ERR_DROP, "GL_State: invalid dst blend state bits"); break;
			}
...
static void SetViewportAndScissor(void) {
	...
	qglScissor(backEnd.viewParms.viewportX, backEnd.viewParms.viewportY, backEnd.viewParms.viewportWidth,
			   backEnd.viewParms.viewportHeight);
}
...
void RB_BeginDrawingView(void) {
	...
	if (r_finish->integer == 0) { glState.finishCalled = qtrue; }
	...
	if (glRefConfig.framebufferObject) {
		...
		// FIXME: HUGE HACK: render to the screen fbo if we've already postprocessed the frame and aren't drawing more
		// world drawing more world check is in case of double renders, such as skyportals
	...
	if (r_measureOverdraw->integer || r_shadows->integer == 2) { clearBits |= GL_STENCIL_BUFFER_BIT; }
	if (r_fastsky->integer && !(backEnd.refdef.rdflags & RDF_NOWORLDMODEL)) {
		clearBits |= GL_COLOR_BUFFER_BIT; // FIXME: only if sky shaders have been used
	}

	// clear to black for cube maps
	if (tr.renderCubeFbo && backEnd.viewParms.targetFbo == tr.renderCubeFbo) { clearBits |= GL_COLOR_BUFFER_BIT; }
...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, int numDrawSurfs) {
	...
	for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
		if (drawSurf->sort == oldSort && drawSurf->cubemapIndex == oldCubemapIndex) {
			if (backEnd.depthFill && shader && shader->sort != SS_OPAQUE) continue;
		...
		if (shader != NULL &&
			(shader != oldShader || fogNum != oldFogNum || dlighted != oldDlighted || pshadowed != oldPshadowed ||
			 cubemapIndex != oldCubemapIndex || (entityNum != oldEntityNum && !shader->entityMergable))) {
			if (oldShader != NULL) { RB_EndSurface(); }
		...
		if (entityNum != oldEntityNum) {
			...
			if (entityNum != REFENTITYNUM_WORLD) {
				...
				if (backEnd.currentEntity->e.renderfx & RF_DEPTHHACK) {
					...
					if (backEnd.currentEntity->e.renderfx & RF_CROSSHAIR) isCrosshair = qtrue;
				}
			} else {
				backEnd.currentEntity	 = &tr.worldEntity;
	...
	if (glRefConfig.framebufferObject) FBO_Bind(fbo);
...
void RB_SetGL2D(void) {
	...
	if (backEnd.projection2D && backEnd.last2DFBO == glState.currentFBO) return;
...
void RE_StretchRaw(int x, int y, int w, int h, int cols, int rows, const byte *data, int client, qboolean dirty) {
	...
	// FIXME: HUGE hack
	if (glRefConfig.framebufferObject) { FBO_Bind(backEnd.framePostProcessed ? NULL : tr.renderFbo); }
...
const void *RB_DrawSurfs(const void *data) {
	...
	if (glRefConfig.framebufferObject && !(backEnd.refdef.rdflags & RDF_NOWORLDMODEL) &&
		(r_depthPrepass->integer || isShadowView)) {
		...
		if (!isShadowView) {
			if (tr.msaaResolveFbo) {
				...
				if (!glRefConfig.intelGraphics)
					qglCopyTextureSubImage2DEXT(tr.renderDepthImage->texnum, GL_TEXTURE_2D, 0, 0, 0, 0, 0,
			...
			if (r_ssao->integer) {
				...
				viewInfo[2] =
					1.0f / ((float)(tr.quarterImage[0]->width) * tan(backEnd.viewParms.fovX * M_PI / 360.0f) * 2.0f);
				viewInfo[3] =
					1.0f / ((float)(tr.quarterImage[0]->height) * tan(backEnd.viewParms.fovY * M_PI / 360.0f) * 2.0f);
	...
	if (!isShadowView) {
		...
		if (r_drawSun->integer) { RB_DrawSun(0.1, tr.sunShader); }
		...
		if (glRefConfig.framebufferObject && r_drawSunRays->integer) {
			...
			if (glRefConfig.occlusionQuery) { qglEndQuery(GL_SAMPLES_PASSED); }
...
const void *RB_DrawBuffer(const void *data) {
	...
	// FBO binding
	if (glRefConfig.framebufferObject) FBO_Bind(NULL);
...
const void *RB_ClearDepth(const void *data) {
	...
	// FBO binding
	if (glRefConfig.framebufferObject) { FBO_Bind(tr.renderFbo ? tr.renderFbo : NULL); }

	// Clear depth buffer
	qglClear(GL_DEPTH_BUFFER_BIT);

	// Clear depth texture for MSAA resolve buffer
	if (tr.msaaResolveFbo) {
...
const void *RB_SwapBuffers(const void *data) {
	...
	if (r_measureOverdraw->integer) {
		unsigned char *stencilReadback =
			(unsigned char *)ri.Hunk_AllocateTempMemory(glConfig.vidWidth * glConfig.vidHeight);
		qglReadPixels(0, 0, glConfig.vidWidth, glConfig.vidHeight, GL_STENCIL_INDEX, GL_UNSIGNED_BYTE, stencilReadback);
		long sum = 0;
		for (int i = 0; i < glConfig.vidWidth * glConfig.vidHeight; i++) { sum += stencilReadback[i]; }
		backEnd.pc.c_overDraw += sum;
		ri.Hunk_FreeTempMemory(stencilReadback);
	}

	// fast blit FBO if using framebuffer object
	if (glRefConfig.framebufferObject && !backEnd.framePostProcessed) {
	...
	if (!glState.finishCalled) { qglFinish(); }
	...
	if (cmd->map != -1) {
		if (cmd->cubeSide != -1) {
			...
			if (tr.pshadowMaps[cmd->map]) {
				qglCopyTextureSubImage2DEXT(tr.pshadowMaps[cmd->map]->texnum, GL_TEXTURE_2D, 0, 0, 0, backEnd.refdef.x,
											glConfig.vidHeight - (backEnd.refdef.y + PSHADOW_MAP_SIZE),
											PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE);
			}
		}
...
const void *RB_PostProcess(const void *data) {
	...
	if (tr.msaaResolveFbo) {
		...
		FBO_FastBlit(tr.renderFbo, NULL, tr.msaaResolveFbo, NULL, GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT,
					 GL_NEAREST);
	...
	if (r_ssao->integer) {
		...
		FBO_Blit(tr.screenSsaoFbo, srcBox, NULL, srcFbo, dstBox, NULL, NULL,
	...
	if (r_drawSunRays->integer) RB_SunRays(NULL, srcBox, NULL, dstBox);
...
void RB_ExecuteRenderCommands(const void *data) {
	...
	while (1) {
		...
		case RC_SET_COLOR: data = RB_SetColor(data); break;
		case RC_STRETCH_PIC: data = RB_StretchPic(data); break;
		case RC_DRAW_SURFS: data = RB_DrawSurfs(data); break;
		case RC_DRAW_BUFFER: data = RB_DrawBuffer(data); break;
