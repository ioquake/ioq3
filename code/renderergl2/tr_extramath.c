...
qboolean Mat4Compare(const mat4_t a, const mat4_t b) {
	return !(a[0] != b[0] || a[4] != b[4] || a[8] != b[8] || a[12] != b[12] || a[1] != b[1] || a[5] != b[5] ||
			 a[9] != b[9] || a[13] != b[13] || a[2] != b[2] || a[6] != b[6] || a[10] != b[10] || a[14] != b[14] ||
...
qboolean SpheresIntersect(vec3_t origin1, float radius1, vec3_t origin2, float radius2) {
	...
	if (DotProduct(diff, diff) <= radiusSum * radiusSum) { return qtrue; }
