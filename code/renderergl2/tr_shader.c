...
void R_RemapShader(const char *shaderName, const char *newShaderName, const char *timeOffset) {
	...
	if (timeOffset) { sh2->timeOffset = atof(timeOffset); }
}
...
static int NameToSrcBlendMode(const char *name) {
	if (!Q_stricmp(name, "GL_ONE")) {
		...
		if (r_ignoreDstAlpha->integer) return GLS_SRCBLEND_ONE;

		return GLS_SRCBLEND_DST_ALPHA;
	} else if (!Q_stricmp(name, "GL_ONE_MINUS_DST_ALPHA")) {
		if (r_ignoreDstAlpha->integer) return GLS_SRCBLEND_ZERO;
...
static int NameToDstBlendMode(const char *name) {
	if (!Q_stricmp(name, "GL_ONE")) {
		...
		if (r_ignoreDstAlpha->integer) return GLS_DSTBLEND_ONE;

		return GLS_DSTBLEND_DST_ALPHA;
	} else if (!Q_stricmp(name, "GL_ONE_MINUS_DST_ALPHA")) {
		if (r_ignoreDstAlpha->integer) return GLS_DSTBLEND_ZERO;
...
static qboolean ParseStage(shaderStage_t *stage, char **text) {
		...
		//
			...
			if (!Q_stricmp(token, "$whiteimage")) {
				...
				if (stage->type == ST_NORMALMAP || stage->type == ST_NORMALPARALLAXMAP) {
					...
					if (stage->type == ST_NORMALPARALLAXMAP) type = IMGTYPE_NORMALHEIGHT;
				} else {
					if (r_genNormalMaps->integer) flags |= IMGFLAG_GENNORMALMAP;
				}
	...
	if (stage->alphaGen == AGEN_IDENTITY) {
		if (stage->rgbGen == CGEN_IDENTITY || stage->rgbGen == CGEN_LIGHTING_DIFFUSE) { stage->alphaGen = AGEN_SKIP; }
	}
...
static void ParseSkyParms(char **text) {
	...
	if (strcmp(token, "-")) {
		for (i = 0; i < 6; i++) {
			...
			shader.sky.outerbox[i] =
				R_FindImageFile((char *)pathname, IMGTYPE_COLORALPHA, imgFlags | IMGFLAG_CLAMPTOEDGE);
	...
	if (!shader.sky.cloudHeight) { shader.sky.cloudHeight = 512; }
...
static qboolean ParseShader(char **text) {
	...
	while (1) {
		...
		// stage definition
			...
			if (!ParseStage(&stages[s], text)) { return qfalse; }
		...
		// sun parms
			...
			if (isGL2Sun) {
				...
				token = COM_ParseExt(text, qfalse);
				if (token[0]) tr.sunShadowScale = atof(token);
			}
		...
		// tonemap parms
			...
			token = COM_ParseExt(text, qfalse);
			if (token[0]) { shader.clampTime = atof(token); }
		}
		...
		// fogParms
			...
			if (!ParseVector(text, 3, shader.fogParms.color)) { return qfalse; }
	...
	//
	if (s == 0 && !shader.isSky && !(shader.contentFlags & CONTENTS_FOG)) { return qfalse; }
...
static void ComputeVertexAttribs(void) {
	...
	if (shader.isPortal) { shader.vertexAttribs |= ATTR_NORMAL; }
	...
	if (shader.numDeforms) {
		for (i = 0; i < shader.numDeforms; i++) {
			...
			case DEFORM_BULGE: shader.vertexAttribs |= ATTR_NORMAL | ATTR_TEXCOORD; break;

			case DEFORM_AUTOSPRITE: shader.vertexAttribs |= ATTR_NORMAL | ATTR_COLOR; break;
			...
			case DEFORM_TEXT5:
			case DEFORM_TEXT6:
			case DEFORM_TEXT7: shader.vertexAttribs |= ATTR_NORMAL; break;
	...
	for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
		...
		if (pStage->glslShaderGroup == tr.lightallShader) {
			...
			case LIGHTDEF_USE_LIGHT_VERTEX: shader.vertexAttribs |= ATTR_LIGHTDIRECTION; break;
			default: break;
			}
		}

		for (i = 0; i < NUM_TEXTURE_BUNDLES; i++) {
			if (pStage->bundle[i].image[0] == 0) { continue; }

			switch (pStage->bundle[i].tcGen) {
			case TCGEN_TEXTURE: shader.vertexAttribs |= ATTR_TEXCOORD; break;
			case TCGEN_LIGHTMAP: shader.vertexAttribs |= ATTR_LIGHTCOORD; break;
			case TCGEN_ENVIRONMENT_MAPPED: shader.vertexAttribs |= ATTR_NORMAL; break;
		...
		case CGEN_ONE_MINUS_VERTEX: shader.vertexAttribs |= ATTR_COLOR; break;

		case CGEN_LIGHTING_DIFFUSE: shader.vertexAttribs |= ATTR_NORMAL; break;

		default: break;
		}

		switch (pStage->alphaGen) {
		case AGEN_LIGHTING_SPECULAR: shader.vertexAttribs |= ATTR_NORMAL; break;

		case AGEN_VERTEX:
		case AGEN_ONE_MINUS_VERTEX: shader.vertexAttribs |= ATTR_COLOR; break;
...
static void CollapseStagesToLightall(shaderStage_t *diffuse, shaderStage_t *normal, shaderStage_t *specular,
									 shaderStage_t *lightmap, qboolean useLightVector, qboolean useLightVertex,
									 qboolean parallax, qboolean tcgen) {
	int defs = 0;
	...
	if (r_normalMapping->integer) {
		...
		if (normal) {
			...
			if (normalImg) {
				...
				VectorSet4(diffuse->normalScale, r_baseNormalX->value, r_baseNormalY->value, 1.0f,
						   r_baseParallax->value);
			}
		}
	}
	...
	if (tcgen || diffuse->bundle[0].numTexMods) { defs |= LIGHTDEF_USE_TCGEN_AND_TCMOD; }
...
static int CollapseStagesToGLSL(void) {
	...
	if (shader.numDeforms != 0) { skip = qtrue; }
	...
	if (!skip) {
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			case TCGEN_VECTOR: break;
			default: skip = qtrue; break;
			}

			switch (pStage->alphaGen) {
			case AGEN_LIGHTING_SPECULAR:
			case AGEN_PORTAL: skip = qtrue; break;
			default: break;
			}
		}
	}
	...
	if (!skip) {
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if (pStage->type != ST_COLORMAP) continue;

			// skip lightmaps
			if (pStage->bundle[0].tcGen == TCGEN_LIGHTMAP) continue;
			...
			for (j = i + 1; j < MAX_SHADER_STAGES; j++) {
				...
				if (!pStage2->active) continue;

				switch (pStage2->type) {
				case ST_NORMALMAP:
					if (!normal) { normal = pStage2; }
					break;
			...
			tcgen = qfalse;
			if (diffuse->bundle[0].tcGen == TCGEN_ENVIRONMENT_MAPPED || diffuse->bundle[0].tcGen == TCGEN_LIGHTMAP ||
				diffuse->bundle[0].tcGen == TCGEN_VECTOR) {
				tcgen = qtrue;
			}

			diffuselit = qfalse;
			if (diffuse->rgbGen == CGEN_LIGHTING_DIFFUSE) { diffuselit = qtrue; }

			vertexlit = qfalse;
			if (diffuse->rgbGen == CGEN_VERTEX_LIT || diffuse->rgbGen == CGEN_EXACT_VERTEX_LIT) { vertexlit = qtrue; }
		...
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if (pStage->bundle[0].tcGen == TCGEN_LIGHTMAP) { pStage->active = qfalse; }
		}
	}
	...
	for (i = 0; i < MAX_SHADER_STAGES; i++) {
		...
		if (pStage->type == ST_NORMALMAP) { pStage->active = qfalse; }

		if (pStage->type == ST_NORMALPARALLAXMAP) { pStage->active = qfalse; }

		if (pStage->type == ST_SPECULARMAP) { pStage->active = qfalse; }
	}
	...
	if (r_sunlightMode->integer && shader.numDeforms == 0) {
		for (i = 0; i < MAX_SHADER_STAGES; i++) {
			...
			if (pStage->adjustColorsForFog) continue;
				...
				for (i = 0, drawSurf = ds_cmd->drawSurfs; i < ds_cmd->numDrawSurfs; i++, drawSurf++) {
					...
					if (sortedIndex >= newShader) {
						...
						drawSurf->sort = (sortedIndex << QSORT_SHADERNUM_SHIFT) | entityNum |
										 (fogNum << QSORT_FOGNUM_SHIFT) | ((int)pshadowMap << QSORT_PSHADOW_SHIFT) |
										 (int)dlightMap;
					}
				}
...
static void SortNewShader(void) {
	...
	for (i = tr.numShaders - 2; i >= 0; i--) {
		if (tr.sortedShaders[i]->sort <= sort) { break; }
...
static void VertexLightingCollapse(void) {
	...
	if (shader.sort == SS_OPAQUE) {
		...
		for (stage = 0; stage < MAX_SHADER_STAGES; stage++) {
			...
			if (pStage->bundle[0].isLightmap) { rank -= 100; }
			if (pStage->bundle[0].tcGen != TCGEN_TEXTURE) { rank -= 5; }
			if (pStage->bundle[0].numTexMods) { rank -= 5; }
			if (pStage->rgbGen != CGEN_IDENTITY && pStage->rgbGen != CGEN_IDENTITY_LIGHTING) { rank -= 3; }
		...
		if (stages[0].bundle[0].isLightmap) { stages[0] = stages[1]; }
...
static void InitShader(const char *name, int lightmapIndex) {
	...
	for (i = 0; i < MAX_SHADER_STAGES; i++) {
		...
		if (r_pbr->integer) {
			...
			stages[i].specularScale[0] = stages[i].specularScale[1] = stages[i].specularScale[2] =
				r_baseSpecular->value;
...
static shader_t *FinishShader(void) {
	...
	for (stage = 0; stage < MAX_SHADER_STAGES;) {
		...
		if (pStage->isDetail && !r_detailTextures->integer) {
			...
			for (index = stage + 1; index < MAX_SHADER_STAGES; index++) {
				if (!stages[index].active) break;
			}

			if (index < MAX_SHADER_STAGES)
				memmove(pStage, pStage + 1, sizeof(*pStage) * (index - stage));
			else {
				if (stage + 1 < MAX_SHADER_STAGES) memmove(pStage, pStage + 1, sizeof(*pStage) * (index - stage - 1));
		...
		if (pStage->bundle[0].isLightmap) {
			if (pStage->bundle[0].tcGen == TCGEN_BAD) { pStage->bundle[0].tcGen = TCGEN_LIGHTMAP; }
			hasLightmapStage = qtrue;
		} else {
			if (pStage->bundle[0].tcGen == TCGEN_BAD) { pStage->bundle[0].tcGen = TCGEN_TEXTURE; }
		}
	...
	if (!shader.sort) { shader.sort = SS_OPAQUE; }
	...
	if (stage == 0 && !shader.isSky) shader.sort = SS_FOG;
...
static char *FindShaderInShaderText(const char *shadername) {
	...
	if (shaderTextHashTable[hash]) {
		for (i = 0; shaderTextHashTable[hash][i]; i++) {
			...
			if (!Q_stricmp(token, shadername)) return p;
		}
	}
...
shader_t *R_FindShaderByName(const char *name) {
	...
	if ((name == NULL) || (name[0] == 0)) { return tr.defaultShader; }
...
shader_t *R_FindShader(const char *name, int lightmapIndex, qboolean mipRawImage) {
	...
	if (name[0] == 0) { return tr.defaultShader; }
	...
	if (shaderText) {
		...
		if (r_printShaders->integer) { ri.Printf(PRINT_ALL, "*SHADER* %s\n", name); }
...
qhandle_t RE_RegisterShaderFromImage(const char *name, int lightmapIndex, image_t *image, qboolean mipRawImage) {
	...
	if (lightmapIndex >= tr.numLightmaps) { lightmapIndex = LIGHTMAP_WHITEIMAGE; }
...
static void ScanAndLoadShaderFiles(void) {
	...
	for (i = 0; i < numShaderFiles; i++) {
		...
		// look for a .mtr file first
		{
			...
			if ((ext = strrchr(filename, '.'))) { strcpy(ext, ".mtr"); }
		...
		// Do a simple check on the shader structure in that file to make sure one bad shader file cannot fuck up all
		// other shaders.
		p = buffers[i];
		...
		while (1) {
			...
			if (token[0] != '{' || token[1] != '\0') {
				ri.Printf(PRINT_WARNING,
						  "WARNING: Ignoring shader file %s. Shader \"%s\" on line %d missing opening brace", filename,
