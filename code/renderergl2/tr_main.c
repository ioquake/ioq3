...
qboolean R_CompareVert(srfVert_t *v1, srfVert_t *v2, qboolean checkST) {
	...
	for (i = 0; i < 3; i++) {
		if (floor(v1->xyz[i] + 0.1) != floor(v2->xyz[i] + 0.1)) { return qfalse; }

		if (checkST && ((v1->st[0] != v2->st[0]) || (v1->st[1] != v2->st[1]))) { return qfalse; }
	}
...
=============
*/
void R_CalcTexDirs(vec3_t sdir, vec3_t tdir, const vec3_t v1, const vec3_t v2, const vec3_t v3, const vec2_t w1,
				   const vec2_t w2, const vec2_t w3) {
...
vec_t R_CalcTangentSpace(vec3_t tangent, vec3_t bitangent, const vec3_t normal, const vec3_t sdir, const vec3_t tdir) {
	...
	if (bitangent) CrossProduct(normal, tangent, bitangent);
...
qboolean R_CalcTangentVectors(srfVert_t *dv[3]) {
	...
	if (fabs(bb) < 0.00000001f) return qfalse;
...
int R_CullLocalBox(vec3_t localBounds[2]) {
	...
	if (r_nocull->integer) { return CULL_CLIP; }
...
int R_CullPointAndRadiusEx(const vec3_t pt, float radius, const cplane_t *frustum, int numPlanes) {
	...
	if (mightBeClipped) { return CULL_CLIP; }
...
void R_LocalPointToWorld(const vec3_t local, vec3_t world) {
	world[0] =
		local[0] * tr.or.axis[0][0] + local[1] * tr.or.axis[1][0] + local[2] * tr.or.axis[2][0] + tr.or.origin[0];
	world[1] =
		local[0] * tr.or.axis[0][1] + local[1] * tr.or.axis[1][1] + local[2] * tr.or.axis[2][1] + tr.or.origin[1];
	world[2] =
...
void R_TransformModelToClip(const vec3_t src, const float *modelMatrix, const float *projectionMatrix, vec4_t eye,
	...
	for (i = 0; i < 4; i++) {
		eye[i] = src[0] * modelMatrix[i + 0 * 4] + src[1] * modelMatrix[i + 1 * 4] + src[2] * modelMatrix[i + 2 * 4] +
				 1 * modelMatrix[i + 3 * 4];
	}

	for (i = 0; i < 4; i++) {
		dst[i] = eye[0] * projectionMatrix[i + 0 * 4] + eye[1] * projectionMatrix[i + 1 * 4] +
				 eye[2] * projectionMatrix[i + 2 * 4] + eye[3] * projectionMatrix[i + 3 * 4];
	}
}
...
static void R_SetFarClip(void) {
	...
	for (i = 0; i < 8; i++) {
		...
		if (distance > farthestCornerDistance) { farthestCornerDistance = distance; }
	}
...
void R_SetupProjection(viewParms_t *dest, float zProj, float zFar, qboolean computeFrustum) {
	...
	if (computeFrustum) R_SetupFrustum(dest, xmin, xmax, ymax, zProj, zFar, stereoSep);
}
...
void R_PlaneForSurface(surfaceType_t *surfType, cplane_t *plane) {
	...
	switch (*surfType) {
	case SF_FACE: *plane = ((srfBspSurface_t *)surfType)->cullPlane; return;
	case SF_TRIANGLES:
...
qboolean R_GetPortalOrientations(drawSurf_t *drawSurf, int entityNum, orientation_t *surface, orientation_t *camera,
	...
	for (i = 0; i < tr.refdef.num_entities; i++) {
		...
		if (e->e.reType != RT_PORTALSURFACE) { continue; }
		...
		if (e->e.oldorigin[0] == e->e.origin[0] && e->e.oldorigin[1] == e->e.origin[1] &&
...
static qboolean SurfIsOffscreen(const drawSurf_t *drawSurf, vec4_t clipDest[128]) {
	...
	for (i = 0; i < tess.numIndexes; i += 3) {
		...
		if (DotProduct(normal, tNormal) >= 0) { numTriangles--; }
	}
	if (!numTriangles) { return qtrue; }

	// mirrors can early out at this point, since we don't do a fade over distance
	// with them (although we could)
	if (IsMirror(drawSurf, entityNum)) { return qfalse; }

	if (shortest > (tess.shader->portalRange * tess.shader->portalRange)) { return qtrue; }
...
qboolean R_MirrorViewBySurface(drawSurf_t *drawSurf, int entityNum) {
	...
	if (r_noportals->integer || (r_fastsky->integer == 1)) { return qfalse; }

	// trivially reject portal/mirror
	if (SurfIsOffscreen(drawSurf, clipDest)) { return qfalse; }
...
int R_SpriteFogNum(trRefEntity_t *ent) {
	...
	if (tr.refdef.rdflags & RDF_NOWORLDMODEL) { return 0; }

	if (ent->e.renderfx & RF_CROSSHAIR) { return 0; }

	for (i = 1; i < tr.world->numfogs; i++) {
		fog = &tr.world->fogs[i];
		for (j = 0; j < 3; j++) {
			if (ent->e.origin[j] - ent->e.radius >= fog->bounds[1][j]) { break; }
			if (ent->e.origin[j] + ent->e.radius <= fog->bounds[0][j]) { break; }
		}
		if (j == 3) { return i; }
	}
...
static ID_INLINE void R_Radix(int byte, int size, drawSurf_t *source, drawSurf_t *dest) {
	...
	for (; sortKey < end; sortKey += sizeof(drawSurf_t)) ++count[*sortKey];
	...
	sortKey = ((unsigned char *)&source[0].sort) + byte;
	for (i = 0; i < size; ++i, sortKey += sizeof(drawSurf_t)) dest[index[*sortKey]++] = source[i];
}
...
void R_AddDrawSurf(surfaceType_t *surface, shader_t *shader, int fogIndex, int dlightMap, int pshadowMap, int cubemap) {
	...
	tr.refdef.drawSurfs[index].sort = (shader->sortedIndex << QSORT_SHADERNUM_SHIFT) | tr.shiftedEntityNum |
									  (fogIndex << QSORT_FOGNUM_SHIFT) | ((int)pshadowMap << QSORT_PSHADOW_SHIFT) |
									  (int)dlightMap;
...
void R_SortDrawSurfs(drawSurf_t *drawSurfs, int numDrawSurfs) {
	...
	for (i = 0; i < numDrawSurfs; i++) {
		...
		if (shader->sort > SS_PORTAL) { break; }

		// no shader should ever have this sort type
		if (shader->sort == SS_BAD) { ri.Error(ERR_DROP, "Shader '%s'with sort == SS_BAD", shader->name); }

		// if the mirror was completely clipped away, we may need to check another surface
		if (R_MirrorViewBySurface((drawSurfs + i), entityNum)) {
			// this is a debug option to see exactly what is being mirrored
			if (r_portalOnly->integer) { return; }
...
static void R_AddEntitySurface(int entityNum) {
	...
	//
	if ((ent->e.renderfx & RF_FIRST_PERSON) && (tr.viewParms.flags & VPF_NOVIEWMODEL)) { return; }
	...
	case RT_RAIL_RINGS:
		...
		if ((ent->e.renderfx & RF_THIRD_PERSON) && !tr.viewParms.isPortal) { return; }
	...
	case RT_MODEL:
		...
		if (!tr.currentModel) {
			...
			case MOD_BAD: // null model axis
				if ((ent->e.renderfx & RF_THIRD_PERSON) && !tr.viewParms.isPortal) { break; }
		...
		break;
	default: ri.Error(ERR_DROP, "R_AddEntitySurfaces: Bad reType");
	}
}
...
void R_AddEntitySurfaces(void) {
	...
	for (i = 0; i < tr.refdef.num_entities; i++) R_AddEntitySurface(i);
}
...
void R_GenerateDrawSurfs(void) {
	...
	if (!(tr.viewParms.flags & VPF_SHADOWMAP)) { R_SetFarClip(); }
...
void R_DebugGraphics(void) {
	if (tr.refdef.rdflags & RDF_NOWORLDMODEL) { return; }
	if (!r_debugSurface->integer) { return; }
...
void R_RenderView(viewParms_t *parms) {
	...
	if (parms->viewportWidth <= 0 || parms->viewportHeight <= 0) { return; }
	...
	if (numDrawSurfs > MAX_DRAWSURFS) { numDrawSurfs = MAX_DRAWSURFS; }
...
void R_RenderPshadowMaps(const refdef_t *fd) {
	...
	for (i = 0; i < tr.refdef.num_entities; i++) {
		...
		if (ent->e.reType == RT_MODEL) {
			...
			if (ent->e.nonNormalizedAxes) { scale = VectorLength(ent->e.axis[0]); }
		...
		for (j = i + 1; j < tr.refdef.num_pshadows; j++) {
			...
			if (SpheresIntersect(ps1->viewOrigin, ps1->viewRadius, ps2->viewOrigin, ps2->viewRadius)) {
				for (k = 0; k < ps1->numEntities; k++) {
					if (SpheresIntersect(ps1->entityOrigins[k], ps1->entityRadiuses[k], ps2->viewOrigin,
										 ps2->viewRadius)) {
						touch = qtrue;
						break;
					}
			...
			if (touch) {
				...
				BoundingSphereOfSpheres(ps1->viewOrigin, ps1->viewRadius, ps2->viewOrigin, ps2->viewRadius, newOrigin,
										&newRadius);
				...
				for (k = j; k < tr.refdef.num_pshadows - 1; k++) { tr.refdef.pshadows[k] = tr.refdef.pshadows[k + 1]; }
	...
	if (tr.refdef.num_pshadows > MAX_DRAWN_PSHADOWS) { tr.refdef.num_pshadows = MAX_DRAWN_PSHADOWS; }
	...
	for (i = 0; i < tr.refdef.num_pshadows; i++) {
		...
		if (shadow->viewRadius * 3.0f > shadow->lightRadius) { shadow->lightRadius = shadow->viewRadius * 3.0f; }
		...
		if (fabsf(DotProduct(up, shadow->lightViewAxis[0])) > 0.9f) { VectorSet(up, -1, 0, 0); }
	...
	for (i = 0; i < tr.refdef.num_pshadows; i++) {
...

		{
			...
			for (j = 0; j < shadow->numEntities; j++) { R_AddEntitySurface(shadow->entityNums[j]); }
...
void R_RenderSunShadowMaps(const refdef_t *fd, int level) {
	...
	if (r_forceSun->integer == 2) {
		...
		{
			lightCol[0] = lightCol[1] = lightCol[2] = CLAMP(sin(angle) * 2.0f, 0.0f, 1.0f) * 2.0f;
			lightCol[3]								= 1.0f;
		} else {
			lightCol[0] = lightCol[1] = lightCol[2] = CLAMP(sin(angle) * 2.0f * 0.1f, 0.0f, 0.1f);
			lightCol[3]								= 1.0f;
		}
	...
	// Create bounds for light projection using slice of view projection
	{
		...
		if (!glRefConfig.depthClamp) lightviewBounds[0][0] = lightviewBounds[1][0] - 8192;
		...
		if (lightViewIndependentOfCameraView) {
			...
			cascadeBound =
				MAX(lightviewBounds[1][0] - lightviewBounds[0][0], lightviewBounds[1][1] - lightviewBounds[0][1]);
...

	{
		...
		if (glRefConfig.framebufferObject) shadowParms.targetFbo = tr.sunShadowFbo[level];
...
void R_RenderCubemapSide(int cubemapIndex, int cubemapSide, qboolean subscene) {
	...
	if (!subscene) {
		...
		// fix involves changing r_FBufScale to fit smaller cubemap image size, or rendering cubemap to framebuffer
		// first
...

	{
		...
		if (scale < 1.0001f && cubemapSide == 0) {
			ri.Printf(PRINT_ALL, "cubemap %d %s (%f, %f, %f) is outside the lightgrid or inside a wall!\n",
					  cubemapIndex, tr.cubemaps[cubemapIndex].name, tr.refdef.vieworg[0], tr.refdef.vieworg[1],
					  tr.refdef.vieworg[2]);
		}
	}
