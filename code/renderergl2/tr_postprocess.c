	...
	if (autoExposure) {
		if (lastFrameCount == 0 || tr.frameCount < lastFrameCount || tr.frameCount - lastFrameCount > 5) {
			...
			while (size > 1) {
				...
				if (size == 1) dstFbo = tr.targetLevelsFbo;

				// FBO_Blit(targetFbo, srcBox, NULL, tr.textureScratchFbo[nextScratch], dstBox,
				// &tr.calclevels4xShader[1], NULL, 0);
		...
		FBO_Blit(tr.targetLevelsFbo, srcBox, NULL, tr.calcLevelsFbo, NULL, NULL, color,
...
void RB_BokehBlur(FBO_t *src, ivec4_t srcBox, FBO_t *dst, ivec4_t dstBox, float blur) {
	...
	if (glRefConfig.framebufferObject) {
		...
		if (blur > 0.0f && blur <= 1.0f) {
			...
			FBO_Blit(tr.quarterFbo[0], NULL, NULL, dst, dstBox, NULL, color,
					 GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA);
		}
#ifndef HQ_BLUR
		// ok blur, but can see some pixelization
			...
			FBO_Blit(tr.textureScratchFbo[0], NULL, NULL, dst, dstBox, NULL, color,
					 GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA);
		} else if (blur > 2.0f) {
			...
			for (i = 0; i < 2; i++) {
				...
				if (i != 0)
					FBO_Blit(tr.textureScratchFbo[0], NULL, blurTexScale, tr.textureScratchFbo[1], NULL,
							 &tr.bokehShader, color, GLS_SRCBLEND_ONE | GLS_DSTBLEND_ONE);
				else
					FBO_Blit(tr.textureScratchFbo[0], NULL, blurTexScale, tr.textureScratchFbo[1], NULL,
							 &tr.bokehShader, color, 0);
			}
...
static void RB_RadialBlur(FBO_t *srcFbo, FBO_t *dstFbo, int passes, float stretch, float x, float y, float w, float h,
						  float xcenter, float ycenter, float alpha) {
	ivec4_t		srcBox, dstBox;
...
void RB_SunRays(FBO_t *srcFbo, ivec4_t srcBox, FBO_t *dstFbo, ivec4_t dstBox) {
	...
	if (dot < cutoff) return;

	if (!RB_UpdateSunFlareVis()) return;
	...
	// initialize quarter buffers
	{
		...
		if (colorize) {
			...
			FBO_Blit(tr.sunRaysFbo, rayBox, NULL, tr.quarterFbo[0], quarterBox, NULL, color,
	...
	// radial blur passes, ping-ponging between the two quarter-size buffers
	{
		...
		int			i;
		for (i = 0; i < 2; ++i) {
			RB_RadialBlur(tr.quarterFbo[i & 1], tr.quarterFbo[(~i) & 1], 5, stretch, 0.f, 0.f, tr.quarterFbo[0]->width,
						  tr.quarterFbo[0]->height, pos[0], pos[1], 1.125f);
			stretch += stretch_add;
		}
	}
...
void RB_GaussianBlur(float blur) {
...

	{
		...
		FBO_BlitFromTexture(tr.whiteImage, NULL, NULL, tr.textureScratchFbo[0], NULL, NULL, color,
		...
		color[3] = factor;
		FBO_Blit(tr.textureScratchFbo[0], srcBox, NULL, NULL, dstBox, NULL, color,
				 GLS_SRCBLEND_SRC_ALPHA | GLS_DSTBLEND_ONE_MINUS_SRC_ALPHA);
	}
}
