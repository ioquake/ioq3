...
static qboolean R_CullSurface(msurface_t *surf) {
	if (r_nocull->integer || surf->cullinfo.type == CULLINFO_NONE) { return qfalse; }

	if (r_nocurves->integer && *surf->data == SF_GRID) { return qtrue; }
	...
	if (surf->cullinfo.type & CULLINFO_PLANE) {
		...
		if (!r_facePlaneCull->integer) { return qfalse; }

		ct = surf->shader->cullType;

		if (ct == CT_TWO_SIDED) { return qfalse; }
		...
		*/
		...
		if (ct == CT_FRONT_SIDED) {
			if (d < surf->cullinfo.plane.dist - 8) { return qtrue; }
		} else {
			if (d > surf->cullinfo.plane.dist + 8) { return qtrue; }
		}
	...
	if (surf->cullinfo.type & CULLINFO_SPHERE) {
		...
		if (sphereCull == CULL_OUT) { return qtrue; }
	}
...
static int R_DlightSurface(msurface_t *surf, int dlightBits) {
	int				i, num_dlights = tr.refdef.num_dlights;
	const dlight_t *dl = tr.refdef.dlights;

	if (surf->cullinfo.type & CULLINFO_PLANE) {
		for (i = 0; i < num_dlights; i++, dl++) {
			if (!(dlightBits & (1u << i))) { continue; }
			const float d = DotProduct(dl->origin, surf->cullinfo.plane.normal) - surf->cullinfo.plane.dist;
	...
	if (surf->cullinfo.type & CULLINFO_SPHERE) {
		for (i = 0; i < num_dlights; i++, dl++) {
			if (!(dlightBits & (1u << i))) { continue; }
			// dl = &tr.refdef.dlights[i];
			if (!SpheresIntersect((vec_t *)dl->origin, dl->radius, (vec_t *)surf->cullinfo.localOrigin,
	...
	if (surf->cullinfo.type & CULLINFO_SPHERE) {
		for (i = 0; i < tr.refdef.num_pshadows; i++) {
			...
			if (!SpheresIntersect(ps->viewOrigin, ps->viewRadius, surf->cullinfo.localOrigin, surf->cullinfo.radius) ||
				DotProduct(surf->cullinfo.localOrigin, ps->cullPlane.normal) - ps->cullPlane.dist <
					-surf->cullinfo.radius) {
				// pshadow doesn't reach the bounds
				pshadowBits &= ~(1 << i);
			}
		...
		if (dlightBits) {
			...
			for (i = 0; i < tr.refdef.num_dlights; i++) {
				...
				if (dlightBits & (1 << i)) {
					...
					if (dist > -dl->radius) { newDlights[0] |= (1 << i); }
					if (dist < dl->radius) { newDlights[1] |= (1 << i); }
				}
			}
		}
		...
		if (pshadowBits) {
			...
			for (i = 0; i < tr.refdef.num_pshadows; i++) {
				...
				if (pshadowBits & (1 << i)) {
					...
					if (dist > -shadow->lightRadius) { newPShadows[0] |= (1 << i); }
					if (dist < shadow->lightRadius) { newPShadows[1] |= (1 << i); }
				}
			}
		}
...
static mnode_t *R_PointInLeaf(const vec3_t p) {
	...
	if (!tr.world) { ri.Error(ERR_DROP, "R_PointInLeaf: bad model"); }
...
static const byte *R_ClusterPVS(int cluster) {
	if (tr.world->vis && cluster >= 0 && cluster < tr.world->numClusters) {
		return tr.world->vis + (size_t)cluster * tr.world->clusterBytes;
	} else {
		return NULL;
	}
}
...
qboolean R_inPVS(const vec3_t p1, const vec3_t p2) {
	mnode_t *leaf1 = R_PointInLeaf(p1);
	mnode_t *leaf2 = R_PointInLeaf(p2);

	if (leaf1->cluster != leaf2->cluster) {
		byte *vis = ri.CM_ClusterPVS(leaf1->cluster);
		int	  idx = leaf2->cluster >> 3;
		int	  bit = 1 << (leaf2->cluster & 7);
		return (vis[idx] & bit) ? qtrue : qfalse;
	}
...
static void R_MarkLeaves(void) {
	...
	for (i = 0; i < MAX_VISCOUNTS; i++) {
		// if the areamask or r_showcluster was modified, invalidate all
		// visclusters this caused doors to open into undrawn areas
	...
	if (r_showcluster->modified || r_showcluster->integer) {
		...
		if (r_showcluster->integer) { ri.Printf(PRINT_ALL, "cluster:%i  area:%i\n", cluster, leaf->area); }
	}
	...
	for (i = 0, leaf = tr.world->nodes; i < tr.world->numnodes; i++, leaf++) {
		cluster = leaf->cluster;
		if (cluster < 0 || cluster >= tr.world->numClusters) { continue; }

		// check general pvs
		if (vis && !(vis[cluster >> 3] & (1 << (cluster & 7)))) { continue; }
...
void R_AddWorldSurfaces(void) {
	...
	if (tr.refdef.num_dlights > MAX_DLIGHTS) { tr.refdef.num_dlights = MAX_DLIGHTS; }
