...
void RE_AddPolyToScene(qhandle_t hShader, int numVerts, const polyVert_t *verts, int numPolys) {
	...
	for (j = 0; j < numPolys; j++) {
		...
		// see if it is in a fog volume
			...
			for (i = 1; i < poly->numVerts; i++) { AddPointToBounds(poly->verts[i].xyz, bounds[0], bounds[1]); }
			...
			if (fogIndex == tr.world->numfogs) { fogIndex = 0; }
		}
...
void RE_AddDynamicLightToScene(const vec3_t org, float intensity, float r, float g, float b, int additive) {
	...
	if (glConfig.hardwareType == GLHW_RIVA128 || glConfig.hardwareType == GLHW_PERMEDIA2) { return; }
...
void RE_BeginScene(const refdef_t *fd) {
	...
	if ((tr.refdef.rdflags & RDF_NOWORLDMODEL) || !(r_depthPrepass->value)) {
		...
		if (r_sunlightMode->integer == 1) {
			tr.refdef.sunAmbCol[0] = tr.refdef.sunAmbCol[1] = tr.refdef.sunAmbCol[2] =
				r_forceSun->integer ? r_forceSunAmbientScale->value : tr.sunShadowScale;
		} else {
			if (r_forceSun->integer)
...
void RE_RenderScene(const refdef_t *fd) {
	...
	if (!tr.world && !(fd->rdflags & RDF_NOWORLDMODEL)) { ri.Error(ERR_DROP, "R_RenderScene: NULL worldmodel"); }
	...
	if (0) //(glRefConfig.framebufferObject && !( fd->rdflags & RDF_NOWORLDMODEL ))
	{
		...
		for (i = 0; i < tr.numCubemaps; i++) {
			for (j = 0; j < 6; j++) { R_RenderCubemapSide(i, j, qtrue); }
		}
	}
	...
	if (!(fd->rdflags & RDF_NOWORLDMODEL)) R_AddPostProcessCmd();
