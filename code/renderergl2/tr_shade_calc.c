...
static float *TableForFunc(genFunc_t func) {
	switch (func) {
	case GF_SIN: return tr.sinTable;
...
void RB_CalcDeformVertexes(deformStage_t *ds) {
	...
	if (ds->deformationWave.frequency == 0) {
		...
		for (i = 0; i < tess.numVertexes; i++, xyz += 4, normal += 4) {
			...
			scale = WAVEVALUE(table, ds->deformationWave.base, ds->deformationWave.amplitude,
...
void RB_CalcMoveVertexes(deformStage_t *ds) {
	...
	scale = WAVEVALUE(table, ds->deformationWave.base, ds->deformationWave.amplitude, ds->deformationWave.phase,
					  ds->deformationWave.frequency);

	VectorScale(ds->moveVector, scale, offset);

	xyz = (float *)tess.xyz;
	for (i = 0; i < tess.numVertexes; i++, xyz += 4) { VectorAdd(xyz, offset, xyz); }
}
...
void DeformText(const char *text) {
	...
	for (i = 0; i < 4; i++) {
		...
		if (tess.xyz[i][2] < bottom) { bottom = tess.xyz[i][2]; }
		if (tess.xyz[i][2] > top) { top = tess.xyz[i][2]; }
	}
...
void RB_DeformTessGeometry(void) {
	...
	for (i = 0; i < tess.shader->numDeforms; i++) {
		...
		case DEFORM_NORMALS: RB_CalcDeformNormals(ds); break;
		...
		case DEFORM_TEXT5:
		case DEFORM_TEXT6:
		case DEFORM_TEXT7: DeformText(backEnd.refdef.text[ds->deformation - DEFORM_TEXT0]); break;
		}
	}
}
...
*/
float RB_CalcWaveAlphaSingle(const waveForm_t *wf) { return EvalWaveFormClamped(wf); }
