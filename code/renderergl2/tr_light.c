...
void R_DlightBmodel(bmodel_t *bmodel) {
	...
	for (i = 0; i < tr.refdef.num_dlights; i++) {
		...
		for (j = 0; j < 3; j++) {
			if (dl->transformed[j] - bmodel->bounds[1][j] > dl->radius) { break; }
			if (bmodel->bounds[0][j] - dl->transformed[j] > dl->radius) { break; }
		}
		if (j < 3) { continue; }
	...
	for (i = 0; i < bmodel->numSurfaces; i++) {
		...
		case SF_FACE:
		case SF_GRID:
		case SF_TRIANGLES: ((srfBspSurface_t *)surf->data)->dlightBits = mask; break;
...
void R_SetupEntityLighting(const trRefdef_t *refdef, trRefEntity_t *ent) {
	...
	if (ent->lightingCalculated) { return; }
	...
	if (r_debugLight->integer) { LogLight(ent); }
...
int R_LightDirForPoint(vec3_t point, vec3_t lightDir, vec3_t normal, world_t *world) {
	...
	if (world->lightGridData == NULL) return qfalse;
