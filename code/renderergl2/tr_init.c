...
static void InitOpenGL(void) {
	...
	if (glConfig.vidWidth == 0) {
		...
		if (glConfig.maxTextureSize <= 0) { glConfig.maxTextureSize = 0; }
		...
		glRefConfig.glslMaxAnimatedBones = Com_Clamp(0, IQM_MAX_JOINTS, (temp - 160.0f) / 16.0f);
		if (glRefConfig.glslMaxAnimatedBones < 12) { glRefConfig.glslMaxAnimatedBones = 0; }
	}
...
void GL_CheckErrs(char *file, int line) {
	...
	if (r_ignoreGLErrors->integer) { return; }
	switch (err) {
...
void RB_TakeScreenshot(int x, int y, int width, int height, char *fileName) {
	...
	// gamma correct
	if (glConfig.deviceSupportsGamma) R_GammaCorrect(allbuf + offset, memcount);
...
void RB_TakeScreenshotJPEG(int x, int y, int width, int height, char *fileName) {
	...
	// gamma correct
	if (glConfig.deviceSupportsGamma) R_GammaCorrect(buffer + offset, memcount);
...
void R_LevelShot(void) {
	...
	// gamma correct
	if (glConfig.deviceSupportsGamma) { R_GammaCorrect(buffer + 18, 128 * 128 * 3); }
...
void R_ScreenShot_f(void) {
	...
	if (!silent) { ri.Printf(PRINT_ALL, "Wrote %s\n", checkname); }
}
...
const void *RB_TakeVideoFrameCmd(const void *data) {
	...
	// gamma correct
	if (glConfig.deviceSupportsGamma) R_GammaCorrect(cBuf, memcount);
...
void GL_SetDefaultState(void) {
	...
	if (glRefConfig.framebufferObject) GL_BindNullFramebuffers();
	...
	if (glRefConfig.vertexArrayObject) qglBindVertexArray(0);
	...
	if (glRefConfig.seamlessCubeMap) qglEnable(GL_TEXTURE_CUBE_MAP_SEAMLESS);
...
void GfxInfo_f(void) {
	...
	if (qglGetStringi) {
		...
		for (i = 0; i < numExtensions; i++) { ri.Printf(PRINT_ALL, "%s ", qglGetStringi(GL_EXTENSIONS, i)); }
	} else {
...
void R_InitQueries(void) {
	if (!glRefConfig.occlusionQuery) return;

	if (r_drawSunRays->integer) qglGenQueries(ARRAY_LEN(tr.sunFlareQuery), tr.sunFlareQuery);
}

void R_ShutDownQueries(void) {
	if (!glRefConfig.occlusionQuery) return;

	if (r_drawSunRays->integer) qglDeleteQueries(ARRAY_LEN(tr.sunFlareQuery), tr.sunFlareQuery);
}
...
void R_Init(void) {
	...
	if ((intptr_t)tess.xyz & 15) { ri.Printf(PRINT_WARNING, "tess.xyz not 16 byte aligned\n"); }
	...
	for (i = 0; i < FUNCTABLE_SIZE; i++) {
		...
		if (i < FUNCTABLE_SIZE / 2) {
			if (i < FUNCTABLE_SIZE / 4) {
				tr.triangleTable[i] = (float)i / ((float)FUNCTABLE_SIZE / 4);
			} else {
	...
	if (max_polys < MAX_POLYS) max_polys = MAX_POLYS;

	max_polyverts = r_maxpolyverts->integer;
	if (max_polyverts < MAX_POLYVERTS) max_polyverts = MAX_POLYVERTS;
	...
	if (glRefConfig.framebufferObject) FBO_Init();
	...
	err = qglGetError();
	if (err != GL_NO_ERROR) ri.Printf(PRINT_ALL, "glGetError() = 0x%x\n", err);
...
void RE_Shutdown(qboolean destroyWindow) {
	...
	if (tr.registered) {
		...
		R_ShutDownQueries();
		if (glRefConfig.framebufferObject) FBO_Shutdown();
		R_DeleteTextures();
		R_ShutdownVaos();
...
void RE_EndRegistration(void) {
	R_IssuePendingRenderCommands();
	if (!ri.Sys_LowPhysicalMemory()) { RB_ShowImages(); }
}
