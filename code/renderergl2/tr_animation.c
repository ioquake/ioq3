...
int R_MDRComputeFogNum(mdrHeader_t *header, trRefEntity_t *ent) {
	...
	for (i = 1; i < tr.world->numfogs; i++) {
		fog = &tr.world->fogs[i];
		for (j = 0; j < 3; j++) {
			if (localOrigin[j] - mdrFrame->radius >= fog->bounds[1][j]) { break; }
			if (localOrigin[j] + mdrFrame->radius <= fog->bounds[0][j]) { break; }
		}
		if (j == 3) { return i; }
	}
...
void R_MDRAddAnimSurfaces(trRefEntity_t *ent) {
	...
	// range checked again.
	//
	if ((ent->e.frame >= header->numFrames) || (ent->e.frame < 0) || (ent->e.oldframe >= header->numFrames) ||
		(ent->e.oldframe < 0)) {
	...
	if (header->numLODs <= 0) return;
	if (header->numLODs <= lodnum) lodnum = header->numLODs - 1;

	lod = (mdrLOD_t *)((byte *)header + header->ofsLODs);
	for (i = 0; i < lodnum; i++) { lod = (mdrLOD_t *)((byte *)lod + lod->ofsEnd); }
...
void RB_MDRSurfaceAnim(mdrSurface_t *surface) {
	...
	for (j = 0; j < indexes; j++) { tess.indexes[baseIndex + j] = baseVertex + triangles[j]; }
