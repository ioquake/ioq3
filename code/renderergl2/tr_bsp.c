...
static void R_LoadLightmaps(lump_t *l, lump_t *surfs) {
	...
	if (numLightmaps <= 1) {
		...
		for (i = 0, surf = (dsurface_t *)(fileBase + surfs->fileofs); i < surfs->filelen / sizeof(dsurface_t);
			 i++, surf++) {
	...
	if (tr.worldDeluxeMapping) numLightmaps >>= 1;
	...
	if (r_mergeLightmaps->integer) {
		...
		while (lightmapCols * lightmapRows < numLightmaps && lightmapCols != maxLightmapsPerAxis) lightmapCols <<= 1;

		while (lightmapCols * lightmapRows < numLightmaps && lightmapRows != maxLightmapsPerAxis) lightmapRows <<= 1;
	...
	if (tr.worldDeluxeMapping) tr.deluxemaps = ri.Hunk_Alloc(tr.numLightmaps * sizeof(image_t *), h_low);
	...
	if (r_hdr->integer) {
		...
		if (ri.FS_FileExists(filename)) textureInternalFormat = GL_RGBA16;
	}
	...
	if (r_mergeLightmaps->integer) {
		...
		for (i = 0; i < tr.numLightmaps; i++) {
			tr.lightmaps[i] = R_CreateImage(va("_fatlightmap%d", i), NULL, width, height, IMGTYPE_COLORALPHA, imgFlags,
											textureInternalFormat);

			if (tr.worldDeluxeMapping)
				tr.deluxemaps[i] =
					R_CreateImage(va("_fatdeluxemap%d", i), NULL, width, height, IMGTYPE_DELUXE, imgFlags, 0);
		}
	}
	...
	for (i = 0; i < numLightmaps; i++) {
		...
		// if (tr.worldLightmapping)
		{
			...
			if (textureInternalFormat == GL_RGBA16) {
				Com_sprintf(filename, sizeof(filename), "maps/%s/lm_%04d.hdr", s_worldData.baseName,
			...
			if (hdrLightmap) {
				...
				if (p >= end) ri.Error(ERR_DROP, "Bad header for %s!", filename);
			...
			if (r_mergeLightmaps->integer)
				...
				tr.lightmaps[i] = R_CreateImage(va("*lightmap%d", i), image, tr.lightmapSize, tr.lightmapSize,
												IMGTYPE_COLORALPHA, imgFlags, textureInternalFormat);

			if (hdrLightmap) ri.FS_FreeFile(hdrLightmap);
		}
		...
		if (tr.worldDeluxeMapping) {
			...
			if (r_mergeLightmaps->integer)
				...
				tr.deluxemaps[i] = R_CreateImage(va("*deluxemap%d", i), image, tr.lightmapSize, tr.lightmapSize,
...
static float FatPackU(float input, int lightmapnum) {
	if (lightmapnum < 0) return input;

	if (tr.worldDeluxeMapping) lightmapnum >>= 1;
...
static int FatLightmap(int lightmapnum) {
	if (lightmapnum < 0) return lightmapnum;

	if (tr.worldDeluxeMapping) lightmapnum >>= 1;

	if (tr.fatLightmapCols > 0) return lightmapnum / (tr.fatLightmapCols * tr.fatLightmapRows);
...
static shader_t *ShaderForShaderNum(int shaderNum, int lightmapNum) {
	...
	if (r_vertexLight->integer || glConfig.hardwareType == GLHW_PERMEDIA2) { lightmapNum = LIGHTMAP_BY_VERTEX; }

	if (r_fullbright->integer) { lightmapNum = LIGHTMAP_WHITEIMAGE; }

	shader = R_FindShader(dsh->shader, lightmapNum, qtrue);

	// if the shader had errors, just use default shader
	if (shader->defaultShader) { return tr.defaultShader; }
...
void LoadDrawVertToSrfVert(srfVert_t *s, drawVert_t *d, int realLightmapNum, float hdrVertColors[3], vec3_t *bounds) {
	...
	if (bounds) AddPointToBounds(s->xyz, bounds[0], bounds[1]);
...
static void ParseFace(dsurface_t *ds, drawVert_t *verts, float *hdrVertColors, msurface_t *surf, int *indexes) {
	...
	if (r_singleShader->integer && !surf->shader->isSky) { surf->shader = tr.defaultShader; }
	...
	for (i = 0; i < numVerts; i++)
		LoadDrawVertToSrfVert(&cv->verts[i], &verts[i], realLightmapNum,
							  hdrVertColors ? hdrVertColors + (ds->firstVert + i) * 3 : NULL, surf->cullinfo.bounds);
	...
	for (i = 0, tri = cv->indexes; i < numIndexes; i += 3, tri += 3) {
		for (j = 0; j < 3; j++) {
			...
			if (tri[j] >= numVerts) { ri.Error(ERR_DROP, "Bad index in face surface"); }
		}
	...
	for (i = 0; i < 3; i++) { cv->cullPlane.normal[i] = LittleFloat(ds->lightmapVecs[2][i]); }
			...
			for (k = 1; k < grid1->width - 1; k++) {
				...
				for (m = 0; m < 2; m++) {
					...
					for (l = 1; l < grid2->height - 1; l++) {
						//
						if (fabs(grid1->verts[k + offset1].xyz[0] - grid2->verts[grid2->width * l + offset2].xyz[0]) >
							.1)
							continue;
						if (fabs(grid1->verts[k + offset1].xyz[1] - grid2->verts[grid2->width * l + offset2].xyz[1]) >
							.1)
							continue;
						if (fabs(grid1->verts[k + offset1].xyz[2] - grid2->verts[grid2->width * l + offset2].xyz[2]) >
							.1)
							continue;
						// ok the points are equal and should have the same lod error
		...
		for (n = 0; n < 2; n++) {
			...
			for (k = 1; k < grid1->height - 1; k++) {
				for (m = 0; m < 2; m++) {
					...
					for (l = 1; l < grid2->width - 1; l++) {
						//
						if (fabs(grid1->verts[grid1->width * k + offset1].xyz[0] - grid2->verts[l + offset2].xyz[0]) >
							.1)
							continue;
						if (fabs(grid1->verts[grid1->width * k + offset1].xyz[1] - grid2->verts[l + offset2].xyz[1]) >
							.1)
							continue;
						if (fabs(grid1->verts[grid1->width * k + offset1].xyz[2] - grid2->verts[l + offset2].xyz[2]) >
							.1)
							continue;
						// ok the points are equal and should have the same lod error
...
void R_FixSharedVertexLodError(void) {
	...
	for (i = 0; i < s_worldData.numsurfaces; i++) {
		...
		// if this surface is not a grid
		if (grid1->surfaceType != SF_GRID) continue;
		//
		if (grid1->lodFixed) continue;
		//
		grid1->lodFixed = 2;
	...
	for (n = 0; n < 2; n++) {
		...
		for (k = 0; k < grid1->width - 2; k += 2) {
			...
			for (m = 0; m < 2; m++) {
				...
				if (grid2->width >= MAX_GRID_SIZE) break;
				if (m)
					offset2 = (grid2->height - 1) * grid2->width;
				else
					offset2 = 0;
				for (l = 0; l < grid2->width - 1; l++) {
					...
					if (fabs(v1[0] - v2[0]) > .1) continue;
					if (fabs(v1[1] - v2[1]) > .1) continue;
					if (fabs(v1[2] - v2[2]) > .1) continue;
					...
					v2 = grid2->verts[l + 1 + offset2].xyz;
					if (fabs(v1[0] - v2[0]) < .01 && fabs(v1[1] - v2[1]) < .01 && fabs(v1[2] - v2[2]) < .01) continue;
					//
					...
					if (m)
						...
						row = 0;
					R_GridInsertColumn(grid2, l + 1, row, grid1->verts[k + 1 + offset1].xyz,
			...
			for (m = 0; m < 2; m++) {
				...
				if (grid2->height >= MAX_GRID_SIZE) break;
				if (m)
					offset2 = grid2->width - 1;
				else
					offset2 = 0;
				for (l = 0; l < grid2->height - 1; l++) {
					...
					if (m)
						...
						column = 0;
					R_GridInsertRow(grid2, l + 1, column, grid1->verts[k + 1 + offset1].xyz,
									grid1->widthLodError[k + 1]);
					grid2->lodStitched					= qfalse;
					s_worldData.surfaces[grid2num].data = (void *)grid2;
					return qtrue;
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->width - 1; l++) {
					...
					if (m)
						...
						row = 0;
					R_GridInsertColumn(grid2, l + 1, row, grid1->verts[grid1->width * (k + 1) + offset1].xyz,
			...
			for (m = 0; m < 2; m++) {
				...
				for (l = 0; l < grid2->height - 1; l++) {
					...
					if (m)
						...
						column = 0;
					R_GridInsertRow(grid2, l + 1, column, grid1->verts[grid1->width * (k + 1) + offset1].xyz,
									grid1->heightLodError[k + 1]);
					grid2->lodStitched					= qfalse;
					s_worldData.surfaces[grid2num].data = (void *)grid2;
					return qtrue;
	...
	for (n = 0; n < 2; n++) {
		...
		for (k = grid1->width - 1; k > 1; k -= 2) {
			...
			for (m = 0; m < 2; m++) {
				...
				if (!grid2 || grid2->width >= MAX_GRID_SIZE) break;
				if (m)
					offset2 = (grid2->height - 1) * grid2->width;
				else
					offset2 = 0;
			...
			for (m = 0; m < 2; m++) {
				...
				if (!grid2 || grid2->height >= MAX_GRID_SIZE) break;
				if (m)
					offset2 = grid2->width - 1;
				else
					offset2 = 0;
...
int R_TryStitchingPatch(int grid1num) {
	...
	for (j = 0; j < s_worldData.numsurfaces; j++) {
		...
		if (grid1->lodOrigin[2] != grid2->lodOrigin[2]) continue;
		//
		while (R_StitchPatches(grid1num, j)) { numstitches++; }
	}
	return numstitches;
}
...
void R_MovePatchSurfacesToHunk(void) {
	...
	for (i = 0; i < s_worldData.numsurfaces; i++) {
		...
		// if this surface is not a grid
		if (grid->surfaceType != SF_GRID) continue;
		//
...
static void R_LoadSurfaces(lump_t *surfs, lump_t *verts, lump_t *indexLump) {
	...
	if (surfs->filelen % sizeof(*in)) ri.Error(ERR_DROP, "LoadMap: funny lump size in %s", s_worldData.name);
	count = surfs->filelen / sizeof(*in);

	dv = (void *)(fileBase + verts->fileofs);
	if (verts->filelen % sizeof(*dv)) ri.Error(ERR_DROP, "LoadMap: funny lump size in %s", s_worldData.name);

	indexes = (void *)(fileBase + indexLump->fileofs);
	if (indexLump->filelen % sizeof(*indexes)) ri.Error(ERR_DROP, "LoadMap: funny lump size in %s", s_worldData.name);
	...
	if (r_hdr->integer) {
		...
		if (hdrVertColors) {
			...
			if (size != sizeof(float) * 3 * (verts->filelen / sizeof(*dv)))
				ri.Error(ERR_DROP, "Bad size for %s (%i, expected %i)!", filename, size,
						 (int)((sizeof(float)) * 3 * (verts->filelen / sizeof(*dv))));
		}
	}
	...
	for (i = 0; i < count; i++, in++, out++) {
		...
		case MST_PATCH: out->data = ri.Hunk_Alloc(sizeof(srfBspSurface_t), h_low); break;
		case MST_TRIANGLE_SOUP: out->data = ri.Hunk_Alloc(sizeof(srfBspSurface_t), h_low); break;
		case MST_PLANAR: out->data = ri.Hunk_Alloc(sizeof(srfBspSurface_t), h_low); break;
		case MST_FLARE: out->data = ri.Hunk_Alloc(sizeof(srfFlare_t), h_low); break;
		default: break;
		}
	}
	...
	for (i = 0; i < count; i++, in++, out++) {
		...
		case MST_FLARE:
			...
			numFlares++;
			break;
		default: ri.Error(ERR_DROP, "Bad surfaceType");
		}
	}

	if (hdrVertColors) { ri.FS_FreeFile(hdrVertColors); }
...
static void R_LoadSubmodels(lump_t *l) {
	...
	if (l->filelen % sizeof(*in)) ri.Error(ERR_DROP, "LoadMap: funny lump size in %s", s_worldData.name);
	...
	for (i = 0; i < count; i++, in++, out++) {
		...
		if (model == NULL) { ri.Error(ERR_DROP, "R_LoadSubmodels: R_AllocModel() failed"); }
...
static void R_LoadNodesAndLeafs(lump_t *nodeLump, lump_t *leafLump) {
	...
	for (i = 0; i < numLeafs; i++, inLeaf++, out++) {
		...
		if (out->cluster >= s_worldData.numClusters) { s_worldData.numClusters = out->cluster + 1; }
...
static void R_LoadFogs(lump_t *l, lump_t *brushesLump, lump_t *sidesLump) {
	...
	if (l->filelen % sizeof(*fogs)) { ri.Error(ERR_DROP, "LoadMap: funny lump size in %s", s_worldData.name); }
	...
	if (sidesLump->filelen % sizeof(*sides)) { ri.Error(ERR_DROP, "LoadMap: funny lump size in %s", s_worldData.name); }
	sidesCount = sidesLump->filelen / sizeof(*sides);

	for (i = 0; i < count; i++, fogs++) {
		out->originalBrushNumber = LittleLong(fogs->brushNum);

		if ((unsigned)out->originalBrushNumber >= brushesCount) { ri.Error(ERR_DROP, "fog brushNumber out of range"); }
		brush = brushes + out->originalBrushNumber;

		firstSide = LittleLong(brush->firstSide);

		if ((unsigned)firstSide > sidesCount - 6) { ri.Error(ERR_DROP, "fog brush sideNumber out of range"); }
		...
		out->colorInt =
			ColorBytes4(shader->fogParms.color[0], shader->fogParms.color[1], shader->fogParms.color[2], 1.0);
	...
	if (r_hdr->integer) {
		...
		if (hdrLightGrid) {
			...
			if (size != sizeof(float) * 6 * numGridPoints)
				ri.Error(ERR_DROP, "Bad size for %s (%i, expected %i)!", filename, size,
						 (int)(sizeof(float)) * 6 * numGridPoints);
		...
		if (hdrLightGrid) ri.FS_FreeFile(hdrLightGrid);
	}
}
...
void R_LoadEntities(lump_t *l) {
	...
	while (1) {
		...
		if (!Q_strncmp(keyname, s, strlen(s))) {
			...
			*s++ = 0;
			if (r_vertexLight->integer) { R_RemapShader(value, s, "0"); }
			continue;
		}
...
void R_RenderMissingCubemaps(void) {
	...
	for (i = 0; i < tr.numCubemaps; i++) {
		if (!tr.cubemaps[i].image) {
			tr.cubemaps[i].image = R_CreateImage(va("*cubeMap%d", i), NULL, r_cubemapSize->integer,
												 r_cubemapSize->integer, IMGTYPE_COLORALPHA, flags, GL_RGBA8);
...
void R_CalcVertexLightDirs(void) {
	...
	for (k = 0, surface = &s_worldData.surfaces[0]; k < s_worldData.numsurfaces /* s_worldData.numWorldSurfaces */;
		 k++, surface++) {
...
void RE_LoadWorldMap(const char *name) {
	...
	if (tr.worldMapLoaded) { ri.Error(ERR_DROP, "ERROR: attempted to redundantly load world map"); }
	...
	if (!buffer.b) { ri.Error(ERR_DROP, "RE_LoadWorldMap: %s not found", name); }
	...
	if (0) {
		...
		for (i = 0; i < lightGridSize; i++, data += 8) {
			...
			if (!(data[0] + data[1] + data[2] + data[3] + data[4] + data[5])) continue;
				...
				for (x = ibounds[0][0]; x <= ibounds[1][0]; x++) {
					for (y = ibounds[0][1]; y <= ibounds[1][1]; y++) {
						for (z = ibounds[0][2]; z <= ibounds[1][2]; z++) {
							uint8_t primaryLight =
								primaryLightGrid[x * 8 + y * 8 * w->lightGridBounds[0] +
												 z * 8 * w->lightGridBounds[0] * w->lightGridBounds[2]];

							if (primaryLight == 0) continue;

							numSamples++;

							if (primaryLight == 1) goodSamples++;
						}
					}
				}
	...
	if (r_cubeMapping->integer) {
		...
		if (!tr.numCubemaps) { R_LoadCubemapEntities("misc_cubemap"); }
