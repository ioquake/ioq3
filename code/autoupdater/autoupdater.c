...
static void windowsWaitForProcessToDie(const DWORD pid) {
	...
	if (WaitForSingleObject(h, INFINITE) != WAIT_OBJECT_0) { die("WaitForSingleObject failed"); }
	CloseHandle(h);
}
...
#define APPENDCMDLINE(str)                                                                                             \
	...
	va_start(ap, exe);
	APPENDCMDLINE(exe);
	while ((arg = va_arg(ap, const char *)) != NULL) { APPENDCMDLINE(arg); }
	va_end(ap);

	if (totallen >= sizeof(cmdline)) { die("command line too long to launch."); }
...
static void		 prepHttpLib(void) {
	 hInternet = InternetOpenA(AUTOUPDATE_USER_AGENT, INTERNET_OPEN_TYPE_PRECONFIG, NULL, NULL, 0);
	 if (!hInternet) { die("InternetOpen failed"); }
}
...
static int runHttpDownload(const char *from, FILE *to) {
	...
	HINTERNET hUrl	   = InternetOpenUrlA(hInternet, from, NULL, 0,
										  INTERNET_FLAG_HYPERLINK | INTERNET_FLAG_IGNORE_REDIRECT_TO_HTTP |
											  ...
											  INTERNET_FLAG_NO_COOKIES | INTERNET_FLAG_NO_UI | INTERNET_FLAG_RESYNCHRONIZE |
...
#define LOADCURLSYM(fn)                                                                                                \
	if ((CURL_##fn = (CURLFN_##fn)dlsym(handle, #fn)) == NULL) { die("Failed to load libcurl symbol '" #fn "'"); }
...
#define curl_global_cleanup CURL_curl_global_cleanup
	...
	if (curl_global_init(CURL_GLOBAL_DEFAULT) != CURLE_OK) { die("curl_global_init() failed!"); }
}

static void shutdownHttpLib(void) {
	if (curl_global_cleanup) { curl_global_cleanup(); }
}
...
static void outOfMemory(void) { die("Out of memory"); }
...
static int64_t fileLength(const char *fname) {
	struct stat statbuf;
	if (stat(fname, &statbuf) == -1) { return -1; }
...
static void parseArgv(int argc, char **argv) {
	...
	for (i = 0; i < argc; i++) { infof("  argv[%d]: %s", i, argv[i]); }
...
static void downloadURL(const char *from, const char *to) {
	...
	buildParentDirs(to);
	io = fopen(to, "wb");
	if (!io) { die("Failed to open output file"); }
	...
	if (fclose(io) == EOF) { die("Can't flush file on close. i/o error? Disk full?"); }
...
static void parseManifest(const char *fname) {
	...
	char		  buf[512];
	if (!io) { die("Failed to open manifest for reading"); }
	...
	while (fgets(buf, sizeof(buf), io)) {
		...
		if (!item && !buf[0]) { continue; /* blank line between items or blank at EOF */ }
...
/* unix replaces the process with execl(), but Windows needs to wait for the parent to terminate. */
	...
	if (!ppid) { die("Can't determine parent process id"); }
	windowsWaitForProcessToDie(ppid);
#endif

	in = fopen(argv0, "rb");
	if (!in) { die("Can't open self for input while upgrading updater"); }

	remove(tempfname);
	if (rename(options.updateself, tempfname) == -1) { die("Can't rename original while upgrading updater"); }

	out = fopen(options.updateself, "wb");
	if (!out) { die("Can't open file for output while upgrading updater"); }

	while (!feof(in) && !why) {
		char		 buf[512];
		const size_t br = fread(buf, 1, sizeof(buf), in);
		if (br > 0) {
			if (fwrite(buf, br, 1, out) != 1) { why = "write failure while upgrading updater"; }
		} else if (ferror(in)) {
	...
	if ((fclose(out) == EOF) && (!why)) { why = "close failure while upgrading updater"; }
...
static void installUpdatedFile(const ManifestItem *item) {
	...
	if (rename(downloadPath, item->fname) == -1) { die("Failed to move updated file to final position"); }
}
...
static void applyUpdates(void) {
	...
	for (item = manifest; item != NULL; item = item->next) {
		...
		if (io != NULL) {
			...
			remove(rollbackPath);
			if (rename(item->fname, rollbackPath) == -1) { die("failed to move to rollback dir"); }
		}
...
static void waitToApplyUpdates(void) {
	if (options.waitforprocess) {
		infof("Waiting for pid " PIDFMT " to die...", (PIDFMTCAST)options.waitforprocess);
		{
			...
			while (kill(options.waitforprocess, 0) == 0) { usleep(100000); }
#endif
		}
...
int main(int argc, char **argv) {
	...
	if (sha256_hash_index == -1) { die("Failed to register sha256 hasher"); }
	...
	if (options.updateself) { upgradeSelfAndRestart(argv[0]); }
