...
static void Demos_MenuEvent(void *ptr, int event) {
	...
	case ID_LEFT: ScrollList_Key(&s_demos.list, K_LEFTARROW); break;

	case ID_RIGHT: ScrollList_Key(&s_demos.list, K_RIGHTARROW); break;
	}
}
...
static void Demos_MenuInit(void) {
	...
	for (j = 0; j < 2; j++) {
		if (s_demos.numDemos > MAX_DEMOS) s_demos.numDemos = MAX_DEMOS;
