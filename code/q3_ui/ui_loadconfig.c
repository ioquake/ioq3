...
static void LoadConfig_MenuEvent(void *ptr, int event) {
	...
	case ID_LEFT: ScrollList_Key(&s_configs.list, K_LEFTARROW); break;

	case ID_RIGHT: ScrollList_Key(&s_configs.list, K_RIGHTARROW); break;
	}
}
...
static void LoadConfig_MenuInit(void) {
	...
	for (i = 0; i < s_configs.list.numitems; i++) {
		...
		if (!Q_stricmp(configname + len - 4, ".cfg")) configname[len - 4] = '\0';
