...
static const char *master_items[] = {"Local",	"Internet", "Master1",	 "Master2", "Master3",
									 "Master4", "Master5",	"Favorites", NULL};

static const char *servertype_items[] = {"All",		   "Free For All",	   "Team Deathmatch",
										 "Tournament", "Capture the Flag", NULL};
...
static int QDECL ArenaServers_Compare(const void *arg1, const void *arg2) {
	...
	case SORT_GAME:
		if (t1->gametype < t2->gametype) { return -1; }
		if (t1->gametype == t2->gametype) { return 0; }
		return 1;

	case SORT_PING:
		if (t1->pingtime < t2->pingtime) { return -1; }
		if (t1->pingtime > t2->pingtime) { return 1; }
...
static void ArenaServers_Go(void) {
	...
	if (servernode) { trap_Cmd_ExecuteText(EXEC_APPEND, va("connect %s\n", servernode->adrstr)); }
}
...
static void ArenaServers_UpdateMenu(void) {
	...
	if (g_arenaservers.numqueriedservers > 0) {
		...
		if (g_arenaservers.refreshservers && (g_arenaservers.currentping <= g_arenaservers.numqueriedservers)) {
			// show progress
			Com_sprintf(g_arenaservers.status.string, MAX_STATUSLENGTH, "%d of %d Arena Servers.",
	...
	for (i = 0, j = 0; i < count; i++, servernodeptr++) {
		...
		if (!g_emptyservers && !servernodeptr->numclients) { continue; }

		if (!g_fullservers && (servernodeptr->numclients == servernodeptr->maxclients)) { continue; }

		switch (g_gametype) {
		case GAMES_ALL: break;

		case GAMES_FFA:
			if (servernodeptr->gametype != GT_FFA) { continue; }
			break;

		case GAMES_TEAMPLAY:
			if (servernodeptr->gametype != GT_TEAM) { continue; }
			break;

		case GAMES_TOURNEY:
			if (servernodeptr->gametype != GT_TOURNAMENT) { continue; }
			break;

		case GAMES_CTF:
			if (servernodeptr->gametype != GT_CTF) { continue; }
			break;
		}
		...
		Com_sprintf(buff, MAX_LISTBOXWIDTH, "%-20.20s %-12.12s %2d/%2d %-8.8s %4s%s%3d " S_COLOR_YELLOW "%s",
					...
					servernodeptr->maxclients, servernodeptr->gamename, netnames[servernodeptr->nettype], pingColor,
...
static void ArenaServers_Insert(char *adrstr, char *info, int pingtime) {
	...
	*/
	...
	if (servernodeptr->nettype < 0 || servernodeptr->nettype >= ARRAY_LEN(netnames) - 1) { servernodeptr->nettype = 0; }
...
static void ArenaServers_DoRefresh(void) {
	...
	if (uis.realtime < g_arenaservers.refreshtime) {
		if (g_servertype != UIAS_FAVORITES) {
			if (g_servertype == UIAS_LOCAL) {
				if (!trap_LAN_GetServerCount(AS_LOCAL)) { return; }
			}
	...
	for (i = 0; i < MAX_PINGREQUESTS && g_arenaservers.currentping < g_arenaservers.numqueriedservers; i++) {
		...
		if (g_servertype == UIAS_FAVORITES) {
			...
			trap_LAN_GetServerAddressString(ArenaServers_SourceForLAN(), g_arenaservers.currentping, adrstr,
											MAX_ADDRESSLENGTH);
		}
...
static void Punkbuster_ConfirmEnable(qboolean result) {
	if (result) { trap_SetPbClStatus(1); }
...
static void ArenaServers_Event(void *ptr, int event) {
	...
	case ID_CREATE: UI_StartServerMenu(qtrue); break;
	...
	case ID_PUNKBUSTER:
		if (g_arenaservers.punkbuster.curvalue) {
			UI_ConfirmMenu_Style("Enable Punkbuster?", UI_CENTER | UI_INVERSE | UI_SMALLFONT, 0,
								 Punkbuster_ConfirmEnable);
		} else {
			UI_ConfirmMenu_Style("Disable Punkbuster?", UI_CENTER | UI_INVERSE | UI_SMALLFONT, 0,
...
static void ArenaServers_MenuInit(void) {
	...
	for (i = 0; i < MAX_LISTBOXITEMS; i++) { g_arenaservers.items[i] = g_arenaservers.table[i].buff; }
