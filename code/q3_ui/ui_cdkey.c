...
static void UI_CDKeyMenu_Event(void *ptr, int event) {
	...
	case ID_ACCEPT:
		if (cdkeyMenuInfo.cdkey.field.buffer[0]) { trap_SetCDKey(cdkeyMenuInfo.cdkey.field.buffer); }
		UI_PopMenu();
		break;
...
static void UI_CDKeyMenu_DrawKey(void *self) {
	...
	if (val == 1) {
		...
		UI_DrawProportionalString(320, 376, "The CD Key appears to be valid, thank you", UI_CENTER | UI_SMALLFONT,
								  color_white);
	} else {
...
static void UI_CDKeyMenu_Init(void) {
	...
	if (uis.menusp) { Menu_AddItem(&cdkeyMenuInfo.menu, &cdkeyMenuInfo.back); }

	trap_GetCDKey(cdkeyMenuInfo.cdkey.field.buffer, cdkeyMenuInfo.cdkey.field.maxchars + 1);
	if (trap_VerifyCDKey(cdkeyMenuInfo.cdkey.field.buffer, NULL) == qfalse) { cdkeyMenuInfo.cdkey.field.buffer[0] = 0; }
}
