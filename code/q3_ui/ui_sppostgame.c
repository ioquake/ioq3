...
char *ui_medalSounds[]	 = {"sound/feedback/accuracy.wav",	  "sound/feedback/impressive_a.wav",
...
static void UI_SPPostgameMenu_NextEvent(void *ptr, int event) {
	...
	if (currentLevel == -1) { currentLevel = postgameMenuInfo.level; }
	currentSet = currentLevel / ARENAS_PER_TIER;

	if (levelSet > currentSet || levelSet == UI_GetNumSPTiers()) { level = currentLevel; }
...
static sfxHandle_t UI_SPPostgameMenu_MenuKey(int key) {
	if (uis.realtime < postgameMenuInfo.ignoreKeysTime) { return 0; }
	...
	if (key == K_ESCAPE || key == K_MOUSE2) { return 0; }
...
static void UI_SPPostgameMenu_MenuDrawScoreLine(int n, int y) {
	...
	if (n > (postgameMenuInfo.numClients + 1)) { n -= (postgameMenuInfo.numClients + 2); }

	if (n >= postgameMenuInfo.numClients) { return; }
...
static void UI_SPPostgameMenu_MenuDraw(void) {
	...
	// draw the scoreboard
	if (!trap_Cvar_VariableValue("ui_spScoreboard")) { return; }
...
void UI_SPPostgameMenu_f(void) {
	...
	if (postgameMenuInfo.numClients > MAX_SCOREBOARD_CLIENTS) { postgameMenuInfo.numClients = MAX_SCOREBOARD_CLIENTS; }
	...
	if (UI_GetSpecialArenaInfo("final")) { postgameMenuInfo.lastTier++; }
}
