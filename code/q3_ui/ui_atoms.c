...
void UI_PopMenu(void) {
	...
	if (uis.menusp < 0) trap_Error("UI_PopMenu: menu stack underflow");
...
void UI_DrawProportionalString(int x, int y, const char *str, int style, vec4_t color) {
	...
	if (style & UI_PULSE) {
		...
		drawcolor[1] = color[1];
		drawcolor[2] = color[2];
		drawcolor[3] = 0.5 + 0.5 * sin(uis.realtime / (float)PULSE_DIVISOR);
...
=================
*/
void UI_DrawProportionalString_AutoWrapped(int x, int y, int xmax, int ystep, const char *str, int style,
										   vec4_t color) {
	int	  width;
	char *s1, *s2, *s3;
	char  c_bcp;
...
void UI_DrawString(int x, int y, const char *str, int style, vec4_t color) {
	...
	if (style & UI_PULSE) {
		...
		UI_LerpColor(color, lowlight, newcolor, 0.5 + 0.5 * sin(uis.realtime / (float)PULSE_DIVISOR));
		drawcolor = newcolor;
	} else
		drawcolor = color;
...
qboolean UI_IsFullscreen(void) {
	if (uis.activemenu && (trap_Key_GetCatcher() & KEYCATCH_UI)) { return uis.activemenu->fullscreen; }

	return qfalse;
}

static void NeedCDAction(qboolean result) {
	if (!result) { trap_Cmd_ExecuteText(EXEC_APPEND, "quit\n"); }
}
...
void UI_SetActiveMenu(uiMenuCommand_t menu) {
	...
	case UIMENU_NEED_CD: UI_ConfirmMenu("Insert the CD", 0, NeedCDAction); return;
	case UIMENU_BAD_CD_KEY: UI_ConfirmMenu("Bad CD Key", 0, NeedCDKeyAction); return;
	case UIMENU_INGAME:
		/*
		//GRank
		UI_RankingsMenu();
...
void UI_DrawTextBox(int x, int y, int width, int lines) {
	UI_FillRect(x + (float)BIGCHAR_WIDTH / 2, y + (float)BIGCHAR_HEIGHT / 2, (width + 1) * BIGCHAR_WIDTH,
				(lines + 1) * BIGCHAR_HEIGHT, colorBlack);
	UI_DrawRect(x + (float)BIGCHAR_WIDTH / 2, y + (float)BIGCHAR_HEIGHT / 2, (width + 1) * BIGCHAR_WIDTH,
