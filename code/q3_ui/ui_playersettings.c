...
static const char *handicap_items[] = {"None", "95", "90", "85", "80", "75", "70", "65", "60", "55", "50",
...
static void PlayerSettings_DrawHandicap(void *self) {
	...
	UI_DrawProportionalString(item->generic.x + 64, item->generic.y + PROP_HEIGHT, handicap_items[item->curvalue],
							  style, color);
}
...
static void PlayerSettings_DrawPlayer(void *self) {
	...
	if (strcmp(buf, s_playersettings.playerModel) != 0) {
		...
		viewangles[PITCH] = 0;
		viewangles[ROLL]  = 0;
		UI_PlayerInfo_SetInfo(&s_playersettings.playerinfo, LEGS_IDLE, TORSO_STAND, viewangles, vec3_origin,
							  WP_MACHINEGUN, qfalse);
	}
...
static sfxHandle_t PlayerSettings_MenuKey(int key) {
	if (key == K_MOUSE2 || key == K_ESCAPE) { PlayerSettings_SaveChanges(); }
...
static void PlayerSettings_MenuEvent(void *ptr, int event) {
	...
	case ID_HANDICAP: trap_Cvar_Set("handicap", va("%i", 100 - 25 * s_playersettings.handicap.curvalue)); break;
