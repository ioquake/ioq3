...
static char *specifyleague_artlist[] = {
	SPECIFYLEAGUE_FRAMEL, SPECIFYLEAGUE_FRAMER, SPECIFYLEAGUE_ARROWS0, SPECIFYLEAGUE_UP,	   SPECIFYLEAGUE_DOWN,
	SPECIFYLEAGUE_BACK0,  SPECIFYLEAGUE_BACK1,	GLOBALRANKINGS_LOGO,   GLOBALRANKINGS_LETTERS, NULL};
...
static void SpecifyLeague_Event(void *ptr, int event) {
	...
	case ID_SPECIFYLEAGUEUP:
		if (event == QM_ACTIVATED) ScrollList_Key(&s_specifyleague.list, K_UPARROW);
		break;

	case ID_SPECIFYLEAGUEDOWN:
		if (event == QM_ACTIVATED) ScrollList_Key(&s_specifyleague.list, K_DOWNARROW);
		break;

	case ID_SPECIFYLEAGUENAME:
		if ((event == QM_LOSTFOCUS) && (Q_strncmp(playername, s_specifyleague.rankname.field.buffer,
...
void SpecifyLeague_MenuInit(void) {
	...
	Q_strncpyz(s_specifyleague.rankname.field.buffer, UI_Cvar_VariableString("name"),
...
void SpecifyLeague_Cache(void) {
	...
	// touch all our pics
	for (i = 0;; i++) {
		if (!specifyleague_artlist[i]) break;
