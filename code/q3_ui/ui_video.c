...
static void DriverInfo_Event(void *ptr, int event) {
	...
	case ID_DRIVERINFOBACK: UI_PopMenu(); break;
	}
}
...
static void DriverInfo_MenuDraw(void) {
	...
	if (s_driverinfo.numstrings & 1)
		UI_DrawString(320, y, s_driverinfo.strings[s_driverinfo.numstrings - 1], UI_CENTER | UI_SMALLFONT,
					  text_color_normal);
}
...
static void UI_DriverInfo_Menu(void) {
	...
	while (s_driverinfo.numstrings < 40 && *eptr) {
		...
		if (*eptr && *eptr != ' ') s_driverinfo.strings[s_driverinfo.numstrings++] = eptr;
...
static int GraphicsOptions_FindBuiltinResolution(int mode) {
	...
	if (!resolutionsDetected) return mode;

	if (mode < 0) return -1;

	for (i = 0; builtinResolutions[i]; i++) {
		if (!Q_stricmp(builtinResolutions[i], detectedResolutions[mode])) return i;
	}
...
static int GraphicsOptions_FindDetectedResolution(int mode) {
	...
	for (i = 0; detectedResolutions[i]; i++) {
		if (!Q_stricmp(builtinResolutions[mode], detectedResolutions[i])) return i;
	}
...
static void GraphicsOptions_GetAspectRatios(void) {
	...
	for (r = 0; resolutions[r]; r++) {
		...
		for (i = 0; ratioBuf[i][0]; i++) {
			if (!Q_stricmp(str, ratioBuf[i])) break;
		}
		if (!ratioBuf[i][0]) {
...
static void GraphicsOptions_GetResolutions(void) {
	...
	if (*resbuf) {
		...
		if (i < ARRAY_LEN(detectedResolutions) - 1) {
			Com_sprintf(currentResolution, sizeof(currentResolution), "%dx%d", uis.glconfig.vidWidth,
						uis.glconfig.vidHeight);

			for (i = 0; detectedResolutions[i]; i++) {
				if (strcmp(detectedResolutions[i], currentResolution) == 0) break;
			}
...
static void GraphicsOptions_CheckConfig(void) {
	...
	for (i = 0; i < NUM_IVO_TEMPLATES - 1; i++) {
		if (s_ivo_templates[i].colordepth != s_graphicsoptions.colordepth.curvalue) continue;
		if (s_ivo_templates[i].driver != s_graphicsoptions.driver.curvalue) continue;
		if (GraphicsOptions_FindDetectedResolution(s_ivo_templates[i].mode) != s_graphicsoptions.mode.curvalue)
			continue;
		if (s_ivo_templates[i].fullscreen != s_graphicsoptions.fs.curvalue) continue;
		if (s_ivo_templates[i].tq != s_graphicsoptions.tq.curvalue) continue;
		if (s_ivo_templates[i].lighting != s_graphicsoptions.lighting.curvalue) continue;
		if (s_ivo_templates[i].geometry != s_graphicsoptions.geometry.curvalue) continue;
		if (s_ivo_templates[i].filter != s_graphicsoptions.filter.curvalue) continue;
...
static void GraphicsOptions_UpdateMenuItems(void) {
	...
	if (s_graphicsoptions.allow_extensions.curvalue == 0) {
		if (s_graphicsoptions.texturebits.curvalue == 0) { s_graphicsoptions.texturebits.curvalue = 1; }
	}
...
static void GraphicsOptions_ApplyChanges(void *unused, int notification) {
	...
	case 0: trap_Cvar_SetValue("r_texturebits", 0); break;
	case 1: trap_Cvar_SetValue("r_texturebits", 16); break;
	case 2: trap_Cvar_SetValue("r_texturebits", 32); break;
	}
...
static void GraphicsOptions_Event(void *ptr, int event) {
	...
	case ID_DRIVERINFO: UI_DriverInfo_Menu(); break;

	case ID_BACK2: UI_PopMenu(); break;

	case ID_GRAPHICS: break;
...
static void GraphicsOptions_SetMenuItems(void) {
	...
	if (s_graphicsoptions.mode.curvalue < 0) {
		if (resolutionsDetected) {
			...
			if (s_graphicsoptions.mode.curvalue < 0) s_graphicsoptions.mode.curvalue = 0;
		} else {
	...
	default:
	case 0: s_graphicsoptions.texturebits.curvalue = 0; break;
	case 16: s_graphicsoptions.texturebits.curvalue = 1; break;
	case 32: s_graphicsoptions.texturebits.curvalue = 2; break;
	}
	...
	default:
	case 0: s_graphicsoptions.colordepth.curvalue = 0; break;
	case 16: s_graphicsoptions.colordepth.curvalue = 1; break;
	case 32: s_graphicsoptions.colordepth.curvalue = 2; break;
	}

	if (s_graphicsoptions.fs.curvalue == 0) { s_graphicsoptions.colordepth.curvalue = 0; }
	if (s_graphicsoptions.driver.curvalue == 1) { s_graphicsoptions.colordepth.curvalue = 1; }
}
...
void GraphicsOptions_MenuInit(void) {
	...
	static const char *s_graphics_options_names[] = {"Very High Quality", "High Quality", "Normal", "Fast",
													 "Fastest",			  "Custom",		  NULL};
