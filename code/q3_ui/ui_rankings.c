...
void Rankings_DrawName(void *self) {
	...
	for (p = f->field.buffer; *p != '\0'; p++) {
		...
		if (!(((*p) >= '0' && (*p) <= '9') || Q_isalpha(*p))) { *p = '\0'; }
	}

	// strip color codes
	Q_CleanStr(f->field.buffer);
	length = strlen(f->field.buffer);
	if (f->field.cursor > length) { f->field.cursor = length; }
...
void Rankings_DrawPassword(void *self) {
	...
	// mask password with *
	for (i = 0; i < length; i++) { f->field.buffer[i] = '*'; }
...
static void Rankings_MenuEvent(void *ptr, int event) {
	...
	case ID_LOGIN: UI_LoginMenu(); break;
	...
	case ID_CREATE: UI_SignupMenu(); break;
