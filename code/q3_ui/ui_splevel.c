...
static void UI_SPLevelMenu_SetBots(void) {
	...
	if (selectedArenaSet > currentSet) { return; }
	...
	while (*p && levelMenuInfo.numBots < 7) {
		...
		if (!botInfo) { botInfo = UI_GetBotInfoByNumber(levelMenuInfo.numBots); }
...
static void UI_SPLevelMenu_SetMenuArena(int n, int level, const char *arenaInfo) {
	...
	if (levelMenuInfo.levelScores[n] > 8) { levelMenuInfo.levelScores[n] = 8; }
...
static void UI_SPLevelMenu_SetMenuItems(void) {
	...
	if (selectedArenaSet == trainingTier || selectedArenaSet == finalTier) { selectedArena = 0; }
	...
	if (selectedArenaSet == trainingTier) {
		...
		if (selectedArena != -1) {
			levelMenuInfo.selectedArenaInfo =
				UI_GetArenaInfoByNumber(selectedArenaSet * ARENAS_PER_TIER + selectedArena);
		}
	}
...
static void UI_SPLevelMenu_ResetDraw(void) {
	...
	UI_DrawProportionalString(SCREEN_WIDTH / 2, 356 + PROP_HEIGHT * 1, "single player game variables.",
							  UI_CENTER | UI_SMALLFONT, color_yellow);
	UI_DrawProportionalString(SCREEN_WIDTH / 2, 356 + PROP_HEIGHT * 2, "Do this only if you want to",
							  UI_CENTER | UI_SMALLFONT, color_yellow);
	UI_DrawProportionalString(SCREEN_WIDTH / 2, 356 + PROP_HEIGHT * 3, "start over from the beginning.",
							  UI_CENTER | UI_SMALLFONT, color_yellow);
}
...
static void UI_SPLevelMenu_LevelEvent(void *ptr, int notification) {
	...
	if (selectedArenaSet == trainingTier || selectedArenaSet == finalTier) { return; }
...
static void UI_SPLevelMenu_LeftArrowEvent(void *ptr, int notification) {
	...
	if (selectedArenaSet == minTier) { return; }
...
static void UI_SPLevelMenu_RightArrowEvent(void *ptr, int notification) {
	...
	if (selectedArenaSet == maxTier) { return; }
...
static void UI_SPLevelMenu_NextEvent(void *ptr, int notification) {
	...
	if (selectedArena == -1) { selectedArena = 0; }
...
static void UI_SPLevelMenu_MenuDraw(void) {
	...
	for (n = 0; n < levelMenuInfo.numMaps; n++) {
		...
		if (n == selectedArena) {
			if (Menu_ItemAtCursor(&levelMenuInfo.menu) == &levelMenuInfo.item_maps[n]) { trap_R_SetColor(color); }
...
static void UI_SPLevelMenu_Init(void) {
	...
	if (skill < 1 || skill > 5) { trap_Cvar_Set("g_spSkill", "2"); }
	...
	for (n = 0; n < 6; n++) { levelMenuInfo.awardLevels[n] = UI_GetAwardLevel(n); }
	...
	for (n = 0; n < count; n++) { Menu_AddItem(&levelMenuInfo.menu, &levelMenuInfo.item_awards[n]); }
