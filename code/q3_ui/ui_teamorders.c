...
static const char *ctfOrders[] = {"I Am the Leader", "Defend the Base",		 "Follow Me", "Get Enemy Flag", "Camp Here",
...
static const char *teamOrders[]	  = {"I Am the Leader",		 "Follow Me", "Roam", "Camp Here", "Report",
...
static void UI_TeamOrdersMenu_SetList(int id) {
	...
	teamOrdersMenuInfo.list.generic.bottom =
		teamOrdersMenuInfo.list.generic.top + teamOrdersMenuInfo.list.numitems * PROP_HEIGHT;
}
...
sfxHandle_t UI_TeamOrdersMenu_Key(int key) {
	...
	if (l != &teamOrdersMenuInfo.list) { return Menu_DefaultKey(&teamOrdersMenuInfo.menu, key); }
...
static void UI_TeamOrdersMenu_BuildBotList(void) {
	...
	for (n = 0; n < 9; n++) { teamOrdersMenuInfo.bots[n] = teamOrdersMenuInfo.botNames[n]; }
...
void UI_TeamOrdersMenu_f(void) {
	...
	trap_GetConfigString(CS_SERVERINFO, info, sizeof(info));
	teamOrdersMenuInfo.gametype = atoi(Info_ValueForKey(info, "g_gametype"));
	if (teamOrdersMenuInfo.gametype < GT_TEAM) { return; }
