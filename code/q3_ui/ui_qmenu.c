	...
	if (b->generic.flags & QMF_GRAYED) {
		...
		if (((b->generic.flags & QMF_PULSE) || (b->generic.flags & QMF_PULSEIFFOCUS)) &&
			...
			if (b->focuscolor) {
				...
				color = pulse_color;
			color[3] = 0.5 + 0.5 * sin((float)uis.realtime / PULSE_DIVISOR);
...
static void RadioButton_Draw(menuradiobutton_s *rb) {
	...
	if (rb->generic.name) UI_DrawString(x - SMALLCHAR_WIDTH, y, rb->generic.name, UI_RIGHT | UI_SMALLFONT, color);
...
static void Slider_Draw(menuslider_s *s) {
	...
	UI_DrawHandlePic((int)(x + 2 * SMALLCHAR_WIDTH + (SLIDER_RANGE - 1) * SMALLCHAR_WIDTH * s->range) - 2, y - 2, 12,
					 20, button);
}
#else
/*
...
static void Slider_Draw(menuslider_s *s) {
	...
	for (i = 0; i < SLIDER_RANGE; i++) UI_DrawChar(x + (i + 2) * SMALLCHAR_WIDTH, y, 129, UI_LEFT | style, color);
...
sfxHandle_t ScrollList_Key(menulist_s *l, int key) {
	...
	case K_MOUSE1:
		if (l->generic.flags & QMF_HASMOUSEFOCUS) {
			...
			y = l->generic.y;
			w = ((l->width + l->separation) * l->columns - l->separation) * SMALLCHAR_WIDTH;
			if (l->generic.flags & QMF_CENTER_JUSTIFY) { x -= w / 2; }
	...
	case K_PGUP:
	case K_KP_PGUP:
		if (l->columns > 1) { return menu_null_sound; }
		...
		if (l->curvalue > 0) {
			...
			if (l->generic.callback) l->generic.callback(l, QM_GOTFOCUS);
	...
	case K_KP_UPARROW:
	case K_UPARROW:
		if (l->curvalue == 0) { return menu_buzz_sound; }
	...
	case K_KP_DOWNARROW:
	case K_DOWNARROW:
		if (l->curvalue == l->numitems - 1) { return menu_buzz_sound; }
	...
	case K_LEFTARROW:
		...
		if (l->curvalue < l->height) { return menu_buzz_sound; }
	...
	case K_RIGHTARROW:
		...
		if (c >= l->numitems) { return menu_buzz_sound; }
...
void ScrollList_Draw(menulist_s *l) {
	...
	for (column = 0; column < l->columns; column++) {
		...
		for (i = base; i < base + l->height; i++) {
			...
			if (i == l->curvalue) {
				u = x - 2;
				if (l->generic.flags & QMF_CENTER_JUSTIFY) { u -= (l->width * SMALLCHAR_WIDTH) / 2 + 1; }
...
void Menu_AddItem(menuframework_s *menu, void *item) {
	...
	if (!(itemptr->flags & QMF_NODEFAULTINIT)) {
		switch (itemptr->type) {
		case MTYPE_ACTION: Action_Init((menuaction_s *)item); break;

		case MTYPE_FIELD: MenuField_Init((menufield_s *)item); break;

		case MTYPE_SPINCONTROL: SpinControl_Init((menulist_s *)item); break;

		case MTYPE_RADIOBUTTON: RadioButton_Init((menuradiobutton_s *)item); break;

		case MTYPE_SLIDER: Slider_Init((menuslider_s *)item); break;

		case MTYPE_BITMAP: Bitmap_Init((menubitmap_s *)item); break;

		case MTYPE_TEXT: Text_Init((menutext_s *)item); break;

		case MTYPE_SCROLLLIST: ScrollList_Init((menulist_s *)item); break;

		case MTYPE_PTEXT: PText_Init((menutext_s *)item); break;

		case MTYPE_BTEXT: BText_Init((menutext_s *)item); break;
...
void Menu_Draw(menuframework_s *menu) {
	...
	for (i = 0; i < menu->nitems; i++) {
		...
		if (itemptr->ownerdraw) {
			...
			switch (itemptr->type) {
			case MTYPE_RADIOBUTTON: RadioButton_Draw((menuradiobutton_s *)itemptr); break;

			case MTYPE_FIELD: MenuField_Draw((menufield_s *)itemptr); break;

			case MTYPE_SLIDER: Slider_Draw((menuslider_s *)itemptr); break;

			case MTYPE_SPINCONTROL: SpinControl_Draw((menulist_s *)itemptr); break;

			case MTYPE_ACTION: Action_Draw((menuaction_s *)itemptr); break;

			case MTYPE_BITMAP: Bitmap_Draw((menubitmap_s *)itemptr); break;

			case MTYPE_TEXT: Text_Draw((menutext_s *)itemptr); break;

			case MTYPE_SCROLLLIST: ScrollList_Draw((menulist_s *)itemptr); break;

			case MTYPE_PTEXT: PText_Draw((menutext_s *)itemptr); break;

			case MTYPE_BTEXT: BText_Draw((menutext_s *)itemptr); break;
...
sfxHandle_t Menu_ActivateItem(menuframework_s *s, menucommon_s *item) {
	if (item->callback) {
		...
		if (!(item->flags & QMF_SILENT)) { return menu_move_sound; }
	}
...
sfxHandle_t Menu_DefaultKey(menuframework_s *m, int key) {
	...
	if (item && !(item->flags & (QMF_GRAYED | QMF_INACTIVE))) {
		switch (item->type) {
		case MTYPE_SPINCONTROL: sound = SpinControl_Key((menulist_s *)item, key); break;

		case MTYPE_RADIOBUTTON: sound = RadioButton_Key((menuradiobutton_s *)item, key); break;

		case MTYPE_SLIDER: sound = Slider_Key((menuslider_s *)item, key); break;

		case MTYPE_SCROLLLIST: sound = ScrollList_Key((menulist_s *)item, key); break;

		case MTYPE_FIELD: sound = MenuField_Key((menufield_s *)item, &key); break;
		}
	...
	case K_F12: trap_Cmd_ExecuteText(EXEC_APPEND, "screenshot\n"); break;
#endif
	case K_KP_UPARROW:
	case K_UPARROW:
