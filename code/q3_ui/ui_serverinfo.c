...
void Favorites_Add(void) {
	...
	for (i = 0; i < MAX_FAVORITESERVERS; i++) {
		...
		if (!adrstr[0] && !best) best = i + 1;
	}

	if (best) trap_Cvar_Set(va("server%d", best), serverbuff);
}
...
=================
*/
static sfxHandle_t ServerInfo_MenuKey(int key) { return (Menu_DefaultKey(&s_serverinfo.menu, key)); }
...
void UI_ServerInfoMenu(void) {
	...
	if (trap_Cvar_VariableValue("sv_running")) { s_serverinfo.add.generic.flags |= QMF_GRAYED; }
	...
	if (s_serverinfo.numlines > 16) s_serverinfo.numlines = 16;
