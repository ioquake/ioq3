...
void TossClientItems(gentity_t *self) {
	...
	if (weapon == WP_MACHINEGUN || weapon == WP_GRAPPLING_HOOK) {
		if (self->client->ps.weaponstate == WEAPON_DROPPING) { weapon = self->client->pers.cmd.weapon; }
		if (!(self->client->ps.stats[STAT_WEAPONS] & (1 << weapon))) { weapon = WP_NONE; }
	}
...
void TossClientPersistantPowerups(gentity_t *ent) {
	...
	if (!ent->client->persistantPowerup) { return; }
...
char *modNames[] = {"MOD_UNKNOWN", "MOD_SHOTGUN",		 "MOD_GAUNTLET",	   "MOD_MACHINEGUN",
...
#ifdef MISSIONPACK
					"MOD_NAIL",	   "MOD_CHAINGUN",		 "MOD_PROXIMITY_MINE", "MOD_KAMIKAZE",
					"MOD_JUICED",
#endif
					"MOD_GRAPPLE"};
...
void CheckAlmostCapture(gentity_t *self, gentity_t *attacker) {
	...
	if (self->client->ps.powerups[PW_REDFLAG] || self->client->ps.powerups[PW_BLUEFLAG] ||
		...
		ent = NULL;
		do { ent = G_Find(ent, FOFS(classname), classname); } while (ent && (ent->flags & FL_DROPPED_ITEM));
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath) {
	...
	if (g_gametype.integer == GT_HARVESTER) { TossClientCubes(self); }
#endif
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if (client->sess.spectatorClient == self->s.number) { Cmd_Score_f(g_entities + i); }
	}
...
#ifdef MISSIONPACK
		if (self->s.eFlags & EF_KAMIKAZE) { Kamikaze_DeathTimer(self); }
#endif
	}
...
int G_InvulnerabilityEffect(gentity_t *targ, vec3_t dir, vec3_t point, vec3_t impactpoint, vec3_t bouncedir) {
	...
	if (n > 0) {
		...
		if (impactpoint) { VectorCopy(intersections[0], impactpoint); }
		if (bouncedir) {
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker, vec3_t dir, vec3_t point, int damage,
	...
	if (targ->client && mod != MOD_JUICED) {
		if (targ->client->invulnerabilityTime > level.time) {
			if (dir && point) { G_InvulnerabilityEffect(targ, dir, point, impactpoint, bouncedir); }
			return;
		}
	}
#endif
	...
	if (targ->s.eType == ET_MOVER) {
		if (targ->use && targ->moverState == MOVER_POS1) { targ->use(targ, inflictor, attacker); }
		return;
	}
#ifdef MISSIONPACK
...
#ifdef MISSIONPACK
		if (bg_itemlist[attacker->client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_GUARD) { max /= 2; }
#endif
	...
	if (attacker->client && client && targ != attacker && targ->health > 0 && targ->s.eType != ET_MISSILE &&
	...
	if (take) {
		...
		if (targ->client) { targ->client->ps.stats[STAT_HEALTH] = targ->health; }
