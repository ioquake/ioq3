...
gentity_t *SelectRandomFurthestSpawnPoint(vec3_t avoidPoint, vec3_t origin, vec3_t angles, qboolean isbot) {
	...
	rnd = random() * (float)(numSpots / 2.0f);
...
gentity_t *SelectInitialSpawnPoint(vec3_t origin, vec3_t angles, qboolean isbot) {
	...
	while ((spot = G_Find(spot, FOFS(classname), "info_player_deathmatch")) != NULL) {
		if (((spot->flags & FL_NO_BOTS) && isbot) || ((spot->flags & FL_NO_HUMANS) && !isbot)) { continue; }

		if ((spot->spawnflags & 0x01)) break;
	}

	if (!spot || SpotWouldTelefrag(spot)) return SelectSpawnPoint(vec3_origin, origin, angles, isbot);
...
static void ClientCleanName(const char *in, char *out, int outSize) {
	...
	if (*out == '\0' || colorlessLen == 0) Q_strncpyz(out, "UnnamedPlayer", outSize);
}
...
void ClientUserinfoChanged(int clientNum) {
	...
	if (client->ps.powerups[PW_GUARD]) {
		...
		if (client->pers.maxHealth < 1 || client->pers.maxHealth > 100) { client->pers.maxHealth = 100; }
	}
#else
	health				   = atoi(Info_ValueForKey(userinfo, "handicap"));
	client->pers.maxHealth = health;
	...
	*/
	...
	if (ent->r.svFlags & SVF_BOT) {
		...
		s = va("n\\%s\\t\\%i\\model\\%s\\hmodel\\%s\\g_redteam\\%s\\g_blueteam\\%s\\c1\\%s\\c2\\%s\\hc\\%i\\w\\%i\\l\\%"
			   "i\\tt\\%d\\tl\\%d",
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
	...
	value = Info_ValueForKey(userinfo, "ip");
	if (!strcmp(value, "localhost")) { client->pers.localClient = qtrue; }
...
void ClientSpawn(gentity_t *ent) {
	...
	for (i = 0; i < MAX_PERSISTANT; i++) { persistant[i] = client->ps.persistant[i]; }
	...
	for (i = 0; i < MAX_PERSISTANT; i++) { client->ps.persistant[i] = persistant[i]; }
	...
	if (ent->client->sess.spectatorState != SPECTATOR_FOLLOW) { ClientEndFrame(ent); }
...
void ClientDisconnect(int clientNum) {
	...
	ent = g_entities + clientNum;
	if (!ent->client || ent->client->pers.connected == CON_DISCONNECTED) { return; }
