...
void G_RegisterCvars(void) {
	...
	for (i = 0, cv = gameCvarTable; i < gameCvarTableSize; i++, cv++) {
		...
		if (cv->teamShader) { remapped = qtrue; }
	}
...
void G_InitGame(int levelTime, int randomSeed, int restart) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) { g_entities[i].classname = "clientslot"; }
	...
	if (g_gametype.integer >= GT_TEAM) { G_CheckTeamItems(); }
...
void AddTournamentPlayer(void) {
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if (client->sess.spectatorState == SPECTATOR_SCOREBOARD || client->sess.spectatorClient < 0) { continue; }

		if (!nextInLine || client->sess.spectatorNum > nextInLine->sess.spectatorNum) nextInLine = client;
	}
...
void RemoveTournamentLoser(void) {
	...
	if (level.clients[clientNum].pers.connected != CON_CONNECTED) { return; }
...
int QDECL SortRanks(const void *a, const void *b) {
	...
	if (ca->sess.sessionTeam == TEAM_SPECTATOR && cb->sess.sessionTeam == TEAM_SPECTATOR) {
		if (ca->sess.spectatorNum > cb->sess.spectatorNum) { return -1; }
		if (ca->sess.spectatorNum < cb->sess.spectatorNum) { return 1; }
		return 0;
	}
...
void CalculateRanks(void) {
	...
	for (i = 0; i < ARRAY_LEN(level.numteamVotingClients); i++) level.numteamVotingClients[i] = 0;
	...
	if (level.intermissiontime) { SendScoreboardMessageToAllClients(); }
}
...
void SendScoreboardMessageToAllClients(void) {
	...
	for (i = 0; i < level.maxclients; i++) {
		if (level.clients[i].pers.connected == CON_CONNECTED) { DeathmatchScoreboardMessage(g_entities + i); }
	}
}
...
void BeginIntermission(void) {
	...
	if (g_gametype.integer == GT_TOURNAMENT) { AdjustTournamentScores(); }
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		// respawn if dead
		if (client->health <= 0) { ClientRespawn(client); }
...
void LogExit(const char *string) {
	...
	for (i = 0; i < numSorted; i++) {
		...
		if (cl->pers.connected == CON_CONNECTING) { continue; }
...
void CheckIntermissionExit(void) {
	...
	if (level.time < level.intermissiontime + 5000) { return; }
	...
	if (level.time < level.exitTime + 10000) { return; }
...
qboolean ScoreIsTied(void) {
	...
	if (g_gametype.integer >= GT_TEAM) { return level.teamScores[TEAM_RED] == level.teamScores[TEAM_BLUE]; }
...
void CheckExitRules(void) {
	...
	if (g_gametype.integer < GT_CTF && g_fraglimit.integer) {
		...
		for (i = 0; i < g_maxclients.integer; i++) {
			cl = level.clients + i;
			if (cl->pers.connected != CON_CONNECTED) { continue; }
			if (cl->sess.sessionTeam != TEAM_FREE) { continue; }
...
void CheckTournament(void) {
	...
	if (level.numPlayingClients == 0) { return; }

	if (g_gametype.integer == GT_TOURNAMENT) {

		// pull in a spectator if needed
		if (level.numPlayingClients < 2) { AddTournamentPlayer(); }
		...
		if (g_gametype.integer >= GT_TEAM) {
			...
			if (counts[TEAM_RED] < 1 || counts[TEAM_BLUE] < 1) { notEnough = qtrue; }
...
void CheckTeamVote(int team) {
	...
	if (!level.teamVoteTime[cs_offset]) { return; }
...
void G_RunThink(gentity_t *ent) {
	...
	if (thinktime <= 0) { return; }
	if (thinktime > level.time) { return; }
...
void G_RunFrame(int levelTime) {
	...
	if (g_listEntity.integer) {
		for (i = 0; i < MAX_GENTITIES; i++) { G_Printf("%4i: %s\n", i, g_entities[i].classname); }
