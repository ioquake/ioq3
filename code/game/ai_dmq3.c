...
qboolean EntityIsDead(aas_entityinfo_t *entinfo) {
	...
	if (entinfo->number >= 0 && entinfo->number < MAX_CLIENTS) {
		...
		if (!BotAI_GetClientState(entinfo->number, &ps)) { return qfalse; }
...
#ifdef MISSIONPACK
	if (entinfo->powerups & (1 << PW_NEUTRALFLAG)) return qtrue;
#endif
	return qfalse;
}
...
qboolean EntityIsChatting(aas_entityinfo_t *entinfo) {
	if (entinfo->flags & EF_TALK) { return qtrue; }
	return qfalse;
}
...
qboolean EntityHasQuad(aas_entityinfo_t *entinfo) {
	if (entinfo->powerups & (1 << PW_QUAD)) { return qtrue; }
	return qfalse;
}
...
qboolean EntityHasKamikaze(aas_entityinfo_t *entinfo) {
	if (entinfo->flags & EF_KAMIKAZE) { return qtrue; }
	return qfalse;
}
...
qboolean EntityCarriesCubes(aas_entityinfo_t *entinfo) {
	...
	if (state.generic1 > 0) return qtrue;
	return qfalse;
}
...
void BotSetTeamStatus(bot_state_t *bs) {
	...
	case LTG_TEAMACCOMPANY:
		...
		break;
	case LTG_DEFENDKEYAREA: teamtask = TEAMTASK_DEFENSE; break;
	case LTG_GETFLAG: teamtask = TEAMTASK_OFFENSE; break;
	case LTG_RUSHBASE: teamtask = TEAMTASK_DEFENSE; break;
	case LTG_RETURNFLAG: teamtask = TEAMTASK_RETRIEVE; break;
	case LTG_CAMP:
	case LTG_CAMPORDER: teamtask = TEAMTASK_CAMP; break;
	case LTG_PATROL: teamtask = TEAMTASK_PATROL; break;
	case LTG_GETITEM: teamtask = TEAMTASK_PATROL; break;
	case LTG_KILL: teamtask = TEAMTASK_PATROL; break;
	case LTG_HARVEST: teamtask = TEAMTASK_OFFENSE; break;
	case LTG_ATTACKENEMYBASE: teamtask = TEAMTASK_OFFENSE; break;
	default: teamtask = TEAMTASK_PATROL; break;
	}
...
int BotSetLastOrderedTask(bot_state_t *bs) {
	...
	if (gametype == GT_CTF) {
		...
		if (bs->lastgoal_ltgtype == LTG_RETURNFLAG) {
			if (BotTeam(bs) == TEAM_RED) {
				if (bs->redflagstatus == 0) { bs->lastgoal_ltgtype = 0; }
			} else {
				if (bs->blueflagstatus == 0) { bs->lastgoal_ltgtype = 0; }
			}
		}
	}
...
void BotCTFSeekGoals(bot_state_t *bs) {
	...
	if (BotCTFCarryingFlag(bs)) {
		...
		if (bs->ltgtype != LTG_RUSHBASE) {
			...
			// if the flag is back
			if (flagstatus == 0) { bs->rushbaseaway_time = 0; }
		}
		return;
	}
	...
	if (bs->ltgtype == LTG_TEAMACCOMPANY && !bs->ordered) {
		...
		BotEntityInfo(bs->teammate, &entinfo);
		if (!EntityCarriesFlag(&entinfo)) { bs->ltgtype = 0; }
	}
	//
	if (BotTeam(bs) == TEAM_RED)
	...
	if (flagstatus == 1) {
		...
		if (bs->owndecision_time < FloatTime()) {
			...
			if (!(bs->ltgtype == LTG_DEFENDKEYAREA &&
				  (bs->teamgoal.number == ctf_redflag.number || bs->teamgoal.number == ctf_blueflag.number))) {
	...
	if (BotTeamLeader(bs)) { return; }
	// if the bot is ordered to do something
	if (bs->lastgoal_ltgtype) { bs->teamgoal_time += 60; }
	// if the bot decided to do something on its own and has a last ordered goal
	if (!bs->ordered && bs->lastgoal_ltgtype) { bs->ltgtype = 0; }
	// if already a CTF or team goal
	if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA ||
		bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_RETURNFLAG ||
	...
	//
	if (BotSetLastOrderedTask(bs)) return;
	//
	if (bs->owndecision_time > FloatTime()) return;
	;
	// if the bot is roaming
	if (bs->ctfroam_time > FloatTime()) return;
...
void Bot1FCTFSeekGoals(bot_state_t *bs) {
	...
	if (bs->neutralflagstatus == 1) {
		if (bs->owndecision_time < FloatTime()) {
			...
			if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA ||
				...
				bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_ATTACKENEMYBASE || bs->ltgtype == LTG_GETITEM ||
				bs->ltgtype == LTG_MAKELOVE_UNDER || bs->ltgtype == LTG_MAKELOVE_ONTOP) {
				return;
			}
	...
	if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA ||
		bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_RETURNFLAG ||
		bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL || bs->ltgtype == LTG_ATTACKENEMYBASE ||
...
void BotHarvesterSeekGoals(bot_state_t *bs) {
	...
	if (bs->ltgtype == LTG_TEAMACCOMPANY && !bs->ordered) {
		// if the team mate being accompanied no longer carries the flag
		BotEntityInfo(bs->teammate, &entinfo);
		if (!EntityCarriesCubes(&entinfo)) { bs->ltgtype = 0; }
	}
	// if the bot is ordered to do something
	if (bs->lastgoal_ltgtype) { bs->teamgoal_time += 60; }
	// if not yet doing something
	if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA ||
		bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_CAMPORDER || bs->ltgtype == LTG_PATROL ||
		bs->ltgtype == LTG_ATTACKENEMYBASE || bs->ltgtype == LTG_HARVEST || bs->ltgtype == LTG_GETITEM ||
		bs->ltgtype == LTG_MAKELOVE_UNDER || bs->ltgtype == LTG_MAKELOVE_ONTOP) {
		return;
	}
	//
...
char *EasyClientName(int client, char *buf, int size) {
	...
	// remove all spaces
	for (ptr = strstr(name, " "); ptr; ptr = strstr(name, " ")) { memmove(ptr, ptr + 1, strlen(ptr + 1) + 1); }
...
void BotCheckItemPickup(bot_state_t *bs, int *oldinventory) {
	...
	if (!oldinventory[INVENTORY_KAMIKAZE] && bs->inventory[INVENTORY_KAMIKAZE] >= 1) { offence = qtrue; }
	if (!oldinventory[INVENTORY_INVULNERABILITY] && bs->inventory[INVENTORY_INVULNERABILITY] >= 1) { offence = qtrue; }
	...
	if (!bs->inventory[INVENTORY_KAMIKAZE] && !bs->inventory[INVENTORY_INVULNERABILITY]) {
		if (!oldinventory[INVENTORY_SCOUT] && bs->inventory[INVENTORY_SCOUT] >= 1) { offence = qtrue; }
		if (!oldinventory[INVENTORY_GUARD] && bs->inventory[INVENTORY_GUARD] >= 1) { offence = qtrue; }
		if (!oldinventory[INVENTORY_DOUBLER] && bs->inventory[INVENTORY_DOUBLER] >= 1) { offence = qfalse; }
		if (!oldinventory[INVENTORY_AMMOREGEN] && bs->inventory[INVENTORY_AMMOREGEN] >= 1) { offence = qfalse; }
	}
		...
		if (offence) {
			if (!(bs->teamtaskpreference & TEAMTP_ATTACKER)) {
				...
				if (BotTeamLeader(bs)) {
					...
					if (bs->ltgtype != LTG_GETFLAG && bs->ltgtype != LTG_ATTACKENEMYBASE &&
						bs->ltgtype != LTG_HARVEST) {
						//
...
void BotUseKamikaze(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_KAMIKAZE] <= 0) return;
	if (bs->kamikaze_time > FloatTime()) return;
	bs->kamikaze_time = FloatTime() + 0.2;
	if (gametype == GT_CTF) {
		// never use kamikaze if the team flag carrier is visible
		if (BotCTFCarryingFlag(bs)) return;
		c = BotTeamFlagCarrierVisible(bs);
		if (c >= 0) {
			BotEntityInfo(c, &entinfo);
			VectorSubtract(entinfo.origin, bs->origin, dir);
			if (VectorLengthSquared(dir) < Square(KAMIKAZE_DIST)) return;
		}
		c = BotEnemyFlagCarrierVisible(bs);
		if (c >= 0) {
			BotEntityInfo(c, &entinfo);
		...
		// never use kamikaze if the team flag carrier is visible
		if (Bot1FCTFCarryingFlag(bs)) return;
		c = BotTeamFlagCarrierVisible(bs);
		if (c >= 0) {
			BotEntityInfo(c, &entinfo);
			VectorSubtract(entinfo.origin, bs->origin, dir);
		...
		//
		if (BotHarvesterCarryingCubes(bs)) return;
...
void BotUseInvulnerability(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_INVULNERABILITY] <= 0) return;
	if (bs->invulnerability_time > FloatTime()) return;
...
void BotBattleUseItems(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_HEALTH] < 60) {
		if (bs->inventory[INVENTORY_MEDKIT] > 0) { trap_EA_Use(bs->client); }
	}
#ifdef MISSIONPACK
	BotUseKamikaze(bs);
...
void BotSetTeleportTime(bot_state_t *bs) {
	if ((bs->cur_ps.eFlags ^ bs->last_eFlags) & EF_TELEPORT_BIT) { bs->teleport_time = FloatTime(); }
...
float BotAggression(bot_state_t *bs) {
	...
	if (bs->inventory[INVENTORY_QUAD]) {
		...
		if (bs->weaponnum != WP_GAUNTLET || bs->inventory[ENEMY_HORIZONTAL_DIST] < 80) { return 70; }
	}
	...
	if (bs->inventory[INVENTORY_BFG10K] > 0 && bs->inventory[INVENTORY_BFGAMMO] > 7) return 100;
	// if the bot can use the railgun
	if (bs->inventory[INVENTORY_RAILGUN] > 0 && bs->inventory[INVENTORY_SLUGS] > 5) return 95;
	// if the bot can use the lightning gun
	if (bs->inventory[INVENTORY_LIGHTNING] > 0 && bs->inventory[INVENTORY_LIGHTNINGAMMO] > 50) return 90;
	// if the bot can use the rocketlauncher
	if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 5) return 90;
	// if the bot can use the plasmagun
	if (bs->inventory[INVENTORY_PLASMAGUN] > 0 && bs->inventory[INVENTORY_CELLS] > 40) return 85;
	// if the bot can use the grenade launcher
	if (bs->inventory[INVENTORY_GRENADELAUNCHER] > 0 && bs->inventory[INVENTORY_GRENADES] > 10) return 80;
	// if the bot can use the shotgun
	if (bs->inventory[INVENTORY_SHOTGUN] > 0 && bs->inventory[INVENTORY_SHELLS] > 10) return 50;
...
float BotFeelingBad(bot_state_t *bs) {
	if (bs->weaponnum == WP_GAUNTLET) { return 100; }
	if (bs->inventory[INVENTORY_HEALTH] < 40) { return 100; }
	if (bs->weaponnum == WP_MACHINEGUN) { return 90; }
	if (bs->inventory[INVENTORY_HEALTH] < 60) { return 80; }
	return 0;
}
...
int BotWantsToRetreat(bot_state_t *bs) {
	...
	if (gametype == GT_CTF) {
		...
		if (Bot1FCTFCarryingFlag(bs)) return qtrue;
	} else if (gametype == GT_OBELISK) {
		// the bots should be dedicated to attacking the enemy obelisk
		if (bs->ltgtype == LTG_ATTACKENEMYBASE) {
			if (bs->enemy != redobelisk.entitynum && bs->enemy != blueobelisk.entitynum) { return qtrue; }
		}
		if (BotFeelingBad(bs) > 50) { return qtrue; }
		return qfalse;
	} else if (gametype == GT_HARVESTER) {
...
int BotWantsToChase(bot_state_t *bs) {
	...
	if (gametype == GT_CTF) {
		...
		if (Bot1FCTFCarryingFlag(bs)) return qfalse;
		// always chase if the enemy is carrying a flag
		BotEntityInfo(bs->enemy, &entinfo);
		if (EntityCarriesFlag(&entinfo)) return qtrue;
	} else if (gametype == GT_OBELISK) {
		// the bots should be dedicated to attacking the enemy obelisk
		if (bs->ltgtype == LTG_ATTACKENEMYBASE) {
			if (bs->enemy != redobelisk.entitynum && bs->enemy != blueobelisk.entitynum) { return qfalse; }
		}
	} else if (gametype == GT_HARVESTER) {
...
int BotHasPersistantPowerupAndWeapon(bot_state_t *bs) {
	...
	if (!bs->inventory[INVENTORY_SCOUT] && !bs->inventory[INVENTORY_GUARD] && !bs->inventory[INVENTORY_DOUBLER] &&
	...
	// if the bot can use the bfg
	if (bs->inventory[INVENTORY_BFG10K] > 0 && bs->inventory[INVENTORY_BFGAMMO] > 7) return qtrue;
	// if the bot can use the railgun
	if (bs->inventory[INVENTORY_RAILGUN] > 0 && bs->inventory[INVENTORY_SLUGS] > 5) return qtrue;
	// if the bot can use the lightning gun
	if (bs->inventory[INVENTORY_LIGHTNING] > 0 && bs->inventory[INVENTORY_LIGHTNINGAMMO] > 50) return qtrue;
	// if the bot can use the rocketlauncher
	if (bs->inventory[INVENTORY_ROCKETLAUNCHER] > 0 && bs->inventory[INVENTORY_ROCKETS] > 5) return qtrue;
	//
	if (bs->inventory[INVENTORY_NAILGUN] > 0 && bs->inventory[INVENTORY_NAILS] > 5) return qtrue;
	//
	if (bs->inventory[INVENTORY_PROXLAUNCHER] > 0 && bs->inventory[INVENTORY_MINES] > 5) return qtrue;
	//
	if (bs->inventory[INVENTORY_CHAINGUN] > 0 && bs->inventory[INVENTORY_BELT] > 40) return qtrue;
	// if the bot can use the plasmagun
	if (bs->inventory[INVENTORY_PLASMAGUN] > 0 && bs->inventory[INVENTORY_CELLS] > 20) return qtrue;
	return qfalse;
}
...
int BotWantsToCamp(bot_state_t *bs) {
	...
	if (bs->ltgtype == LTG_TEAMHELP || bs->ltgtype == LTG_TEAMACCOMPANY || bs->ltgtype == LTG_DEFENDKEYAREA ||
		bs->ltgtype == LTG_GETFLAG || bs->ltgtype == LTG_RUSHBASE || bs->ltgtype == LTG_CAMP ||
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
	...
	if (gametype == GT_OBELISK) {
		...
		BotAI_Trace(&trace, bs->eye, NULL, NULL, target, bs->client, CONTENTS_SOLID);
		if (trace.fraction >= 1 || trace.ent == goal->entitynum) {
			if (goal->entitynum == bs->enemy) { return qfalse; }
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	if (bs->enemy >= MAX_CLIENTS) {
		...
		if (bs->enemy == redobelisk.entitynum || bs->enemy == blueobelisk.entitynum) { target[2] += 32; }
#endif
		// aim at the obelisk
	...
	if (enemyvisible) {
		...
		if (trace.fraction <= 1 && trace.ent != entinfo.number) { bestorigin[2] += 16; }
		...
		if (wi.speed) {
			...
			if (!(dist > 100 && VectorLengthSquared(dir) < Square(32))) {
				...
				if (aim_skill > 0.8 &&
					...
					// VectorLength(dir), dist * 10 / wi.speed);
				}
...
void BotCheckAttack(bot_state_t *bs) {
	...
	if (attackentity >= MAX_CLIENTS) {
		...
		if (entinfo.number == redobelisk.entitynum || entinfo.number == blueobelisk.entitynum) {
			...
			if (g_entities[entinfo.number].activator && g_entities[entinfo.number].activator->s.frame == 2) { return; }
		}
#endif
	}
	//
	...
	if (bs->weaponnum == WP_GAUNTLET) {
		if (VectorLengthSquared(dir) > Square(60)) { return; }
	}
	if (VectorLengthSquared(dir) < Square(100))
		fov = 120;
	else
	...
	if (wi.flags & WFL_FIRERELEASED) {
		if (bs->flags & BFL_ATTACKED) { trap_EA_Attack(bs->client); }
	} else {
...
void BotMapScripts(bot_state_t *bs) {
	...
	if (!Q_stricmp(mapname, "q3tourney6") || !Q_stricmp(mapname, "q3tourney6_ctf") ||
		...
		if (bs->origin[0] > mins[0] && bs->origin[0] < maxs[0]) {
			if (bs->origin[1] > mins[1] && bs->origin[1] < maxs[1]) {
				if (bs->origin[2] > mins[2] && bs->origin[2] < maxs[2]) { return; }
			}
		}
		shootbutton = qfalse;
		...
		if (shootbutton) {
			...
			bs->ideal_viewangles[YAW] = AngleMod(bs->ideal_viewangles[YAW]);
			//
			if (InFieldOfVision(bs->viewangles, 20, bs->ideal_viewangles)) { trap_EA_Attack(bs->client); }
		}
	}
}
...
int BotModelMinsMaxs(int modelindex, int eType, int contents, vec3_t mins, vec3_t maxs) {
	...
	for (i = 0; i < level.num_entities; i++, ent++) {
		...
		if (eType && ent->s.eType != eType) { continue; }
		if (contents && ent->r.contents != contents) { continue; }
		if (ent->s.modelindex == modelindex) {
			if (mins) VectorAdd(ent->r.currentOrigin, ent->r.mins, mins);
			if (maxs) VectorAdd(ent->r.currentOrigin, ent->r.maxs, maxs);
			return i;
		}
	}
...
int BotFuncButtonActivateGoal(bot_state_t *bs, int bspent, bot_activategoal_t *activategoal) {
	...
	if (!*model) return qfalse;
	modelindex = atoi(model + 1);
	if (!modelindex) return qfalse;
	...
	if (health) {
		...
		if (bsptrace.fraction >= 1.0 || bsptrace.ent == entitynum) {
			...
			for (i = numareas - 1; i >= 0; i--) {
				if (trap_AAS_AreaReachability(areas[i])) { break; }
			}
			if (i < 0) {
...
int BotPopFromActivateGoalStack(bot_state_t *bs) {
	if (!bs->activatestack) return qfalse;
...
void BotClearActivateGoalStack(bot_state_t *bs) {
	while (bs->activatestack) BotPopFromActivateGoalStack(bs);
}
...
void BotEnableActivateGoalAreas(bot_activategoal_t *activategoal, int enable) {
	...
	if (activategoal->areasdisabled == !enable) return;
	for (i = 0; i < activategoal->numareas; i++) trap_AAS_EnableRoutingArea(activategoal->areas[i], enable);
...
int BotIsGoingToActivateEntity(bot_state_t *bs, int entitynum) {
	...
	for (a = bs->activatestack; a; a = a->next) {
		if (a->time < FloatTime()) continue;
		if (a->goal.entitynum == entitynum) return qtrue;
	}
	for (i = 0; i < MAX_ACTIVATESTACK; i++) {
		if (bs->activategoalheap[i].inuse) continue;
		//
		if (bs->activategoalheap[i].goal.entitynum == entitynum) {
			// if the bot went for this goal less than 2 seconds ago
			if (bs->activategoalheap[i].justused_time > FloatTime() - 2) return qtrue;
		}
	}
	return qfalse;
}
...
int BotGetActivateGoal(bot_state_t *bs, int entitynum, bot_activategoal_t *activategoal) {
	...
	if (!strcmp(classname, "func_door")) {
		...
		if (spawnflags & 1) return 0;
		// get the door origin
		if (!trap_AAS_VectorForBSPEpairKey(ent, "origin", origin)) { VectorClear(origin); }
		// if the door is open or opening already
		if (!VectorCompare(origin, entinfo.origin)) return 0;
		...
		if (*model) {
			...
			if (modelindex) {
				...
				for (i = 0; i < numareas; i++) {
					if (activategoal->numareas >= MAX_ACTIVATEAREAS) break;
					if (!trap_AAS_AreaReachability(areas[i])) { continue; }
	...
	if (!strcmp(classname, "func_button")) { return 0; }
	...
	for (i = 0; i >= 0 && i < 10;) {
		...
		if (!strcmp(classname, "func_button")) {
			//
			if (!BotFuncButtonActivateGoal(bs, ent, activategoal)) continue;
		...
		// invisible trigger multiple box
			...
			//
			if (!BotTriggerMultipleActivateGoal(bs, ent, activategoal)) continue;
...
int BotGoForActivateGoal(bot_state_t *bs, bot_activategoal_t *activategoal) {
	...
	if (!activategoal->time) activategoal->time = FloatTime() + 10;
...
void BotPrintActivateGoalInfo(bot_state_t *bs, bot_activategoal_t *activategoal, int bspent) {
	...
	if (activategoal->shoot) {
		Com_sprintf(buf, sizeof(buf), "%s: I have to shoot at a %s from %1.1f %1.1f %1.1f in area %d\n", netname,
					classname, activategoal->goal.origin[0], activategoal->goal.origin[1], activategoal->goal.origin[2],
...
void BotAIBlocked(bot_state_t *bs, bot_moveresult_t *moveresult, int activate) {
	...
	if (activate && entinfo.modelindex > 0 && entinfo.modelindex <= max_bspmodelindex) {
		...
		if (bspent) {
			//
			if (bs->activatestack && !bs->activatestack->inuse) bs->activatestack = NULL;
			...
			if (!(moveresult->flags & MOVERESULT_ONTOPOFOBSTACLE) && trap_AAS_AreaReachability(bs->areanum)) return;
		} else {
			// enable any routing areas that were disabled
			BotEnableActivateGoalAreas(&activategoal, qtrue);
		}
...
int BotAIPredictObstacles(bot_state_t *bs, bot_goal_t *goal) {
	...
	if (!bot_predictobstacles.integer) return qfalse;
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	...
	if (state->eType != ET_MISSILE || state->weapon != WP_GRENADE_LAUNCHER) return;
...
void BotCheckForProxMines(bot_state_t *bs, entityState_t *state) {
	...
	if (state->eType != ET_MISSILE || state->weapon != WP_PROX_LAUNCHER) return;
	// if this prox mine is from someone on our own team
	if (state->generic1 == BotTeam(bs)) return;
	...
	trap_BotAddAvoidSpot(bs->ms, state->pos.trBase, 160, AVOID_ALWAYS);
	//
	if (bs->numproxmines >= MAX_PROXMINES) return;
...
void BotCheckForKamikazeBody(bot_state_t *bs, entityState_t *state) {
	...
	if (!(state->eFlags & EF_KAMIKAZE)) return;
	// if this entity isn't dead
	if (!(state->eFlags & EF_DEAD)) return;
...
void BotCheckEvents(bot_state_t *bs, entityState_t *state) {
	...
	if (bs->entityeventTime[state->number] == g_entities[state->number].eventTime) { return; }
...
void BotCheckAir(bot_state_t *bs) {
	if (bs->inventory[INVENTORY_ENVIRONMENTSUIT] <= 0) {
		if (trap_AAS_PointContents(bs->eye) & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA)) { return; }
	}
...
bot_goal_t *BotAlternateRoute(bot_state_t *bs, bot_goal_t *goal) {
	...
	if (bs->altroutegoal.areanum) {
		//
		if (bs->reachedaltroutegoal_time) return goal;
...
int BotGetAlternateRouteGoal(bot_state_t *bs, int base) {
	...
	if (!numaltroutegoals) return qfalse;
	rnd = (float)random() * numaltroutegoals;
	if (rnd >= numaltroutegoals) rnd = numaltroutegoals - 1;
	...
	if (gametype == GT_CTF) {
		...
		if (ctf_neutralflag.areanum) {
			...
			red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
				ctf_neutralflag.origin, ctf_neutralflag.areanum, ctf_redflag.origin, ctf_redflag.areanum, TFL_DEFAULT,
				red_altroutegoals, MAX_ALTROUTEGOALS, ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
			blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
				ctf_neutralflag.origin, ctf_neutralflag.areanum, ctf_blueflag.origin, ctf_blueflag.areanum, TFL_DEFAULT,
				blue_altroutegoals, MAX_ALTROUTEGOALS, ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
		}
	} else if (gametype == GT_1FCTF) {
		...
		//
		red_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
			neutralobelisk.origin, neutralobelisk.areanum, redobelisk.origin, redobelisk.areanum, TFL_DEFAULT,
			red_altroutegoals, MAX_ALTROUTEGOALS, ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
		blue_numaltroutegoals = trap_AAS_AlternativeRouteGoals(
			neutralobelisk.origin, neutralobelisk.areanum, blueobelisk.origin, blueobelisk.areanum, TFL_DEFAULT,
			blue_altroutegoals, MAX_ALTROUTEGOALS, ALTROUTEGOAL_CLUSTERPORTALS | ALTROUTEGOAL_VIEWPORTALS);
	} else if (gametype == GT_HARVESTER) {
		if (trap_BotGetLevelItemGoal(-1, "Neutral Obelisk", &neutralobelisk) < 0)
...
void BotDeathmatchAI(bot_state_t *bs, float thinktime) {
	...
	if (!bs->ainode) { AIEnter_Seek_LTG(bs, "BotDeathmatchAI: no ai node"); }
...
void BotSetEntityNumForGoalWithModel(bot_goal_t *goal, int eType, char *modelname) {
	...
	for (i = 0; i < level.num_entities; i++, ent++) {
		if (!ent->inuse) { continue; }
		if (eType && ent->s.eType != eType) { continue; }
		if (ent->s.modelindex != modelindex) { continue; }
...
void BotSetEntityNumForGoal(bot_goal_t *goal, char *classname) {
	...
	for (i = 0; i < level.num_entities; i++, ent++) {
		if (!ent->inuse) { continue; }
		if (Q_stricmp(ent->classname, classname) != 0) { continue; }
		VectorSubtract(goal->origin, ent->s.origin, dir);
		if (VectorLengthSquared(dir) < Square(10)) {
			goal->entitynum = i;
			return;
...
void BotSetEntityNumForGoalWithActivator(bot_goal_t *goal, char *classname) {
	...
	ent = &g_entities[0];
	for (i = 0; i < level.num_entities; i++, ent++) {
		if (!ent->inuse || !ent->activator) { continue; }
		if (Q_stricmp(ent->activator->classname, classname) != 0) { continue; }
		VectorSubtract(goal->origin, ent->s.origin, dir);
		if (VectorLengthSquared(dir) < Square(10)) {
			goal->entitynum = i;
			return;
...
int BotGoalForBSPEntity(char *classname, bot_goal_t *goal) {
	...
	for (ent = trap_AAS_NextBSPEntity(0); ent; ent = trap_AAS_NextBSPEntity(ent)) {
		if (!trap_AAS_ValueForBSPEpairKey(ent, "classname", value, sizeof(value))) continue;
		if (!strcmp(value, classname)) {
			if (!trap_AAS_VectorForBSPEpairKey(ent, "origin", origin)) return qfalse;
...
void BotSetupDeathmatchAI(void) {
	...
	for (ent = trap_AAS_NextBSPEntity(0); ent; ent = trap_AAS_NextBSPEntity(ent)) {
		...
		if (model[0] == '*') {
			...
			if (modelnum > max_bspmodelindex) max_bspmodelindex = modelnum;
		}
	}
