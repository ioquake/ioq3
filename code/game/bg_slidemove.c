	...
	for (bumpcount = 0; bumpcount < numbumps; bumpcount++) {
		...
		for (i = 0; i < numplanes; i++) {
			...
			for (j = 0; j < numplanes; j++) {
				...
				if (gravity) { PM_ClipVelocity(endClipVelocity, planes[j], endClipVelocity, OVERCLIP); }
			...
			if (gravity) { VectorCopy(endClipVelocity, endVelocity); }
...
void PM_StepSlideMove(qboolean gravity) {
	...
	if (trace.fraction < 1.0) { PM_ClipVelocity(pm->ps->velocity, trace.plane.normal, pm->ps->velocity, OVERCLIP); }
