...
void G_TransposeMatrix(vec3_t matrix[3], vec3_t transpose[3]) {
	int i, j;
	for (i = 0; i < 3; i++) {
		for (j = 0; j < 3; j++) { transpose[i][j] = matrix[j][i]; }
	}
}
...
qboolean G_TryPushingEntity(gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove) {
	...
	if ((pusher->s.eFlags & EF_MOVER_STOP) && check->s.groundEntityNum != pusher->s.number) { return qfalse; }
	...
	if (check->client) { VectorCopy((pushed_p - 1)->origin, check->client->ps.origin); }
...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle) {
	...
	*obstacle = NULL;
	...
	for (e = 0; e < listedEntities; e++) {
		...
		if (check->s.eType != ET_ITEM && check->s.eType != ET_PLAYER && !check->physicsObject) { continue; }
...
void G_MoverTeam(gentity_t *ent) {
	...
	if (part) {
		...
		if (ent->blocked) { ent->blocked(ent, obstacle); }
		return;
	}
...
void G_RunMover(gentity_t *ent) {
	...
	if (ent->s.pos.trType != TR_STATIONARY || ent->s.apos.trType != TR_STATIONARY) { G_MoverTeam(ent); }
...
void MatchTeam(gentity_t *teamLeader, int moverState, int time) {
	...
	for (slave = teamLeader; slave; slave = slave->teamchain) { SetMoverState(slave, moverState, time); }
}
...
void ReturnToPos1(gentity_t *ent) {
	...
	// starting sound
	if (ent->sound2to1) { G_AddEvent(ent, EV_GENERAL_SOUND, ent->sound2to1); }
}
...
void Reached_BinaryMover(gentity_t *ent) {
	...
	if (ent->moverState == MOVER_1TO2) {
		...
		// fire targets
		if (!ent->activator) { ent->activator = ent; }
		...
		// play sound
		if (ent->soundPos1) { G_AddEvent(ent, EV_GENERAL_SOUND, ent->soundPos1); }

		// close areaportals
		if (ent->teammaster == ent || !ent->teammaster) { trap_AdjustAreaPortalState(ent, qfalse); }
	} else {
...
void Use_BinaryMover(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	if (ent->moverState == MOVER_POS1) {
		...
		// open areaportal
		if (ent->teammaster == ent || !ent->teammaster) { trap_AdjustAreaPortalState(ent, qtrue); }
		return;
	}
...
void InitMover(gentity_t *ent) {
	...
	if (G_SpawnString("noise", "100", &sound)) { ent->s.loopSound = G_SoundIndex(sound); }
...
void Blocked_Door(gentity_t *ent, gentity_t *other) {
	...
	if (ent->damage) { G_Damage(other, ent, ent, NULL, NULL, ent->damage, 0, MOD_CRUSH); }
	if (ent->spawnflags & 4) {
...
void Think_SpawnNewDoorTrigger(gentity_t *ent) {
	...
	for (other = ent; other; other = other->teamchain) { other->takedamage = qtrue; }
...
void Touch_Plat(gentity_t *ent, gentity_t *other, trace_t *trace) {
	if (!other->client || other->client->ps.stats[STAT_HEALTH] <= 0) { return; }
...
void Touch_PlatCenterTrigger(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (ent->parent->moverState == MOVER_POS1) { Use_BinaryMover(ent->parent, ent, other); }
}
...
void SP_func_plat(gentity_t *ent) {
	...
	if (!G_SpawnFloat("height", "0", &height)) { height = (ent->r.maxs[2] - ent->r.mins[2]) - lip; }
...
void Touch_Button(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	if (ent->moverState == MOVER_POS1) { Use_BinaryMover(ent, other, other); }
}
...
to also draw "speed"		the number of degrees each way the pendulum swings, (30 default)
"phase"		the 0.0 to 1.0 offset in the cycle to start at
"dmg"		damage to inflict when blocked (2 default)
"color"		constantLight color
"light"		constantLight radius
