...
static void G_LoadArenas(void) {
	...
	for (n = 0; n < g_numArenas; n++) { Info_SetValueForKey(g_arenaInfos[n], "num", va("%i", n)); }
}
...
const char *G_GetArenaInfoByMap(const char *map) {
	...
	for (n = 0; n < g_numArenas; n++) {
		if (Q_stricmp(Info_ValueForKey(g_arenaInfos[n], "map"), map) == 0) { return g_arenaInfos[n]; }
	}
...
int G_CountBotPlayersByName(const char *name, int team) {
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (team >= 0 && cl->sess.sessionTeam != team) { continue; }
		if (name && Q_stricmp(name, cl->pers.netname)) { continue; }
		num++;
	}
	return num;
}
...
int G_SelectRandomBotInfo(int team) {
	...
	if (team != -1 && G_CountBotPlayersByName(NULL, -1) < g_numBots) { team = -1; }

	num		  = 0;
	bestCount = MAX_CLIENTS;
	for (n = 0; n < g_numBots; n++) {
		value = Info_ValueForKey(g_botInfos[n], "funname");
		if (!value[0]) { value = Info_ValueForKey(g_botInfos[n], "name"); }
		//
...
void G_CheckMinimumPlayers(void) {
	...
	if (checkminimumplayers_time > level.time - 10000) { return; }
	...
	if (g_gametype.integer >= GT_TEAM) {
		if (minplayers >= g_maxclients.integer / 2) { minplayers = (g_maxclients.integer / 2) - 1; }
		...
		if (minplayers >= g_maxclients.integer) { minplayers = g_maxclients.integer - 1; }
...
void G_CheckBotSpawn(void) {
	...
	for (n = 0; n < BOT_SPAWN_QUEUE_DEPTH; n++) {
		if (!botSpawnQueue[n].spawnTime) { continue; }
		if (botSpawnQueue[n].spawnTime > level.time) { continue; }
...
void G_InitBots(qboolean restart) {
	...
	if (g_gametype.integer == GT_SINGLE_PLAYER) {
		...
		if (Q_stricmp(strValue, "training") == 0) { basedelay += 10000; }

		if (!restart) { G_SpawnBots(Info_ValueForKey(arenainfo, "bots"), basedelay); }
	}
}
