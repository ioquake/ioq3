...
qboolean CheckGauntletAttack(gentity_t *ent) {
	...
	if (ent->client->noclip) { return qfalse; }
...
void Bullet_Fire(gentity_t *ent, float spread, int damage, int mod) {
	...
	for (i = 0; i < 10; i++) {
		...
		if (traceEnt->takedamage && traceEnt->client) {
			...
			if (LogAccuracyHit(traceEnt, ent)) { ent->client->accuracy_hits++; }
		} else {
...
#endif
			if (LogAccuracyHit(traceEnt, ent)) { hitClient = qtrue; }
...
void weapon_railgun_fire(gentity_t *ent) {
	...
	do {
		...
		if (traceEnt->takedamage) {
			...
			if (traceEnt->client && traceEnt->client->invulnerabilityTime > level.time) {
				...
				if (LogAccuracyHit(traceEnt, ent)) { hits++; }
	...
	if (hits == 0) {
		...
		if (ent->client->accurateCount >= 2) {
			...
			ent->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST |
...
void Weapon_GrapplingHook_Fire(gentity_t *ent) {
	if (!ent->client->fireHeld && !ent->client->hook) fire_grapple(ent, muzzle, forward);
...
qboolean LogAccuracyHit(gentity_t *target, gentity_t *attacker) {
	if (!target->takedamage) { return qfalse; }

	if (target == attacker) { return qfalse; }

	if (!target->client) { return qfalse; }

	if (!attacker->client) { return qfalse; }

	if (target->client->ps.stats[STAT_HEALTH] <= 0) { return qfalse; }

	if (OnSameTeam(target, attacker)) { return qfalse; }
...
#ifdef MISSIONPACK
	case WP_NAILGUN: Weapon_Nailgun_Fire(ent); break;
	case WP_PROX_LAUNCHER: weapon_proxlauncher_fire(ent); break;
	case WP_CHAINGUN: Bullet_Fire(ent, CHAINGUN_SPREAD, CHAINGUN_DAMAGE, MOD_CHAINGUN); break;
#endif
	default:
...
static void KamikazeRadiusDamage(vec3_t origin, gentity_t *attacker, float damage, float radius) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		if (ent->kamikazeTime > level.time) { continue; }
...
static void KamikazeShockWave(vec3_t origin, gentity_t *attacker, float damage, float push, float radius) {
	...
	for (e = 0; e < numListedEntities; e++) {
		...
		// don't hit things we have already hit
		if (ent->kamikazeShockTime > level.time) { continue; }
