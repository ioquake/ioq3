...
==================
*/
void BotVoiceChat_DefendFlag(bot_state_t *bs, int client, int mode) { BotVoiceChat_Defend(bs, client, mode); }
...
void BotVoiceChat_FollowFlagCarrier(bot_state_t *bs, int client, int mode) {
	...
	if (carrier >= 0) BotVoiceChat_FollowMe(bs, carrier, mode);
#ifdef DEBUG
	BotPrintTeamGoal(bs);
#endif // DEBUG
}
...
int BotVoiceChatCommand(bot_state_t *bs, int mode, char *voiceChat) {
	...
	if (!BotSameTeam(bs, clientNum)) { return qfalse; }
