...
void multi_trigger(gentity_t *ent, gentity_t *activator) {
	...
	if (activator->client) {
		if ((ent->spawnflags & 1) && activator->client->sess.sessionTeam != TEAM_RED) { return; }
		if ((ent->spawnflags & 2) && activator->client->sess.sessionTeam != TEAM_BLUE) { return; }
	}
...
void hurt_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	// play sound
	if (!(self->spawnflags & 4)) { G_Sound(other, CHAN_AUTO, self->noise_index); }
