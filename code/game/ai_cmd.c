...
int BotGetItemTeamGoal(char *goalname, bot_goal_t *goal) {
	...
	do {
		...
		if (i > 0) {
			...
			if (goal->flags & GFL_DROPPED) continue;
			return qtrue;
		}
	} while (i > 0);
	return qfalse;
...
void BotMatch_GetFlag(bot_state_t *bs, bot_match_t *match) {
	...
	if (gametype == GT_CTF) {
		...
		if (!ctf_neutralflag.areanum || !ctf_redflag.areanum || !ctf_blueflag.areanum) return;
	}
#endif
	else {
		return;
...
void BotMatch_AttackEnemyBase(bot_state_t *bs, bot_match_t *match) {
	...
	if (gametype == GT_CTF) {
		...
		if (!redobelisk.areanum || !blueobelisk.areanum) return;
	}
#endif
	else {
		return;
...
void BotMatch_Harvest(bot_state_t *bs, bot_match_t *match) {
	...
	if (gametype == GT_HARVESTER) {
		if (!neutralobelisk.areanum || !redobelisk.areanum || !blueobelisk.areanum) return;
	} else {
		return;
	}
	// if not addressed to this bot
...
void BotMatch_WhoIsTeamLeader(bot_state_t *bs, bot_match_t *match) {
	...
	if (!Q_stricmp(netname, bs->teamleader)) { trap_EA_SayTeam(bs->client, "I'm the team leader\n"); }
}
...
void BotMatch_EnterGame(bot_state_t *bs, bot_match_t *match) {
	...
	trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	client = FindClientByName(netname);
	if (client >= 0) { notleader[client] = qfalse; }
...
void BotMatch_NewLeader(bot_state_t *bs, bot_match_t *match) {
	...
	trap_BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	client = FindClientByName(netname);
	if (!BotSameTeam(bs, client)) return;
