...
void ClientImpacts(gentity_t *ent, pmove_t *pm) {
	...
	for (i = 0; i < pm->numtouch; i++) {
		...
		if ((ent->r.svFlags & SVF_BOT) && (ent->touch)) { ent->touch(ent, other, &trace); }
...
void G_TouchTriggers(gentity_t *ent) {
	...
	if (ent->client->ps.stats[STAT_HEALTH] <= 0) { return; }
	...
	for (i = 0; i < num; i++) {
		...
		if (hit->s.eType == ET_ITEM) {
			if (!BG_PlayerTouchesItem(&ent->client->ps, &hit->s, level.time)) { continue; }
		} else {
		...
		if ((ent->r.svFlags & SVF_BOT) && (ent->touch)) { ent->touch(ent, hit, &trace); }
	}
	...
	while (client->timeResidual >= 1000) {
		...
		if (maxHealth) {
			if (ent->health < maxHealth) {
				ent->health += 15;
				if (ent->health > maxHealth * 1.1) { ent->health = maxHealth * 1.1; }
				G_AddEvent(ent, EV_POWERUP_REGEN, 0);
			} else if (ent->health < maxHealth * 2) {
				ent->health += 5;
				if (ent->health > maxHealth * 2) { ent->health = maxHealth * 2; }
				G_AddEvent(ent, EV_POWERUP_REGEN, 0);
			}
#else
		...
		if (client->ps.stats[STAT_ARMOR] > client->ps.stats[STAT_MAX_HEALTH]) { client->ps.stats[STAT_ARMOR]--; }
	}
#ifdef MISSIONPACK
	if (bg_itemlist[client->ps.stats[STAT_PERSISTANT_POWERUP]].giTag == PW_AMMOREGEN) {
		int w, max, inc, t, i;
		int weapList[] = {WP_MACHINEGUN, WP_SHOTGUN, WP_GRENADE_LAUNCHER, WP_ROCKET_LAUNCHER, WP_LIGHTNING, WP_RAILGUN,
		...
		for (i = 0; i < weapCount; i++) {
			...
			if (client->ps.ammo[w] >= max) { client->ammoTimes[w] = 0; }
			if (client->ammoTimes[w] >= t) {
				while (client->ammoTimes[w] >= t) client->ammoTimes[w] -= t;
				client->ps.ammo[w] += inc;
				if (client->ps.ammo[w] > max) { client->ps.ammo[w] = max; }
			}
		}
	}
#endif
...
void ClientThink_real(gentity_t *ent) {
	...
	if (client->sess.sessionTeam == TEAM_SPECTATOR) {
		if (client->sess.spectatorState == SPECTATOR_SCOREBOARD) { return; }
	...
	if (level.time > client->rewardTime) {
		client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST |
	...
	if (client->ps.weapon == WP_GAUNTLET && !(ucmd->buttons & BUTTON_TALK) && (ucmd->buttons & BUTTON_ATTACK) &&
	...
	if (client->ps.stats[STAT_HEALTH] <= 0) {
		...
		if (level.time > client->respawnTime) {
			...
			if (ucmd->buttons & (BUTTON_ATTACK | BUTTON_USE_HOLDABLE)) { ClientRespawn(ent); }
		}
		return;
	}
...
void G_RunClient(gentity_t *ent) {
	if (!(ent->r.svFlags & SVF_BOT) && !g_synchronousClients.integer) { return; }
...
void SpectatorClientEndFrame(gentity_t *ent) {
	...
	if (ent->client->sess.spectatorState == SPECTATOR_FOLLOW) {
		...
		if (ent->client->ps.pm_flags & PMF_FOLLOW) {
			...
			if (ent->client->sess.spectatorClient >= 0) { ent->client->sess.spectatorState = SPECTATOR_FREE; }
...
void ClientEndFrame(gentity_t *ent) {
	...
	for (i = 0; i < MAX_POWERUPS; i++) {
		if (ent->client->ps.powerups[i] < level.time) { ent->client->ps.powerups[i] = 0; }
	}
	...
	if (ent->client->invulnerabilityTime > level.time) { ent->client->ps.powerups[PW_INVULNERABILITY] = level.time; }
#endif
