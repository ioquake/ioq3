...
void BotTeamplayReport(void) {
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		// skip spectators
		if (atoi(Info_ValueForKey(buf, "t")) == TEAM_RED) { BotReportStatus(botstates[i]); }
	}
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if (!strlen(buf) || !strlen(Info_ValueForKey(buf, "n"))) continue;
		// skip spectators
		if (atoi(Info_ValueForKey(buf, "t")) == TEAM_BLUE) { BotReportStatus(botstates[i]); }
	}
}
...
void BotSetInfoConfigString(bot_state_t *bs) {
	...
	strcpy(carrying, "  ");
	if (gametype == GT_CTF) {
		if (BotCTFCarryingFlag(bs)) { strcpy(carrying, "F "); }
	}
#ifdef MISSIONPACK
	else if (gametype == GT_1FCTF) {
		if (Bot1FCTFCarryingFlag(bs)) { strcpy(carrying, "F "); }
	} else if (gametype == GT_HARVESTER) {
		if (BotHarvesterCarryingCubes(bs)) {
			if (BotTeam(bs) == TEAM_RED)
...
void BotInterbreeding(void) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		if (botstates[i] && botstates[i]->inuse) { BotAIShutdownClient(botstates[i]->client, qfalse); }
	}
...
==============
*/
void BotEntityInfo(int entnum, aas_entityinfo_t *info) { trap_AAS_EntityInfo(entnum, info); }
...
void BotWriteSessionData(bot_state_t *bs) {
	...
	s = va("%i %i %i %i %i %i %i %i"
		   ...
		   " %f %f %f"
		   " %f",
		   bs->lastgoal_decisionmaker, bs->lastgoal_ltgtype, bs->lastgoal_teammate, bs->lastgoal_teamgoal.areanum,
		   ...
		   bs->lastgoal_teamgoal.maxs[2], bs->formation_dist);
...
int BotAISetupClient(int client, struct bot_settings_s *settings, qboolean restart) {
	...
	if (bot_interbreed) { trap_BotMutateGoalFuzzyLogic(bs->gs, 1); }
	// if we kept the bot client
	if (restart) { BotReadSessionData(bs); }
...
int BotAIShutdownClient(int client, qboolean restart) {
	...
	if (restart) { BotWriteSessionData(bs); }

	if (BotChat_ExitGame(bs)) { trap_BotEnterChat(bs->cs, bs->client, CHAT_ALL); }
...
int BotAIShutdown(int restart) {
	...
	if (restart) {
		...
		for (i = 0; i < MAX_CLIENTS; i++) {
			if (botstates[i] && botstates[i]->inuse) { BotAIShutdownClient(botstates[i]->client, restart); }
		}
