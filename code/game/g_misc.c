...
void TeleportPlayer(gentity_t *player, vec3_t origin, vec3_t angles) {
	...
	if (player->client->sess.sessionTeam != TEAM_SPECTATOR) { G_KillBox(player); }
...
void Use_Shooter(gentity_t *ent, gentity_t *other, gentity_t *activator) {
	...
	switch (ent->s.weapon) {
	case WP_GRENADE_LAUNCHER: fire_grenade(ent, ent->s.origin, dir); break;
	case WP_ROCKET_LAUNCHER: fire_rocket(ent, ent->s.origin, dir); break;
	case WP_PLASMAGUN: fire_plasma(ent, ent->s.origin, dir); break;
	}
...
static void PortalTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	while ((destination = G_Find(destination, FOFS(classname), "hi_portal destination")) != NULL) {
		if (destination->count == self->count) { break; }
	}

	// if there is not one, die!
	if (!destination) {
		if (self->pos1[0] || self->pos1[1] || self->pos1[2]) { TeleportPlayer(other, self->pos1, self->s.angles); }
