...
void DeathmatchScoreboardMessage(gentity_t *ent) {
	...
	for (i = 0; i < numSorted; i++) {
		...
		Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i %i %i %i %i %i %i %i %i", level.sortedClients[i],
	...
	if (checkNums) {
		...
		if (StringIsInteger(s)) {
			...
			if (idnum >= 0 && idnum < level.maxclients) {
				...
				if (cl->pers.connected == CON_CONNECTED) { return idnum; }
			}
		}
	}
...
void Cmd_Give_f(gentity_t *ent) {
	...
	if (give_all || Q_stricmp(name, "ammo") == 0) {
		for (i = 0; i < MAX_WEAPONS; i++) { ent->client->ps.ammo[i] = 999; }
		if (!give_all) return;
	}
...
void SetTeam(gentity_t *ent, const char *s) {
	...
	if (client->ps.stats[STAT_HEALTH] <= 0 && client->pers.connected == CON_CONNECTED) { CopyToBodyQue(ent); }
	...
	if (team == TEAM_SPECTATOR && oldTeam != team) AddTournamentQueue(client);
...
void StopFollowing(gentity_t *ent) {
	...
	if (ent->client->ps.stats[STAT_HEALTH] <= 0) { ent->client->ps.stats[STAT_HEALTH] = 1; }
}
...
void Cmd_Follow_f(gentity_t *ent) {
	...
	if (ent->client->sess.sessionTeam != TEAM_SPECTATOR) { SetTeam(ent, "spectator"); }
...
void Cmd_FollowCycle_f(gentity_t *ent, int dir) {
	...
	if (dir != 1 && dir != -1) { G_Error("Cmd_FollowCycle_f: bad dir %i", dir); }
...
static void G_SayTo(gentity_t *ent, gentity_t *other, int mode, int color, const char *name, const char *message) {
	...
	trap_SendServerCommand(other - g_entities, va("%s \"%s%c%c%s\"", mode == SAY_TEAM ? "tchat" : "chat", name,
...
static void Cmd_Tell_f(gentity_t *ent) {
	...
	if (ent != target && !(ent->r.svFlags & SVF_BOT)) { G_Say(ent, ent, SAY_TELL, p); }
}
...
void G_Voice(gentity_t *ent, gentity_t *target, int mode, const char *id, qboolean voiceonly) {
	...
	// echo the text to the console
	if (g_dedicated.integer) { G_Printf("voice: %s %s\n", ent->client->pers.netname, id); }
...
static void Cmd_VoiceTell_f(gentity_t *ent, qboolean voiceonly) {
	...
	// don't tell to the player self if it was already directed to this player
	// also don't send the chat back to a bot
	if (ent != target && !(ent->r.svFlags & SVF_BOT)) { G_Voice(ent, ent, SAY_TELL, id, voiceonly); }
}
...
static void Cmd_VoiceTaunt_f(gentity_t *ent) {
	...
	if (ent->enemy && ent->enemy->client && ent->enemy->client->lastkilled_client == ent->s.number) {
		// i am a dead corpse
		if (!(ent->enemy->r.svFlags & SVF_BOT)) { G_Voice(ent, ent->enemy, SAY_TELL, VOICECHAT_DEATHINSULT, qfalse); }
		if (!(ent->r.svFlags & SVF_BOT)) { G_Voice(ent, ent, SAY_TELL, VOICECHAT_DEATHINSULT, qfalse); }
		ent->enemy = NULL;
		return;
	}
	...
	if (ent->client->lastkilled_client >= 0 && ent->client->lastkilled_client != ent->s.number) {
		...
		if (who->client) {
			...
			if (who->client->lasthurt_mod == MOD_GAUNTLET) {
				...
				if (!(ent->r.svFlags & SVF_BOT)) { G_Voice(ent, ent, SAY_TELL, VOICECHAT_KILLGAUNTLET, qfalse); }
			} else {
				if (!(who->r.svFlags & SVF_BOT)) {
					G_Voice(ent, who, SAY_TELL, VOICECHAT_KILLINSULT, qfalse); // and I killed them with something else
				}
				if (!(ent->r.svFlags & SVF_BOT)) { G_Voice(ent, ent, SAY_TELL, VOICECHAT_KILLINSULT, qfalse); }
			}
		...
		for (i = 0; i < MAX_CLIENTS; i++) {
			...
			if (who->client && who != ent && who->client->sess.sessionTeam == ent->client->sess.sessionTeam) {
				if (who->client->rewardTime > level.time) {
					if (!(who->r.svFlags & SVF_BOT)) { G_Voice(ent, who, SAY_TELL, VOICECHAT_PRAISE, qfalse); }
					if (!(ent->r.svFlags & SVF_BOT)) { G_Voice(ent, ent, SAY_TELL, VOICECHAT_PRAISE, qfalse); }
					return;
				}
			}
		}
...
void Cmd_GameCommand_f(gentity_t *ent) {
	...
	// don't tell to the player self if it was already directed to this player
	// also don't send the chat back to a bot
	if (ent != target && !(ent->r.svFlags & SVF_BOT)) { G_Say(ent, ent, SAY_TELL, gc_orders[order]); }
}
...
static const char *gameNames[] = {"Free For All",	  "Tournament",	  "Single Player", "Team Deathmatch",
...
void Cmd_CallVote_f(gentity_t *ent) {
	...
	if (!Q_stricmp(arg1, "map_restart")) {
		...
		trap_SendServerCommand(
			ent - g_entities, "print \"Vote commands are: map_restart, nextmap, map <mapname>, g_gametype <n>, kick "
							  "<player>, clientkick <clientnum>, g_doWarmup, timelimit <time>, fraglimit <frags>.\n\"");
		return;
	}
...
void Cmd_CallTeamVote_f(gentity_t *ent) {
	...
	if (!Q_stricmp(arg1, "leader")) {
		...
		if (!arg2[0]) {
			...
			for (i = 0; i < 3; i++) {
				if (!arg2[i] || arg2[i] < '0' || arg2[i] > '9') break;
			}
			if (i >= 3 || !arg2[i]) {
				i = atoi(arg2);
	...
	for (i = 0; i < level.maxclients; i++) {
		if (level.clients[i].sess.sessionTeam == team) level.clients[i].ps.eFlags &= ~EF_TEAMVOTED;
	}
...
void ClientCommand(int clientNum) {
	...
	if (!ent->client || ent->client->pers.connected != CON_CONNECTED) {
		if (ent->client && ent->client->pers.localClient) {
			...
			if (Q_stricmp(cmd, "team") == 0) { Cmd_Team_f(ent); }
		}
