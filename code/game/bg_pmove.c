...
static void PM_Friction(void) {
	...
	if (pm->waterlevel) { drop += speed * pm_waterfriction * pm->waterlevel * pml.frametime; }

	// apply flying friction
	if (pm->ps->powerups[PW_FLIGHT]) { drop += speed * pm_flightfriction * pml.frametime; }

	if (pm->ps->pm_type == PM_SPECTATOR) { drop += speed * pm_spectatorfriction * pml.frametime; }
...
static void PM_WaterMove(void) {
	...
	if (wishspeed > pm->ps->speed * pm_swimScale) { wishspeed = pm->ps->speed * pm_swimScale; }
...
static void PM_AirMove(void) {
	...
	for (i = 0; i < 2; i++) { wishvel[i] = pml.forward[i] * fmove + pml.right[i] * smove; }
	wishvel[2] = 0;
...
static void PM_WalkMove(void) {
	...
	if (pm->ps->pm_flags & PMF_DUCKED) {
		if (wishspeed > pm->ps->speed * pm_duckScale) { wishspeed = pm->ps->speed * pm_duckScale; }
	}
	...
	if (pm->waterlevel) {
		...
		if (wishspeed > pm->ps->speed * waterScale) { wishspeed = pm->ps->speed * waterScale; }
	}
...
static void PM_Footsteps(void) {
	...
	if (pm->ps->groundEntityNum == ENTITYNUM_NONE) {
		...
		if (pm->ps->powerups[PW_INVULNERABILITY]) { PM_ContinueLegsAnim(LEGS_IDLECR); }
		// airborne leaves position in cycle intact, but doesn't advance
		if (pm->waterlevel > 1) { PM_ContinueLegsAnim(LEGS_SWIM); }
		return;
	}
...
static void PM_Weapon(void) {
	...
	if (pm->ps->weaponTime <= 0 || pm->ps->weaponstate != WEAPON_FIRING) {
		if (pm->ps->weapon != pm->cmd.weapon) { PM_BeginWeaponChange(pm->cmd.weapon); }
	}
	...
	if (pm->ps->ammo[pm->ps->weapon] != -1) { pm->ps->ammo[pm->ps->weapon]--; }
...
void Pmove(pmove_t *pmove) {
	...
	if (finalTime > pmove->ps->commandTime + 1000) { pmove->ps->commandTime = finalTime - 1000; }
