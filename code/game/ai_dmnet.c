...
void BotDumpNodeSwitches(bot_state_t *bs) {
	...
	for (i = 0; i < numnodeswitches; i++) { BotAI_Print(PRT_MESSAGE, "%s", nodeswitch[i]); }
...
int BotReachedGoal(bot_state_t *bs, bot_goal_t *goal) {
	if (goal->flags & GFL_ITEM) {
		...
		if (trap_BotTouchingGoal(bs->origin, goal)) {
			if (!(goal->flags & GFL_DROPPED)) { trap_BotSetAvoidGoalTime(bs->gs, goal->number, -1); }
			return qtrue;
		}
		...
		if (bs->areanum == goal->areanum) {
			if (bs->origin[0] > goal->origin[0] + goal->mins[0] && bs->origin[0] < goal->origin[0] + goal->maxs[0]) {
				if (bs->origin[1] > goal->origin[1] + goal->mins[1] &&
					...
					if (!trap_AAS_Swimming(bs->origin)) { return qtrue; }
				}
			}
		}
...
int BotGetLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal) {
	...
	if (bs->ltgtype == LTG_KILL && !retreat) {
		...
		//
		if (bs->killedenemy_time > bs->teamgoal_time - TEAM_KILL_SOMEONE &&
	...
	if ((bs->ltgtype == LTG_CAMP || bs->ltgtype == LTG_CAMPORDER) && !retreat) {
		...
		if (VectorLengthSquared(dir) < Square(60)) {
			...
			if (!bs->arrive_time) {
				if (bs->ltgtype == LTG_CAMPORDER) {
					BotAI_BotInitialChat(bs, "camp_arrive", EasyClientName(bs->teammate, netname, sizeof(netname)),
										 NULL);
					trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
			...
			if (bs->attackcrouch_time < FloatTime() - 5) {
				croucher = trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_CROUCHER, 0, 1);
				if (random() < bs->thinktime * croucher) { bs->attackcrouch_time = FloatTime() + 5 + croucher * 15; }
			}
			// if the bot wants to crouch
			if (bs->attackcrouch_time > FloatTime()) { trap_EA_Crouch(bs->client); }
			// don't crouch when swimming
			if (trap_AAS_Swimming(bs->origin)) bs->attackcrouch_time = FloatTime() - 1;
			...
			if (trap_PointContents(bs->eye, bs->entitynum) & (CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA)) {
				if (bs->ltgtype == LTG_CAMPORDER) {
					...
					trap_BotEnterChat(bs->cs, bs->decisionmaker, CHAT_TELL);
					//
					if (bs->lastgoal_ltgtype == LTG_CAMPORDER) { bs->lastgoal_ltgtype = 0; }
				}
				bs->ltgtype = 0;
			}
			//
	...
	if (gametype == GT_CTF) {
		...
		if (bs->ltgtype == LTG_ATTACKENEMYBASE && bs->attackaway_time < FloatTime()) {
			...
			if (bs->teamgoal_time < FloatTime()) { bs->ltgtype = 0; }
			// if touching the base flag the bot should loose the enemy flag
			if (trap_BotTouchingGoal(bs->origin, goal)) { bs->attackaway_time = FloatTime() + 2 + 5 * random(); }
			return qtrue;
		}
		// returning flag
		if (bs->ltgtype == LTG_RETURNFLAG) {
		...
		if (bs->ltgtype == LTG_ATTACKENEMYBASE && bs->attackaway_time < FloatTime()) {
			...
			if (BotFeelingBad(bs) > 50) { return BotGetItemLongTermGoal(bs, tfl, goal); }
			// if touching the obelisk
			if (trap_BotTouchingGoal(bs->origin, goal)) { bs->attackaway_time = FloatTime() + 3 + 5 * random(); }
			// or very close to the obelisk
			VectorSubtract(bs->origin, goal->origin, dir);
			if (VectorLengthSquared(dir) < Square(60)) { bs->attackaway_time = FloatTime() + 3 + 5 * random(); }
			// quit rushing after 2 minutes
			if (bs->teamgoal_time < FloatTime()) { bs->ltgtype = 0; }
			BotAlternateRoute(bs, goal);
		...
		if (bs->ltgtype == LTG_HARVEST && bs->harvestaway_time < FloatTime()) {
			...
			if (bs->teamgoal_time < FloatTime()) { bs->ltgtype = 0; }
			//
			if (trap_BotTouchingGoal(bs->origin, goal)) { bs->harvestaway_time = FloatTime() + 4 + 3 * random(); }
			return qtrue;
		}
	}
#endif
...
int BotLongTermGoal(bot_state_t *bs, int tfl, int retreat, bot_goal_t *goal) {
	...
	if (bs->lead_time > 0 && !retreat) {
		...
		if (bs->leadvisible_time < FloatTime() - 1) { bs->leadbackup_time = FloatTime() + 2; }
		...
		if (bs->leadbackup_time > FloatTime()) {
			...
			if (squaredist < Square(100)) { bs->leadbackup_time = 0; }
...
void AIEnter_Intermission(bot_state_t *bs, char *s) {
	...
	if (BotChat_EndLevel(bs)) { trap_BotEnterChat(bs->cs, 0, bs->chatto); }
...
int AINode_Observer(bot_state_t *bs) {
	...
	if (!BotIsObserver(bs)) { AIEnter_Stand(bs, "observer: left observer"); }
	return qtrue;
}
...
int AINode_Respawn(bot_state_t *bs) {
	...
	if (bs->respawnchat_time && bs->respawnchat_time < FloatTime() - 0.5) { trap_EA_Talk(bs->client); }
	//
	return qtrue;
}
...
void BotClearPath(bot_state_t *bs, bot_moveresult_t *moveresult) {
	...
	if (moveresult->flags & MOVERESULT_BLOCKEDBYAVOIDSPOT) { bs->blockedbyavoidspot_time = FloatTime() + 5; }
...
int AINode_Seek_ActivateEntity(bot_state_t *bs) {
	...
	if (bs->activatestack->shoot) {
		...
		if (bsptrace.fraction >= 1.0 || bsptrace.ent == goal->entitynum) {
			...
			if (bs->cur_ps.weapon == bs->activatestack->weapon) {
				...
				// if the bot is pretty close with its aim
				if (InFieldOfVision(bs->viewangles, 20, ideal_viewangles)) { trap_EA_Attack(bs->client); }
			}
		}
	}
	...
	if (targetvisible) {
		...
		// predict obstacles
		if (BotAIPredictObstacles(bs, goal)) return qfalse;
...
int AINode_Seek_NBG(bot_state_t *bs) {
	...
	if (BotInLavaOrSlime(bs)) bs->tfl |= TFL_LAVA | TFL_SLIME;
	//
	if (BotCanAndWantsToRocketJump(bs)) { bs->tfl |= TFL_ROCKETJUMP; }
	// map specific code
	BotMapScripts(bs);
	// no enemy
	bs->enemy = -1;
...
int AINode_Seek_LTG(bot_state_t *bs) {
	...
	if (bs->killedenemy_time > FloatTime() - 2) {
		if (random() < bs->thinktime * 1) { trap_EA_Gesture(bs->client); }
	}
	// if there is an enemy
	if (BotFindEnemy(bs, -1)) {
		if (BotWantsToRetreat(bs)) {
	...
	if (!BotLongTermGoal(bs, bs->tfl, qfalse, &goal)) { return qtrue; }
	...
	if (bs->check_time < FloatTime()) {
		...
		if (gametype == GT_CTF) {
			...
			if (Bot1FCTFCarryingFlag(bs)) range = 50;
		} else if (gametype == GT_HARVESTER) {
			if (BotHarvesterCarryingCubes(bs)) range = 80;
		}
#endif
		//
...
int AINode_Battle_Fight(bot_state_t *bs) {
	...
	if (bs->enemydeath_time) {
		if (bs->enemydeath_time < FloatTime() - 1.0) {
			...
			if (bs->enemysuicide) { BotChat_EnemySuicide(bs); }
		...
		if (EntityIsDead(&entinfo)) { bs->enemydeath_time = FloatTime(); }
	}
	...
	if (bs->enemy >= MAX_CLIENTS) {
		...
		if (bs->enemy == redobelisk.entitynum || bs->enemy == blueobelisk.entitynum) { target[2] += 16; }
#endif
	}
