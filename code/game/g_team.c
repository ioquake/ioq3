...
static __attribute__((format(printf, 2, 3))) void QDECL PrintMsg(gentity_t *ent, const char *fmt, ...) {
	...
	va_start(argptr, fmt);
	if (Q_vsnprintf(msg, sizeof(msg), fmt, argptr) >= sizeof(msg)) { G_Error("PrintMsg overrun"); }
	va_end(argptr);
...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker) {
	...
	if (!targ->client || !attacker->client || targ == attacker || OnSameTeam(targ, attacker)) return;
	...
	tokens = 0;
#ifdef MISSIONPACK
	if (g_gametype.integer == GT_HARVESTER) { tokens = targ->client->ps.generic1; }
#endif
	...
	if (g_gametype.integer == GT_OBELISK) {
		...
		case TEAM_RED: c = "team_redobelisk"; break;
		case TEAM_BLUE: c = "team_blueobelisk"; break;
		default: return;
		}
		...
		for (i = 0; i < g_maxclients.integer; i++) {
			...
			if (carrier->inuse && carrier->client->ps.powerups[flag_pw]) break;
			carrier = NULL;
		}
#ifdef MISSIONPACK
	}
	...
	while ((flag = G_Find(flag, FOFS(classname), c)) != NULL) {
		if (!(flag->flags & FL_DROPPED_ITEM)) break;
	}
...
#ifdef MISSIONPACK
	if (g_gametype.integer == GT_1FCTF) { flag_pw = PW_NEUTRALFLAG; }
#endif
...
int Team_TouchOurFlag(gentity_t *ent, gentity_t *other, int team) {
	...
	// add the sprite over the player's head
	other->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST |
	...
	for (i = 0; i < g_maxclients.integer; i++) {
		...
		if (!player->inuse || player == other) continue;
		...
		if (player->client->sess.sessionTeam != cl->sess.sessionTeam) {
			...
			if (player->client->pers.teamState.lastreturnedflag + CTF_RETURN_FLAG_ASSIST_TIMEOUT > level.time) {
				...
				// add the sprite over the player's head
				player->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET |
											   EF_AWARD_ASSIST | EF_AWARD_DEFEND | EF_AWARD_CAP);
...
int Pickup_Team(gentity_t *ent, gentity_t *other) {
	...
	if (g_gametype.integer == GT_HARVESTER) {
		...
		if (ent->spawnflags != cl->sess.sessionTeam) { cl->ps.generic1 += 1; }
		G_FreeEntity(ent);
		return 0;
	}
#endif
...
#ifdef MISSIONPACK
	if (g_gametype.integer == GT_1FCTF) {
		if (team == TEAM_FREE) { return Team_TouchEnemyFlag(ent, other, cl->sess.sessionTeam); }
		if (team != cl->sess.sessionTeam) { return Team_TouchOurFlag(ent, other, cl->sess.sessionTeam); }
		return 0;
	}
#endif
	// GT_CTF
	if (team == cl->sess.sessionTeam) { return Team_TouchOurFlag(ent, other, team); }
...
gentity_t *Team_GetLocation(gentity_t *ent) {
	...
	for (eloc = level.locationHead; eloc; eloc = eloc->nextTrain) {
		...
		if (!trap_InPVS(origin, eloc->r.currentOrigin)) { continue; }
...
static int QDECL SortClients(const void *a, const void *b) { return *(int *)a - *(int *)b; }
...
void TeamplayInfoMessage(gentity_t *ent) {
	...
	if (!ent->client->pers.teamInfo) return;

	// send team info to spectator for team of followed client
	if (ent->client->sess.sessionTeam == TEAM_SPECTATOR) {
		if (ent->client->sess.spectatorState != SPECTATOR_FOLLOW || ent->client->sess.spectatorClient < 0) { return; }
	...
	for (i = 0, cnt = 0; i < g_maxclients.integer && cnt < TEAM_MAXOVERLAY; i++) {
		...
		if (player->inuse && player->client->sess.sessionTeam == team) { clients[cnt++] = level.sortedClients[i]; }
	}
	...
	for (i = 0, cnt = 0; i < g_maxclients.integer && cnt < TEAM_MAXOVERLAY; i++) {
		...
		if (player->inuse && player->client->sess.sessionTeam == team) {
			...
			Com_sprintf(entry, sizeof(entry), " %i %i %i %i %i %i",
						...
						i, player->client->pers.teamState.location, h, a, player->client->ps.weapon,
						player->s.powerups);
			j = strlen(entry);
...
void CheckTeamStatus(void) {
	...
	if (level.time - level.lastTeamLocationTime > TEAM_LOCATION_UPDATE_TIME) {
		...
		for (i = 0; i < g_maxclients.integer; i++) {
			...
			if (ent->inuse) { TeamplayInfoMessage(ent); }
		}
	}
}
...
static void ObeliskRegen(gentity_t *self) {
	...
	if (self->health >= g_obeliskHealth.integer) { return; }

	G_AddEvent(self, EV_POWERUP_REGEN, 0);
	self->health += g_obeliskRegenAmount.integer;
	if (self->health > g_obeliskHealth.integer) { self->health = g_obeliskHealth.integer; }
...
static void ObeliskDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod) {
	...
	// add the sprite over the player's head
	attacker->client->ps.eFlags &= ~(EF_AWARD_IMPRESSIVE | EF_AWARD_EXCELLENT | EF_AWARD_GAUNTLET | EF_AWARD_ASSIST |
									 EF_AWARD_DEFEND | EF_AWARD_CAP);
...
static void ObeliskTouch(gentity_t *self, gentity_t *other, trace_t *trace) {
	...
	if (OtherTeam(other->client->sess.sessionTeam) != self->spawnflags) { return; }

	tokens = other->client->ps.generic1;
	if (tokens <= 0) { return; }

	PrintMsg(NULL, "%s" S_COLOR_WHITE " brought in %i %s.\n", other->client->pers.netname, tokens,
			 (tokens == 1) ? "skull" : "skulls");
...
static void ObeliskPain(gentity_t *self, gentity_t *attacker, int damage) {
	...
	self->activator->s.modelindex2 = self->health * 0xff / g_obeliskHealth.integer;
	if (!self->activator->s.frame) { G_AddEvent(self, EV_OBELISKPAIN, 0); }
...
qboolean CheckObeliskAttack(gentity_t *obelisk, gentity_t *attacker) {
	...
	if (obelisk->die != ObeliskDie) { return qfalse; }

	// if the attacker is a client
	if (!attacker->client) { return qfalse; }

	// if the obelisk is on the same team as the attacker then don't hurt it
	if (obelisk->spawnflags == attacker->client->sess.sessionTeam) { return qtrue; }
