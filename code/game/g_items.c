...
int Pickup_Powerup(gentity_t *ent, gentity_t *other) {
	...
	for (i = 0; i < level.maxclients; i++) {
		...
		if (client == other->client) { continue; }
		if (client->pers.connected == CON_DISCONNECTED) { continue; }
		if (client->ps.stats[STAT_HEALTH] <= 0) { continue; }

		// if same team in team game, no sound
		// cannot use OnSameTeam as it expects to g_entities, not clients
		if (g_gametype.integer >= GT_TEAM && other->client->sess.sessionTeam == client->sess.sessionTeam) { continue; }
		...
		if (DotProduct(delta, forward) < 0.4) { continue; }
...
int Pickup_PersistantPowerup(gentity_t *ent, gentity_t *other) {
	...
	case PW_GUARD:
		...
		if (handicap <= 0.0f || handicap > 100.0f) { handicap = 100.0f; }
		max = (int)(2 * handicap);
...
int Pickup_Holdable(gentity_t *ent, gentity_t *other) {
	...
	if (ent->item->giTag == HI_KAMIKAZE) { other->client->ps.eFlags |= EF_KAMIKAZE; }
...
void Add_Ammo(gentity_t *ent, int weapon, int count) {
	...
	if (ent->client->ps.ammo[weapon] > 200) { ent->client->ps.ammo[weapon] = 200; }
}
...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
	...
	if (g_gametype.integer == GT_TEAM) { return g_weaponTeamRespawn.integer; }
...
int Pickup_Health(gentity_t *ent, gentity_t *other) {
	...
	if (other->health > max) { other->health = max; }
...
int Pickup_Armor(gentity_t *ent, gentity_t *other) {
	...
	if (other->client->ps.stats[STAT_ARMOR] > upperBound) { other->client->ps.stats[STAT_ARMOR] = upperBound; }
#else
	other->client->ps.stats[STAT_ARMOR] += ent->item->quantity;
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	case IT_POWERUP:
		...
		break;
#ifdef MISSIONPACK
	case IT_PERSISTANT_POWERUP: respawn = Pickup_PersistantPowerup(ent, other); break;
#endif
...
void G_CheckTeamItems(void) {
	...
	if (g_gametype.integer == GT_OBELISK) {
		...
		if (!ent) { G_Printf(S_COLOR_YELLOW "WARNING: No team_redobelisk in map\n"); }

		ent = NULL;
		ent = G_Find(ent, FOFS(classname), "team_blueobelisk");
		if (!ent) { G_Printf(S_COLOR_YELLOW "WARNING: No team_blueobelisk in map\n"); }
	}
	...
	if (g_gametype.integer == GT_HARVESTER) {
		...
		if (!ent) { G_Printf(S_COLOR_YELLOW "WARNING: No team_neutralobelisk in map\n"); }
	}
#endif
}
...
void RegisterItem(gitem_t *item) {
	if (!item) { G_Error("RegisterItem: NULL"); }
...
#ifdef MISSIONPACK
	if (item->giType == IT_PERSISTANT_POWERUP) { ent->s.generic1 = ent->spawnflags; }
#endif
}
