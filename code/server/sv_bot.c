...
==================
*/
static int BotImport_PointContents(vec3_t point) { return SV_PointContents(point, -1); }
...
static void *BotImport_HunkAlloc(int size) {
	if (Hunk_CheckMark()) { Com_Error(ERR_DROP, "SV_Bot_HunkAlloc: Alloc with marks already set"); }
...
int BotImport_DebugPolygonCreate(int color, int numPoints, vec3_t *points) {
	...
	for (i = 1; i < bot_maxdebugpolys; i++) {
		if (!debugpolygons[i].inuse) break;
	}
	if (i >= bot_maxdebugpolys) return 0;
	poly			= &debugpolygons[i];
	poly->inuse		= qtrue;
	poly->color		= color;
...
==================
*/
static void BotImport_DebugLineDelete(int line) { BotImport_DebugPolygonDelete(line); }
...
int SV_BotGetSnapshotEntity(int client, int sequence) {
	...
	cl	  = &svs.clients[client];
	frame = &cl->frames[cl->netchan.outgoingSequence & PACKET_MASK];
	if (sequence < 0 || sequence >= frame->num_entities) { return -1; }
