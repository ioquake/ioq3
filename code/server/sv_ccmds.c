...
static void SV_Map_f(void) {
	...
	if (Q_stricmpn(cmd, "sp", 2) == 0) {
		...
		if (sv_gametype->integer == GT_SINGLE_PLAYER) { Cvar_SetValue("g_gametype", GT_FFA); }
	}
...
static void SV_MapRestart_f(void) {
	...
	for (i = 0; i < sv_maxclients->integer; i++) {
		if (svs.clients[i].state == CS_PRIMED) { svs.clients[i].oldServerTime = sv.restartTime; }
	}
...
static void SV_Kick_f(void) {
	...
	if (!cl) {
		if (!Q_stricmp(Cmd_Argv(1), "all")) {
			for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
				if (!cl->state) { continue; }
				if (cl->netchan.remoteAddress.type == NA_LOOPBACK) { continue; }
...
static void SV_Ban_f(void) {
	...
	if (svs.authorizeAddress.type != NA_BAD) {
		NET_OutOfBandPrint(NS_SERVER, svs.authorizeAddress, "banUser %i.%i.%i.%i", cl->netchan.remoteAddress.ip[0],
						   cl->netchan.remoteAddress.ip[1], cl->netchan.remoteAddress.ip[2],
...
static void SV_RehashBans_f(void) {
	...
	// make sure server is running
	if (!com_sv_running->integer) { return; }

	serverBansCount = 0;

	if (!sv_banFile->string || !*sv_banFile->string) return;
...
static qboolean SV_ParseCIDRNotation(netadr_t *dest, int *mask, char *adrstr) {
	...
	if (!NET_StringToAdr(adrstr, dest, NA_UNSPEC)) return qtrue;

	if (suffix) {
		*mask = atoi(suffix);

		if (dest->type == NA_IP) {
			if (*mask < 1 || *mask > 32) *mask = 32;
		} else {
			if (*mask < 1 || *mask > 128) *mask = 128;
		}
	...
	if (strchr(banstring, '.') || strchr(banstring, ':')) {
		...
		while (index < serverBansCount) {
			...
			if (curban->isexception == isexception && curban->subnet >= mask &&
				NET_CompareBaseAdrMask(curban->ip, ip, mask)) {
				Com_Printf("Deleting %s %s/%d\n", isexception ? "exception" : "ban", NET_AdrToString(curban->ip),
						   curban->subnet);
...
static void SV_CompleteMapName(char *args, int argNum) {
	if (argNum == 2) { Field_CompleteFilename("maps", "bsp", qtrue, qfalse); }
}
