...
void SV_GetChallenge(netadr_t from) {
	...
	if (Cvar_VariableValue("g_gametype") == GT_SINGLE_PLAYER || Cvar_VariableValue("ui_singlePlayerActive")) { return; }
	...
	for (i = 0; i < MAX_CHALLENGES; i++, challenge++) {
		if (!challenge->connected && NET_CompareAdr(from, challenge->adr)) {
			...
			if (challenge->time < oldestClientTime) oldestClientTime = challenge->time;
		}
	...
	if (challenge->adr.type == NA_IP && !com_standalone->integer && !Sys_IsLANAddress(from)) {
		...
		if (svs.authorizeAddress.type == NA_BAD) {
			...
			if (NET_StringToAdr(AUTHORIZE_SERVER_NAME, &svs.authorizeAddress, NA_IP)) {
				...
				Com_Printf("%s resolved to %i.%i.%i.%i:%i\n", AUTHORIZE_SERVER_NAME, svs.authorizeAddress.ip[0],
						   svs.authorizeAddress.ip[1], svs.authorizeAddress.ip[2], svs.authorizeAddress.ip[3],
						   BigShort(svs.authorizeAddress.port));
			}
		}

		// we couldn't contact the auth server, let them in.
		if (svs.authorizeAddress.type == NA_BAD) Com_Printf("Couldn't resolve auth server address\n");
		...
		// let them in, assuming the id server is down
			...
			if (game[0] == 0) { game = BASEGAME; }

			// the 0 is for backwards compatibility with obsolete sv_allowanonymous flags
			// getIpAuthorize <challenge> <IP> <game> 0 <auth-flag>
			NET_OutOfBandPrint(NS_SERVER, svs.authorizeAddress, "getIpAuthorize %i %i.%i.%i.%i %s 0 %s",
							   challenge->challenge, from.ip[0], from.ip[1], from.ip[2], from.ip[3], game,
...
void SV_AuthorizeIpPacket(netadr_t from) {
	...
	for (i = 0; i < MAX_CHALLENGES; i++) {
		if (svs.challenges[i].challenge == challenge) { break; }
	}
	if (i == MAX_CHALLENGES) {
...
static qboolean SV_IsBanned(netadr_t *from, qboolean isexception) {
	...
	if (!isexception) {
		...
		if (SV_IsBanned(from, qtrue)) return qfalse;
	}
...
void SV_DirectConnect(netadr_t from) {
	...
	if (!NET_IsLocalAddress(from)) {
		...
		for (i = 0; i < MAX_CHALLENGES; i++) {
			if (NET_CompareAdr(from, svs.challenges[i].adr)) {
				if (challenge == svs.challenges[i].challenge) break;
			}
		}
	...
	if (!newcl) {
		if (NET_IsLocalAddress(from)) {
			...
			for (i = startIndex; i < sv_maxclients->integer; i++) {
				cl = &svs.clients[i];
				if (cl->netchan.remoteAddress.type == NA_BOT) { count++; }
			}
			// if they're all bots
...
gotnewcl:
	...
	*newcl		   = temp;
	...
	count = 0;
	for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
		if (svs.clients[i].state >= CS_CONNECTED) { count++; }
	}
	if (count == 1 || count == sv_maxclients->integer) { SV_Heartbeat_f(); }
}
...
void SV_DropClient(client_t *drop, const char *reason) {
	...
	for (i = 0; i < sv_maxclients->integer; i++) {
		if (svs.clients[i].state >= CS_CONNECTED) { break; }
	}
...
static void SV_CloseDownload(client_t *cl) {
	...
	// EOF
	if (cl->download) { FS_FCloseFile(cl->download); }
	cl->download	  = 0;
	*cl->downloadName = 0;
...
int SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
	...
	if (!*cl->downloadName) return 0; // Nothing being downloaded
	...
	while (cl->downloadCurrentBlock - cl->downloadClientBlock < MAX_DOWNLOAD_WINDOW &&
		...
		if (!cl->downloadBlocks[curindex]) cl->downloadBlocks[curindex] = Z_Malloc(MAX_DOWNLOAD_BLKSIZE);
	...
	if (cl->downloadClientBlock == cl->downloadCurrentBlock) return 0; // Nothing to transmit
	...
	if (cl->downloadXmitBlock == 0) MSG_WriteLong(msg, cl->downloadSize);
...
int SV_SendQueuedMessages(void) {
	...
	for (i = 0; i < sv_maxclients->integer; i++) {
		...
		if (cl->state) {
			...
			if (!nextFragT) nextFragT = SV_Netchan_TransmitNextFragment(cl);

			if (nextFragT >= 0 && (retval == -1 || retval > nextFragT)) retval = nextFragT;
		}
	}
...
static void SV_VerifyPaks_f(client_t *cl) {
	...
	if (sv_pure->integer != 0) {
		...
		if (bGood) bGood = (FS_FileIsInPAK("vm/ui.qvm", &nChkSum2) == 1);
		...
		while (bGood) {
			...
			for (i = 0; nCurArg < nClientPaks; i++) { nClientChkSum[i] = atoi(Cmd_Argv(nCurArg++)); }
			...
			if (nServerPaks > 1024) nServerPaks = 1024;

			for (i = 0; i < nServerPaks; i++) { nServerChkSum[i] = atoi(Cmd_Argv(i)); }

			// check if the client has provided any pure checksums of pk3 files not loaded by the server
			for (i = 0; i < nClientPaks; i++) {
				for (j = 0; j < nServerPaks; j++) {
					if (nClientChkSum[i] == nServerChkSum[j]) { break; }
				}
				if (j >= nServerPaks) {
					bGood = qfalse;
					break;
			...
			for (i = 0; i < nClientPaks; i++) { nChkSum1 ^= nClientChkSum[i]; }
...
void SV_UserinfoChanged(client_t *cl) {
	...
	if (strlen(val)) {
		i = atoi(val);
		if (i <= 0 || i > 100 || strlen(val) > 4) { Info_SetValueForKey(cl->userinfo, "handicap", "100"); }
	}
...
static void SV_UpdateVoipIgnore(client_t *cl, const char *idstr, qboolean ignore) {
	if ((*idstr >= '0') && (*idstr <= '9')) {
		...
		if ((id >= 0) && (id < MAX_CLIENTS)) { cl->ignoreVoipFromClient[id] = ignore; }
	}
}
...
static void SV_UserVoip(client_t *cl, msg_t *msg, qboolean ignoreData) {
	...
	if (msg->readcount > msg->cursize) return; // short/invalid packet, bail.

	if (packetsize > sizeof(encoded)) { // overlarge packet?
		int bytesleft = packetsize;
		while (bytesleft) {
			int br = bytesleft;
			if (br > sizeof(encoded)) br = sizeof(encoded);
	...
	if (ignoreData || SV_ShouldIgnoreVoipSender(cl)) return; // Blacklisted, disabled, etc.
	...
	for (i = 0, client = svs.clients; i < sv_maxclients->integer; i++, client++) {
		...
		if (!(flags & (VOIP_SPATIAL | VOIP_DIRECT))) continue; // not addressed to this player.
