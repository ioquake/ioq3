...
static void SV_Netchan_Encode(client_t *client, msg_t *msg, const char *clientCommandString) {
	...
	if (msg->cursize < SV_ENCODE_START) { return; }
...
#ifdef LEGACY_PROTOCOL
	if (client->compat) SV_Netchan_Encode(client, &netbuf->msg, netbuf->clientCommandString);
#endif
...
#ifdef LEGACY_PROTOCOL
		if (client->compat) SV_Netchan_Encode(client, msg, client->lastClientCommandString);
#endif
...
#ifdef LEGACY_PROTOCOL
	if (client->compat) SV_Netchan_Decode(client, msg);
#endif
