...
void SV_AddServerCommand(client_t *client, const char *cmd) {
	...
	if (client->state < CS_PRIMED) return;
...
void QDECL SV_SendServerCommand(client_t *cl, const char *fmt, ...) {
	...
	if (strlen((char *)message) > 1022) { return; }
...
void SV_MasterHeartbeat(const char *message) {
	...
	for (i = 0; i < MAX_MASTER_SERVERS; i++) {
		if (!sv_master[i]->string[0]) continue;
		...
		if (adr[i][0].type == NA_BAD && adr[i][1].type == NA_BAD) { continue; }
		...
		if (adr[i][0].type != NA_BAD) NET_OutOfBandPrint(NS_SERVER, adr[i][0], "heartbeat %s\n", message);
		if (adr[i][1].type != NA_BAD) NET_OutOfBandPrint(NS_SERVER, adr[i][1], "heartbeat %s\n", message);
	}
}
...
static long SVC_HashForAddress(netadr_t address) {
	...
	for (i = 0; i < size; i++) { hash += (long)(ip[i]) * (i + 119); }
...
static leakyBucket_t *SVC_BucketForAddress(netadr_t address, int burst, int period) {
	...
	for (bucket = bucketHashes[hash]; bucket; bucket = bucket->next) {
		switch (bucket->type) {
		case NA_IP:
			if (memcmp(bucket->ipv._4, address.ip, 4) == 0) { return bucket; }
			break;

		case NA_IP6:
			if (memcmp(bucket->ipv._6, address.ip6, 16) == 0) { return bucket; }
			break;
	...
	for (i = 0; i < MAX_BUCKETS; i++) {
		...
		if (bucket->type == NA_BAD) {
			...
			if (bucketHashes[hash] != NULL) { bucketHashes[hash]->prev = bucket; }
...
static void SVC_Status(netadr_t from) {
	...
	if (strlen(Cmd_Argv(1)) > 128) return;
...
void SVC_Info(netadr_t from) {
	...
	for (i = sv_privateClients->integer; i < sv_maxclients->integer; i++) {
		if (svs.clients[i].state >= CS_CONNECTED) {
			count++;
			if (svs.clients[i].netchan.remoteAddress.type != NA_BOT) { humans++; }
		}
	}
	...
	if (sv_minPing->integer) { Info_SetValueForKey(infostring, "minPing", va("%i", sv_minPing->integer)); }
	if (sv_maxPing->integer) { Info_SetValueForKey(infostring, "maxPing", va("%i", sv_maxPing->integer)); }
	gamedir = Cvar_VariableString("fs_game");
	if (*gamedir) { Info_SetValueForKey(infostring, "game", gamedir); }
...
static void SV_ConnectionlessPacket(netadr_t from, msg_t *msg) {
	...
	if (!Q_strncmp("connect", (char *)&msg->data[4], 7)) { Huff_Decompress(msg, 12); }
...
static void SV_CalcPings(void) {
	...
	for (i = 0; i < sv_maxclients->integer; i++) {
		...
		for (j = 0; j < PACKET_BACKUP; j++) {
			if (cl->frames[j].messageAcked <= 0) { continue; }
...
static qboolean SV_CheckPaused(void) {
	...
	if (!cl_paused->integer) { return qfalse; }

	// only pause if there is just a single client connected
	count = 0;
	for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
		if (cl->state >= CS_CONNECTED && cl->netchan.remoteAddress.type != NA_BOT) { count++; }
	}

	if (count > 1) {
		// don't pause
		if (sv_paused->integer) Cvar_Set("sv_paused", "0");
		return qfalse;
	}

	if (!sv_paused->integer) Cvar_Set("sv_paused", "1");
	return qtrue;
}
...
void SV_Frame(int msec) {
	...
	if (sv_fps->integer < 1) { Cvar_Set("sv_fps", "10"); }
...
int SV_SendQueuedPackets() {
	...
	if (sv_dlRate->integer) {
		...
		if (deltaT > 0) {
			if (deltaT < timeVal) timeVal = deltaT + 1;
		} else {
		...
		if (SV_SendDownloadMessages()) timeVal = 0;
	}
