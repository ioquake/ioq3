...
void InitLevelItemHeap(void) {
	...
	for (i = 0; i < max_levelitems - 1; i++) { levelitemheap[i].next = &levelitemheap[i + 1]; } // end for
...
void BotInitLevelItems(void) {
	...
	for (i = 0; i < ic->numiteminfo; i++) {
		if (!ic->iteminfo[i].modelindex) { Log_Write("item %s has modelindex 0", ic->iteminfo[i].classname); } // end if
	} // end for
	...
	if (avoidtime < 0) {
		...
		for (li = levelitems; li; li = li->next) {
			if (li->number == number) {
				...
				if (!avoidtime) avoidtime = AVOID_DEFAULT_TIME;
				if (avoidtime < AVOID_MINIMUM_TIME) avoidtime = AVOID_MINIMUM_TIME;
...
void BotUpdateEntityItems(void) {
	...
	for (ent = AAS_NextEntity(0); ent; ent = AAS_NextEntity(ent)) {
		...
		for (i = 0; i < ic->numiteminfo; i++) {
			if (ic->iteminfo[i].modelindex == modelindex) { break; } // end if
		}															 // end for
...
int BotChooseLTGItem(int goalstate, vec3_t origin, int *inventory, int travelflags) {
	...
	gs = BotGoalStateFromHandle(goalstate);
	if (!gs) return qfalse;
	if (!gs->itemweightconfig) return qfalse;
	...
	for (li = levelitems; li; li = li->next) {
		...
		// obelisk)
		if (!li->entitynum && !(li->flags & IFL_ROAM)) continue;
		...
		if (li->timeout) weight += droppedweight->value;
#endif // DROPPEDWEIGHT
	   // use weight scale for item_botroam
		...
		if (weight > 0) {
			...
			if (t > 0) {
				...
				if (avoidtime - t * 0.009 > 0) continue;
				//
	...
	goal.flags	   = GFL_ITEM;
	if (bestitem->timeout) goal.flags |= GFL_DROPPED;
	if (bestitem->flags & IFL_ROAM) goal.flags |= GFL_ROAM;
...
int BotItemGoalInVisButNotVisible(int viewer, vec3_t eye, vec3_t viewangles, bot_goal_t *goal) {
	...
	if (trace.fraction >= 1) {
		...
		if (goal->entitynum <= 0) return qfalse;
		//
		...
		if (entinfo.ltime < AAS_Time() - 0.5) return qtrue;
	} // end if
	return qfalse;
...
void BotShutdownGoalAI(void) {
	...
	for (i = 1; i <= MAX_CLIENTS; i++) {
		if (botgoalstates[i]) { BotFreeGoalState(i); } // end if
	}												   // end for
