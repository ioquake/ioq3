...
int BotReachabilityArea(vec3_t origin, int client) {
	...
	if (!bsptrace.startsolid && bsptrace.fraction < 1 && bsptrace.ent != ENTITYNUM_NONE) {
		...
		if (bsptrace.ent == ENTITYNUM_WORLD) { return BotFuzzyPointReachabilityArea(origin); } // end if
		...
		if (AAS_Swimming(origin)) { return BotFuzzyPointReachabilityArea(origin); } // end if
		//
		...
		if (!trace.startsolid) { VectorCopy(trace.endpos, org); } // end if
		//
...
int BotOnMover(vec3_t origin, int entnum, aas_reachability_t *reach) {
	...
	if (!trace.startsolid && !trace.allsolid) {
		...
		if (trace.ent != ENTITYNUM_NONE && AAS_EntityModelNum(trace.ent) == modelnum) { return qtrue; } // end if
	}																									// end if
	return qfalse;
...
int BotAvoidSpots(vec3_t origin, aas_reachability_t *reach, bot_avoidspot_t *avoidspots, int numavoidspots) {
	...
	for (i = 0; i < numavoidspots; i++) {
		...
		if (type == AVOID_ALWAYS) return type;
	} // end for
	return type;
...
void BotAddAvoidSpot(int movestate, vec3_t origin, float radius, int type) {
	...
	if (ms->numavoidspots >= MAX_AVOIDSPOTS) return;
...
int BotGetReachabilityToGoal(vec3_t origin, int areanum, int lastgoalareanum, int lastareanum, int *avoidreach,
	...
	if (!areanum) return 0;
	//
	if (AAS_AreaDoNotEnter(areanum) || AAS_AreaDoNotEnter(goal->areanum)) { travelflags |= TFL_DONOTENTER; } // end if
	...
	for (reachnum = AAS_NextAreaReachability(areanum, 0); reachnum;
...
#ifdef DEBUG
			if (botDeveloper) { botimport.Print(PRT_MESSAGE, "avoiding reachability %d\n", avoidreach[i]); } // end if
#endif																										 // DEBUG
			continue;
		} // end if
#endif	  // AVOIDREACH
	   // get the reachability from the number
		...
		if (BotAvoidSpots(origin, &reach, avoidspots, numavoidspots)) {
			if (flags) { *flags |= MOVERESULT_BLOCKEDBYAVOIDSPOT; }
			continue;
		}
...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type) {
	...
	if (ms->moveflags & MFL_ONGROUND) {
		...
		if (ms->moveflags & MFL_BARRIERJUMP) {
			...
			if (ms->velocity[2] < 50) { EA_Move(ms->client, dir, speed); } // end if
		}																   // end if
...
bot_moveresult_t BotTravel_Elevator(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if (BotOnMover(ms->origin, ms->entitynum, reach)) {
	   ...
	   // move to the end point
			...
			hordir[2] = 0;
			VectorNormalize(hordir);
			if (!BotCheckBarrierJump(ms, hordir, 100)) { EA_Move(ms->client, hordir, 400); } // end if
			VectorCopy(hordir, result.movedir);
		} // end else
...
int GrappleState(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	for (i = AAS_NextEntity(0); i; i = AAS_NextEntity(i)) {
		if (AAS_EntityType(i) == (int)entitytypemissile->value) {
			...
			if (entinfo.weapon == (int)weapindex_grapple->value) { return 1; } // end if
		}																	   // end if
	}																		   // end for
...
void BotResetGrapple(bot_movestate_t *ms) {
	...
	if ((reach.traveltype & TRAVELTYPE_MASK) != TRAVEL_GRAPPLEHOOK) {
		if ((ms->moveflags & MFL_ACTIVEGRAPPLE) || ms->grapplevisible_time) {
			if (offhandgrapple->value) EA_Command(ms->client, cmd_grappleoff->string);
...
bot_moveresult_t BotTravel_Grapple(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if (ms->moveflags & MFL_ACTIVEGRAPPLE) {
	   ...
	   //
		...
		//
		if (!(int)offhandgrapple->value) { EA_Attack(ms->client); } // end if
	...
	if (ms->moveflags & (MFL_ONGROUND | MFL_SWIMMING | MFL_AGAINSTLADDER)) {
		...
		if (reachnum) {
			...
			// special grapple hook case
				...
				if (ms->reachability_time < AAS_Time() || (ms->moveflags & MFL_GRAPPLERESET)) { reachnum = 0; } // end
																												// if
			} // end if
			// special elevator case
				...
				if (ms->areanum == reach.areanum || ms->reachability_time < AAS_Time()) { reachnum = 0; } // end if
			}																							  // end if
			else {
#ifdef DEBUG
				if (botDeveloper) {
	   ...
	   // or the area changed
		...
		if (!reachnum) {
			...
			if (!AAS_AreaReachability(ms->areanum)) {
#ifdef DEBUG
				if (botDeveloper) { botimport.Print(PRT_MESSAGE, "area %d no reachability\n", ms->areanum); } // end if
#endif																										  // DEBUG
			}																								  // end if
			// get a new reachability leading towards the goal
			reachnum = BotGetReachabilityToGoal(ms->origin, ms->areanum, ms->lastgoalareanum, ms->lastareanum,
	   ...
	   //
		...
		for (i = numareas - 1; i >= 0; i--) {
			if (AAS_AreaJumpPad(areas[i])) {
				...
				// areas[i]);
				foundjumppad = qtrue;
				lastreachnum = BotGetReachabilityToGoal(end, areas[i], ms->lastgoalareanum, ms->lastareanum,
														ms->avoidreach, ms->avoidreachtimes, ms->avoidreachtries, goal,
														TFL_JUMPPAD, ms->avoidspots, ms->numavoidspots, NULL);
				if (lastreachnum) {
