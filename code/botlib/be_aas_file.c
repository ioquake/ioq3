...
void AAS_SwapAASData(void) {
	...
	for (i = 0; i < aasworld.numvertexes; i++) {
		for (j = 0; j < 3; j++) aasworld.vertexes[i][j] = LittleFloat(aasworld.vertexes[i][j]);
	} // end for
	// planes
	for (i = 0; i < aasworld.numplanes; i++) {
		for (j = 0; j < 3; j++) aasworld.planes[i].normal[j] = LittleFloat(aasworld.planes[i].normal[j]);
...
void AAS_DData(unsigned char *data, int size) {
	...
	for (i = 0; i < size; i++) { data[i] ^= (unsigned char)i * 119; } // end for
...
int AAS_LoadAASFile(char *filename) {
	...
	//
	if (header.version == AASVERSION) { AAS_DData((unsigned char *)&header + 8, sizeof(aas_header_t) - 8); } // end if
	//
...
int AAS_WriteAASLump(fileHandle_t fp, aas_header_t *h, int lumpnum, void *data, int length) {
	...
	if (length > 0) { botimport.FS_Write(data, length, fp); } // end if
