...
void AAS_JumpReachRunStart(aas_reachability_t *reach, vec3_t runstart) {
	...
	if (move.stopevent & (SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE)) { VectorCopy(start, runstart); } // end if
...
int AAS_ClientMovementPrediction(struct aas_clientmove_s *move, int entnum, vec3_t origin, int presencetype,
	...
	for (n = 0; n < maxframes; n++) {
		...
		if (crouch) {
			...
			if (AAS_PointPresenceType(org) & PRESENCE_NORMAL) { presencetype = PRESENCE_NORMAL; } // end if
		}																						  // end else
		...
		do {
			...
			if (trace.fraction < 1.0) {
				...
				if (!step) {
					...
					if (DotProduct(plane->normal, up) > phys_maxsteepness) { onground = qtrue; } // end if
		...
		if (frame_test_vel[2] <= 10) {
			...
			//
			areanum = AAS_PointAreaNum(org);
			if (aasworld.areasettings[areanum].contents & AREACONTENTS_LAVA) event |= SE_ENTERLAVA;
			if (aasworld.areasettings[areanum].contents & AREACONTENTS_SLIME) event |= SE_ENTERSLIME;
			if (aasworld.areasettings[areanum].contents & AREACONTENTS_WATER) event |= SE_ENTERWATER;
			// if in lava or slime
			if (event & stopevent) {
				VectorCopy(org, move->endpos);
...
void AAS_TestMovementPrediction(int entnum, vec3_t origin, vec3_t dir) {
	...
	if (move.stopevent & SE_LEAVEGROUND) { botimport.Print(PRT_MESSAGE, "leave ground\n"); } // end if
