...
int AAS_BestReachableLinkArea(aas_link_t *areas) {
	...
	for (link = areas; link; link = link->next_area) {
		if (AAS_AreaGrounded(link->areanum) || AAS_AreaSwim(link->areanum)) { return link->areanum; } // end if
	}																								  // end for
	//
	for (link = areas; link; link = link->next_area) {
		...
		for (j = 0; j < area2->numfaces; j++) {
			...
			if (face1num == face2num) {
				...
				if (AAS_PointContents(start) & (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER)) {
					...
					if (AAS_AreaVolume(area2num) < 800) lreach->traveltime += 200;
				...
				if (!trace.startsolid && trace.fraction >= 1.0) {
					...
					if (AAS_PointAreaNum(trace.endpos) == area2num) {
						...
						for (i = 0; i < numareas; i++)
							if (AAS_AreaClusterPortal(areas[i])) break;
						if (i >= numareas) {
...
int AAS_Reachability_Jump(int area1num, int area2num) {
	...
	if (bestdist > 4 && bestdist < maxjumpdistance) {
		...
		if (bestdist <= 48 && fabs(beststart[2] - bestend[2]) < 8) {
			...
			if (!AAS_HorizontalVelocityForJump(phys_jumpvel, beststart, bestend, &speed)) return qfalse;
			speed *= 1.05f;
			...
			dir[2] = 0;
			if (VectorLength(dir) < 10) return qfalse;
		} // end if
		//
		VectorSubtract(bestend, beststart, dir);
		VectorNormalize(dir);
		...
		if (trace.fraction < 1) {
			...
			if (DotProduct(plane->normal, up) >= 0.7) {
				...
				if (!(AAS_PointContents(trace.endpos) & (CONTENTS_LAVA | CONTENTS_SLIME))) {
					if (teststart[2] - trace.endpos[2] <= aassettings.phys_maxbarrier) return qfalse;
				} // end if
			}	  // end if
		}		  // end if
		//
		...
		if (!AAS_AreaClusterPortal(area1num) && !AAS_AreaClusterPortal(area2num)) stopevent |= SE_TOUCHCLUSTERPORTAL;
		//
		for (i = 0; i < 3; i++) {
			//
			if (i == 1)
			...
			for (j = 0; j < numareas; j++) {
				if (areas[j] == area2num) break;
			} // end for
			if (j < numareas) break;
		}
		if (i >= 3) return qfalse;
			//
#ifdef REACH_DEBUG
...
int AAS_Reachability_Ladder(int area1num, int area2num) {
	...
	if (ladderface1 && ladderface2) {
		...
		if (ladderface1vertical) {
			...
			//
#ifdef REACH_DEBUG
			if (trace.startsolid) { Log_Write("trace from area %d started in solid\r\n", area1num); } // end if
#endif																								  // REACH_DEBUG
	   //
			trace.endpos[2] += 1;
...
int AAS_TravelFlagsForTeam(int ent) {
	...
	if (!AAS_IntForBSPEpairKey(ent, "bot_notteam", &notteam)) return 0;
	if (notteam == 1) return TRAVELFLAG_NOTTEAM1;
	if (notteam == 2) return TRAVELFLAG_NOTTEAM2;
	return 0;
		...
		if (!strcmp(classname, "trigger_multiple")) {
			...
			for (dest = AAS_NextBSPEntity(0); dest; dest = AAS_NextBSPEntity(dest)) {
				...
				if (!strcmp(classname, "target_teleporter")) {
					...
					if (!strcmp(targetname, target)) { break; } // end if
				}												// end if
			}													// end for
			if (!dest) { continue; }							// end if
...
void AAS_Reachability_FuncBobbing(void) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		...
		if (!AAS_VectorForBSPEpairKey(ent, "origin", origin)) VectorSet(origin, 0, 0, 0);
		//
...
int AAS_Reachability_Grapple(int area1num, int area2num) {
	...
	for (i = 0; i < area2->numfaces; i++) {
		...
		if (aasworld.areasettings[areanum].contents & (AREACONTENTS_SLIME | AREACONTENTS_LAVA)) { continue; } // end if
				...
				for (l = 0; l < face2->numedges; l++) {
					...
					if (abs(edge1num) == abs(edge2num)) {
						...
						for (p = 0; p < numareas; p++)
							if (AAS_AreaClusterPortal(areas[p])) break;
						if (p < numareas) break;
...
int AAS_ContinueInitReachability(float time) {
	...
	for (i = aasworld.numreachabilityareas; i < aasworld.numareas && i < todo; i++) {
		...
		if (aasworld.areasettings[i].contents & AREACONTENTS_JUMPPAD) { continue; } // end if
		// loop over the areas
		...
		if (aasworld.areasettings[i].contents & (AREACONTENTS_TELEPORTER | AREACONTENTS_JUMPPAD)) { continue; } // end
																												// if
		// loop over the areas
		for (j = 1; j < aasworld.numareas; j++) {
			if (i == j) continue;
