...
void AAS_RemoveClusterAreas(void) {
	...
	for (i = 1; i < aasworld.numareas; i++) { aasworld.areasettings[i].cluster = 0; } // end for
...
void AAS_ClearCluster(int clusternum) {
	...
	for (i = 1; i < aasworld.numareas; i++) {
		if (aasworld.areasettings[i].cluster == clusternum) { aasworld.areasettings[i].cluster = 0; } // end if
	}																								  // end for
	...
	if (!nofaceflood) {
		for (i = 0; i < area->numfaces; i++) {
			...
			if (face->frontarea == areanum) {
				if (face->backarea)
					if (!AAS_FloodClusterAreas_r(face->backarea, clusternum)) return qfalse;
			} // end if
			else {
				if (face->frontarea)
					if (!AAS_FloodClusterAreas_r(face->frontarea, clusternum)) return qfalse;
			} // end else
		}	  // end for
	}		  // end if
	...
	for (i = 1; i < aasworld.numareas; i++) {
		...
		for (j = 0; j < aasworld.areasettings[i].numreachableareas; j++) {
			...
			if (aasworld.areasettings[areanum].cluster) {
				if (!AAS_FloodClusterAreas_r(i, clusternum)) return qfalse;
				i = 0;
				break;
			} // end if
		}	  // end for
...
int AAS_FindClusters(void) {
	...
	for (i = 1; i < aasworld.numareas; i++) {
		...
		if (nofaceflood) {
			if (!aasworld.areasettings[i].numreachableareas) continue;
		} // end if
		...
		if (!AAS_FloodClusterAreas_r(i, aasworld.numclusters)) return qfalse;
		if (!AAS_FloodClusterAreasUsingReachabilities(aasworld.numclusters)) return qfalse;
...
void AAS_FindPossiblePortals(void) {
	...
	for (i = 1; i < aasworld.numareas; i++) { numpossibleportals += AAS_CheckAreaForPossiblePortals(i); } // end for
...
void AAS_InitClustering(void) {
	...
	if (aasworld.numclusters >= 1) {
		...
		if (!((int)LibVarGetValue("forceclustering")) && !((int)LibVarGetValue("forcereachability"))) return;
#endif
	} // end if
	...
	while (1) {
		...
		removedPortalAreas++;
		// find the clusters
		if (!AAS_FindClusters()) continue;
		// test the portals
		if (!AAS_TestPortals()) continue;
		//
		break;
	} // end while
