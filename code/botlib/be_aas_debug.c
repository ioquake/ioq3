...
void AAS_ShowFace(int facenum) {
	...
	if (facenum >= aasworld.numfaces) { botimport.Print(PRT_ERROR, "facenum %d out of range\n", facenum); } // end if
	...
	for (i = 0; i < face->numedges; i++) {
		...
		if (edgenum >= aasworld.numedges) { botimport.Print(PRT_ERROR, "edgenum %d out of range\n", edgenum); } // end
																												// if
...
void AAS_ShowArea(int areanum, int groundfacesonly) {
	...
	for (i = 0; i < area->numfaces; i++) {
		...
		for (j = 0; j < face->numedges; j++) {
			...
			if (n == numareaedges && numareaedges < MAX_DEBUGLINES) { areaedges[numareaedges++] = edgenum; } // end if
		}																									 // end for
	...
	for (n = 0; n < numareaedges; n++) {
		for (line = 0; line < MAX_DEBUGLINES; line++) {
			...
			if (!debuglinevisible[line]) { break; } // end else
		}											// end for
...
void AAS_FloodAreas_r(int areanum, int cluster, int *done) {
	...
	for (i = 0; i < area->numfaces; i++) {
		...
		if (!nextareanum) continue;
		if (done[nextareanum]) continue;
		done[nextareanum] = qtrue;
		if (aasworld.areasettings[nextareanum].contents & AREACONTENTS_VIEWPORTAL) continue;
		if (AAS_AreaCluster(nextareanum) != cluster) continue;
