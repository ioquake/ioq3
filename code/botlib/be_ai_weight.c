...
#endif // DEBUG
	//
	if (!LibVarGetValue("bot_reloadcharacters")) { weightFileList[avail] = config; } // end if
	//
	return config;
...
int FindFuzzyWeight(weightconfig_t *wc, char *name) {
	...
	for (i = 0; i < wc->numweights; i++) {
		if (!strcmp(wc->weights[i].name, name)) { return i; } // end if
	}														  // end if
	return -1;
...
void EvolveWeightConfig(weightconfig_t *config) {
	...
	for (i = 0; i < config->numweights; i++) { EvolveFuzzySeperator_r(config->weights[i].firstseperator); } // end for
...
void ScaleFuzzySeperatorBalanceRange_r(fuzzyseperator_t *fs, float scale) {
	if (fs->child) {
		...
		if (fs->maxweight < fs->minweight) { fs->maxweight = fs->minweight; } // end if
	}																		  // end else if
...
int InterbreedFuzzySeperator_r(fuzzyseperator_t *fs1, fuzzyseperator_t *fs2, fuzzyseperator_t *fsout) {
	if (fs1->child) {
		...
		if (!InterbreedFuzzySeperator_r(fs2->child, fs2->child, fsout->child)) { return qfalse; } // end if
	}																							  // end if
	...
	if (fs1->next) {
		...
		if (!InterbreedFuzzySeperator_r(fs1->next, fs2->next, fsout->next)) { return qfalse; } // end if
	}																						   // end if
	return qtrue;
