...
int AAS_PointReachabilityAreaIndex(vec3_t origin) {
	...
	if (!origin) {
		index = 0;
		for (i = 0; i < aasworld.numclusters; i++) { index += aasworld.clusters[i].numreachabilityareas; } // end for
		return index;
	} // end if

	areanum = AAS_PointAreaNum(origin);
	if (!areanum || !AAS_AreaReachability(areanum)) return 0;
	...
	index = 0;
	for (i = 0; i < cluster; i++) { index += aasworld.clusters[i].numreachabilityareas; } // end for
	index += areanum;
	return index;
...
int AAS_TraceAreas(vec3_t start, vec3_t end, int *areas, vec3_t *points, int maxareas) {
	...
	while (1) {
		...
		// if the trace stack is empty (ended up with a piece of the
		// line to be traced in an area)
		if (tstack_p < tracestack) { return numareas; } // end if
		// number of the current node to test the line against
		nodenum = tstack_p->nodenum;
		// if it is an area
		if (nodenum < 0) {
