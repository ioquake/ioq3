	...
	for (done = 0, numparms = 0, indent = 0; !done;) {
		...
		while (!done) {
			...
			if (!strcmp(token.string, "(")) {
				...
				if (--indent <= 0) {
					if (!parms[define->numparms - 1]) { SourceWarning(source, "too few define parms"); } // end if
					done = 1;
					break;
				} // end if
			}	  // end if
...
void PC_PrintDefineHashTable(define_t **definehash) {
	...
	for (i = 0; i < DEFINEHASHSIZE; i++) {
		Log_Write("%4d:", i);
		for (d = definehash[i]; d; d = d->hashnext) { Log_Write(" %s", d->name); } // end for
		Log_Write("\n");
	} // end for
...
int PC_ExpandDefine(source_t *source, token_t *deftoken, define_t *define, token_t **firsttoken, token_t **lasttoken) {
	...
	if (define->builtin) { return PC_ExpandBuiltinDefine(source, deftoken, define, firsttoken, lasttoken); } // end if
	...
	if (define->numparms) {
		...
		for (i = 0; i < define->numparms; i++) {
			...
			for (pt = parms[i]; pt; pt = pt->next) { Log_Write("%s", pt->string); } // end for
		}																			// end for
#endif																				// DEBUG_EVAL
	}																				// end if
	// empty list at first
	...
	for (dt = define->tokens; dt; dt = dt->next) {
		...
		if (dt->type == TT_NAME) { parmnum = PC_FindDefineParm(define, dt->string); } // end if
...
int PC_Directive_include(source_t *source) {
	...
	if (token.type == TT_STRING) {
		...
		if (*token.string != '>') { SourceWarning(source, "#include missing trailing >"); } // end if
		if (!strlen(path)) {
...
int PC_EvaluateTokens(source_t *source, token_t *tokens, signed long int *intvalue, float *floatvalue, int integer) {
	...
	while (!error && firstoperator) {
		...
		for (o = firstoperator; o->next; o = o->next) {
			...
			if (o->operator!= P_LOGIC_NOT && o->operator!= P_BIN_NOT) v = v->next;
...
int PC_ReadDirective(source_t *source) {
	...
	if (token.type == TT_NAME) {
		...
		for (i = 0; directives[i].name; i++) {
			if (!strcmp(directives[i].name, token.string)) { return directives[i].func(source); } // end if
		}																						  // end for
	}																							  // end if
...
int PC_ReadDollarDirective(source_t *source) {
	...
	if (token.type == TT_NAME) {
		...
		for (i = 0; dollardirectives[i].name; i++) {
			if (!strcmp(dollardirectives[i].name, token.string)) { return dollardirectives[i].func(source); } // end if
		}																									  // end for
	}																										  // end if
	PC_UnreadSourceToken(source, &token);
	SourceError(source, "unknown precompiler directive %s", token.string);
...
int PC_FreeSourceHandle(int handle) {
	if (handle < 1 || handle >= MAX_SOURCEFILES) return qfalse;
	if (!sourceFiles[handle]) return qfalse;
...
int PC_ReadTokenHandle(int handle, pc_token_t *pc_token) {
	...
	if (pc_token->type == TT_STRING) StripDoubleQuotes(pc_token->string);
	return ret;
...
// Changes Globals:		-
//============================================================================
void PC_SetBaseFolder(char *path) { PS_SetBaseFolder(path); } // end of the function PC_SetBaseFolder
//============================================================================
//
// Parameter:			-
// Returns:				-
