...
static void SNDDMA_AudioCallback(void *userdata, Uint8 *stream, int len) {
	...
	if (sdlMasterGain != 1.0f) {
		...
		if (dma.isfloat && (dma.samplebits == 32)) {
			...
			len /= sizeof(*ptr);
			for (i = 0; i < len; i++, ptr++) { *ptr *= sdlMasterGain; }
		} else if (dma.samplebits == 16) {
			Sint16 *ptr = (Sint16 *)stream;
			len /= sizeof(*ptr);
			for (i = 0; i < len; i++, ptr++) { *ptr = (Sint16)(((float)*ptr) * sdlMasterGain); }
		} else if (dma.samplebits == 8) {
			Uint8 *ptr = (Uint8 *)stream;
			len /= sizeof(*ptr);
			for (i = 0; i < len; i++, ptr++) { *ptr = (Uint8)(((float)*ptr) * sdlMasterGain); }
		}
	}
#endif
}
...
static void SNDDMA_PrintAudiospec(const char *str, const SDL_AudioSpec *spec) {
	...
	for (i = 0; i < formatToStringTableSize; i++) {
		if (spec->format == formatToStringTable[i].enumFormat) { fmt = formatToStringTable[i].stringFormat; }
	}
...
qboolean SNDDMA_Init(void) {
	...
	if (!tmp) tmp = (obtained.samples * obtained.channels) * 10;
...
===============
*/
void SNDDMA_Submit(void) { SDL_UnlockAudioDevice(sdlPlaybackDevice); }
...
===============
*/
void SNDDMA_BeginPainting(void) { SDL_LockAudioDevice(sdlPlaybackDevice); }
...
#ifdef USE_SDL_AUDIO_CAPTURE
	if (sdlCaptureDevice) { SDL_PauseAudioDevice(sdlCaptureDevice, 1); }
#endif
}
