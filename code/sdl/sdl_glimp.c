...
===============
*/
void GLimp_Minimize(void) { SDL_MinimizeWindow(SDL_window); }
...
static void GLimp_DetectAvailableModes(void) {
	...
	if (!modes) { ri.Error(ERR_FATAL, "Out of memory"); }
	...
	for (i = 0; i < numSDLModes; i++) {
		...
		if (windowMode.format != mode.format) continue;

		// SDL can give the same resolution with different refresh rates.
		// Only list resolution once.
		for (j = 0; j < numModes; j++) {
			if (mode.w == modes[j].w && mode.h == modes[j].h) break;
		}

		if (j != numModes) continue;
	...
	if (numModes > 1) qsort(modes, numModes, sizeof(SDL_Rect), GLimp_CompareModes);
...
#define GLE(ret, name, ...)                                                                                            \
	...
	if (!qglGetString) { Com_Error(ERR_FATAL, "glGetString is NULL"); }

	version = (const char *)qglGetString(GL_VERSION);

	if (!version) { Com_Error(ERR_FATAL, "GL_VERSION is NULL"); }
	...
	if (QGL_VERSION_ATLEAST(3, 0) || QGLES_VERSION_ATLEAST(3, 0)) { QGL_3_0_PROCS; }
...
static int GLimp_SetMode(int mode, qboolean fullscreen, qboolean noborder, qboolean fixedFunction) {
	...
	if (r_allowResize->integer) flags |= SDL_WINDOW_RESIZABLE;
	...
	if (fullscreen) {
		...
		if (noborder) flags |= SDL_WINDOW_BORDERLESS;

		glConfig.isFullscreen = qfalse;
	}

	colorBits = r_colorbits->value;
	if ((!colorBits) || (colorBits >= 32)) colorBits = 24;
	...
	for (i = 0; i < 16; i++) {
		...
		if ((i % 4) == 3) { // reduce colorBits
			if (testColorBits == 24) testColorBits = 16;
		}
		...
		if (perChannelColorBits == 4) perChannelColorBits = 0; /* Use minimum size for 16-bit color */
		...
		if (!fixedFunction) {
			...
			if ((SDL_glContext = SDL_GL_CreateContext(SDL_window)) == NULL) {
				...
				if (!renderer || (strstr(renderer, "Software Renderer") || strstr(renderer, "Software Rasterizer"))) {
					if (renderer) ri.Printf(PRINT_ALL, "GL_RENDERER is %s, rejecting context\n", renderer);
	...
	if (fixedFunction) {
		...
		if (SDL_GL_ExtensionSupported("GL_EXT_compiled_vertex_array")) {
			if (r_ext_compiled_vertex_array->value) {
				...
				if (!qglLockArraysEXT || !qglUnlockArraysEXT) { ri.Error(ERR_FATAL, "bad getprocaddress"); }
			} else {
...
void GLimp_Init(qboolean fixedFunction) {
	...
	if (GLimp_StartDriverAndSetMode(r_mode->integer, r_fullscreen->integer, qfalse, fixedFunction)) goto success;

	// Finally, try the default screen resolution
	if (r_mode->integer != R_MODE_FALLBACK) {
		ri.Printf(PRINT_ALL, "Setting r_mode %d failed, falling back on r_mode %d\n", r_mode->integer, R_MODE_FALLBACK);

		if (GLimp_StartDriverAndSetMode(R_MODE_FALLBACK, qfalse, qfalse, fixedFunction)) goto success;
	}
...
success:
	...
	if (qglGetStringi) {
		...
		for (i = 0; i < numExtensions; i++) {
			...
			if ((listLength + extensionLength + 1) >= sizeof(glConfig.extensions_string)) break;
...
void GLimp_EndFrame(void) {
	...
	if (Q_stricmp(r_drawBuffer->string, "GL_FRONT") != 0) { SDL_GL_SwapWindow(SDL_window); }
	...
	if (r_fullscreen->modified) {
		...
		if (needToToggle) {
			...
			if (!sdlToggled) ri.Cmd_ExecuteText(EXEC_APPEND, "vid_restart\n");
