...
static const orderTask_t validOrders[] = {
	{VOICECHAT_GETFLAG, TEAMTASK_OFFENSE},		   {VOICECHAT_OFFENSE, TEAMTASK_OFFENSE},
	{VOICECHAT_DEFEND, TEAMTASK_DEFENSE},		   {VOICECHAT_DEFENDFLAG, TEAMTASK_DEFENSE},
	{VOICECHAT_PATROL, TEAMTASK_PATROL},		   {VOICECHAT_CAMP, TEAMTASK_CAMP},
	{VOICECHAT_FOLLOWME, TEAMTASK_FOLLOW},		   {VOICECHAT_RETURNFLAG, TEAMTASK_RETRIEVE},
...
static int CG_ValidOrder(const char *p) {
	...
	for (i = 0; i < numValidOrders; i++) {
		if (Q_stricmp(p, validOrders[i].order) == 0) { return validOrders[i].taskNum; }
	}
	return -1;
}
#endif
...
static void CG_AddToTeamChat(const char *str) {
	...
	*p = 0;
	...
	if (cgs.teamChatPos - cgs.teamLastChatPos > chatHeight) cgs.teamLastChatPos = cgs.teamChatPos - chatHeight;
}
...
int CG_ParseVoiceChats(const char *filename, voiceChatList_t *voiceChatList, int maxVoiceChats) {
	...
	for (i = 0; i < maxVoiceChats; i++) { voiceChats[i].id[0] = 0; }
	...
	while (1) {
		...
		while (1) {
			...
			if (!token[0]) { return qtrue; }
			Com_sprintf(
				voiceChats[voiceChatList->numVoiceChats].chats[voiceChats[voiceChatList->numVoiceChats].numSounds],
...
int CG_HeadModelVoiceChats(char *filename) {
	...
	for (i = 0; i < MAX_VOICEFILES; i++) {
		if (!Q_stricmp(token, voiceChatLists[i].name)) { return i; }
	}
...
voiceChatList_t *CG_VoiceChatListForClient(int clientNum) {
	...
	for (k = 0; k < 2; k++) {
		...
		for (i = 0; i < MAX_HEADMODELS; i++) {
			if (!Q_stricmp(headModelVoiceChat[i].headmodel, headModelName)) { break; }
		}
		if (i < MAX_HEADMODELS) { return &voiceChatLists[headModelVoiceChat[i].voiceChatNum]; }
		...
		for (i = 0; i < MAX_HEADMODELS; i++) {
			if (!strlen(headModelVoiceChat[i].headmodel)) {
				...
				if (voiceChatNum == -1) break;
				Com_sprintf(headModelVoiceChat[i].headmodel, sizeof(headModelVoiceChat[i].headmodel), "%s",
							headModelName);
				...
				if (voiceChatLists[i].gender == gender) {
					...
					for (j = 0; j < MAX_HEADMODELS; j++) {
						if (!strlen(headModelVoiceChat[j].headmodel)) {
							Com_sprintf(headModelVoiceChat[j].headmodel, sizeof(headModelVoiceChat[j].headmodel), "%s",
										headModelName);
...
void CG_VoiceChatLocal(int mode, qboolean voiceOnly, int clientNum, int color, const char *cmd) {
	...
	if (mode == SAY_ALL && cgs.gametype >= GT_TEAM && cg_teamChatsOnly.integer) { return; }
...
static void CG_ServerCommand(void) {
	...
	if (!strcmp(cmd, "chat")) {
		if (cgs.gametype >= GT_TEAM && cg_teamChatsOnly.integer) { return; }
