...
void CG_CheckAmmo(void) {
	...
	for (i = WP_MACHINEGUN; i < WP_NUM_WEAPONS; i++) {
		if (!(weapons & (1 << i))) { continue; }
		if (cg.snap->ps.ammo[i] < 0) { continue; }
		switch (i) {
		...
		case WP_PROX_LAUNCHER:
			...
			break;
		default: total += cg.snap->ps.ammo[i] * 200; break;
		}
		if (total >= 5000) {
			cg.lowAmmoWarning = 0;
			return;
	...
	if (cg.lowAmmoWarning != previous) { trap_S_StartLocalSound(cgs.media.noAmmoSound, CHAN_LOCAL_SOUND); }
}
...
void CG_CheckLocalSounds(playerState_t *ps, playerState_t *ops) {
	...
	if (ps->stats[STAT_HEALTH] < ops->stats[STAT_HEALTH] - 1) {
		if (ps->stats[STAT_HEALTH] > 0) { CG_PainEvent(&cg.predictedPlayerEntity, ps->stats[STAT_HEALTH]); }
	}
	...
	if (cgs.fraglimit > 0 && cgs.gametype < GT_CTF) {
		...
		if (cgs.gametype == GT_TEAM && cgs.scores2 > highScore) { highScore = cgs.scores2; }
