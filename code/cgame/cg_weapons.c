...
static void CG_MachineGunEjectBrass(centity_t *cent) {
	...
	if (cg_brassTime.integer <= 0) { return; }
	...
	le->endTime	  = le->startTime + cg_brassTime.integer + ((float)cg_brassTime.integer / 4) * random();
...
static void CG_RocketTrail(centity_t *ent, const weaponInfo_t *wi) {
	...
	for (; t <= ent->trailTime; t += step) {
		...
		smoke = CG_SmokePuff(lastPos, up, wi->trailRadius, 1, 1, 1, 0.33f, wi->wiTrailTime, t, 0, 0,
...
void CG_RegisterWeapon(int weaponNum) {
	...
	for (ammo = bg_itemlist + 1; ammo->classname; ammo++) {
		if (ammo->giType == IT_AMMO && ammo->giTag == weaponNum) { break; }
	}
	if (ammo->classname && ammo->world_model[0]) { weaponInfo->ammoModel = trap_R_RegisterModel(ammo->world_model[0]); }
...
void CG_RegisterItemVisuals(int itemNum) {
	...
	if (item->giType == IT_POWERUP || item->giType == IT_HEALTH || item->giType == IT_ARMOR ||
		...
		if (item->world_model[1]) { itemInfo->models[1] = trap_R_RegisterModel(item->world_model[1]); }
	}
}
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps, centity_t *cent, int team) {
	...
	if ((weaponNum == WP_LIGHTNING || weaponNum == WP_GAUNTLET || weaponNum == WP_GRAPPLING_HOOK) &&
		...
		// impulse flash
		if (cg.time - cent->muzzleFlashTime > MUZZLE_FLASH_TIME) { return; }
	}
...
void CG_DrawWeaponSelect(void) {
	...
	for (i = 1; i < MAX_WEAPONS; i++) {
		...
		if (i == cg.weaponSelect) { CG_DrawPic(x - 4, y - 4, 40, 40, cgs.media.selectShader); }
...
static qboolean CG_WeaponSelectable(int i) {
	if (!cg.snap->ps.ammo[i]) { return qfalse; }
	if (!(cg.snap->ps.stats[STAT_WEAPONS] & (1 << i))) { return qfalse; }
...
void CG_NextWeapon_f(void) {
	...
	for (i = 0; i < MAX_WEAPONS; i++) {
		cg.weaponSelect++;
		if (cg.weaponSelect == MAX_WEAPONS) { cg.weaponSelect = 0; }
		if (cg.weaponSelect == WP_GAUNTLET) {
			continue; // never cycle to gauntlet
		}
		if (CG_WeaponSelectable(cg.weaponSelect)) { break; }
	}
	if (i == MAX_WEAPONS) { cg.weaponSelect = original; }
}
...
void CG_PrevWeapon_f(void) {
	...
	for (i = 0; i < MAX_WEAPONS; i++) {
		cg.weaponSelect--;
		if (cg.weaponSelect == -1) { cg.weaponSelect = MAX_WEAPONS - 1; }
		if (cg.weaponSelect == WP_GAUNTLET) {
			continue; // never cycle to gauntlet
		}
		if (CG_WeaponSelectable(cg.weaponSelect)) { break; }
...
void CG_Weapon_f(void) {
	...
	if (num < 1 || num > MAX_WEAPONS - 1) { return; }
...
void CG_FireWeapon(centity_t *cent) {
	...
	if (ent->weapon == WP_RAILGUN) { cent->pe.railFireTime = cg.time; }
	...
	// do brass ejection
	if (weap->ejectBrassFunc && cg_brassTime.integer > 0) { weap->ejectBrassFunc(cent); }
}
...
void CG_Bullet(vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh, int fleshEntityNum) {
	...
	if (sourceEntityNum >= 0 && cg_tracerChance.value > 0) {
		if (CG_CalcMuzzlePoint(sourceEntityNum, start)) {
			...
			// draw a tracer
			if (random() < cg_tracerChance.value) { CG_Tracer(start, end); }
		}
	}
