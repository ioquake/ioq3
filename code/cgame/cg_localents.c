...
void CG_InitLocalEntities(void) {
	...
	for (i = 0; i < MAX_LOCAL_ENTITIES - 1; i++) { cg_localEntities[i].next = &cg_localEntities[i + 1]; }
}
...
void CG_FragmentBounceMark(localEntity_t *le, trace_t *trace) {
	...
	if (le->leMarkType == LEMT_BLOOD) {
		...
		radius = 16 + (rand() & 31);
		CG_ImpactMark(cgs.media.bloodMarkShader, trace->endpos, trace->plane.normal, random() * 360, 1, 1, 1, 1, qtrue,
					  radius, qfalse);
	} else if (le->leMarkType == LEMT_BURN) {

		radius = 8 + (rand() & 15);
		CG_ImpactMark(cgs.media.burnMarkShader, trace->endpos, trace->plane.normal, random() * 360, 1, 1, 1, 1, qtrue,
					  radius, qfalse);
	}
...
static void CG_AddMoveScaleFade(localEntity_t *le) {
	...
	if (!(le->leFlags & LEF_PUFF_DONT_SCALE)) { re->radius = le->radius * (1.0 - c) + 8; }
...
===================
*/
void CG_AddInvulnerabilityImpact(localEntity_t *le) { trap_R_AddRefEntityToScene(&le->refEntity); }
...
void CG_AddScorePlum(localEntity_t *le) {
	...
	re->radius = (float)NUMBER_SIZE / 2.0f;
...
void CG_AddLocalEntities(void) {
	...
	for (; le != &cg_activeLocalEntities; le = next) {
		...
		case LE_SCOREPLUM: CG_AddScorePlum(le); break;
