...
static void CG_NextOrder_f(void) {
	...
	if (ci) {
		if (!ci->teamLeader && sortedTeamPlayers[cg_currentSelectedPlayer.integer] != cg.snap->ps.clientNum) { return; }
	}
	if (cgs.currentOrder < TEAMTASK_CAMP) {
		cgs.currentOrder++;

		if (cgs.currentOrder == TEAMTASK_RETRIEVE) {
			if (!CG_OtherTeamHasFlag()) { cgs.currentOrder++; }
		}

		if (cgs.currentOrder == TEAMTASK_ESCORT) {
			if (!CG_YourTeamHasFlag()) { cgs.currentOrder++; }
		}
...
static void CG_DenyOrder_f(void) {
	...
	if (cg.time < cgs.acceptOrderTime) { cgs.acceptOrderTime = 0; }
}
...
void CG_InitConsoleCommands(void) {
	...
	for (i = 0; i < ARRAY_LEN(commands); i++) { trap_AddCommand(commands[i].cmd); }
