...
Q_EXPORT intptr_t vmMain(int command, int arg0, int arg1, int arg2, int arg3, int arg4, int arg5, int arg6, int arg7,
	...
	case CG_DRAW_ACTIVE_FRAME: CG_DrawActiveFrame(arg0, arg1, arg2); return 0;
...
void CG_UpdateCvars(void) {
	...
	for (i = 0, cv = cvarTable; i < cvarTableSize; i++, cv++) { trap_Cvar_Update(cv->vmCvar); }
...
int CG_CrosshairPlayer(void) {
	if (cg.time > (cg.crosshairClientTime + 1000)) { return -1; }
	return cg.crosshairClientNum;
}

int CG_LastAttacker(void) {
	if (!cg.attackerTime) { return -1; }
...
static void CG_RegisterItemSounds(int itemNum) {
	...
	if (item->pickup_sound) { trap_S_RegisterSound(item->pickup_sound, qfalse); }
	...
	while (*s) {
		...
		if (!strcmp(data + len - 3, "wav")) { trap_S_RegisterSound(data, qfalse); }
	}
}
...
static void CG_RegisterGraphics(void) {
	...
	for (i = 0; i < 11; i++) { cgs.media.numberShaders[i] = trap_R_RegisterShader(sb_nums[i]); }
	...
	for (i = 1; i < cgs.numInlineModels; i++) {
		...
		for (j = 0; j < 3; j++) { cgs.inlineModelMidpoints[i][j] = mins[j] + 0.5 * (maxs[j] - mins[j]); }
	}
...
qboolean CG_Asset_Parse(int handle) {
	...
	while (1) {
		...
		if (Q_stricmp(token.string, "cursor") == 0) {
			if (!PC_String_Parse(handle, &cgDC.Assets.cursorStr)) { return qfalse; }
			cgDC.Assets.cursor = trap_R_RegisterShaderNoMip(cgDC.Assets.cursorStr);
			continue;
		}

		if (Q_stricmp(token.string, "fadeClamp") == 0) {
			if (!PC_Float_Parse(handle, &cgDC.Assets.fadeClamp)) { return qfalse; }
			continue;
		}

		if (Q_stricmp(token.string, "fadeCycle") == 0) {
			if (!PC_Int_Parse(handle, &cgDC.Assets.fadeCycle)) { return qfalse; }
			continue;
		}

		if (Q_stricmp(token.string, "fadeAmount") == 0) {
			if (!PC_Float_Parse(handle, &cgDC.Assets.fadeAmount)) { return qfalse; }
			continue;
		}

		if (Q_stricmp(token.string, "shadowX") == 0) {
			if (!PC_Float_Parse(handle, &cgDC.Assets.shadowX)) { return qfalse; }
			continue;
		}

		if (Q_stricmp(token.string, "shadowY") == 0) {
			if (!PC_Float_Parse(handle, &cgDC.Assets.shadowY)) { return qfalse; }
			continue;
		}

		if (Q_stricmp(token.string, "shadowColor") == 0) {
			if (!PC_Color_Parse(handle, &cgDC.Assets.shadowColor)) { return qfalse; }
...
void CG_ParseMenu(const char *menuFile) {
	...
	if (!handle) handle = trap_PC_LoadSource("ui/testhud.menu");
	if (!handle) return;
...
void CG_LoadMenus(const char *menuFile) {
	...
	if (!f) {
		...
		if (!f) { CG_Error(S_COLOR_RED "default menu file not found: ui/hud.txt, unable to continue!"); }
	}
...
static qboolean CG_OwnerDrawHandleKey(int ownerDraw, int flags, float *special, int key) { return qfalse; }
...
void CG_SetScoreSelection(void *p) {
	...
	for (i = 0; i < cg.numScores; i++) {
		...
		if (ps->clientNum == cg.scores[i].client) { cg.selectedScore = i; }
	}
	...
	*handle = -1;
	...
	if (info && info->infoValid) {
		...
		case 1:
			...
			break;
		case 2:
			if (cg.snap->ps.stats[STAT_CLIENTS_READY] & (1 << sp->client)) { return "Ready"; }
			if (team == -1) {
				...
				if (info->teamLeader) { return "Leader"; }
			}
			break;
		case 3: return info->name; break;
		case 4: return va("%i", info->score); break;
		case 5: return va("%4i", sp->time); break;
		case 6:
...
static void CG_FeederSelection(float feederID, int index) {
	if (cgs.gametype >= GT_TEAM) {
		...
		for (i = 0; i < cg.numScores; i++) {
			if (cg.scores[i].team == team) {
				if (index == count) { cg.selectedScore = i; }
				count++;
			}
		}
	} else {
...
static int CG_OwnerDrawWidth(int ownerDraw, float scale) {
	switch (ownerDraw) {
	case CG_GAME_TYPE: return CG_Text_Width(CG_GameTypeString(), scale, 0);
	case CG_GAME_STATUS: return CG_Text_Width(CG_GetGameStatusText(), scale, 0); break;
	case CG_KILLER: return CG_Text_Width(CG_GetKillerText(), scale, 0); break;
	case CG_RED_NAME: return CG_Text_Width(cg_redTeamName.string, scale, 0); break;
	case CG_BLUE_NAME: return CG_Text_Width(cg_blueTeamName.string, scale, 0); break;
	}
	return 0;
}
...
void CG_LoadHudMenu(void) {
	...
	hudSet = buff;
	if (hudSet[0] == '\0') { hudSet = "ui/hud.txt"; }
...
void CG_Init(int serverMessageNum, int serverCommandSequence, int clientNum) {
	...
	if (strcmp(s, GAME_VERSION)) { CG_Error("Client/Server game mismatch: %s/%s", GAME_VERSION, s); }
