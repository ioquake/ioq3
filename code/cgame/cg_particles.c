...
void CG_AddParticleToScene(cparticle_t *p, vec3_t org, float alpha) {
	...
	if (p->type == P_WEATHER || p->type == P_WEATHER_TURBULENT || p->type == P_WEATHER_FLURRY || p->type == P_BUBBLE ||
		...
		if (p->type != P_WEATHER_FLURRY) {
			if (p->type == P_BUBBLE || p->type == P_BUBBLE_TURBULENT) {
				...
				if (org[2] < p->end) {
					...
					while (p->org[2] < p->end) { p->org[2] += (p->start - p->end); }
		...
		if (cgs.glconfig.hardwareType == GLHW_RAGEPRO) alpha = 1;
	...
	// Ridah
		...
		time2 = p->endtime - p->time;
		ratio = time / time2;
		if (ratio >= 1.0f) { ratio = 0.9999f; }

		width  = p->width + (ratio * (p->endwidth - p->width));
		height = p->height + (ratio * (p->endheight - p->height));

		// if we are "inside" this sprite, don't draw
		if (Distance(cg.snap->ps.origin, org) < width / 1.5) { return; }
...
void CG_ParticleExplosion(char *animStr, vec3_t origin, vec3_t vel, int duration, int sizeStart, int sizeEnd) {
	...
	// Add randomized acceleration to create an explosion effect
	p->accel[0] = crandom() * 500;
	p->accel[1] = crandom() * 500;
	p->accel[2] = crandom() * 500;
}
...
void CG_ParticleImpactSmokePuff(qhandle_t pshader, vec3_t origin) {
	...
	for (i = 0; i < numParticles; i++) {
		...
		p->accel[0] = (rand() % 20) - 10;
		p->accel[1] = (rand() % 20) - 10;
		p->accel[2] = -(rand() % 20) + 100; // + 100 gravity
		...
		// Fade out over time
		p->endtime	 = cg.time + (rand() % 1000) + 1000;
		p->startfade = cg.time + (rand() % 500) + 500;
	}
}
...
void CG_Particle_Bleed(qhandle_t pshader, vec3_t start, vec3_t dir, int fleshEntityNum, int duration) {
	...
	VectorScale(dir, -100, p->vel);
	VectorSet(p->accel, 0, 0, 500);
...
void CG_OilSlickRemove(centity_t *cent) {
	...
	if (!id) CG_Printf("CG_OilSlickRevove NULL id\n");
...
void CG_ParticleBloodCloud(centity_t *cent, vec3_t origin, vec3_t dir) {
	float		 length, dist, crittersize;
	vec3_t		 angles, forward, right, up, point;
	cparticle_t *p;
	int			 i;
	...
	if (length) dist = length / crittersize;
	...
	for (i = 0; i < dist; i++) {
		...
		// p->pshader = cgs.media.bloodMistShader; // Write shader for bloodmist
		p->pshader = cgs.media.smokePuffShader;

		p->endtime = cg.time + 750 + (crandom() * 250);
		...
		p->vel[0] = (crandom() * 50.0f) * right[0] + (crandom() * 50.0f) * up[0];
		p->vel[1] = (crandom() * 50.0f) * right[1] + (crandom() * 50.0f) * up[1];
		p->vel[2] = (crandom() * 50.0f) * right[2] + (crandom() * 50.0f) * up[2] + 800;

		// VectorCopy(cg.gravity, p->accel);
