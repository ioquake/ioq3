...
static void CG_Item(centity_t *cent) {
	...
	if (es->modelindex >= bg_numItems) { CG_Error("Bad item index %i on entity", es->modelindex); }
	...
	if ((item->giType == IT_WEAPON) || (item->giType == IT_ARMOR)) { ent.renderfx |= RF_MINLIGHT; }
...
static void CG_Missile(centity_t *cent) {
	...
	s1 = &cent->currentState;
	if (s1->weapon >= WP_NUM_WEAPONS) { s1->weapon = 0; }
...
static void CG_InterpolateEntityPosition(centity_t *cent) {
	...
	if (cg.nextSnap == NULL) { CG_Error("CG_InterpoateEntityPosition: cg.nextSnap == NULL"); }
...
static void CG_TeamBase(centity_t *cent) {
	...
	if (cgs.gametype == GT_CTF) {
		...
		if (cent->currentState.frame == 2) {
			if (!cent->miscTime) { cent->miscTime = cg.time; }
...
static void CG_AddCEntity(centity_t *cent) {
	...
	default: CG_Error("Bad entity type: %i", cent->currentState.eType); break;
	case ET_INVISIBLE:
