...
void CG_DrawInformation(void) {
	...
	if (!levelshot) { levelshot = trap_R_RegisterShaderNoMip("menu/art/unknownmap"); }
	...
	if (cgs.gametype < GT_CTF) {
		...
		if (value) {
			UI_DrawProportionalString(320, y, va("fraglimit %i", value), UI_CENTER | UI_SMALLFONT | UI_DROPSHADOW,
									  colorWhite);
			y += PROP_HEIGHT;
		}
	}
	...
	if (cgs.gametype >= GT_CTF) {
		...
		if (value) {
			UI_DrawProportionalString(320, y, va("capturelimit %i", value), UI_CENTER | UI_SMALLFONT | UI_DROPSHADOW,
									  colorWhite);
		}
	}
}
