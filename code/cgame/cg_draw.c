		...
		while (s && *s && count < len) {
			...
			if (Q_IsColorString(s)) {
				...
				if (style == ITEM_TEXTSTYLE_SHADOWED || style == ITEM_TEXTSTYLE_SHADOWEDMORE) {
					...
					CG_Text_PaintChar(x + ofs, y - yadj + ofs, glyph->imageWidth, glyph->imageHeight, useScale,
				...
				// CG_DrawPic(x, y - yadj, scale * cgDC.Assets.textFont.glyphs[text[i]].imageWidth, scale *
				// cgDC.Assets.textFont.glyphs[text[i]].imageHeight, cgDC.Assets.textFont.glyphs[text[i]].glyph);
...
static void CG_DrawField(int x, int y, int width, int value) {
	...
	while (*ptr && l) {
		...
		CG_DrawPic(x, y, CG_CHAR_WIDTH, CG_CHAR_HEIGHT, cgs.media.numberShaders[frame]);
		x += CG_CHAR_WIDTH;
		ptr++;
		l--;
	}
...
void CG_DrawFlagModel(float x, float y, float w, float h, int team, qboolean force2D) {
	...
	if (!force2D && cg_draw3dIcons.integer) {
		...
		if (item) { CG_DrawPic(x, y, w, h, cg_items[ITEM_INDEX(item)].icon); }
	}
}
...
static void CG_DrawStatusBar(void) {
	...
	if (cent->currentState.weapon && cg_weapons[cent->currentState.weapon].ammoModel) {
		...
		CG_Draw3DModel(CG_CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE,
	...
	if (ps->stats[STAT_ARMOR]) {
		...
		CG_Draw3DModel(370 + CG_CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE, cgs.media.armorModel, 0,
					   origin, angles);
	}
	//
	// ammo
	...
	if (cent->currentState.weapon) {
		...
		if (value > -1) {
			...
			if (!cg_draw3dIcons.integer && cg_drawIcons.integer) {
				...
				if (icon) { CG_DrawPic(CG_CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE, icon); }
			}
		}
	}
	...
	if (value > 0) {
		...
		if (!cg_draw3dIcons.integer && cg_drawIcons.integer) {
			CG_DrawPic(370 + CG_CHAR_WIDTH * 3 + TEXT_ICON_SPACE, 432, ICON_SIZE, ICON_SIZE, cgs.media.armorIcon);
		}
	}
}
#endif
...
static float CG_DrawFPS(float y) {
	...
	if (index > FPS_FRAMES) {
		...
		total = 0;
		for (i = 0; i < FPS_FRAMES; i++) { total += previousTimes[i]; }
		if (!total) { total = 1; }
...
static float CG_DrawTeamOverlay(float y, qboolean right, qboolean upper) {
	...
	if (pwidth > TEAM_OVERLAY_MAXNAME_WIDTH) pwidth = TEAM_OVERLAY_MAXNAME_WIDTH;
...
static void CG_DrawUpperRight(stereoFrame_t stereoFrame) {
	...
	if (cgs.gametype >= GT_TEAM && cg_drawTeamOverlay.integer == 1) { y = CG_DrawTeamOverlay(y, qtrue, qtrue); }
	if (cg_drawSnapshot.integer) { y = CG_DrawSnapshot(y); }
	if (cg_drawFPS.integer && (stereoFrame == STEREO_CENTER || stereoFrame == STEREO_RIGHT)) { y = CG_DrawFPS(y); }
	if (cg_drawTimer.integer) { y = CG_DrawTimer(y); }
	if (cg_drawAttacker.integer) { CG_DrawAttacker(y); }
}
...
static void CG_DrawLowerRight(void) {
	...
	if (cgs.gametype >= GT_TEAM && cg_drawTeamOverlay.integer == 2) { y = CG_DrawTeamOverlay(y, qtrue, qfalse); }
...
static void CG_DrawLowerLeft(void) {
	...
	if (cgs.gametype >= GT_TEAM && cg_drawTeamOverlay.integer == 3) { y = CG_DrawTeamOverlay(y, qfalse, qfalse); }
...
static void CG_DrawLagometer(void) {
	...
	if (cg_nopredict.integer || cg_synchronousClients.integer) { CG_DrawBigString(x, y, "snc", 1.0); }
...
static qboolean CG_DrawScoreboard(void) {
	...
	if (++cg.deferredPlayerLoading > 10) { CG_LoadDeferredPlayers(); }
...
static void CG_DrawWarmup(void) {
	...
	if (sec != cg.warmupCount) {
		cg.warmupCount = sec;
		switch (sec) {
		case 0: trap_S_StartLocalSound(cgs.media.count1Sound, CHAN_ANNOUNCER); break;
		case 1: trap_S_StartLocalSound(cgs.media.count2Sound, CHAN_ANNOUNCER); break;
		case 2: trap_S_StartLocalSound(cgs.media.count3Sound, CHAN_ANNOUNCER); break;
		default: break;
		}
	}
	...
	case 0: scale = 0.54f; break;
	case 1: scale = 0.51f; break;
	case 2: scale = 0.48f; break;
	default: scale = 0.45f; break;
	}
...
static void CG_Draw2D(stereoFrame_t stereoFrame) {
#ifdef MISSIONPACK
	if (cgs.orderPending && cg.time > cgs.orderTime) { CG_CheckOrderPending(); }
#endif
	...
	*/
...
#ifdef MISSIONPACK
	if (!cg_paused.integer) { CG_DrawUpperRight(stereoFrame); }
#else
	...
	if (!CG_DrawFollow()) { CG_DrawWarmup(); }
