...
static void CG_Obituary(entityState_t *ent) {
	...
	if (attacker == target) {
		...
		switch (mod) {
#ifdef MISSIONPACK
		case MOD_KAMIKAZE: message = "goes out with a bang"; break;
#endif
	...
	if (attacker != ENTITYNUM_WORLD) {
		...
		case MOD_BFG_SPLASH:
			...
			break;
#ifdef MISSIONPACK
		case MOD_NAIL: message = "was nailed by"; break;
		case MOD_CHAINGUN:
		...
		case MOD_KAMIKAZE:
			...
			break;
		case MOD_JUICED: message = "was juiced by"; break;
#endif
		case MOD_TELEFRAG:
...
void CG_PainEvent(centity_t *cent, int health) {
	...
	if (cg.time - cent->pe.painTime < 500) { return; }
...
*/
#define DEBUGNAME(x)                                                                                                   \
	if (cg_debugEvents.integer) { CG_Printf(x "\n"); }
void CG_EntityEvent(centity_t *cent, vec3_t position) {
	...
	case EV_STEP_16: // smooth out step up transitions
		DEBUGNAME("EV_STEP");
		{
			...
			if (cg.stepChange > MAX_STEP_CHANGE) { cg.stepChange = MAX_STEP_CHANGE; }
			cg.stepTime = cg.time;
			break;
		}
...
#ifdef MISSIONPACK
				switch (item->giTag) {
				case PW_SCOUT: trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.scoutSound); break;
				case PW_GUARD: trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.guardSound); break;
				case PW_DOUBLER: trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.doublerSound); break;
				case PW_AMMOREGEN: trap_S_StartSound(NULL, es->number, CHAN_AUTO, cgs.media.ammoregenSound); break;
				}
#endif
			} else {
	...
	case EV_NOAMMO:
		...
		if (es->number == cg.snap->ps.clientNum) { CG_OutOfAmmoChange(); }
		break;
	...
	case EV_PAIN:
		...
		DEBUGNAME("EV_PAIN");
		if (cent->currentState.number != cg.snap->ps.clientNum) { CG_PainEvent(cent, es->eventParm); }
		break;
	...
	case EV_GIB_PLAYER:
		...
		if (!(es->eFlags & EF_KAMIKAZE)) { trap_S_StartSound(NULL, es->number, CHAN_BODY, cgs.media.gibSound); }
