...
static void UI_PositionEntityOnTag(refEntity_t *entity, const refEntity_t *parent, clipHandle_t parentModel,
	...
	// lerp the tag
	trap_CM_LerpTag(&lerped, parentModel, parent->oldframe, parent->frame, 1.0f - parent->backlerp, tagName);
...
static void UI_PlayerAngles(playerInfo_t *pi, vec3_t legs[3], vec3_t torso[3], vec3_t head[3]) {
	...
	/*//NOTE(raute): it seems that made trouble with turning animated meshes in ui_playersettings.c ... probably there
	   is a lot things like this in here (so things that we don't need for wop, but which are needed in vq3) if ( (
...
void UI_DrawPlayer(float x, float y, float w, float h, playerInfo_t *pi, int time) {
	...
	//
	if (pi->chat) {
		UI_PlayerFloatSprite(pi, origin, trap_R_RegisterShaderNoMip("icons/hint_textchat"));
	}
