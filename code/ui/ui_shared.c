...
const char *String_Alloc(const char *p) {
	...
	if (*p == 0) { return staticNULL; }

	hash = hashForString(p);

	str = strHandle[hash];
	while (str) {
		if (strcmp(p, str->str) == 0) { return str->str; }
		str = str->next;
	}
...
void String_Init(void) {
	int i;
	for (i = 0; i < HASH_TABLE_SIZE; i++) { strHandle[i] = NULL; }
	strHandleCount = 0;
	strPoolIndex   = 0;
	menuCount	   = 0;
	openMenuCount  = 0;
	...
	if (DC && DC->getBindingBuf) { Controls_GetConfig(); }
}
...
qboolean Color_Parse(char **p, vec4_t *c) {
	...
	for (i = 0; i < 4; i++) {
		if (!Float_Parse(p, &f)) { return qfalse; }
		(*c)[i] = f;
	}
	return qtrue;
}
...
qboolean PC_Color_Parse(int handle, vec4_t *c) {
	...
	for (i = 0; i < 4; i++) {
		if (!PC_Float_Parse(handle, &f)) { return qfalse; }
		(*c)[i] = f;
	}
	return qtrue;
}
	...
	if (Float_Parse(p, &r->x)) {
		if (Float_Parse(p, &r->y)) {
			if (Float_Parse(p, &r->w)) {
				if (Float_Parse(p, &r->h)) { return qtrue; }
			}
		}
	}
	return qfalse;
	...
	if (PC_Float_Parse(handle, &r->x)) {
		if (PC_Float_Parse(handle, &r->y)) {
			if (PC_Float_Parse(handle, &r->w)) {
				if (PC_Float_Parse(handle, &r->h)) { return qtrue; }
			}
		}
	}
	return qfalse;
		...
		if (DC->realTime > *nextTime) {
			*nextTime = DC->realTime + offsetTime;
			if (*flags & WINDOW_FADINGOUT) {
				*f -= fadeAmount;
				if (bFlags && *f <= 0.0) { *flags &= ~(WINDOW_FADINGOUT | WINDOW_VISIBLE); }
			} else {
				*f += fadeAmount;
				if (*f >= clamp) {
					*f = clamp;
					if (bFlags) { *flags &= ~WINDOW_FADINGIN; }
				}
			}
		}
	}
...
void Window_Paint(Window *w, float fadeAmount, float fadeClamp, float fadeCycle) {
	...
	if (w->style == 0 && w->border == 0) { return; }
	...
	if (w->style == WINDOW_STYLE_FILLED) {
		...
		if (w->flags & WINDOW_FORECOLORSET) { DC->setColor(w->foreColor); }
		DC->drawHandlePic(fillRect.x, fillRect.y, fillRect.w, fillRect.h, w->background);
		DC->setColor(NULL);
		...
		if (w->cinematic == -1) {
			...
			if (w->cinematic == -1) { w->cinematic = -2; }
		}
		if (w->cinematic >= 0) {
...
void Menu_UpdatePosition(menuDef_t *menu) {
	...
	for (i = 0; i < menu->itemCount; i++) { Item_SetScreenCoords(menu->items[i], x, y); }
}
...
itemDef_t *Menu_ClearFocus(menuDef_t *menu) {
	...
	for (i = 0; i < menu->itemCount; i++) {
		if (menu->items[i]->window.flags & WINDOW_HASFOCUS) { ret = menu->items[i]; }
		menu->items[i]->window.flags &= ~WINDOW_HASFOCUS;
		if (menu->items[i]->leaveFocus) { Item_RunScript(menu->items[i], menu->items[i]->leaveFocus); }
	}
...
qboolean Rect_ContainsPoint(rectDef_t *rect, float x, float y) {
	if (rect) {
		if (x > rect->x && x < rect->x + rect->w && y > rect->y && y < rect->y + rect->h) { return qtrue; }
	}
	return qfalse;
}
...
itemDef_t *Menu_GetMatchingItemByNumber(menuDef_t *menu, int index, const char *name) {
	...
	for (i = 0; i < menu->itemCount; i++) {
		if (Q_stricmp(menu->items[i]->window.name, name) == 0 ||
			(menu->items[i]->window.group && Q_stricmp(menu->items[i]->window.group, name) == 0)) {
			if (count == index) { return menu->items[i]; }
			count++;
		}
	}
	return NULL;
...
void Script_SetBackground(itemDef_t *item, char **args) {
	const char *name;
	// expecting name to set asset to
	if (String_Parse(args, &name)) { item->window.background = DC->registerShaderNoMip(name); }
}

itemDef_t *Menu_FindItemByName(menuDef_t *menu, const char *p) {
	int i;
	if (menu == NULL || p == NULL) { return NULL; }

	for (i = 0; i < menu->itemCount; i++) {
		if (Q_stricmp(p, menu->items[i]->window.name) == 0) { return menu->items[i]; }
	}
...
void Script_SetTeamColor(itemDef_t *item, char **args) {
	if (DC->getTeamColor) {
		...
		vec4_t color;
		DC->getTeamColor(&color);
		for (i = 0; i < 4; i++) { item->window.backColor[i] = color[i]; }
	}
}
...
void Script_SetItemColor(itemDef_t *item, char **args) {
	...
	if (String_Parse(args, &itemname) && String_Parse(args, &name)) {
		...
		if (!Color_Parse(args, &color)) { return; }
...
menuDef_t *Menus_FindByName(const char *p) {
	...
	for (i = 0; i < menuCount; i++) {
		if (Q_stricmp(Menus[i].window.name, p) == 0) { return &Menus[i]; }
	}
	return NULL;
}
...
void Script_Show(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { Menu_ShowItemByName(item->parent, name, qtrue); }
}

void Script_Hide(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { Menu_ShowItemByName(item->parent, name, qfalse); }
}

void Script_FadeIn(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { Menu_FadeItemByName(item->parent, name, qfalse); }
}

void Script_FadeOut(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { Menu_FadeItemByName(item->parent, name, qtrue); }
}

void Script_Open(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { Menus_OpenByName(name); }
}
...
void Script_Close(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { Menus_CloseByName(name); }
}
...
void Script_SetFocus(itemDef_t *item, char **args) {
	...
	if (String_Parse(args, &name)) {
		...
		if (focusItem && !(focusItem->window.flags & WINDOW_DECORATION) &&
			...
			focusItem->window.flags |= WINDOW_HASFOCUS;
			if (focusItem->onFocus) { Item_RunScript(focusItem, focusItem->onFocus); }
			if (DC->Assets.itemFocusSound) { DC->startLocalSound(DC->Assets.itemFocusSound, CHAN_LOCAL_SOUND); }
		}
	}
}
...
void Script_SetPlayerModel(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { DC->setCVar("team_model", name); }
}

void Script_SetPlayerHead(itemDef_t *item, char **args) {
	const char *name;
	if (String_Parse(args, &name)) { DC->setCVar("team_headmodel", name); }
}

void Script_SetCvar(itemDef_t *item, char **args) {
	const char *cvar, *val;
	if (String_Parse(args, &cvar) && String_Parse(args, &val)) { DC->setCVar(cvar, val); }
}

void Script_Exec(itemDef_t *item, char **args) {
	const char *val;
	if (String_Parse(args, &val)) { DC->executeText(EXEC_APPEND, va("%s ; ", val)); }
}

void Script_Play(itemDef_t *item, char **args) {
	const char *val;
	if (String_Parse(args, &val)) { DC->startLocalSound(DC->registerSound(val, qfalse), CHAN_LOCAL_SOUND); }
}
...
void Item_RunScript(itemDef_t *item, const char *s) {
	...
	if (item && s && s[0]) {
		...
		while (1) {
			...
			if (!String_Parse(&p, &command)) { return; }

			if (command[0] == ';' && command[1] == '\0') { continue; }
...
qboolean Item_EnableShowViaCvar(itemDef_t *item, int flag) {
	...
	if (item && item->enableCvar && *item->enableCvar && item->cvarTest && *item->cvarTest) {
		...
		while (1) {
			...
			if (!String_Parse(&p, &val)) { return (item->cvarFlags & flag) ? qfalse : qtrue; }

			if (val[0] == ';' && val[1] == '\0') { continue; }

			// enable it if any of the values are true
			if (item->cvarFlags & flag) {
				if (Q_stricmp(buff, val) == 0) { return qtrue; }
			} else {
				// disable it if any of the values are true
				if (Q_stricmp(buff, val) == 0) { return qfalse; }
			}
		}
...
qboolean Item_SetFocus(itemDef_t *item, float x, float y) {
	...
	if (item->cvarFlags & (CVAR_ENABLE | CVAR_DISABLE) && !Item_EnableShowViaCvar(item, CVAR_ENABLE)) { return qfalse; }

	if (item->cvarFlags & (CVAR_SHOW | CVAR_HIDE) && !Item_EnableShowViaCvar(item, CVAR_SHOW)) { return qfalse; }
	...
	if (item->type == ITEM_TYPE_TEXT) {
		...
		if (Rect_ContainsPoint(&r, x, y)) {
			...
			if (item->focusSound) { sfx = &item->focusSound; }
			playSound = qtrue;
		} else {
			if (oldFocus) {
				oldFocus->window.flags |= WINDOW_HASFOCUS;
				if (oldFocus->onFocus) { Item_RunScript(oldFocus, oldFocus->onFocus); }
			}
		}
	} else {
		item->window.flags |= WINDOW_HASFOCUS;
		if (item->onFocus) { Item_RunScript(item, item->onFocus); }
		if (item->focusSound) { sfx = &item->focusSound; }
		playSound = qtrue;
	}
...
int Item_Slider_OverSlider(itemDef_t *item, float x, float y) {
	...
	if (Rect_ContainsPoint(&r, x, y)) { return WINDOW_LB_THUMB; }
	return 0;
}
...
int Item_ListBox_OverLB(itemDef_t *item, float x, float y) {
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		...
		if (Rect_ContainsPoint(&r, x, y)) { return WINDOW_LB_LEFTARROW; }
		// check if on right arrow
		r.x = item->window.rect.x + item->window.rect.w - SCROLLBAR_SIZE;
		if (Rect_ContainsPoint(&r, x, y)) { return WINDOW_LB_RIGHTARROW; }
		// check if on thumb
		...
		r.w = thumbstart - r.x;
		if (Rect_ContainsPoint(&r, x, y)) { return WINDOW_LB_PGUP; }
		r.x = thumbstart + SCROLLBAR_SIZE;
		r.w = item->window.rect.x + item->window.rect.w - SCROLLBAR_SIZE;
		if (Rect_ContainsPoint(&r, x, y)) { return WINDOW_LB_PGDN; }
	} else {
		r.x = item->window.rect.x + item->window.rect.w - SCROLLBAR_SIZE;
		...
		if (!(item->window.flags &
			...
			if (listPtr->elementStyle == LISTBOX_IMAGE) {
				...
				if (Rect_ContainsPoint(&r, x, y)) {
					...
					if (listPtr->cursorPos >= listPtr->endPos) { listPtr->cursorPos = listPtr->endPos; }
				}
			} else {
				// text hit..
			}
...
void Item_MouseEnter(itemDef_t *item, float x, float y) {
	...
	if (item) {
		...
		// items can be enabled and disabled based on cvars
		if (item->cvarFlags & (CVAR_ENABLE | CVAR_DISABLE) && !Item_EnableShowViaCvar(item, CVAR_ENABLE)) { return; }

		if (item->cvarFlags & (CVAR_SHOW | CVAR_HIDE) && !Item_EnableShowViaCvar(item, CVAR_SHOW)) { return; }
		...
		if (Rect_ContainsPoint(&r, x, y)) {
			...
			if (item->type == ITEM_TYPE_LISTBOX) { Item_ListBox_MouseEnter(item, x, y); }
		}
	}
}
...
itemDef_t *Menu_HitTest(menuDef_t *menu, float x, float y) {
	int i;
	for (i = 0; i < menu->itemCount; i++) {
		if (Rect_ContainsPoint(&menu->items[i]->window.rect, x, y)) { return menu->items[i]; }
	}
	return NULL;
}
		...
		if (item->window.flags & WINDOW_HORIZONTAL) {
			...
			if (key == K_LEFTARROW || key == K_KP_LEFTARROW) {
				if (!listPtr->notselectable) {
					listPtr->cursorPos--;
					if (listPtr->cursorPos < 0) { listPtr->cursorPos = 0; }
					if (listPtr->cursorPos < listPtr->startPos) { listPtr->startPos = listPtr->cursorPos; }
			...
			if (key == K_RIGHTARROW || key == K_KP_RIGHTARROW) {
				if (!listPtr->notselectable) {
					listPtr->cursorPos++;
					if (listPtr->cursorPos < listPtr->startPos) { listPtr->startPos = listPtr->cursorPos; }
					if (listPtr->cursorPos >= count) { listPtr->cursorPos = count - 1; }
					if (listPtr->cursorPos >= listPtr->startPos + viewmax) {
						listPtr->startPos = listPtr->cursorPos - viewmax + 1;
					}
					item->cursorPos = listPtr->cursorPos;
					...
					listPtr->startPos++;
					if (listPtr->startPos >= count) listPtr->startPos = count - 1;
				}
				return qtrue;
			}
		} else {
...
int Item_Multi_FindCvarByValue(itemDef_t *item) {
	...
	if (multiPtr) {
		...
		for (i = 0; i < multiPtr->count; i++) {
			if (multiPtr->strDef) {
				if (Q_stricmp(buff, multiPtr->cvarStr[i]) == 0) { return i; }
			} else {
				if (multiPtr->cvarValue[i] == value) { return i; }
			}
		}
	}
	return 0;
...
const char *Item_Multi_Setting(itemDef_t *item) {
	...
	if (multiPtr) {
		...
		for (i = 0; i < multiPtr->count; i++) {
			if (multiPtr->strDef) {
				if (Q_stricmp(buff, multiPtr->cvarStr[i]) == 0) { return multiPtr->cvarList[i]; }
			} else {
				if (multiPtr->cvarValue[i] == value) { return multiPtr->cvarList[i]; }
			}
		}
	}
	return "";
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key) {
	...
	if (item->cvar) {
		...
		DC->getCVarString(item->cvar, buff, sizeof(buff));
		len = strlen(buff);
		if (editPtr->maxChars && len > editPtr->maxChars) { len = editPtr->maxChars; }
		if (key & K_CHAR_FLAG) {
			key &= ~K_CHAR_FLAG;
			...
			//
			if (key < 32 || !item->cvar) { return qtrue; }

			if (item->type == ITEM_TYPE_NUMERICFIELD) {
				if (key < '0' || key > '9') { return qfalse; }
			}

			if (!DC->getOverstrikeMode()) {
				if ((len == MAX_EDITFIELD - 1) || (editPtr->maxChars && len >= editPtr->maxChars)) { return qtrue; }
				memmove(&buff[item->cursorPos + 1], &buff[item->cursorPos], len + 1 - item->cursorPos);
			} else {
				if (editPtr->maxChars && item->cursorPos >= editPtr->maxChars) { return qtrue; }
			}
			...
			if (item->cursorPos < len + 1) {
				item->cursorPos++;
				if (editPtr->maxPaintChars && item->cursorPos > editPtr->maxPaintChars) { editPtr->paintOffset++; }
			}
			...
			if (key == K_RIGHTARROW || key == K_KP_RIGHTARROW) {
				...
				if (item->cursorPos < len) { item->cursorPos++; }
				return qtrue;
			}
			...
			if (key == K_END || key == K_KP_END) { // ( tolower(key) == 'e' && trap_Key_IsDown( K_CTRL ) ) ) {
				item->cursorPos = len;
				if (item->cursorPos > editPtr->maxPaintChars) { editPtr->paintOffset = len - editPtr->maxPaintChars; }
				return qtrue;
			}
		...
		if (key == K_ENTER || key == K_KP_ENTER || key == K_ESCAPE) { return qfalse; }
...
static void Scroll_ListBox_AutoFunc(void *p) {
	...
	if (DC->realTime > si->nextAdjustTime) {
		...
		if (si->adjustValue > SCROLL_TIME_FLOOR) { si->adjustValue -= SCROLL_TIME_ADJUSTOFFSET; }
	}
}
...
static void Scroll_ListBox_ThumbFunc(void *p) {
	...
	if (si->item->window.flags & WINDOW_HORIZONTAL) {
		if (DC->cursorx == si->xStart) { return; }
...
qboolean Item_Slider_HandleKey(itemDef_t *item, int key, qboolean down) {
	...
	if (item->cvar) {
		if (key == K_MOUSE1 || key == K_MOUSE2 || key == K_MOUSE3) {
			editFieldDef_t *editDef = item->typeData;
			if (editDef && Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) &&
				item->window.flags & WINDOW_HASFOCUS) {
				rectDef_t testRect;
				width = SLIDER_WIDTH;
				if (item->text) {
...
qboolean Item_HandleKey(itemDef_t *item, int key, qboolean down) {
	...
	if (itemCapture) {
		...
		if (down && (key == K_MOUSE1 || key == K_MOUSE2 || key == K_MOUSE3)) { Item_StartCapture(item, key); }
	}

	if (!down) { return qfalse; }

	switch (item->type) {
	case ITEM_TYPE_BUTTON: return qfalse; break;
	case ITEM_TYPE_RADIOBUTTON: return qfalse; break;
	case ITEM_TYPE_CHECKBOX: return qfalse; break;
	case ITEM_TYPE_EDITFIELD:
	case ITEM_TYPE_NUMERICFIELD:
		// return Item_TextField_HandleKey(item, key);
		return qfalse;
		break;
	case ITEM_TYPE_COMBO: return qfalse; break;
	case ITEM_TYPE_LISTBOX: return Item_ListBox_HandleKey(item, key, down, qfalse); break;
	case ITEM_TYPE_YESNO: return Item_YesNo_HandleKey(item, key); break;
	case ITEM_TYPE_MULTI: return Item_Multi_HandleKey(item, key); break;
	case ITEM_TYPE_OWNERDRAW: return Item_OwnerDraw_HandleKey(item, key); break;
	case ITEM_TYPE_BIND: return Item_Bind_HandleKey(item, key, down); break;
	case ITEM_TYPE_SLIDER: return Item_Slider_HandleKey(item, key, down); break;
	// case ITEM_TYPE_IMAGE:
	//   Item_Image_Paint(item);
	//   break;
	default: return qfalse; break;
	}
...
void Item_Action(itemDef_t *item) {
	if (item) { Item_RunScript(item, item->action); }
}
...
static void Display_CloseCinematics(void) {
	int i;
	for (i = 0; i < menuCount; i++) { Menu_CloseCinematics(&Menus[i]); }
}
...
int Display_VisibleMenuCount(void) {
	...
	count = 0;
	for (i = 0; i < menuCount; i++) {
		if (Menus[i].window.flags & (WINDOW_FORCED | WINDOW_VISIBLE)) { count++; }
	}
	return count;
}
...
int UI_SelectForKey(int key) {
	...
	case K_JOY2:
	case K_JOY3:
	case K_JOY4: return 1; // next

	case K_MOUSE2:
	case K_LEFTARROW:
	case K_KP_LEFTARROW: return -1; // previous
	}
...
void Menu_HandleKey(menuDef_t *menu, int key, qboolean down) {
	...
	for (i = 0; i < menu->itemCount; i++) {
		if (menu->items[i]->window.flags & WINDOW_HASFOCUS) { item = menu->items[i]; }
	}
	...
	case K_F11:
		if (DC->getCVarValue("developer")) { debugMode ^= 1; }
		break;
	...
	case K_F12:
		...
		break;
	case K_KP_UPARROW:
	case K_UPARROW: Menu_SetPrevCursorItem(menu); break;
	...
	case K_TAB:
	case K_KP_DOWNARROW:
	case K_DOWNARROW: Menu_SetNextCursorItem(menu); break;

	case K_MOUSE1:
	case K_MOUSE2:
		if (item) {
			if (item->type == ITEM_TYPE_TEXT) {
				if (Rect_ContainsPoint(Item_CorrectedTextRect(item), DC->cursorx, DC->cursory)) { Item_Action(item); }
				...
				if (Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory)) { Item_Action(item); }
			}
		}
		break;
...
void Item_Text_Paint(itemDef_t *item) {
	...
	/*
		...
		if (item->textStyle == ITEM_TEXTSTYLE_SHADOWED || item->textStyle == ITEM_TEXTSTYLE_OUTLINESHADOWED) {
			Fade(&item->window.flags, &DC->Assets.shadowColor[3], DC->Assets.fadeClamp, &item->window.nextTime,
	   DC->Assets.fadeCycle, qfalse); DC->drawText(item->textRect.x + DC->Assets.shadowX, item->textRect.y +
	   DC->Assets.shadowY, item->textscale, DC->Assets.shadowColor, textPtr, adjust);
		}
	*/
...
void Item_TextField_Paint(itemDef_t *item) {
	...
	if (item->cvar) { DC->getCVarString(item->cvar, buff, sizeof(buff)); }
...
void Controls_SetConfig(qboolean restart) {
	...
	for (i = 0; i < g_bindCount; i++) {
		...
		if (g_bindings[i].bind1 != -1) {
			...
			if (g_bindings[i].bind2 != -1) DC->setBinding(g_bindings[i].bind2, g_bindings[i].command);
		}
	}
...
int BindingIDFromName(const char *name) {
	int i;
	for (i = 0; i < g_bindCount; i++) {
		if (Q_stricmp(name, g_bindings[i].command) == 0) { return i; }
	}
	return -1;
}
...
void Item_Bind_Paint(itemDef_t *item) {
	...
	menuDef_t	   *parent	 = (menuDef_t *)item->parent;
	editFieldDef_t *editPtr	 = (editFieldDef_t *)item->typeData;
	if (editPtr) { maxChars = editPtr->maxPaintChars; }
	...
	if (item->text) {
		...
		DC->drawText(item->textRect.x, item->textRect.y, item->textscale, newColor, "FIXME", 0, maxChars,
					 item->textStyle);
	}
}
...
qboolean Item_Bind_HandleKey(itemDef_t *item, int key, qboolean down) {
	...
	if (!g_waitingForKey) {
		if (down &&
			((key == K_MOUSE1 && Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory)) || key == K_ENTER ||
			 key == K_KP_ENTER || key == K_JOY1 || key == K_JOY2 || key == K_JOY3 || key == K_JOY4)) {
			g_waitingForKey = qtrue;
			g_bindItem		= item;
		}
		return qtrue;
	} else {
		if (g_bindItem == NULL) { return qtrue; }

		if (key & K_CHAR_FLAG) { return qtrue; }

		switch (key) {
		case K_ESCAPE: g_waitingForKey = qfalse; return qtrue;
	...
	if (key != -1) {
		...
		for (i = 0; i < g_bindCount; i++) {
			...
			if (g_bindings[i].bind2 == key) { g_bindings[i].bind2 = -1; }
...
void Item_ListBox_Paint(itemDef_t *item) {
	...
	if (item->window.flags & WINDOW_HORIZONTAL) {
		...
		if (thumb > x - SCROLLBAR_SIZE - 1) { thumb = x - SCROLLBAR_SIZE - 1; }
		...
		// thumb
		thumb = Item_ListBox_ThumbDrawPosition(item); // Item_ListBox_ThumbPosition(item);
		if (thumb > y - SCROLLBAR_SIZE - 1) { thumb = y - SCROLLBAR_SIZE - 1; }
				...
				if (listPtr->numColumns > 0) {
					...
					for (j = 0; j < listPtr->numColumns; j++) {
						...
						if (optionalImage >= 0) {
							...
							DC->drawText(x + 4 + listPtr->columnInfo[j].pos, y + listPtr->elementHeight,
...
void Item_Paint(itemDef_t *item) {
	...
	if (item->window.flags & WINDOW_INTRANSITION) {
		if (DC->realTime > item->window.nextTime) {
			...
			if (done == 4) { item->window.flags &= ~WINDOW_INTRANSITION; }
		}
	}
	...
	if (item->cvarFlags & (CVAR_SHOW | CVAR_HIDE)) {
		if (!Item_EnableShowViaCvar(item, CVAR_SHOW)) { return; }
	}

	if (item->window.flags & WINDOW_TIMEDVISIBLE) {}

	if (!(item->window.flags & WINDOW_VISIBLE)) { return; }
	...
	switch (item->type) {
	case ITEM_TYPE_OWNERDRAW: Item_OwnerDraw_Paint(item); break;
	case ITEM_TYPE_TEXT:
	case ITEM_TYPE_BUTTON: Item_Text_Paint(item); break;
	case ITEM_TYPE_RADIOBUTTON: break;
	case ITEM_TYPE_CHECKBOX: break;
	case ITEM_TYPE_EDITFIELD:
	case ITEM_TYPE_NUMERICFIELD: Item_TextField_Paint(item); break;
	case ITEM_TYPE_COMBO: break;
	case ITEM_TYPE_LISTBOX: Item_ListBox_Paint(item); break;
	// case ITEM_TYPE_IMAGE:
	//   Item_Image_Paint(item);
	//   break;
	case ITEM_TYPE_MODEL: Item_Model_Paint(item); break;
	case ITEM_TYPE_YESNO: Item_YesNo_Paint(item); break;
	case ITEM_TYPE_MULTI: Item_Multi_Paint(item); break;
	case ITEM_TYPE_BIND: Item_Bind_Paint(item); break;
	case ITEM_TYPE_SLIDER: Item_Slider_Paint(item); break;
	default: break;
	}
}
...
itemDef_t *Menu_GetFocusedItem(menuDef_t *menu) {
	...
	if (menu) {
		for (i = 0; i < menu->itemCount; i++) {
			if (menu->items[i]->window.flags & WINDOW_HASFOCUS) { return menu->items[i]; }
		}
	}
	return NULL;
}
...
menuDef_t *Menu_GetFocused(void) {
	int i;
	for (i = 0; i < menuCount; i++) {
		if (Menus[i].window.flags & WINDOW_HASFOCUS && Menus[i].window.flags & WINDOW_VISIBLE) { return &Menus[i]; }
	}
	return NULL;
}
...
qboolean Menus_AnyFullScreenVisible(void) {
	int i;
	for (i = 0; i < menuCount; i++) {
		if (Menus[i].window.flags & WINDOW_VISIBLE && Menus[i].fullScreen) { return qtrue; }
	}
	return qfalse;
}
...
menuDef_t *Menus_ActivateByName(const char *p) {
	...
	for (i = 0; i < menuCount; i++) {
		if (Q_stricmp(Menus[i].window.name, p) == 0) {
			m = &Menus[i];
			Menus_Activate(m);
			if (openMenuCount < MAX_OPEN_MENUS && focus != NULL) { menuStack[openMenuCount++] = focus; }
		} else {
...
void Menu_HandleMouseMove(menuDef_t *menu, float x, float y) {
	...
	if (!(menu->window.flags & (WINDOW_VISIBLE | WINDOW_FORCED))) { return; }
	...
	if (g_waitingForKey || g_editingField) { return; }
	...
	for (pass = 0; pass < 2; pass++) {
		for (i = 0; i < menu->itemCount; i++) {
			...
			if (!(menu->items[i]->window.flags & (WINDOW_VISIBLE | WINDOW_FORCED))) { continue; }
			...
			if (Rect_ContainsPoint(&menu->items[i]->window.rect, x, y)) {
				if (pass == 1) {
					...
					if (overItem->type == ITEM_TYPE_TEXT && overItem->text) {
						if (!Rect_ContainsPoint(Item_CorrectedTextRect(overItem), x, y)) { continue; }
					}
					...
					if (IsVisible(overItem->window.flags)) {
						...
						if (!focusSet) { focusSet = Item_SetFocus(overItem, x, y); }
					}
				}
...
void Menu_Paint(menuDef_t *menu, qboolean forcePaint) {
	...
	if (!(menu->window.flags & WINDOW_VISIBLE) && !forcePaint) { return; }

	if (menu->window.ownerDrawFlags && DC->ownerDrawVisible && !DC->ownerDrawVisible(menu->window.ownerDrawFlags)) {
		return;
	}

	if (forcePaint) { menu->window.flags |= WINDOW_FORCED; }
	...
	if (menu->fullScreen) {
		...
		// UI_DrawHandlePic(menu->window.rect.x, menu->window.rect.y, menu->window.rect.w, menu->window.rect.h,
		// menu->backgroundShader);
	}

	// paint the background and or border
	Window_Paint(&menu->window, menu->fadeAmount, menu->fadeClamp, menu->fadeCycle);

	for (i = 0; i < menu->itemCount; i++) { Item_Paint(menu->items[i]); }
...
keywordHash_t *KeywordHash_Find(keywordHash_t *table[], char *keyword) {
	...
	for (key = table[hash]; key; key = key->next) {
		if (!Q_stricmp(key->keyword, keyword)) return key;
	}
	return NULL;
}
...
qboolean ItemParse_name(itemDef_t *item, int handle) {
	if (!PC_String_Parse(handle, &item->window.name)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_focusSound(itemDef_t *item, int handle) {
	const char *temp;
	if (!PC_String_Parse(handle, &temp)) { return qfalse; }
...
qboolean ItemParse_text(itemDef_t *item, int handle) {
	if (!PC_String_Parse(handle, &item->text)) { return qfalse; }
	return qtrue;
}

// group <string>
qboolean ItemParse_group(itemDef_t *item, int handle) {
	if (!PC_String_Parse(handle, &item->window.group)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_model_origin(itemDef_t *item, int handle) {
	...
	if (PC_Float_Parse(handle, &modelPtr->origin[0])) {
		if (PC_Float_Parse(handle, &modelPtr->origin[1])) {
			if (PC_Float_Parse(handle, &modelPtr->origin[2])) { return qtrue; }
		}
	}
	return qfalse;
}
...
qboolean ItemParse_model_fovx(itemDef_t *item, int handle) {
	...
	if (!PC_Float_Parse(handle, &modelPtr->fov_x)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_model_fovy(itemDef_t *item, int handle) {
	...
	if (!PC_Float_Parse(handle, &modelPtr->fov_y)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_model_rotation(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &modelPtr->rotationSpeed)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_model_angle(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &modelPtr->angle)) { return qfalse; }
	return qtrue;
}

// rect <rectangle>
qboolean ItemParse_rect(itemDef_t *item, int handle) {
	if (!PC_Rect_Parse(handle, &item->window.rectClient)) { return qfalse; }
	return qtrue;
}

// style <integer>
qboolean ItemParse_style(itemDef_t *item, int handle) {
	if (!PC_Int_Parse(handle, &item->window.style)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_notselectable(itemDef_t *item, int handle) {
	...
	if (item->type == ITEM_TYPE_LISTBOX && listPtr) { listPtr->notselectable = qtrue; }
	return qtrue;
}
...
qboolean ItemParse_elementwidth(itemDef_t *item, int handle) {
	...
	Item_ValidateTypeData(item);
	listPtr = (listBoxDef_t *)item->typeData;
	if (!PC_Float_Parse(handle, &listPtr->elementWidth)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_elementheight(itemDef_t *item, int handle) {
	...
	Item_ValidateTypeData(item);
	listPtr = (listBoxDef_t *)item->typeData;
	if (!PC_Float_Parse(handle, &listPtr->elementHeight)) { return qfalse; }
	return qtrue;
}

// feeder <float>
qboolean ItemParse_feeder(itemDef_t *item, int handle) {
	if (!PC_Float_Parse(handle, &item->special)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_elementtype(itemDef_t *item, int handle) {
	...
	Item_ValidateTypeData(item);
	if (!item->typeData) return qfalse;
	listPtr = (listBoxDef_t *)item->typeData;
	if (!PC_Int_Parse(handle, &listPtr->elementStyle)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_columns(itemDef_t *item, int handle) {
	...
	if (PC_Int_Parse(handle, &num)) {
		if (num > MAX_LB_COLUMNS) { num = MAX_LB_COLUMNS; }
...
qboolean ItemParse_border(itemDef_t *item, int handle) {
	if (!PC_Int_Parse(handle, &item->window.border)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_bordersize(itemDef_t *item, int handle) {
	if (!PC_Float_Parse(handle, &item->window.borderSize)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_visible(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &i)) { return qfalse; }
	if (i) { item->window.flags |= WINDOW_VISIBLE; }
	return qtrue;
}
...
qboolean ItemParse_align(itemDef_t *item, int handle) {
	if (!PC_Int_Parse(handle, &item->alignment)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_textalign(itemDef_t *item, int handle) {
	if (!PC_Int_Parse(handle, &item->textalignment)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_textalignx(itemDef_t *item, int handle) {
	if (!PC_Float_Parse(handle, &item->textalignx)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_textaligny(itemDef_t *item, int handle) {
	if (!PC_Float_Parse(handle, &item->textaligny)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_textscale(itemDef_t *item, int handle) {
	if (!PC_Float_Parse(handle, &item->textscale)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_textstyle(itemDef_t *item, int handle) {
	if (!PC_Int_Parse(handle, &item->textStyle)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_outlinecolor(itemDef_t *item, int handle) {
	if (!PC_Color_Parse(handle, &item->window.outlineColor)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_cinematic(itemDef_t *item, int handle) {
	if (!PC_String_Parse(handle, &item->window.cinematicName)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_doubleClick(itemDef_t *item, int handle) {
	...
	if (!PC_Script_Parse(handle, &listPtr->doubleClick)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_onFocus(itemDef_t *item, int handle) {
	if (!PC_Script_Parse(handle, &item->onFocus)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_leaveFocus(itemDef_t *item, int handle) {
	if (!PC_Script_Parse(handle, &item->leaveFocus)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_mouseEnter(itemDef_t *item, int handle) {
	if (!PC_Script_Parse(handle, &item->mouseEnter)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_mouseExit(itemDef_t *item, int handle) {
	if (!PC_Script_Parse(handle, &item->mouseExit)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_mouseEnterText(itemDef_t *item, int handle) {
	if (!PC_Script_Parse(handle, &item->mouseEnterText)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_mouseExitText(itemDef_t *item, int handle) {
	if (!PC_Script_Parse(handle, &item->mouseExitText)) { return qfalse; }
	return qtrue;
}

qboolean ItemParse_action(itemDef_t *item, int handle) {
	if (!PC_Script_Parse(handle, &item->action)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_cvarTest(itemDef_t *item, int handle) {
	if (!PC_String_Parse(handle, &item->cvarTest)) { return qfalse; }
	return qtrue;
}
...
qboolean ItemParse_maxChars(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &maxChars)) { return qfalse; }
	editPtr			  = (editFieldDef_t *)item->typeData;
...
qboolean ItemParse_cvarFloat(itemDef_t *item, int handle) {
	...
	if (!item->typeData) return qfalse;
	editPtr = (editFieldDef_t *)item->typeData;
	if (PC_String_Parse(handle, &item->cvar) && PC_Float_Parse(handle, &editPtr->defVal) &&
...
qboolean ItemParse_cvarStrList(itemDef_t *item, int handle) {
	...
	while (1) {
		...
		if (*token.string == '}') { return qtrue; }

		if (*token.string == ',' || *token.string == ';') { continue; }
		...
		if (pass == 0) {
			...
			pass							   = 0;
			multiPtr->count++;
			if (multiPtr->count >= MAX_MULTI_CVARS) { return qfalse; }
		}
	}
	return qfalse;
}
...
qboolean ItemParse_cvarFloatList(itemDef_t *item, int handle) {
	...
	while (1) {
		...
		multiPtr->cvarList[multiPtr->count] = String_Alloc(token.string);
		if (!PC_Float_Parse(handle, &multiPtr->cvarValue[multiPtr->count])) { return qfalse; }
...
void Item_SetupKeywordHash(void) {
	...
	for (i = 0; itemParseKeywords[i].keyword; i++) { KeywordHash_Add(itemParseKeywordHash, &itemParseKeywords[i]); }
}

static const char *builtinResolutions[] = {"320x240",	"400x300",	"512x384",	"640x480",	 "800x600",
...
static void Item_ApplyHacks(itemDef_t *item) {
	...
	if (item->type == ITEM_TYPE_EDITFIELD && item->cvar &&
		(!Q_stricmp(item->cvar, "ui_Name") || !Q_stricmp(item->cvar, "ui_findplayer"))) {
		editFieldDef_t *editField = (editFieldDef_t *)item->typeData;

		// enough to hold a full player name
		if (editField->maxChars < MAX_NAME_LENGTH) {
			if (editField->maxPaintChars > editField->maxChars) { editField->maxPaintChars = editField->maxChars; }
	...
	if (item->type == ITEM_TYPE_MULTI && item->cvar && !Q_stricmp(item->cvar, "r_mode")) {
		...
		if (multiPtr->count < MAX_MULTI_CVARS) {
			...
			Com_sprintf(currentResolution, sizeof(currentResolution), "%dx%d", DC->glconfig.vidWidth,
						DC->glconfig.vidHeight);
			for (i = 0; i < multiPtr->count; i++) {
				if (!Q_stricmp(multiPtr->cvarStr[i], currentResolution)) { break; }
			}
...
qboolean MenuParse_font(itemDef_t *item, int handle) {
	...
	if (!PC_String_Parse(handle, &menu->font)) { return qfalse; }
...
qboolean MenuParse_name(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_String_Parse(handle, &menu->window.name)) { return qfalse; }
...
qboolean MenuParse_fullscreen(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &fullScreen.i)) { return qfalse; }
...
qboolean MenuParse_rect(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Rect_Parse(handle, &menu->window.rect)) { return qfalse; }
	return qtrue;
}

qboolean MenuParse_style(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Int_Parse(handle, &menu->window.style)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_visible(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &i)) { return qfalse; }
	if (i) { menu->window.flags |= WINDOW_VISIBLE; }
	return qtrue;
}

qboolean MenuParse_onOpen(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Script_Parse(handle, &menu->onOpen)) { return qfalse; }
	return qtrue;
}

qboolean MenuParse_onClose(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Script_Parse(handle, &menu->onClose)) { return qfalse; }
	return qtrue;
}

qboolean MenuParse_onESC(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Script_Parse(handle, &menu->onESC)) { return qfalse; }
	return qtrue;
}

qboolean MenuParse_border(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Int_Parse(handle, &menu->window.border)) { return qfalse; }
	return qtrue;
}

qboolean MenuParse_borderSize(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Float_Parse(handle, &menu->window.borderSize)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_outlinecolor(itemDef_t *item, int handle) {
	menuDef_t *menu = (menuDef_t *)item;
	if (!PC_Color_Parse(handle, &menu->window.outlineColor)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_background(itemDef_t *item, int handle) {
	...
	if (!PC_String_Parse(handle, &buff)) { return qfalse; }
...
qboolean MenuParse_cinematic(itemDef_t *item, int handle) {
	...
	if (!PC_String_Parse(handle, &menu->window.cinematicName)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_ownerdraw(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &menu->window.ownerDraw)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_soundLoop(itemDef_t *item, int handle) {
	...
	if (!PC_String_Parse(handle, &menu->soundName)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_fadeClamp(itemDef_t *item, int handle) {
	...
	if (!PC_Float_Parse(handle, &menu->fadeClamp)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_fadeAmount(itemDef_t *item, int handle) {
	...
	if (!PC_Float_Parse(handle, &menu->fadeAmount)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_fadeCycle(itemDef_t *item, int handle) {
	...
	if (!PC_Int_Parse(handle, &menu->fadeCycle)) { return qfalse; }
	return qtrue;
}
...
qboolean MenuParse_itemDef(itemDef_t *item, int handle) {
	...
	if (menu->itemCount < MAX_MENUITEMS) {
		...
		if (!menu->items[menu->itemCount]) { return qfalse; }
		Item_Init(menu->items[menu->itemCount]);
		if (!Item_Parse(handle, menu->items[menu->itemCount])) { return qfalse; }
...
keywordHash_t menuParseKeywords[] = {{"font", MenuParse_font, NULL},
...
void Menu_SetupKeywordHash(void) {
	...
	for (i = 0; menuParseKeywords[i].keyword; i++) { KeywordHash_Add(menuParseKeywordHash, &menuParseKeywords[i]); }
}
...
void Menu_PaintAll(void) {
	int i;
	if (captureFunc) { captureFunc(captureData); }

	for (i = 0; i < Menu_Count(); i++) { Menu_Paint(&Menus[i], qfalse); }
...
void *Display_CaptureItem(int x, int y) {
	...
	for (i = 0; i < menuCount; i++) {
		// turn off focus each item
		// menu->items[i].window.flags &= ~WINDOW_HASFOCUS;
		if (Rect_ContainsPoint(&Menus[i].window.rect, x, y)) { return &Menus[i]; }
	}
	return NULL;
}
...
qboolean Display_MouseMove(void *p, int x, int y) {
	...
	if (menu == NULL) {
		...
		for (i = 0; i < menuCount; i++) { Menu_HandleMouseMove(&Menus[i], x, y); }
	} else {
...
int Display_CursorType(int x, int y) {
	...
	for (i = 0; i < menuCount; i++) {
		...
		r2.w = r2.h = 7;
		if (Rect_ContainsPoint(&r2, x, y)) { return CURSOR_SIZER; }
	}
	return CURSOR_ARROW;
}
...
void Display_HandleKey(int key, qboolean down, int x, int y) {
	...
	if (menu == NULL) { menu = Menu_GetFocused(); }
	if (menu) { Menu_HandleKey(menu, key, down); }
}
...
static void Item_CacheContents(itemDef_t *item) {
	if (item) { Window_CacheContents(&item->window); }
}
...
static void Menu_CacheContents(menuDef_t *menu) {
	if (menu) {
		...
		for (i = 0; i < menu->itemCount; i++) { Item_CacheContents(menu->items[i]); }

		if (menu->soundName && *menu->soundName) { DC->registerSound(menu->soundName, qfalse); }
	}
}

void Display_CacheAll(void) {
	int i;
	for (i = 0; i < menuCount; i++) { Menu_CacheContents(&Menus[i]); }
}
	...
	if (menu && menu->window.flags & (WINDOW_VISIBLE | WINDOW_FORCED)) {
		if (Rect_ContainsPoint(&menu->window.rect, x, y)) {
			...
			for (i = 0; i < menu->itemCount; i++) {
				...
				if (menu->items[i]->window.flags & WINDOW_DECORATION) { continue; }
