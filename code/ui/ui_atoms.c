...
void UI_LoadBestScores(const char *map, int game) {
	...
	if (trap_FS_FOpenFile(fileName, &f, FS_READ) >= 0) {
		...
		if (size == sizeof(postGameInfo_t)) { trap_FS_Read(&newInfo, sizeof(postGameInfo_t), f); }
...
static void UI_CalcPostGameStats(void) {
	...
	if (trap_FS_FOpenFile(fileName, &f, FS_READ) >= 0) {
		...
		size = 0;
		trap_FS_Read(&size, sizeof(int), f);
		if (size == sizeof(postGameInfo_t)) { trap_FS_Read(&oldInfo, sizeof(postGameInfo_t), f); }
		trap_FS_FCloseFile(f);
	}
	...
	if (newInfo.skillBonus <= 0) { newInfo.skillBonus = 1; }
	...
	if (newInfo.time < oldInfo.time) { uiInfo.newBestTime = uiInfo.uiDC.realTime + 20000; }
...
qboolean UI_CursorInRect(int x, int y, int width, int height) {
	if (uiInfo.uiDC.cursorx < x || uiInfo.uiDC.cursory < y || uiInfo.uiDC.cursorx > x + width ||
