...
void SHOWNET(msg_t *msg, char *s) {
	if (cl_shownet->integer >= 2) { Com_Printf("%3i:%s\n", msg->readcount - 1, s); }
}
...
void CL_ParsePacketEntities(msg_t *msg, clSnapshot_t *oldframe, clSnapshot_t *newframe) {
	...
	while (1) {
		...
		if (msg->readcount > msg->cursize) { Com_Error(ERR_DROP, "CL_ParsePacketEntities: end of message"); }

		while (oldnum < newnum) {
			// one or more entities from the old packet are unchanged
			if (cl_shownet->integer == 3) { Com_Printf("%3i:  unchanged: %i\n", msg->readcount, oldnum); }
		...
		if (oldnum == newnum) {
			...
			if (cl_shownet->integer == 3) { Com_Printf("%3i:  delta: %i\n", msg->readcount, newnum); }
		...
		if (oldnum > newnum) {
			// delta from baseline
			if (cl_shownet->integer == 3) { Com_Printf("%3i:  baseline: %i\n", msg->readcount, newnum); }
...
void CL_ParseGamestate(msg_t *msg) {
	...
	if (cl_autoRecordDemo->integer && clc.demorecording) CL_StopRecord_f();
...
static void CL_PlayVoip(int sender, int samplecnt, const byte *data, int flags) {
	if (flags & VOIP_DIRECT) { S_RawSamples(sender + 1, samplecnt, 48000, 2, 1, data, clc.voipGain[sender], -1); }

	if (flags & VOIP_SPATIAL) { S_RawSamples(sender + MAX_CLIENTS + 1, samplecnt, 48000, 2, 1, data, 1.0f, sender); }
}
...
static void CL_ParseVoip(msg_t *msg, qboolean ignoreData) {
	...
	if (written > 0) CL_PlayVoip(sender, written, (const byte *)decoded, flags);
