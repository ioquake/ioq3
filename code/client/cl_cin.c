...
void CIN_CloseAllVideos(void) {
	...
	for (i = 0; i < MAX_VIDEO_HANDLES; i++) {
		if (cinTable[i].fileName[0] != 0) { CIN_StopCinematic(i); }
	}
}
...
static int CIN_HandleForVideo(void) {
	...
	for (i = 0; i < MAX_VIDEO_HANDLES; i++) {
		if (cinTable[i].fileName[0] == 0) { return i; }
	}
	...
	if (!cinTable[currentHandle].half) {
		...
		if (cinTable[currentHandle].samplesPerPixel == 2) {
			...
			for (i = 0; i < four; i++) {
				...
				ibptr.s = vq2 + (*input++) * 2;
				ibptr.i += (*input++) * 2;
				for (j = 0; j < 2; j++) { VQ2TO2(iaptr.i, ibptr.i, icptr.i, idptr.i); }
			}
		}
	}
}
...
static void readQuadInfo(byte *qData) {
	...
	if (cls.glconfig.hardwareType == GLHW_RAGEPRO || cls.glconfig.maxTextureSize <= 256) {
		if (cinTable[currentHandle].drawX > 256) { cinTable[currentHandle].drawX = 256; }
		if (cinTable[currentHandle].drawY > 256) { cinTable[currentHandle].drawY = 256; }
...
redump:
	...
	case ROQ_QUAD_VQ:
		...
		break;
	case ROQ_CODEBOOK: decodeCodeBook(framedata, (unsigned short)cinTable[currentHandle].roq_flags); break;
	case ZA_SOUND_MONO:
	...
	case ROQ_PACKET:
		...
		break;
	case ROQ_QUAD_HANG: cinTable[currentHandle].RoQFrameSize = 0; break;
	case ROQ_QUAD_JPEG: break;
	default: cinTable[currentHandle].status = FMV_EOF; break;
	}
	//
...
static void RoQ_init(void) {
	...
	if (cinTable[currentHandle].RoQFrameSize > 65536 || !cinTable[currentHandle].RoQFrameSize) { return; }
}
...
e_status CIN_StopCinematic(int handle) {
	...
	if (cinTable[currentHandle].alterGameState) {
		if (clc.state != CA_CINEMATIC) { return cinTable[currentHandle].status; }
	}
	cinTable[currentHandle].status = FMV_EOF;
	RoQShutdown();
...
int CIN_PlayCinematic(const char *arg, int x, int y, int w, int h, int systemBits) {
	...
	if (!(systemBits & CIN_system)) {
		for (i = 0; i < MAX_VIDEO_HANDLES; i++) {
			if (!strcmp(cinTable[i].fileName, name)) { return i; }
		}
	}
	...
	if (cinTable[currentHandle].alterGameState) {
		// close the menu
		if (uivm) { VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_NONE); }
	} else {
	...
	if (RoQID == 0x1084) {
		...
		if (cinTable[currentHandle].alterGameState) { clc.state = CA_CINEMATIC; }

		Con_Close();

		if (!cinTable[currentHandle].silent) { s_rawend[0] = s_soundtime; }
...
void CIN_ResampleCinematic(int handle, int *buf2) {
	...
	if (xm == 2 && ym == 2) {
		...
		for (iy = 0; iy < 256; iy++) {
			for (ix = 0; ix < 256; ix++) { buf2[(iy << 8) + ix] = buf3[((iy * ym) << ll) + (ix * xm)]; }
		}
	}
}
...
void CL_PlayCinematic_f(void) {
	...
	int	  timeout	= 10; // wait for maximum of 10 seconds for first frame

	printf("CL_PlayCinematic_f\n");

	if (clc.state == CA_CINEMATIC) { SCR_StopCinematic(); }
	...
	if (CL_handle >= 0) {
		...
		if (cinTable[currentHandle].buf == NULL) { printf("Error: Timeout waiting for first frame.\n"); }
	} else {
		printf("Error: Failed to play cinematic.\n");
	}
}

void SCR_DrawCinematic(void) {
	if (CL_handle >= 0 && CL_handle < MAX_VIDEO_HANDLES) { CIN_DrawCinematic(CL_handle); }
}

void SCR_RunCinematic(void) {
	if (CL_handle >= 0 && CL_handle < MAX_VIDEO_HANDLES) { CIN_RunCinematic(CL_handle); }
}
...
void CIN_UploadCinematic(int handle) {
	if (handle >= 0 && handle < MAX_VIDEO_HANDLES) {
		...
		if (cinTable[handle].dirty && (cinTable[handle].CIN_WIDTH != cinTable[handle].drawX ||
			...
			re.UploadCinematic(cinTable[handle].CIN_WIDTH, cinTable[handle].CIN_HEIGHT, 256, 256, (byte *)buf2, handle,
							   qtrue);
			cinTable[handle].dirty = qfalse;
			Hunk_FreeTempMemory(buf2);
		} else {
