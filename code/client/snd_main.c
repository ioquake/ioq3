...
void S_StartSound(vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx) {
	if (si.StartSound) { si.StartSound(origin, entnum, entchannel, sfx); }
}
...
void S_StartLocalSound(sfxHandle_t sfx, int channelNum) {
	if (si.StartLocalSound) { si.StartLocalSound(sfx, channelNum); }
}
...
void S_StartBackgroundTrack(const char *intro, const char *loop) {
	if (si.StartBackgroundTrack) { si.StartBackgroundTrack(intro, loop); }
}
...
void S_StopBackgroundTrack(void) {
	if (si.StopBackgroundTrack) { si.StopBackgroundTrack(); }
}
...
void S_RawSamples(int stream, int samples, int rate, int width, int channels, const byte *data, float volume,
				  int entityNum) {
	if (si.RawSamples) si.RawSamples(stream, samples, rate, width, channels, data, volume, entityNum);
}
...
void S_StopAllSounds(void) {
	if (si.StopAllSounds) { si.StopAllSounds(); }
}
...
void S_ClearLoopingSounds(qboolean killall) {
	if (si.ClearLoopingSounds) { si.ClearLoopingSounds(killall); }
}
...
void S_AddLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx) {
	if (si.AddLoopingSound) { si.AddLoopingSound(entityNum, origin, velocity, sfx); }
}
...
void S_AddRealLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfx) {
	if (si.AddRealLoopingSound) { si.AddRealLoopingSound(entityNum, origin, velocity, sfx); }
}
...
void S_StopLoopingSound(int entityNum) {
	if (si.StopLoopingSound) { si.StopLoopingSound(entityNum); }
}
...
void S_Respatialize(int entityNum, const vec3_t origin, vec3_t axis[3], int inwater) {
	if (si.Respatialize) { si.Respatialize(entityNum, origin, axis, inwater); }
}
...
void S_UpdateEntityPosition(int entityNum, const vec3_t origin) {
	if (si.UpdateEntityPosition) { si.UpdateEntityPosition(entityNum, origin); }
}
...
void S_DisableSounds(void) {
	if (si.DisableSounds) { si.DisableSounds(); }
}
...
void S_BeginRegistration(void) {
	if (si.BeginRegistration) { si.BeginRegistration(); }
}
...
void S_ClearSoundBuffer(void) {
	if (si.ClearSoundBuffer) { si.ClearSoundBuffer(); }
}
...
void S_StartCapture(void) {
	if (si.StartCapture) { si.StartCapture(); }
}
...
int S_AvailableCaptureSamples(void) {
	if (si.AvailableCaptureSamples) { return si.AvailableCaptureSamples(); }
	return 0;
}
...
void S_Capture(int samples, byte *data) {
	if (si.Capture) { si.Capture(samples, data); }
}
...
void S_StopCapture(void) {
	if (si.StopCapture) { si.StopCapture(); }
}
...
void S_MasterGain(float gain) {
	if (si.MasterGain) { si.MasterGain(gain); }
}
#endif
...
void S_Play_f(void) {
	...
	if (!si.RegisterSound || !si.StartLocalSound) { return; }
	...
	for (i = 1; i < c; i++) {
		...
		if (h) { si.StartLocalSound(h, CHAN_LOCAL_SOUND); }
	}
}
...
void S_Music_f(void) {
	...
	if (!si.StartBackgroundTrack) { return; }
...
void S_StopMusic_f(void) {
	if (!si.StopBackgroundTrack) return;
...
void S_Init(void) {
	...
	if (!cv->integer) {
		...
		if (started) {
			if (!S_ValidSoundInterface(&si)) { Com_Error(ERR_FATAL, "Sound interface invalid"); }
