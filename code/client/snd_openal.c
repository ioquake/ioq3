...
static void S_AL_ClearError(qboolean quiet) {
	...
	if (quiet) return;
	if (error != AL_NO_ERROR) { Com_Printf(S_COLOR_YELLOW "WARNING: unhandled AL error: %s\n", S_AL_ErrorMsg(error)); }
}
...
static sfxHandle_t S_AL_BufferFind(const char *filename) {
	...
	if (!filename) { Com_Error(ERR_FATAL, "Sound name is NULL"); }
...
static void S_AL_BufferUseDefault(sfxHandle_t sfx) {
	if (sfx == default_sfx) Com_Error(ERR_FATAL, "Can't load default sound effect %s", knownSfx[sfx].filename);
...
static void S_AL_BufferUnload(sfxHandle_t sfx) {
	if (knownSfx[sfx].filename[0] == '\0') return;

	if (!knownSfx[sfx].inMemory) return;
...
static qboolean S_AL_BufferEvict(void) {
	...
	for (i = 0; i < numSfx; i++) {
		if (!knownSfx[i].filename[0]) continue;

		if (!knownSfx[i].inMemory) continue;
...
static void S_AL_BufferLoad(sfxHandle_t sfx, qboolean cache) {
	...
	// Nothing?
	if (curSfx->filename[0] == '\0') return;

	// Already done?
	if ((curSfx->inMemory) || (curSfx->isDefault) || (!cache && curSfx->isDefaultChecked)) return;
...
static void S_AL_BufferUse(sfxHandle_t sfx) {
	...
	if ((!knownSfx[sfx].inMemory) && (!knownSfx[sfx].isDefault)) S_AL_BufferLoad(sfx, qtrue);
...
static qboolean S_AL_BufferInit(void) {
	if (alBuffersInitialised) return qtrue;
...
static sfxHandle_t S_AL_RegisterSound(const char *sample, qboolean compressed) {
	...
	if ((!knownSfx[sfx].inMemory) && (!knownSfx[sfx].isDefault)) S_AL_BufferLoad(sfx, s_alPrecache->integer);
	knownSfx[sfx].lastUsedTime = Com_Milliseconds();

	if (knownSfx[sfx].isDefault) { return 0; }
...
static void S_AL_ScaleGain(src_t *chksrc, vec3_t origin) {
	...
	if (!chksrc->local) distance = Distance(origin, lastListenerOrigin);
...
static qboolean S_AL_SrcInit(void) {
	...
	for (i = 0; i < limit; i++) {
		...
		if (qalGetError() != AL_NO_ERROR) break;
		srcCount++;
	}
...
static void S_AL_SrcShutdown(void) {
	...
	if (!alSourcesInitialised) return;

	// Destroy all the sources
	for (i = 0; i < srcCount; i++) {
		curSource = &srcList[i];

		if (curSource->isLocked) Com_DPrintf(S_COLOR_YELLOW "WARNING: Source %d is locked\n", i);

		if (curSource->entity > 0) entityList[curSource->entity].srcAllocated = qfalse;
...
=================
*/
static void S_AL_SrcSetup(srcHandle_t src, sfxHandle_t sfx, alSrcPriority_t priority, int entity, int channel,
						  qboolean local) {
	src_t *curSource;
...
static void S_AL_NewLoopMaster(src_t *rmSource, qboolean iskilled) {
	...
	if (rmSource->isPlaying) curSfx->loopActiveCnt--;
		...
		if (rmSource->priority == SRCPRI_ENTITY) {
			...
			if (iskilled || curSfx->loopActiveCnt) {
				for (index = 0; index < srcCount; index++) {
					...
					if (curSource->sfx == rmSource->sfx && curSource != rmSource && curSource->isActive &&
						curSource->isLooping && curSource->priority == SRCPRI_AMBIENT) {
		...
		if (curSource->isPlaying) {
			if (weakest_isplaying && curSource->priority < priority &&
				(curSource->priority < weakest_pri ||
				 (!curSource->isLooping &&
				  (curSource->scaleGain < weakest_gain || curSource->lastUsedTime < weakest_time)))) {
			...
			if (weakest < 0 || knownSfx[curSource->sfx].loopCnt > weakest_numloops ||
...
static void S_AL_StartLocalSound(sfxHandle_t sfx, int channel) {
	...
	if (S_AL_CheckInput(0, sfx)) return;
...
static void S_AL_StartSound(vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx) {
	...
	if (origin) {
		...
		if (S_AL_CheckInput(entnum, sfx)) return;
	...
	if (!origin) curSource->isTracking = qtrue;
...
=================
*/
static void S_AL_SrcLoop(alSrcPriority_t priority, sfxHandle_t sfx, const vec3_t origin, const vec3_t velocity,
						 int entityNum) {
	int		   src;
	...
	if (S_AL_CheckInput(entityNum, sfx)) return;
...
static void S_AL_StopLoopingSound(int entityNum) {
	if (entityList[entityNum].srcAllocated) S_AL_SrcKill(entityList[entityNum].srcIndex);
}
...
static void S_AL_SrcUpdate(void) {
	...
	for (i = 0; i < srcCount; i++) {
		...
		if (!curSource->isActive) continue;

		// Update source parameters
		if ((s_alGain->modified) || (s_volume->modified)) curSource->curGain = s_alGain->value * s_volume->value;
		if ((s_alRolloff->modified) && (!curSource->local))
			qalSourcef(curSource->alSource, AL_ROLLOFF_FACTOR, s_alRolloff->value);
		if (s_alMinDistance->modified) qalSourcef(curSource->alSource, AL_REFERENCE_DISTANCE, s_alMinDistance->value);
				...
				if (!curSource->isPlaying) {
					...
					if (curSource->priority == SRCPRI_AMBIENT) {
						...
						if (curSfx->loopActiveCnt) {
							...
							// This loop sound used to be played, but all sources are stopped. Use last sample
							// position/time to calculate offset so the player thinks the sources continued playing
							// while they were inaudible.
...
static void S_AL_SrcShutup(void) {
	int i;
	for (i = 0; i < srcCount; i++) S_AL_SrcKill(i);
}
...
static void S_AL_AllocateStreamChannel(int stream, int entityNum) {
	...
	if ((stream < 0) || (stream >= MAX_RAW_STREAMS)) return;
...
static void S_AL_RawSamples(int stream, int samples, int rate, int width, int channels, const byte *data, float volume,
	...
	if (numBuffers == streamNumBuffers[stream]) {
		...
		if (!S_AL_GenBuffers(1, &buffer, "stream")) return;
...
static void S_AL_StreamUpdate(int stream) {
	...
	if (state == AL_STOPPED) {
		...
		if (!numBuffers) S_AL_FreeStreamChannel(stream);
	}
...
static void S_AL_StartBackgroundTrack(const char *intro, const char *loop) {
	...
	if ((!intro || !*intro) && (!loop || !*loop)) return;
	...
	if (!S_AL_GenBuffers(NUM_MUSIC_BUFFERS, musicBuffers, "music")) return;
...
#if defined(_WINDOWS)
#define ALDRIVER_DEFAULT "oal_soft.dll"
...
static void S_AL_StopAllSounds(void) {
	...
	S_AL_SrcShutup();
	S_AL_StopBackgroundTrack();
	for (i = 0; i < MAX_RAW_STREAMS; i++) S_AL_StreamDie(i);
}
...
static void S_AL_Update(void) {
	...
	if (s_muted->modified) {
		...
		for (i = 0; i < srcCount; i++) {
			if (srcList[i].isActive) S_AL_Gain(srcList[i].alSource, srcList[i].scaleGain);
		}
	...
	// Update streams
	for (i = 0; i < MAX_RAW_STREAMS; i++) S_AL_StreamUpdate(i);
	S_AL_MusicUpdate();
...
static void S_AL_StartCapture(void) {
	if (alCaptureDevice != NULL) qalcCaptureStart(alCaptureDevice);
}
...
static void S_AL_Capture(int samples, byte *data) {
	if (alCaptureDevice != NULL) qalcCaptureSamples(alCaptureDevice, data, samples);
}

void S_AL_StopCapture(void) {
	if (alCaptureDevice != NULL) qalcCaptureStop(alCaptureDevice);
}
...
static void S_AL_SoundInfo(void) {
	...
	if (enumeration_all_ext || enumeration_ext) Com_Printf("  Available Devices:\n%s", s_alAvailableDevices->string);
...
qboolean S_AL_Init(soundInterface_t *si) {
	...
	if (!QAL_Init(s_alDriver->string)) {
		...
		if (!Q_stricmp(s_alDriver->string, ALDRIVER_DEFAULT) || !QAL_Init(ALDRIVER_DEFAULT)) { return qfalse; }
	}
	...
	if (inputdevice && !*inputdevice) inputdevice = NULL;
	...
	if (enumeration_ext || enumeration_all_ext) {
		...
		if (!device && defaultdevice && !strcmp(defaultdevice, "Generic Hardware")) device = "Generic Software";
#endif
	...
	if (!s_alCapture->integer) {
...
#endif
		{
			...
			s_alAvailableInputDevices =
				Cvar_Get("s_alAvailableInputDevices", inputdevicenames, CVAR_ROM | CVAR_NORESTART);
