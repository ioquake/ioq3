...
void S_TransferStereo16(unsigned long *pbuf, int endtime) {
	...
	while (ls_paintedtime < endtime) {
		...
		if (ls_paintedtime + snd_linear_count > endtime) snd_linear_count = endtime - ls_paintedtime;
...
static void S_PaintChannelFrom16_scalar(channel_t *ch, const sfx_t *sc, int count, int sampleOffset, int bufferOffset) {
	...
	if (!ch->doppler || ch->dopplerScale == 1.0f) {
		...
		for (i = 0; i < count; i++) {
			...
			fdiv = 256.0f * (boff - aoff) / sc->soundChannels;
...
void S_PaintChannels(int endtime) {
	...
	while (s_paintedtime < endtime) {
		...
		if (endtime - s_paintedtime > PAINTBUFFER_SIZE) { end = s_paintedtime + PAINTBUFFER_SIZE; }
		...
		for (i = 0; i < MAX_CHANNELS; i++, ch++) {
			if (!ch->thesfx || (ch->leftvol < 0.25 && ch->rightvol < 0.25)) { continue; }
