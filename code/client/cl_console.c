...
void Con_Clear_f(void) {
	...
	for (i = 0; i < CON_TEXTSIZE; i++) { con.text[i] = (ColorIndex(COLOR_WHITE) << 8) | ' '; }
...
void Cmd_CompleteTxtName(char *args, int argNum) {
	if (argNum == 2) { Field_CompleteFilename("", "txt", qfalse, qtrue); }
}
...
void CL_ConsolePrint(char *txt) {
	...
	if (cl_noprint && cl_noprint->integer) { return; }
	...
	while ((c = *((unsigned char *)txt)) != 0) {
		...
		// word wrap
		if (l != con.linewidth && (con.x + l >= con.linewidth)) { Con_Linefeed(skipnotify); }

		txt++;

		switch (c) {
		case '\n': Con_Linefeed(skipnotify); break;
		case '\r': con.x = 0; break;
...
void Con_DrawNotify(void) {
	...
	for (i = con.current - NUM_CON_TIMES + 1; i <= con.current; i++) {
		...
		if (cl.snap.ps.pm_type != PM_INTERMISSION && Key_GetCatcher() & (KEYCATCH_UI | KEYCATCH_CGAME)) { continue; }
...
void Con_DrawSolidConsole(float frac) {
	...
	if (con.display != con.current) {
		...
		re.SetColor(g_color_table[ColorIndex(COLOR_RED)]);
		for (x = 0; x < con.linewidth; x += 4) SCR_DrawSmallChar(con.xadjust + (x + 1) * SMALLCHAR_WIDTH, y, '^');
		y -= SMALLCHAR_HEIGHT;
		rows--;
	}
...
void Con_PageUp(void) {
	con.display -= 2;
	if (con.current - con.display >= con.totallines) { con.display = con.current - con.totallines + 1; }
}
