...
static void SCR_DrawChar(int x, int y, float size, int ch) {
	...
	float ax = x, ay = y, aw = size, ah = size;
	...
	const float charSize = 0.0625;

	re.DrawStretchPic(ax, ay, aw, ah, fcol, frow, fcol + charSize, frow + charSize, cls.charSetShader);
}
...
void SCR_DrawSmallChar(int x, int y, int ch) {
	...
	if (y < -SMALLCHAR_HEIGHT || ch == ' ') { return; }
...
void SCR_DrawScreenField(stereoFrame_t stereoFrame) {
	...
	if (uivm && !uiFullscreen) {
		switch (clc.state) {
		default: Com_Error(ERR_FATAL, "SCR_DrawScreenField: bad clc.state"); break;
		case CA_CINEMATIC: SCR_DrawCinematic(); break;
	...
	// the menu draws next
	if (Key_GetCatcher() & KEYCATCH_UI && uivm) { VM_Call(uivm, UI_REFRESH, cls.realtime); }

	// console draws next
	Con_DrawConsole();

	// debug graph can be drawn on top of anything
	if (cl_debuggraph->integer || cl_timegraph->integer || cl_debugMove->integer) { SCR_DrawDebugGraph(); }
}
