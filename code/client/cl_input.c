...
void IN_MLookUp(void) {
	in_mlooking = qfalse;
	if (!cl_freelook->integer) { IN_CenterView(); }
}
...
void CL_JoystickEvent(int axis, int value, int time) {
	if (axis < 0 || axis >= MAX_JOYSTICK_AXIS) { Com_Error(ERR_DROP, "CL_JoystickEvent: bad axis %i", axis); }
...
void CL_JoystickMove(usercmd_t *cmd) {
	...
	if (!(in_speed.active ^ cl_run->integer)) { cmd->buttons |= BUTTON_WALKING; }
...
void CL_MouseMove(usercmd_t *cmd) {
	...
	if (cl_mouseAccel->value != 0.0f) {
		if (cl_mouseAccelStyle->integer == 0) {
			...
			// amplification NOTE: decouple the config cvars for independent acceleration setup along X and Y?
...
void CL_CmdButtons(usercmd_t *cmd) {
	...
	//
	for (i = 0; i < 15; i++) {
		if (in_buttons[i].active || in_buttons[i].wasPressed) { cmd->buttons |= 1 << i; }
		in_buttons[i].wasPressed = qfalse;
	}

	if (Key_GetCatcher()) { cmd->buttons |= BUTTON_TALK; }

	// allow the game to know if any key at all is
	// currently pressed, even if it isn't bound to anything
	if (anykeydown && Key_GetCatcher() == 0) { cmd->buttons |= BUTTON_ANY; }
}
...
usercmd_t CL_CreateCmd(void) {
	...
	if (cl_debugMove->integer) {
		if (cl_debugMove->integer == 1) { SCR_DebugGraph(fabs(cl.viewangles[YAW] - oldAngles[YAW])); }
		if (cl_debugMove->integer == 2) { SCR_DebugGraph(fabs(cl.viewangles[PITCH] - oldAngles[PITCH])); }
	}
...
qboolean CL_ReadyToSendPacket(void) {
	...
	if (clc.demoplaying || clc.state == CA_CINEMATIC) { return qfalse; }

	// If we are downloading, we send no less than 50ms between packets
	if (*clc.downloadTempName && cls.realtime - clc.lastPacketSentTime < 50) { return qfalse; }
	...
	if (clc.netchan.remoteAddress.type == NA_LOOPBACK) { return qtrue; }

	// send every frame for LAN
	if (cl_lanForcePackets->integer && Sys_IsLANAddress(clc.netchan.remoteAddress)) { return qtrue; }
...
void CL_WritePacket(void) {
	...
	// don't send anything if playing back a demo
	if (clc.demoplaying || clc.state == CA_CINEMATIC) { return; }
	...
	if (count >= 1) {
		if (cl_showSend->integer) { Com_Printf("(%i)", count); }
	...
	if (cl_showSend->integer) { Com_Printf("%i ", buf.cursize); }
...
void CL_SendCmd(void) {
	...
	if (com_sv_running->integer && sv_paused->integer && cl_paused->integer) { return; }
	...
	if (!CL_ReadyToSendPacket()) {
		if (cl_showSend->integer) { Com_Printf(". "); }
		return;
	}
