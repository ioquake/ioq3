...
CURLMcode (*qcurl_multi_fdset)(CURLM *multi_handle, fd_set *read_fd_set, fd_set *write_fd_set, fd_set *exc_fd_set,
							   int *max_fd);
...
void CL_cURL_BeginDownload(const char *localName, const char *remoteURL) {
	...
	if (com_developer->integer) qcurl_easy_setopt_warn(clc.downloadCURL, CURLOPT_VERBOSE, 1);
...
void CL_cURL_PerformDownload(void) {
	...
	if (res == CURLM_CALL_MULTI_PERFORM) return;
	...
	if (msg->msg == CURLMSG_DONE && msg->data.result == CURLE_OK) {
		...
		Com_Error(ERR_DROP, "Download Error: %s Code: %ld URL: %s", qcurl_easy_strerror(msg->data.result), code,
				  clc.downloadURL);
	}
