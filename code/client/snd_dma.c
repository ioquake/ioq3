...
#ifdef USE_VOIP
static void S_Base_StartCapture(void) { SNDDMA_StartCapture(); }

static int S_Base_AvailableCaptureSamples(void) { return SNDDMA_AvailableCaptureSamples(); }

static void S_Base_Capture(int samples, byte *data) { SNDDMA_Capture(samples, data); }

static void S_Base_StopCapture(void) { SNDDMA_StopCapture(); }

static void S_Base_MasterGain(float val) { SNDDMA_MasterGain(val); }
#endif
...
static sfx_t *S_FindName(const char *name) {
	...
	if (!name) { Com_Error(ERR_FATAL, "Sound name is NULL"); }
	...
	while (sfx) {
		if (!Q_stricmp(sfx->soundName, name)) { return sfx; }
		sfx = sfx->next;
	}

	// find a free sfx
	for (i = 0; i < s_numSfx; i++) {
		if (!s_knownSfx[i].soundName[0]) { break; }
	}

	if (i == s_numSfx) {
		if (s_numSfx == MAX_SFX) { Com_Error(ERR_FATAL, "S_FindName: out of sfx_t"); }
		s_numSfx++;
	}
...
void S_DefaultSound(sfx_t *sfx) {
	...
	for (i = 0; i < sfx->soundLength; i++) { sfx->soundData->sndChunk[i] = i; }
}
...
====================
*/
static void S_Base_StartSoundEx(vec3_t origin, int entityNum, int entchannel, sfxHandle_t sfxHandle,
								qboolean localSound) {
	channel_t *ch;
	sfx_t	  *sfx;
	...
	if (sfx->inMemory == qfalse) { S_memoryLoad(sfx); }

	if (s_show->integer == 1) { Com_Printf("%i : %s\n", s_paintedtime, sfx->soundName); }
	...
	fullVolume = qfalse;
	if (localSound || S_Base_HearingThroughEntity(entityNum, origin)) { fullVolume = qtrue; }
...
void S_Base_AddLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfxHandle) {
	...
	if (!sfx->soundLength) { Com_Error(ERR_DROP, "%s has length 0", sfx->soundName); }
...
void S_AddLoopSounds(void) {
	...
	for (i = 0; i < MAX_GENTITIES; i++) {
		...
		for (j = (i + 1); j < MAX_GENTITIES; j++) {
			loop2 = &loopSounds[j];
			if (!loop2->active || loop2->doppler || loop2->sfx != loop->sfx) { continue; }
		...
		numLoopChannels++;
		if (numLoopChannels == MAX_CHANNELS) { return; }
	}
}
...
void S_ByteSwapRawSamples(int samples, int width, int s_channels, const byte *data) {
	...
	for (i = 0; i < samples; i++) { ((short *)data)[i] = LittleShort(((short *)data)[i]); }
}
...
qboolean S_ScanChannelStarts(void) {
	...
	for (i = 0; i < MAX_CHANNELS; i++, ch++) {
		...
		if (ch->startSample + (ch->thesfx->soundLength) <= s_paintedtime) { S_ChannelFree(ch); }
	}
...
void S_Update_(void) {
	...
	if (endtime - s_soundtime > dma.fullsamples) endtime = s_soundtime + dma.fullsamples;
...
void S_UpdateBackgroundTrack(void) {
	...
	if (s_musicVolume->value <= 0) { return; }

	// see how many samples should be copied into the raw buffer
	if (s_rawend[0] < s_soundtime) { s_rawend[0] = s_soundtime; }
	...
	while (s_rawend[0] < s_soundtime + MAX_RAW_SAMPLES) {
		...
		if (r < fileBytes) { fileSamples = r / (s_backgroundStream->info.width * s_backgroundStream->info.channels); }

		if (r > 0) {
			// add to raw buffer
			S_Base_RawSamples(0, fileSamples, s_backgroundStream->info.rate, s_backgroundStream->info.width,
							  s_backgroundStream->info.channels, raw, s_musicVolume->value, -1);
		} else {
			// loop
