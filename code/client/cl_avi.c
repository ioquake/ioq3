...
static ID_INLINE void SafeFS_Write(const void *buffer, int len, fileHandle_t f) {
	if (FS_Write(buffer, len, f) < len) Com_Error(ERR_DROP, "Failed to write avi file");
}
...
static ID_INLINE void START_CHUNK(const char *s) {
	if (afd.chunkStackTop == MAX_RIFF_CHUNKS) { Com_Error(ERR_DROP, "ERROR: Top of chunkstack breached"); }
...
static ID_INLINE void END_CHUNK(void) {
	...
	if (afd.chunkStackTop <= 0) { Com_Error(ERR_DROP, "ERROR: Bottom of chunkstack breached"); }
...
qboolean CL_OpenAVIForWriting(const char *fileName) {
	...
	if ((afd.f = FS_FOpenFileWrite(fileName)) <= 0) return qfalse;
...
#define MAX_PACK_LEN 16
	...
	if (afd.a.rate % afd.frameRate) {
		...
		while ((afd.a.rate % suggestRate) && suggestRate >= 1) suggestRate--;
	...
	if (!Cvar_VariableIntegerValue("s_initsound")) {
		...
		if (afd.a.bits != 16 || afd.a.channels != 2) {
			Com_Printf(S_COLOR_YELLOW "WARNING: Audio format of %d bit/%d channels not supported", afd.a.bits,
					   afd.a.channels);
			afd.audio = qfalse;
		} else
			afd.audio = qtrue;
...
void CL_WriteAVIVideoFrame(const byte *imageBuffer, int size) {
	...
	if (!afd.fileOpen) return;

	// Chunk header + contents + padding
	if (CL_CheckFileSize(8 + size + 2)) return;
...
void CL_WriteAVIAudioFrame(const byte *pcmBuffer, int size) {
	...
	// Chunk header + contents + padding
	if (CL_CheckFileSize(8 + bytesInBuffer + size + 2)) return;
