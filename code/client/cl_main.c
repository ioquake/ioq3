...
void		CL_Record_f(void) {
	   ...
	   if (clc.demorecording) {
		   if (!clc.spDemoRecording) { Com_Printf("Already recording.\n"); }
		   return;
	   }
...
#ifdef LEGACY_PROTOCOL
		if (demo_protocols[i] == com_legacyprotocol->integer) continue;
#endif
		if (demo_protocols[i] == com_protocol->integer) continue;
...
void CL_PlayDemo_f(void) {
	...
	while (clc.state >= CA_CONNECTED && clc.state < CA_PRIMED) { CL_ReadDemoMessage(); }
...
void CL_Disconnect(qboolean showMainMenu) {
	...
	*clc.downloadTempName = *clc.downloadName = 0;
	...
	if (clc.voipCodecInitialized) {
		...
		for (i = 0; i < MAX_CLIENTS; i++) { opus_decoder_destroy(clc.opusDecoder[i]); }
	...
	if (uivm && showMainMenu) { VM_Call(uivm, UI_SET_ACTIVE_MENU, UIMENU_NONE); }
...
void CL_RequestAuthorization(void) {
	...
	if (!cls.authorizeServer.port) {
		...
		Com_Printf("%s resolved to %i.%i.%i.%i:%i\n", AUTHORIZE_SERVER_NAME, cls.authorizeServer.ip[0],
				   cls.authorizeServer.ip[1], cls.authorizeServer.ip[2], cls.authorizeServer.ip[3],
				   BigShort(cls.authorizeServer.port));
	}
	if (cls.authorizeServer.type == NA_BAD) { return; }
	...
	if (l > 32) { l = 32; }
	for (i = 0; i < l; i++) {
		if ((cl_cdkey[i] >= '0' && cl_cdkey[i] <= '9') || (cl_cdkey[i] >= 'a' && cl_cdkey[i] <= 'z') ||
...
void CL_ForwardToServer_f(void) {
	...
	if (Cmd_Argc() > 1) { CL_AddReliableCommand(Cmd_Args(), qfalse); }
}
...
void CL_Connect_f(void) {
	...
	if (clc.serverAddress.port == 0) { clc.serverAddress.port = BigShort(PORT_SERVER); }
...
void CL_DownloadsComplete(void) {
	...
	if (clc.state != CA_LOADING) { return; }
...
void CL_CheckForResend(void) {
	...
	if (clc.state != CA_CONNECTING && clc.state != CA_CHALLENGING) { return; }

	if (cls.realtime - clc.connectTime < RETRANSMIT_TIMEOUT) { return; }
...
#ifdef LEGACY_PROTOCOL
		if (com_legacyprotocol->integer == com_protocol->integer) clc.compat = qtrue;
	...
	default: Com_Error(ERR_FATAL, "CL_CheckForResend: bad clc.state");
	}
}
...
void CL_MotdPacket(netadr_t from) {
	...
	if (!NET_CompareAdr(from, cls.updateServer)) { return; }

	info = Cmd_Argv(1);

	// check challenge
	challenge = Info_ValueForKey(info, "challenge");
	if (strcmp(challenge, cls.updateChallenge)) { return; }
...
void CL_ServersResponsePacket(const netadr_t *from, msg_t *msg, qboolean extended) {
	...
	do {
		if (*buffptr == '\\' || (extended && *buffptr == '/')) break;
	...
	for (i = 0; i < numservers && count < MAX_GLOBAL_SERVERS; i++) {
		...
		for (j = 0; j < count; j++) {
			if (NET_CompareAdr(cls.globalServers[j].adr, addresses[i])) break;
		}
...
void CL_PacketEvent(netadr_t from, msg_t *msg) {
	...
	//
	if (clc.demorecording && !clc.demowaiting) { CL_WriteDemoMessage(msg, headerBytes); }
}
...
qboolean CL_CheckPaused(void) {
	...
	// lag behind.
	if (cl_paused->integer || cl_paused->modified) return qtrue;
...
void CL_Frame(int msec) {
	...
	if (cl_timegraph->integer) { SCR_DebugGraph(cls.realFrametime * 0.25); }
...
void CL_StartHunkUsers(qboolean rendererOnly) {
	...
	if (com_dedicated->integer) { return; }
...
void CL_InitRef(void) {
	...
	if (!GetRefAPI) { Com_Error(ERR_FATAL, "Can't load symbol GetRefAPI: '%s'", Sys_LibraryError()); }
#endif
	...
	if (!ret) { Com_Error(ERR_FATAL, "Couldn't initialize refresh"); }
...
static void CL_GenerateQKey(void) {
	...
	if (len == QKEY_SIZE) {
		...
		if (len > 0) { Com_Printf("QKEY file size != %d, regenerating\n", QKEY_SIZE); }
...
static void CL_SetServerInfoByAddress(netadr_t from, const char *info, int ping) {
	...
	for (i = 0; i < MAX_OTHER_SERVERS; i++) {
		if (NET_CompareAdr(from, cls.localServers[i].adr)) { CL_SetServerInfo(&cls.localServers[i], info, ping); }
	}

	for (i = 0; i < MAX_GLOBAL_SERVERS; i++) {
		if (NET_CompareAdr(from, cls.globalServers[i].adr)) { CL_SetServerInfo(&cls.globalServers[i], info, ping); }
	}

	for (i = 0; i < MAX_OTHER_SERVERS; i++) {
		if (NET_CompareAdr(from, cls.favoriteServers[i].adr)) { CL_SetServerInfo(&cls.favoriteServers[i], info, ping); }
	}
}
...
void CL_ServerInfoPacket(netadr_t from, msg_t *msg) {
	...
	for (i = 0; i < MAX_PINGREQUESTS; i++) {
		if (cl_pinglist[i].adr.port && !cl_pinglist[i].time && NET_CompareAdr(from, cl_pinglist[i].adr)) {
			...
			switch (from.type) {
			case NA_BROADCAST:
			case NA_IP: type = 1; break;
			case NA_IP6: type = 2; break;
	...
	if (cls.pingUpdateSource != AS_LOCAL) { return; }

	for (i = 0; i < MAX_OTHER_SERVERS; i++) {
		// empty slot
		if (cls.localServers[i].adr.port == 0) { break; }

		// avoid duplicate
		if (NET_CompareAdr(from, cls.localServers[i].adr)) { return; }
	}
...
serverStatus_t *CL_GetServerStatus(netadr_t from) {
	...
	for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
		if (NET_CompareAdr(from, cl_serverStatusList[i].address)) { return &cl_serverStatusList[i]; }
	}
	for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
		if (cl_serverStatusList[i].retrieved) { return &cl_serverStatusList[i]; }
	}
	oldest	   = -1;
	oldestTime = 0;
	for (i = 0; i < MAX_SERVERSTATUSREQUESTS; i++) {
...
int CL_ServerStatus(char *serverAddress, char *serverStatusString, int maxLen) {
	...
	// get the address
	if (!NET_StringToAdr(serverAddress, &to, NA_UNSPEC)) { return qfalse; }
...
void CL_ServerStatusResponse(netadr_t from, msg_t *msg) {
	...
	if (serverStatus->print) { serverStatus->retrieved = qtrue; }
}
...
void CL_Ping_f(void) {
	...
	if (!NET_StringToAdr(server, &to, family)) { return; }
...
qboolean CL_UpdateVisiblePings_f(int source) {
	...
	if (source < 0 || source > AS_FAVORITES) { return qfalse; }
		...
		for (i = 0; i < max; i++) {
			if (server[i].visible) {
				if (server[i].ping == -1) {
					...
					if (slots >= MAX_PINGREQUESTS) { break; }
					for (j = 0; j < MAX_PINGREQUESTS; j++) {
						if (!cl_pinglist[j].adr.port) { continue; }
					...
					if (j >= MAX_PINGREQUESTS) {
						status = qtrue;
						for (j = 0; j < MAX_PINGREQUESTS; j++) {
							if (!cl_pinglist[j].adr.port) { break; }
						}
	...
	if (slots) { status = qtrue; }
	for (i = 0; i < MAX_PINGREQUESTS; i++) {
		if (!cl_pinglist[i].adr.port) { continue; }
...
qboolean CL_CDKeyValidate(const char *key, const char *checksum) {
	...
	len = strlen(key);
	if (len != CDKEY_LEN) { return qfalse; }

	if (checksum && strlen(checksum) != CDCHKSUM_LEN) { return qfalse; }
	...
	if (checksum && !Q_stricmp(chs, checksum)) { return qtrue; }

	if (!checksum) { return qtrue; }
