...
qboolean CL_GetUserCmd(int cmdNumber, usercmd_t *ucmd) {
	...
	if (cmdNumber > cl.cmdNumber) { Com_Error(ERR_DROP, "CL_GetUserCmd: %i >= %i", cmdNumber, cl.cmdNumber); }
...
qboolean CL_GetParseEntityState(int parseEntityNumber, entityState_t *state) {
	...
	if (parseEntityNumber <= cl.parseEntitiesNum - MAX_PARSE_ENTITIES) { return qfalse; }
...
qboolean CL_GetServerCommand(int serverCommandNumber) {
	...
	if (serverCommandNumber <= clc.serverCommandSequence - MAX_RELIABLE_COMMANDS) {
		...
		if (clc.demoplaying) return qfalse;
...
rescan:
	...
	if (!strcmp(cmd, "clientLevelShot")) {
		...
		if (!com_sv_running->integer) { return qfalse; }
		// close the console
		Con_Close();
...
intptr_t CL_CgameSystemCalls(intptr_t *args) {
	...
	case CG_MEMSET: memset(VMA(1), args[2], args[3]); return 0;
	case CG_MEMCPY: memcpy(VMA(1), VMA(2), args[3]); return 0;
	...
	case CG_PC_READ_TOKEN: return botlib_export->PC_ReadTokenHandle(args[1], VMA(2));
	case CG_PC_SOURCE_FILE_AND_LINE: return botlib_export->PC_SourceFileAndLine(args[1], VMA(2), VMA(3));
...
void CL_AdjustTimeDelta(void) {
	...
	if (deltaDelta > RESET_TIME) {
		...
		if (cl_showTimeDelta->integer) { Com_Printf("<RESET> "); }
	} else if (deltaDelta > 100) {
		// fast adjust, cut the difference in half
		if (cl_showTimeDelta->integer) { Com_Printf("<FAST> "); }
	...
	if (cl_showTimeDelta->integer) { Com_Printf("%i ", cl.serverTimeDelta); }
}
...
void CL_SetCGameTime(void) {
	...
	if (!cl.snap.valid) { Com_Error(ERR_DROP, "CL_SetCGameTime: !cl.snap.valid"); }
	...
	if (clc.demoplaying && cl_freezeDemo->integer) {
		...
		if (cl.serverTime < cl.oldServerTime) { cl.serverTime = cl.oldServerTime; }
		cl.oldServerTime = cl.serverTime;

		// note if we are almost past the latest frame (without timeNudge),
		// so we will try and adjust back a bit when the next snapshot arrives
		if (cls.realtime + cl.serverTimeDelta >= cl.snap.serverTime - 5) { cl.extrapolatedSnapshot = qtrue; }
	}
	...
	if (cl_timedemo->integer) {
		...
		if (clc.timeDemoFrames > 0) {
			if (frameDuration > clc.timeDemoMaxDuration) clc.timeDemoMaxDuration = frameDuration;

			if (frameDuration < clc.timeDemoMinDuration) clc.timeDemoMinDuration = frameDuration;

			// 255 ms = about 4fps
			if (frameDuration > UCHAR_MAX) frameDuration = UCHAR_MAX;
