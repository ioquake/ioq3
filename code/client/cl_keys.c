...
void Field_VariableSizeDraw(field_t *edit, int x, int y, int width, int size, qboolean showCursor,
	...
	if (drawLen >= MAX_STRING_CHARS) { Com_Error(ERR_DROP, "drawLen >= MAX_STRING_CHARS"); }
...
void Console_Key(int key) {
	...
	if (key == K_ENTER || key == K_KP_ENTER) {
		...
		if (clc.state != CA_ACTIVE && con_autochat->integer && g_consoleField.buffer[0] &&
		...
		if (g_consoleField.buffer[0] == '\\' || g_consoleField.buffer[0] == '/') {
			...
			if (!g_consoleField.buffer[0]) {
				...
				if (con_autochat->integer) { Cbuf_AddText("cmd say "); }
...
qboolean Key_IsDown(int keynum) {
	if (keynum < 0 || keynum >= MAX_KEYS) { return qfalse; }
...
int Key_StringToKeynum(char *str) {
	...
	if (!str[1]) { return tolower(str[0]); }

	// check for hex code
	n = Com_HexStrToInt(str);
	if (n >= 0 && n < MAX_KEYS) { return n; }
...
char *Key_KeynumToString(int keynum) {
	...
	if (keynum < 0 || keynum >= MAX_KEYS) { return "<OUT OF RANGE>"; }
...
void Key_SetBinding(int keynum, const char *binding) {
	...
	// free old bindings
	if (keys[keynum].binding) { Z_Free(keys[keynum].binding); }
...
int Key_GetKey(const char *binding) {
	...
	if (binding) {
		for (i = 0; i < MAX_KEYS; i++) {
			if (keys[i].binding && Q_stricmp(binding, keys[i].binding) == 0) { return i; }
		}
	}
	return -1;
}
...
void Key_Unbindall_f(void) {
	...
	for (i = 0; i < MAX_KEYS; i++)
		if (keys[i].binding) Key_SetBinding(i, "");
}
...
void Key_KeynameCompletion(void (*callback)(const char *s)) {
	...
	for (i = 0; keynames[i].name != NULL; i++) callback(keynames[i].name);
}
...
void CL_KeyDownEvent(int key, unsigned time) {
	...
	if (keys[K_ALT].down && key == K_ENTER) {
		...
		if (keys[K_ENTER].repeats > 1) { return; }
	...
	if (Key_GetCatcher() & KEYCATCH_CONSOLE) {
		...
		if (cgvm) { VM_Call(cgvm, CG_KEY_EVENT, key, qtrue); }
...
void CL_KeyUpEvent(int key, unsigned time) {
	...
	if (key == K_CONSOLE || (key == K_ESCAPE && keys[K_SHIFT].down)) return;
...
void CL_LoadConsoleHistory(void) {
	...
	if (consoleSaveBufferSize < MAX_CONSOLE_SAVE_BUFFER &&
		...
		for (i = COMMAND_HISTORY - 1; i >= 0; i--) {
			if (!*(token = COM_Parse(&text_p))) break;
		...
		for (i = numLines; i < COMMAND_HISTORY; i++) Field_Clear(&historyEditLines[i]);
		...
		if (historyEditLines[i].buffer[0]) {
			...
			if (saveBufferLength + additionalLength < MAX_CONSOLE_SAVE_BUFFER) {
				Q_strcat(consoleSaveBuffer, MAX_CONSOLE_SAVE_BUFFER,
						 va("%d %d %d %s ", historyEditLines[i].cursor, historyEditLines[i].scroll, lineLength,
